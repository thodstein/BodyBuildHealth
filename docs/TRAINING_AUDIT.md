...
## P12-wire #4 — калибровка Wilks ✅
- Wilks реализован на стандартных опубликованных коэффициентах (муж: a=-216.0475145, b=16.2606339, c=-2.1688383e-3, d=1.1375105e-5, e=-3.49e-9).
- Сверка с официальной таблицей Wilks (verify-wilks-cal.mts, 7/7 PASS): 50кг→0.8437 (impl 0.8432, −0.06%), 75кг→0.5034 (impl 0.5020, −0.28%), 100кг→0.3573 (impl 0.3573, точно) — в пределах 0.3%.
- Wilks намеренно сильнее «штрафует» тяжёлых vs DOTS (DOTS/Wilks-отношение растёт с весом: 50кг×1.18, 75кг×1.43, 100кг×1.72) — ожидаемое поведение; Wilks устарел (DOTS — текущий стандарт IPF), оставлен для исторического сравнения.
## Реструктуризация: 3 плоские трассы (ПЛ / ББ / Ручной сбор) ✅
- Заменил вложенный 2-уровневый переключатель (Авто/Конструктор + внутри СРЦ/ББ) на **3 равноправные трассы одним кликом**:
  - 🏆 ПЛ (сила) → SRCBBScreen track='pl' (mode forced 'src', внутренний тумблер СРЦ/ББ скрыт) — авто-подбор цикла СРЦ, PM-прогрессия, 12 нед, блины, пик, метрики.
  - 💪 ББ → SRCBBScreen track='bb' (mode forced 'bb', тумблер скрыт) — авто-подбор BB-сплита, объём тяж/памп, PED, метрики.
  - 🛠 Ручной сбор → конструктор (план/циклы/программы/мои/методики/ручной калькулятор).
- PlanningTrack (pl/bb/manual) в shared.ts (localStorage he_training_planning_track), бэкворд-совместимо с PlanningMode. SRCBBScreen: prop track?.
- Верификация (render): 3-кнопочный селектор присутствует; ПЛ — нет внутреннего тумблера, показывает силовой цикл; ББ — нет тумблера, ББ-план; Ручной сбор — конструктор-вкладки. PE: 0. tsc ✓, vite ✓.
## Доработка «Ручной сбор» — проверка контента ✅
- Runtime-рендер всех вкладок ручного конструктора (puppeteer, PE: 0):
  - 📋 План: параметры (цель/уровень/сплит/периодизация/цикл/дни/восст/усталость/вес/слабые зоны) + «Сгенерировать план» → вывод: сплит-имя + фаза + ОСН/ДОП/АКС-упражнения (сеты×пов, RIR, схема, отдых, замены) + объём по группам + рекомендации. Полнофункционально, детально.
  - 🔄 Циклы: макроцикл-чарт (объём/интенсивность по неделям, фазы, навигация). Рендерится ✓.
  - 📚 Программы: 140 готовых программ (фильтры Сила/Масса/ПЛ/…). Рендерится ✓.
  - ⭐ Мои: кастомные упражнения. Рендерится ✓.
  - 🧠 Методики: композиция (одна из категории) + библиотека. Рендерится ✓.
  - 🛠️ Ручной конструктор: 4-шаговый калькулятор программы. Рендерится ✓.
- Все 6 вкладок ручного конструктора работают, рендерятся без page-errors. tsc ✓, vite ✓.
- Контент уже на уровне готового/удобного — план-вью детальнее ПЛ/ББ-трасс (ОСН/ДОП/АКС + замены + схема + отдых).

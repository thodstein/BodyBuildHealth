...
### Фаза 3 — единый дизайн (P1)
- Карточки ПЛ на `BbCard/BbFoldCard`/`training-ui` токены; серый→белый; единые кнопки 44px.

### Фаза 4 — гигиена данных и кода (P2)
- Псевдо-упражнения и фильтры метрик; weakpoint-pl резолвер; привязка имён; чистка `LMS_EXERCISES`; мёртвые экспорты движков; `sessionsPerWeek`.

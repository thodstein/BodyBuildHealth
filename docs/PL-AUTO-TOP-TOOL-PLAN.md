...
- ⏸ Остаток (требует решения/отдельной волны): псевдо-упражнения «Отдых»/«Тест: проходка (до макс)» в КПШ/тоннаже; PL-делод-инференс (меняет планы); dangling `canReplace`-ссылки на `ohp_bar` в каталоге (~60 записей — не влияют на выдачу, но врут движку замен); мёртвые экспорты движков (список аудита); `sessionsPerWeek` несоответствия в блок-циклах.

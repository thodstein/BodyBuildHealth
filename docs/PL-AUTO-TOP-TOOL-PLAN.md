...
1. **⏳ BB-ветка `SRCBBScreen`** (решение требуется). Экран монтируется только
   ...
   пишет профиль. Нужно отдельное решение: удалять целиком или оставить как неактивный резерв.
...
3. **⏸ Потребитель `catalogId`** (`LMS_EXERCISES`) — решение требуется: данные и lock есть,
   но `diagnosticGroupForExercise` и `lift-assistance` пока на именах/`groups`. Перевод
   обязан быть синхронным (parity-лок), иначе разойдутся протоколы диагностики.

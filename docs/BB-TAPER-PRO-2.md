...
- **Проверено**: PRO-2 тест 27/27 + prep-cycle 48/48 + соседи 311/311 + `tsc` 0 по своим файлам
  ...
  Добивка-3: PRO-2 33/33 + соседи 359/359 + UI 28/28 + `tsc` 0 по своим (1 ошибка — чужой `BBDiagnosticsHub` implicit any) + apk-verify OK.
- **Шторм-процедура (честная запись о гонке)**: в `BbAutoConstructor.tsx` параллельный агент держал
  ...
  Но между проверкой стейджа и коммитом параллельный процесс выполнил `git add -A` (известный
  шторм-паттерн) — индекс перезаписался версией worktree (моё + чужое), и pathspec-коммит
  `2b47e49a` утянул чужой ханк (36 строк PRO-3 R2) под моим именем. Попытка чистки через
  `hash-object -w` + `update-index --cacheinfo` была перезатёрта следующим `add -A`, а затем агент
  диагностики запушил поверх два своих коммита (`729d3a730`, `2b68186a0`) — amend стал небезопасен
  (порвал бы им историю), поэтому НЕ делался. Итог честно: чужой код ЦЕЛ и на месте (потеря/отката
  нет, в их коммитах он тоже учтён как живой worktree), дефект только в АТРИБУЦИИ 36 строк в `2b47e49a`.
  Урок шторма: стейдж-фильтр + коммит выполнять в ОДНОМ вызове терминала без пауз (окно гонки — миллисекунды),
  а при активном `add -A` соседа — сверять `git ls-files -s` прямо перед коммитом.

## 9. Добивка-3: закрыты микро-гэпы дозы (код, к коммиту pathspec)

По требованию «выполняй полностью» закрыты все три остатка из разбора (без оговорок).

- **Оверлей Macrocycle-пути**: `applyPeakWeekOverlayToBBPlan(plan, cfg, {weekNumber, carbDoseGPerKg})` —
  пик-неделя оверлея строится с дозой trial + пометка в `prepProtocol` (без дозы — байт-в-байт).
  Сам `MacrocyclePanel`/`SRCBBScreen` не тронуты (чужая зона) — опция движка готова к вызову.
- **Display-таблица**: строка пик-недели `buildPrepNutritionPlan` читает `plan.peakWeek.carbDoseGPerKg`
  (пометка «доза trial X г/кг»); без дозы — старая строка.
- **Live-пересчёт load-дней**: NEW `recarbLoadFromVisual(days, visual)` (flat +75 / spill −100,
  делится нацело по load-дням, ккал-инвариант, вход не мутируется, пустой вход → []) + UI-блок
  в чек-ине (чипы Плоско/Норма/Залило 44px + «Пересчитать load-дни» + таблица результата +
  персист `he_peak_recarb_<showDate>`). Честное ограничение: цифры — для приёмов пищи,
  дневник/рацион не переписываются (`IndividualPlanContext` — чужая активная зона, не тронут).
- Тесты +3 (оверлей с/без дозы, таблица с/без дозы, recarb-математика/инвариант/немутабельность).
- Проверено: PRO-2 33/33 + соседи 359/359 + UI 28/28 + `verify:apk-design` OK + `tsc` 0 по своим
  (1 ошибка — чужой `BBDiagnosticsHub.tsx:1765` implicit any, файл не тронут).
- **Харденинг (без смены логики)**: двум тяжёлым display-тестам (20/12 нед, полные сборки) задан
  явный `testTimeout 60000` — под штормовой нагрузкой параллельных прогонов дефолтные 5с
  роняли их таймаутом при корректной математике (доказано прогоном с расширенным лимитом: 8.8с, зелёный).

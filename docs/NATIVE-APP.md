...
Волна 8 — доступность и честность офлайна (`apk-top-pack` 15/15, `tsc` 0
по своим, `vite build` OK — native-CSS отдельными чанками, `dynamic-color`
отдельным чанком):

- **TalkBack в виджетах**: `contentDescription` в корнях всех 4 виджетов
  (ссылаются на существующие desc-строки). `previewLayout` уже был везде.
- **Офлайн-пилюля** (`NativeOfflinePill`, только APK): «Офлайн — данные
  сохраняются локально» над навбаром; в сети не рендерится вообще.
...
Волна 10 — QS Tile воды (`apk-top-pack` 16/16, `tsc` 0 по своим, XML валидны):

- **Quick Settings Tile «💧 +250»** (`WaterTileService.java` + манифест +
  строки): тап из шторки без открытия приложения кладёт воду в ту же
  очередь `WidgetStore`, что виджет/JS, обновляет виджет питания, показывает
  тост; сабтайтл (API 29+) — размер очереди. Проверено тестом проводки
  (манифест ↔ строки ↔ иконка ↔ Java-логика).
- Таймеры уже пищат/вибрируют на финише везде (`CardioSessionTimer`) —
  дублировать нативным хептиком нечего, проверено чтением кода.

Волна 11 — chrome recent-apps (`apk-top-pack` 17/17, `tsc` 0 по своим):

- **theme-color за темой**: `themeMetaColor()` в `appearance.ts` красит
  `<meta name="theme-color">` 1-в-1 с фоном (тёмная `#050b16`, AMOLED
  `#000000`, светлая `#eef2f6`) — только APK, в TG/web DOM не трогается
  (тест проверяет и маппинг, и no-op).
- По ходу пойман и убран дублированный хвост `applyApkTheme` от прошлой
  правки (tsc/esbuild падали — починено до коммита).
- JDK/SDK локально нет (`javac` отсутствует, `JAVA_HOME` пуст; SDK-каталог
  есть) — сборка Java-слоя остаётся за CI, как и раньше.

Волна 12 — anti-flash и мелочи (`apk-top-pack` 20/20, `tsc` 0 по своим):

- **Boot anti-flash**: инлайн-скрипт в `index.html` красит static-заглушку
  под сохранённые тему/акцент ДО первого paint (в TG ключей нет — no-op);
  `initApkAppearance` снимает override после гидрации, иначе переживший
  атрибут спорил бы со сменой темы (поймано анализом специфичности до бага).
- **`accent-color: var(--accent)`**: нативные чекбоксы/радио/ренджи
  следуют за акцентом бесплатно.

Волна 13 — бейдж №2 и pull-to-refresh (`apk-top-pack` 22/22 + PTR 4/4,
`tsc` 0 по своим):

- **Бейдж №2 честный**: дот на Профиле при заполненности < 50% — ровно там,
  где hero-CTA «Заполните профиль» (та же метрика 1-в-1, `PROFILE_BADGE_THRESHOLD`;
  копия 13 проверок осознанная — UI-компонент в мост тянуть нельзя).
- **Pull-to-refresh Главной**: `usePullToRefresh` (только верх скролла,
  вертикаль, игнор форм/горизонтали/оверлеев; высота — напрямую в DOM,
  в state лишь переходы; stale-closure закрыт ref-зеркалом) + индикатор
  `.native-ptr`. Обновление — повторный синк виджетов с честным сообщением.
- По ходу пойман молчаливый откат `HeroImg` в `DashboardScreen.native`
  (чужой worktree-паттерн): переприменён + guard-тест на WebP-source.
  Урок: перед каждым коммитом сверять ключевые маркеры, не только статус.

Волна 14 — guard-скрипт и полная регрессия (`verify:apk-design` OK,
`ui/__tests__` 20 файлов / 175 тестов, `tsc` 0 ПО ВСЕМУ проекту):

- **`npm run verify:apk-design`** (`scripts/check-apk-markers.mjs`, без
  зависимостей): 10 HeroImg-точек, 9 WebP, CSS-изоляция, var-изация,
  монтаж App/main, манифест/ресурсы, boot-ключи — ловит молчаливые откаты
  shared-worktree за секунды вместо раскопок.
...
Волна 17 — WCAG-контрасты (`apk-top-pack` 23/23, `tsc` 0 по своим):

- Замерены все пары текст/фон (тёмная + светлая + 4 акцента): провалы —
  `text-faint` 3.95 и светлые акценты 3.01/4.05. Исправлено токенами:
  faint `0.45→0.52` (4.96), light-акцент `#4d9e00→#3a7300` (5.14),
  light-accent2 `#00875a→#006b45` (5.85).
- Новый guard-тест считает ratio по токенам из CSS (hex + rgba + var-rgb
  с композитингом): текст 7+, остальное 4.5+ во всех темах/акцентах.
- По ходу пойман баг в формуле временного скрипта (ненормализованный g) —
  перепроверено исправленной копией, прежде чем трогать токены.

Волна 18 — общий слой (вариант 1: TG только лучше, не иначе; `shared-contrast`
3/3 + UI 176/176, `tsc` 0 по своим):

- **`::selection`** в обеих темах (фирменный акцент вместо системного синего).
- **Контрасты shared-токенов**: тёмная faint `0.35→0.5` (3.15→5.37), светлая
  dim `0.7→0.75` (4.27→4.93), faint `0.45→0.6` (2.34→3.33, Apple-tertiary
  parity — декоративный порог 3.0), **акцент светлой `#00e68a→#006b45`**
  (1.52→6.05 — был нечитаем везде, где текст; залитые кнопки не задеты —
  градиент захардкожен, белый текст стал только контрастнее 1.9→6.6).
- Guard-тест `shared-contrast.test.tsx`: тёмная 7+/4.5+, светлая 7+/4.5+
  (faint декоративный 3.0+).

Волна 19 — shared-мелочи (UI 178/178, `tsc` 0 по своим; вариант 1):

- **Табличные цифры** (`td/.stat-value/[data-stat]`) — статистика не прыгает.
- **Ленивые фото** ниже сгиба (отчёты, миниатюры дневника, таймлайн) +
  `decoding="async"` превью; hero — eager, не тронуты.
- По ходу пойман дубль `loading="lazy"` в `PhotoTimeline` (аудит-grep не
  видит многострочные теги — `[^>]*` не матчит `\n`): убран, tsc — страховка.

Волна 20 — мелочи общего слоя (`shared-contrast` 6/6, `tsc` 0 по своим):

- **`<html lang>` за переключателем RU/EN**: скринридеры больше не читают
  английский русским произношением (дефолт `ru` в `index.html` и так совпадал).
- Мультилайн-перепроверка `<img>`: дублей/пропусков `loading` нет.

Волна 21 — EN для native-Главной (`dashboard-first-run` 3/3, `tsc` 0):

- Вся `DashboardScreen.native` за таблицей строк RU/EN (приветствие, дата
  через локаль, 9 разделов, статистика, варнинги, first-run, очередь):
  TTS-тексты осознанно не тронуты (русский текст + `ru-RU` корректны;
  перевод подсказок — отдельный i18n-эпик, как и 100+ `ru-RU` в чужих
  движках/экранах/экспортах).
- RU-рендер побайтово прежний (старые тесты зелёные без правок).

Волна 22 — EN для native-компонентов (`apk-top-pack` 24/24 + §4.4 4/4,
`tsc` 0 по своим; прецедент волны 21):

- `NativeFab` (speed-dial, тосты), `NativeOfflinePill`, `AppearanceSetupCard`
  (темы, акценты, системный чип) — таблицы RU/EN, RU-рендер прежний.
- Процесс-урок: один edit из пачки упал (`strings()` не вставился), остальные
  легли — тесты поймали `ReferenceError` до коммита. Правило: упавший edit
  в пачке = стоп и проверка, tsc — до тестов, а не в одной команде.

Волна 23 — EN для §4.4 целиком (9/9 тестов, `tsc` 0 по своим):

- `WidgetsSetupCard`, `BiometrySetupCard`, `NativeFeaturesCard` — таблицы
  RU/EN (строки скопированы дословно, старые RU-тесты зелёные без правок).
  EN-тест рендерит карточки напрямую при `setLocale('en')`.
...
- **Stagger-каскад**: плитки Главной и карточки хабов (тренинг/риски/фарма)
  входят лесенкой 45–55мс (инлайн `animationDelay`, в TG без эффекта).
- **Press-scale** плиток/карточек на тап (десктопный hover-JS не задет).
- **FAB**: Escape закрывает меню (a11y).
- **Офлайн-пилюля**: показывает размер очереди виджета (`⏳ N`).

Волна 25 — видимый редизайн (UI 189/189, `tsc`/`vite build` чистые,
проверено скриншотами headless-Chrome 390×844, web + app-native):

- **Главная начисто**: hero + 3 кнопки (Профиль/Магазин/Статьи) + FAB.
  Приветствия, статистика, плитки, last-card убраны (живут в своих разделах).
- **THEME V2 глобально** (конец `styles.css`): navy-токены, лайм-акцент,
  плавающая pill-навигация с читаемыми лейблами и бейджами, стекло-карточки,
  кнопки, entrance-motion, hero всегда cover, отступ под плавающий навбар.
  Светлая тема безопасна (акцент затемнён ранее). Нижняя навигация (7 табов)
  структурно НЕ тронута — только визуал.
- `index.html`: фон/ theme-color под navy.
- **Hero без обрезанного текста**: запечённый в `hero-main.png` заголовок
  резался cover-кропом («...abolic Insig») — верхние 200px срезаны канвасом
  прямо в исходнике (имя файла то же, тесты/прелоад целы), WebP перегенерит.
- **Табы не обрезаются**: «Анализы» влезает (9.5px/0.2px, паддинги) —
  проверено скриншотом.
- Нижняя навигация структурно НЕ тронута (7 табов) — только визуал.

Волна 26 — Профиль в APK по скриншотам (`tsc` 0, guard зелёный):

- Шапка вкладок не режется пилюлями (`padding-right` + `!important`
  против инлайна), антропометрия 4→2 колонки (единственная такая сетка),
  значения редакторов — 2 строки вместо обрубка. Всё строго под
  `html.app-native`, TG 1-в-1.
- Проверено headless-Chrome 390×844 в native-режиме (до/после).
- Попутно: `VITE_APP_PLATFORM`-оверрайд молча не работал в проде
  (каст с `?.` не подхватывается подстановкой Vite) — прямой доступ,
  `app-platform` 14/14. Без этого native-превью в браузере невозможно.

Аварийный фикс: вечный цикл «ошибка → перезагрузка» в TG (`error-handler`
2/2, `tsc` 0 по своим; на чистых данных TG-эммуляция без оверлея):

- Причина: оверлей «Приложение восстановлено» показывался на ЛЮБУЮ ошибку,
  включая фоновый шум (реджекты синка, ресурсы без `e.error`). После тапа
  «Перезагрузить» шум повторялся — вечный цикл.
- Теперь: ошибки ресурсов вообще не краш (только лог); оверлей — только для
  фатального краша ДО первого рендера (`markBooted()` в `main.tsx` после
  `root.render`); после — тихо в `he_crash_reports` для диагностики.

Осознанные остатки (не баги): конкретный реджект у пользователя не
пойман (чистые данные не воспроизводят — ждём `he_crash_reports` после
деплоя); Gradle-сборка Java-слоя — только CI.

## 12. Самообновление APK без сторов

APK запекает `dist/` в момент сборки, поэтому сам ничего не подтягивает.
Лента обновлений — GitHub Releases, проверка — при каждом запуске APK:

1. `AppUpdateBanner` (только native, в TG/web — null) читает установленную
   версию через `App.getInfo()` и спрашивает фид `fetchLatestRelease()`
   (по умолчанию `.../releases/latest`, оверрайд — `VITE_APP_UPDATE_URL`).
2. Версия новее + не пропущена → баннер «⬇ Обновить» (проверка не чаще
   раза в сутки, пропуск версии запоминается в `he_app_update_v1`).
3. Скачивание — `DownloadManager` в приватный external-files
   (плагин `AppUpdater`, `MainActivity`), установка — системный установщик
   через FileProvider. Разрешение `REQUEST_INSTALL_PACKAGES` в манифесте;
   без гранта «неизвестные источники» баннер ведёт в настройки.
4. CI публикует APK в Release по тегу `v*` (job `publish`): подписанный
   `app-release.apk` приоритетом, иначе debug. Движок предпочитает
   подписанный ассет (`pickApkAssetUrl`).

Критично — единая подпись: Android ставит обновление поверх только при
совпадении подписи. CI подписывает ВСЕ сборки одним ключом
(`build.gradle`: debug тоже берёт `signingConfigs.release`, когда есть
`keystore.properties`; без него локально — обычный debug-ключ). Разовый
переезд debug→подписанная сборка = переустановка с переносом данных,
дальше обновления встают поверх молча.

## 12.1. Быстрое обновление без переустановки (OTA, как в TG)

Полный APK нужен только при смене нативного слоя (плагины/Java/манифест).
Обычные правки `src/` подтягиваются файлами внутрь установленного APK
(плагин `@capawesome/capacitor-live-update@8`, self-hosted, без облаков):

1. CI кладёт в каждый тег-релиз `web-bundle-<tag>.zip` (zip `dist/`,
   корень = index.html) рядом с APK.
2. Баннер при совместимом мажоре (JS 3.x на нативе 3.x) предлагает
   «⚡ Быстрое обновление»: `downloadBundle → setNextBundle` → «🔄 Перезапустить».
   Мажор разошёлся или .zip нет — остаётся только полный APK.
3. `readyTimeout: 10000` + `markAppReady()` в `main.tsx`: не стартовал за 10с —
   авто-откат на встроенный dist. Данные (IndexedDB/localStorage) общие,
   бандл меняет только код.
4. Откат вручную: `resetToBuiltInBundle()` из `src/core/live-update.ts`.

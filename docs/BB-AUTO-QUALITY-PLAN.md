...
- Тесты: bb-область 1357 зелёные; полный прогон 6844/6845 (единственное падение —
  пред-существующее `bb-macrocycle.test.ts` v7, чужой WIP).

**Осталось (опционально, UI-полировка):**
- ⏳ Лаборатория-привязка (темп/отдых/техника по уровню) — движок `bb-tempo-rest`/`bb-exercise-instructions`
  уже используются, нужна глубже в UI.

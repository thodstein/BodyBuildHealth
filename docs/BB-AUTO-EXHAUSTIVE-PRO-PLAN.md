...
- [ ] **Волна 1 — часть 2 (остаток)**: cooldown-ротация enforced
  (`canUseExercise` 4 нед / max 3 повтора), единый конвейер темпов
  (техники→taper→схемы; `modifyTempo` хаба → `workSets`), мёртвое темпов
  (`REST_BY_CHARACTER`/`tutForSet`/пустая ветка deload), `pauseSeconds`
  каталога в темп/подбор, делоад сохраняет выбор упражнений (Delphi) —
  вариант opt-in/травмы.

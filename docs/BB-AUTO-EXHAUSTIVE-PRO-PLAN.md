...
- [x] **Волна 5 — 5.3/5.4/5.5 закрыты** (коммит этого раунда):
  ...
  - Проверено: полный bb-круг **2500 passed / 1 failed (чужое пред-существующее
    ...
    **0 по проекту**, `verify:apk-design` OK.
- [x] **Аудит Sep 2026 «цель — качественный цикл»** (коммит этого раунда): дамп ВСЕХ 38 BB-циклов
  (× пол, intermediate) по реальному UI-пути ПРОФ-цикл + 8 новых классов проверок (порядок,
  прогрессия веса, делод, диапазоны reps/RIR, кап сетов, ратио к ПМ, рампы, делод-повторы).
  Найдены и починены 2 реальных дефекта НАГРУЗКИ:
  - **Конвертер терял мульти-спек источника** (`cycleTemplateToFullProgram`): брался только
    `sets[0]` (обычно рамп 1×12@40%), а `finalPct` был мёртвой переменной — downstream
    `parseWorkSetSpecs` не находил % в notes → fallback `PCT_FOR_RIR` (мышечный, БЕЗ учёта
    повторов) → все упражнения мышцы получали ОДИН вес: bench `[{0.4,12,1},{0.6,10,4}]` → 1-2
    сета на 40%, «разводка = жиму», французский жим 120-153% ПМ, пуловер 96% ПМ.
    Теперь схема `%×reps` едет в `notes` (canonical parse, как 5/3/1-схемы), сумма сетов
    сохраняется, RIR недели/делод применяются к каждому спеку — в обоих ветках конвертера
    (generated из week1 и explicit weeks).
  - **Филлеры наследовали шаблон**: fill (`structuredClone(session.exercises[0])`), баланс
    ширины спины и спец-частота (spread `...template`) копировали ВЕС и `executionProfile`
    лид-упражнения: добор предплечий получал 63.7 кг (153% ПМ) и «Паттерн: приседательный
    паттерн». Теперь вес считается от workMax СВОЕЙ мышцы (compound ×0.5 / изоляция ×0.3),
    профиль/комментарий достраивает enrich по своему имени, хвосты шаблона
    (superset/optional/techniqueTag/executionProfile) не переносятся.
  - Осознанно НЕ менялось (проверено дампом): source-driven ундуляция % week-to-week
    (`cycle-08` W2<W1 — так в источнике), делод-повторы нейральных разгрузок источника,
    ратио франц. жима 85% (scheme-overlay Brzycki, не утечка), warmup-активаторы первыми
    (дизайн `addWarmupActivator`), «тяжёлый компаунд перед primary памп-дня» (loadRank).
  - NEW `bb-cycle-load-quality` **6/6** (мульти-спек→5 сетов+схема в notes; рамп-сет легче
    рабочих; разводка < 0.7×жима; ≤1.2×ПМ по своей мышце; предплечья ≤0.6×ПМ; комментарии
    филлера — свои). Проверено: полный bb-круг **2506 passed / 1 failed (чужое
    `bb-diagnostics-max-pro` female-symmetry) / 20 skipped**, гейт-матрица
    `BB_CYCLE_AUDIT_FULL=1` **24/24** (4 уровня × 38 циклов × 2 пола × 2 цели + библиотека),
    `tsc --noEmit` 0 по проекту, `verify:apk-design` OK.

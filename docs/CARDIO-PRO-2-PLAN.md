...
- **Проверено**: NEW `cardio-pro2` 38/38 + движки-соседи 489/489 (13 файлов) + импорт/annual/macro 46/46 + UI 190/190 (13 файлов); `cardio-diary-slot` date-флейк починен фейковыми часами (добивка); `tsc --noEmit` **0 по всему проекту** (12GB heap); `verify:apk-design` OK.
...
- Добивка-2 «кроме P1» (коммит pathspec, без пуша): №2 — паритет свитча честно ±20% (HIIT 15 мин vs MISS 20 мин); №3 — кнопка «⚡ +HIIT в неделю 1» в карточке валидации при варнинге (`onAddHiit` → SIT 8×20 с undo-версией); №4 — предиктор подтягивает лучший рекорд из журнала (`run10k→run5k→runHalf`, пусто — честная подсказка); №5 — NEW `cardio-pro2-ui` 10/10 (чек-лист/предиктор/рекорды/6 пресетов/кнопка/сквозняки тоглов); №6 — подпись «до 16 лет».
...
- Добивка-5 «2–4, третий круг» (коммит pathspec, без пуша): №2 — `AnnualCardioBuildOptions.redFlags` → мед-блок в каждом цикле года + проброс из конструктора; №3 — чек-лист в `buildCardioPrintHtml` (только при taper-неделях); №4 — `paramsDirty` сравнивает флаги как множества (порядок кликов не важен). Проверено: движки 526/526 + UI 190/190 + `tsc` 0 + apk-verify OK.
- Добивка-6 «гигиена» (коммит pathspec, без пуша): честные счётчики (pro2 38, UI 10) + годовой флеш говорит про мед-блок + durability-нота при урезке днями.
- Добивка-8 «доделай тесты» (коммит pathspec, без пуша): UI 10→15 (годовой флеш про мед-блок + selectVariant хранит флаги). Проверено: движки 527/527 + UI 199/199 + `tsc` 0 + apk-verify OK.
- Добивка-7 «всё кроме 1» (коммит pathspec, без пуша): editConfig сбрасывает 3 флага (else-ветки) + `same()`-множества для equipment/legDays + UI 10→13 (editConfig-очистка/reset/dirty-порядок). Проверено: движки 527/527 + UI 193/193 + `tsc` 0 + apk-verify OK.

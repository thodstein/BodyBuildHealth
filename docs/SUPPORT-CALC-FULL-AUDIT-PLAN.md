...
12. ⏳ **Остальные пункты §6.2 — осознанно отложены**: `risk-engine-v7-matrix` LAB_REFERENCES и `training-calendar` (отдельные движки с множеством потребителей); `notification-engine` (женские алерты требуют фазовой модели E2 — иначе ложные тревоги ×2-3 в неделю); `estimateCardioRisk(entries, {sex})` — контракт: параметр пока не используется, менять семантику риск-скоринга без клинической модели не стали.

...
> ⚠️ **ИСТОРИЧЕСКИЙ ПЛАН (не гарантия текущего соответствия).** Проверка по коду
> (аудит-раунд) выявила дефекты, которых этот статус не покрывал; все закрыты кодом:
> - **P0 — гейт болгарского daily-max обходился.** Гейт жил только в раннере
>   (`bulgarianGate`), а `buildSSCyclePlan` его не звал: новичку / 50+ / без согласия /
>   при ACWR caution-dangerous 6-дневный протокол максимумов собирался молча, и rationale
>   утверждал «согласие получено» (врёт). Второй вектор — годовой план
>   (`buildAnnualFromSSCycles`), и сохранённый `cycleId` (вариант/мост), где раннер не участвует.
>   Теперь гейт fail-closed в самом билдере (единая точка для всех трёх путей);
>   лок-тест `ss-bulgarian-gate.test.ts` (11) + паритет «раннер ⇔ билдер» по 11 комбинациям.
> - **P1 — DUP «event_day» для стронгмена был мёртв:** `applyDUP` определяет ветку по
>   `plan.mode`, но `mode` не передавался ни в билдер, ни в cycle-путь. Тесты были
>   вакуумными (без `dupMode` во входе + `if`-обёртки + нестрогие ассерты) — зелёные
>   при нерабочей фиче. Исправлено + 2 мутационно-проверяемых лока.
> - **P1 — симулятор контеста считал по чужим показателям:** кросс-ивентные фолбэки
>   (йок ← становя, камень ← гантели, царапина ← йок) + выдуманные дефолты 180/140/100,
>   хотя `StrengthSportWorkMax` прямо объявляет «отдельный ввод, не фолбэк через deadlift».
>   Теперь event-specific ПМ, тяговые (вес снаряда ≠ нагрузка атлета) не оцениваются,
>   medley — по слабейшему элементу, `Math.random()` из чистой функции удалён,
>   нет данных → `noData` + честная строка, а не число из воздуха.
> - **P1/P2 — storage молчал** при потере записи (квота) → `console.warn` + честные
>   значения; **CSV-формулы** в экспортах ТА/стронг (`= + - @` → префикс `'`);
>   **даты** в отчётах/чек-инах переведены на локальные (было UTC → «вчера» в UTC+).
>
> Статус ниже относится к Sep 12 2026 и сохраняется как история.

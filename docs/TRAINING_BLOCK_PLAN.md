...
## Этап U — объединение планировщиков (ПЛ + ББ, наполнение из старого автомата)

> Подтверждённая архитектура: ЕДИНЫЙ планировщик = SRCBBScreen с двумя трассами
> **ПЛ (СРЦ/сила)** и **ББ (бодибилдинг)**. Наполнение старого конструктора подключается
> аддитивно (старое не ломаем). Ручная правка — слой поверх сгенерированного плана.

- [x] **U1** Методики: одна из каждой категории (была одна total). `appliedMethods: Record<category,name>`,
  композиция + чипы + «Применить». MethodsTab + TrainingScreen. Render: 2 из 6 категорий ✓.
- [x] **U2** 📚 Программы (140 готовых: 5/3/1, nSuns, SS, PPL…) — вью в SRCBBScreen (reuse ProgramsTab). Render ✓.
- [x] **U3** 🧠 Методики (44 метода, 6 категорий, композиция) — вью в SRCBBScreen (reuse MethodsTab + useDataLink). Render ✓.
- [x] **U4** Ручная правка поверх сгенерированного плана: редактирование сетов/весов/повторов плана (inline-редактор дня).
- [x] **U5** Выбор упражнений из каталога (536) в план: добавить/заменить упражнение в дне.
- [x] **U6** Фикс детерминизма циклов старого конструктора (убрать Math.random jitter в generatePlan).
- [x] **U7** Связать композицию методик с движком (периодизация/интенс-техники влияют на генерацию), не только инфо.
- [x] **U8** Аналитика/визуализация из дневника — вью в SRCBBScreen (reuse AnalyticsTab/VisualTab).

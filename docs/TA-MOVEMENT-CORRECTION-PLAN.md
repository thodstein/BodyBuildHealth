...
- **C10 (реаудит C9 + «продолжай» №7): две пойманные регрессии.** (1) Library-путь
  `buildSpecProtocols` игнорировал equipment/mobility-фильтры: при зале без
  штанги доза просачивалась мимо ранжира (раньше фаза скипалась) — добавлен
  гейт: при активных фильтрах только строгий путь ранжира + lock-тест.
  (2) Бейдж показывался при пустом `correctiveDetail` ([] truthy) — условие
  переведено на length>0 (wizard недостижим: parse всегда даёт null вместо []).
  Проверено: итого **224/224 (16 файлов)**; `tsc` **0 по всему проекту**;
  `verify:apk-design` OK. НЕ ПУШИЛ.
- **C11 («продолжай» №8): mobility-спрос в библиотеке.** Разрыв: при
  mobility-причине библиотека снижала дозу, но первым могла поставить
  упражнение со спросом на больной сустав (дефицит при голеностопе) — ранжир
  такое деприоритизирует (−15), библиотека нет. Фикс: demand-сеты
  (ankle/overhead/hip, паритет ранжира) + `mobilityRestrictions` во все
  функции (`correctivesForWeakPoint`, `correctiveSessionFor`,
  `protocolForPreferred`, `correctiveExportLines`, `correctiveBlockFor`) +
  `profileMobility` в 5 вызовов хаба (карточки/сессия/волна/доза/экспорт/мост).
  Доза не меняется (только порядок). По пути поймано тестами дважды:
  high-pull честно остаётся первым при плече (нет оверхед-фиксации —
  чинил тест), `overhead_hold` не чинил сед (цели) — добавлен `snatch_catch`
  (удержание выполняется в седе). Проверено: `ta-corrective` 20→**22/22** +
  UI 10→**11/11** (профиль→карточка end-to-end) + итого **227/227
  (16 файлов)**; `tsc` **0 по своим** (1 ошибка — чужой nutrition
  `recipe-db-p39.ts`, в работе); `verify:apk-design` OK. НЕ ПУШИЛ.

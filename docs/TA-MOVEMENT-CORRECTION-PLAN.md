...
- **C10 (реаудит C9 + «продолжай» №7): две пойманные регрессии.** (1) Library-путь
  ...
  `verify:apk-design` OK. НЕ ПУШИЛ.
- **C12 (ответ на «что осталось», «продолжай» №9): волна несла базу вместо дозы
  причины + demand-сеты без замка.** Волна `correctiveBlockFor` считала pct от
  базового протокола, тогда как карточка/сессия/экспорт/вставка — от
  скорректированного (сила +5 / mobility −5): пик волны врал на ±5 п.п. Фикс в
  одну строку (`protocolAdj.pct`) + тест «сила выше базы, мобильность ниже».
  Demand-сеты вынесены в экспортируемый `MOBILITY_DEMAND` + spot-lock
  (дефициты/становая/оверхед-фиксация на местах, RDL вне hip — как в ранжире,
  все id существуют). Проверено: `ta-corrective` 22→**24/24** + итого
  **229/229 (16 файлов)**; `tsc` **0 по своим** (1 ошибка — чужой nutrition
  `recipe-db-p39`, в работе); `verify:apk-design` OK. НЕ ПУШИЛ.

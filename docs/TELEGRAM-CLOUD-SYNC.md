...
## Привязка АПК кодом (без логина)

- ТГ: Профиль → Настройки → «4.5 Синхронизация» → «Создать код для АПК» (код `XXX-XXX`, 10 мин, одноразовый).
- АПК: Профиль → Настройки → «4.5 Синхронизация» → ввод кода → токен `tk_*` сохраняется
  в `he_sync_tg_token_v1` (в EXCLUDED_KEYS, не синкается), синк стартует как второе устройство.
- Поле ввода кода видно везде, кроме Telegram Mini App (АПК + браузер как второй экран).
  Режим определяется живыми детекторами (`isTelegramContext`/`isCapacitorNative`), а не
  мемоизированной платформой — сборка с `VITE_APP_PLATFORM=telegram` больше не прячет ввод в АПК.
- Если вместо поля ввода виден текст «Облако не настроено в ЭТОЙ сборке» — этот APK собран
  без `.env` (нет `VITE_SUPABASE_URL`/`VITE_SUPABASE_ANON_KEY`): пересоберите
  `npm run build:native` + `cap sync` из окружения с теми же ключами, что у ТГ-билда.
- Переустановка АПК: локальное стирается, облако цело → новый код из ТГ возвращает все данные.
- Таблица `link_codes` (миграция `supabase/migrations/20260912_link_codes.sql`) — применить в SQL Editor.
- `dev_*` без токена — локальный режим `off` как раньше; с токеном — полный синк.

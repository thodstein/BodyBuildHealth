...
1. **Только реальные id каталога** — соответствующий каталог (`exercise-catalog.ts` / `exercise-catalog-arm.ts` / `exercise-catalog-ta-supplement.ts`); «синтетических» упражнений нет (проектное правило). Lock-тест: каждый id существует.
...
1) Армрестлинг (src/engines/arm/arm-weakpoint-corrections.ts + arm-correction-pro2.engine.ts):
   доведи exercises[] до ≥6 реальных id на каждую из 12 точек (id ТОЛЬКО из src/core/exercise-catalog-arm.ts),
   POOL_TOPUP на все 12 точек, разметь CORRECTION_ROLE для каждого нового id. Дефолтный топ-3 не сдвигать
   (или осознанный re-baseline «было→стало»).

2) Армлифтинг (src/engines/arm/armlift-correction.engine.ts): +≥16 записей (реальные exId из arm-каталога),
   ≥2 на каждую причину×фазу×implement (RT/Axle/Pinch/Hub/CoC/Excalibur/Silver); каждой — cues/progression/
   equipmentAlt/gentle; дефолтные топ-3 (armlift-correction-pro) не сдвигать.
...
Правила: только реальные id каталогов (без синтетики), lock-тесты на существование id/покрытие/дозы,
ранжир — non-regression (иначе re-baseline «было→стало»), только Edit/Write, коммит pathspec без пуша.
Проверь: tsc 0; круги engines/arm + engines/strength-sport; UI TrainingScreen_parts; vite build.
```

## 6. Границы (не делать)
- Не менять математику объёма/капов/MRV (только контент коррекции).
- Не ломать калиброванные тесты без осознанного re-baseline.
- Не добавлять синтетические упражнения и фиктивные id.

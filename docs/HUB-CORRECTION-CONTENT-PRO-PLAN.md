...
| **ББ (hub 5)** | **52 → 76** записи; зоны/сигналы/драйверы доведены до **≥3**; закрыты `recovery` (strength 0) и `volume` (stability 0); NEW движок **сессии/блока коррекции** (волна 3-3-4-4-4-4-3-3) + карточка в хабе + HTML/CSV-экспорт | зона/сигнал/драйвер ≥3; причина×фаза ≥2; блок не выгружается пустым | `c3edbb09`, `181a4049`, `e69321d8` |

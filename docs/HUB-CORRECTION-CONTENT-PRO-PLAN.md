...
| Пороги round-10 (третья ступень) | ББ зона/сигнал/драйвер ≥**5** (+9 `spare`), стронг причина × вид ≥**4** (+4), армлифтинг причина × фаза ≥**4** (+14), ТА причина-лимитер × фаза ≥**4** (+3 коррекции + 3 реальные каталог-записи), арм пул точки ≥**7** (контент уже давал 7–10) | Все новые id — реальные записи каталога; канонические причины (`max_strength`/`technique`/`fatigue`, не `strength`/`stability`); непустой `howNot` у каждой ТА-записи | `a56a060c`, `0e35b8ff` |

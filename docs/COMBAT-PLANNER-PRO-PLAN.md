...
- **Финал-4** (`53b1dd04`): закрыты все 7 пунктов реаудита — живой `cb-redflags` баннер скринера в шаге Атлет (был мёртв в проде); taper-константы `TAPER_*` единым источником (кондиция импортирует, сплит — тонкая обёртка, спарринг-гейт потребляет сплит; lock-тест консистентности); `isCombatPlanBlocked` в движке + гейт внутри `exportToUserProgram` (оборона в глубину); `neckLevelOverride`/`weakSide` (override планирует шею, teen-фолбэк чинит обход авто-добавки мимо пула, асимметрия — rationale+warning; приёмник пишет состояние); приоритет main/secondary при ручном бое (селект `CombatPopupSelect`, не нативный — гард зоны); combat-раздел общего `CycleCatalog` (сегмент/счётчики/фильтры/избранное `combat:`/мост; чужой файл — только аддитивно; соседний promax-тест 6→7 калиброван по прецеденту кардио-таба). Проверено **605/605 (40 файлов)** + `tsc 0` + apk-markers OK. Честно: реаудит показал, что таблицы/ruleset/мульти-бой/таб галереи уже были в финал-3 (нулевой дифф — только верификация, без дублей).
- **Финал-5** (`f77d11f6`): закрыты 3 мелких остатка — дедуп `face_pull` (финализатор больше не вставляет второй экземпляр при имеющемся: честный warning «добейте вручную»; поймано консольным ворнингом React в тестах); удалён мёртвый `needsCombatMedicalBlock` (единый API — `screenCombatRedFlags`); BJJ-пометка про вес в кимоно (`weightClassRulesetNote` + баннер в весогонке + тест). Проверено **607/607 (40 файлов)** + `tsc 0` + apk-markers OK.

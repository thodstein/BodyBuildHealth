...
**Тесты:** 498 → **~618** (outside 10 + strength 221 + combat **367** = matrix 245 + builder 7 + pro 16 + p2-polish 12 + balance 4 + dup 3 + issn-pro-max 31 + extra 8 + **v2-polar 7** — детерминизм 240, ATR multi-cycle, VBT per-lift EWMA, neck auto, XLSX, weightCut ISSN fiber/ORS/window/confirmed, conditioning Jamieson + outside≥5 Zone2, neck 2.0 Collins 4 плоскости, graded травмы, diary grip, fightStyle, annual 2-4, PED cap 1.18, print). Запуск `npx vitest run src/engines/combat` — **367/367**.

...
- Мониторинг `combat-monitoring.engine.ts` — `combatACWR` (0.8/1.3/1.5), `vbtVelocityForPct`/`vbtRecommendation` (loss>20% → RIR+1), `hrvGrade`
- Core `combat-core.engine.ts` — **Boxing Science 4 функции** anti_extension/rotation/lateral/hip_flexion + rotation_power, 4 уровня, `coreWeeklyPlan`
...
- Финализация `combat-finalize.engine.ts` — neck/grip/rot/coreAnti vs MEV/MRV, **push/pull horiz+vert отдельно 1.8/0.55, unilateral vs bilateral, outside конфликт, weightCut deficit**
- Builder `combat-builder.engine.ts` — `POOL_BY_TAG` 15/12/15/... **64 упражнения**, `filterPool` (cable/sled fallback), `gentleFactor` 0.6-0.7, `weightForCombatExercise` через workMax, **budget =112×ped×lab×outside×recovery×nutrition×acwr**, **taper к дате боя, весогонка volume 0.65/0.82, ACWR RIR+1-2, conditioning 2-3 системы, DUP+intensity post-hoc**
- DUP `combat-dup.ts` — `off/power_endurance/heavy_light/conjugate` (3 волны)
- Intensity `combat-intensity.ts` — `none/rest_pause/myo_reps/cluster/contrast`
- Мезоцикл `combat-mesocycle.ts` — **+2.5кг compound / +1кг изоляция**, ACWR-aware (danger → 0.97), weight_cut без бампа
- Годовой `combat-annual.ts` — **ATR 50/30/20 + transition 2нед на 52нед**, `buildAnnualATR`, `annualCBPhaseForWeek`, `buildAnnualPrintHtml/Ics`, `addCompetitionToAnnual`
- Интеграция `combat-integration.engine.ts` — `combatDiaryStatsFromSessions`, `combatToNutritionPayload`, `combatToCardioPayload`
- UI `CombatConstructor.tsx` — params/outside/split/plan, fightDate+weightCut protocol, heatmap neck/grip/core, diary 7д, годовой ATR
...
**Тесты:** 498 → **~565** (outside 10 + strength 221 + combat 283+16 PRO — матрицы 240 детерминизм, ATR 10, taper, weightCut, conditioning, ACWR/VBT, Core4, PED cap, mesocycle, annual ATR, DUP heavy_light/conjugate, intensity myo/cluster, workMax/budget). Запуск `npx vitest run src/engines/combat` — 283/283.

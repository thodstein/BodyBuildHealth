...
- WorkMax `combat-workmax.ts` — `getCombatWorkMax` (workMaxByExercise → workMax[muscle] → BW coeff) + 40 дефолтов, шаг 2.5/0.5кг для шеи
 ...
 - Дневник `combat-diary.engine.ts` — **per-group e1RM + grip изометрия holdSec×10** `buildDiaryTrendCB`, `gripIsometricVolume`
  - UI `CombatConstructor.tsx` — PRO MAX: `periodizationModel/conditioningMode/weighInType/orsSodium/confirmed`, `fightDate/taperWeeks/startDate`, `bodyweight/sex/age + workMax 7 + workMaxByExercise 70 (showExactWM)`, `ACWR/VBT/HRV line + EWMA + VBT per-lift history`, ISSN `water/sodium/carb/fiber/ORS/heat/confirmed + female RED-S`, `DUP 4 / intensity 5 / fightStyle 3`, `POOL 70 + heatmap шея 4 плоскости (Collins) auto`, ` diary 7д + VBT per-lift`, ` годовой ATR 12/24/36/52 multi-cycle 1-4 + бои + print/ics/xlsx`, ` план печать HTML/CSV/XLS/ICS` `buildCombatPrintHtml` + `buildCombatXlsxHtml`
  - XLSX `combat-xlsx.engine.ts` — `buildCombatXlsxHtml/downloadCombatXlsx` Excel HTML Gantt+heatmap+meta
  - Builder: **VBT per-lift EWMA** `velocityLossPerLift + vbtHistory` > скаляр, **neck auto** 4 плоскости как core
...
**Тесты:** 498 → **~626** (outside 10 + strength 226 + combat **375** = matrix 245 + builder 7 + pro 16 + p2-polish 12 + balance 4 + dup 3 + issn-pro-max 31 + extra 8 + v2-polar 7 + **integration 3 + strength-weight-cut 5** — детерминизм 240, ATR multi-cycle, VBT per-lift EWMA (bench/squat/row), neck auto, XLSX, weightCut ISSN fiber/ORS/window/confirmed, conditioning Jamieson + outside≥5 Zone2, neck 2.0 Collins 4 плоскости, graded травмы, diary grip, fightStyle, annual 2-4, PED cap 1.18, print). Запуск `npx vitest run src/engines/combat src/engines/strength-sport/__tests__/strength-weight-cut.test.ts` — **375/375**.

...
## §8. Добивка C4: ноль фантомов в ранжире (паритет C3-фиксу инъекции)

- **Найдено своим lock-тестом**: `rankCorrectionsForSM` отдавал в топ первое слово
  строки без скобок (`'Yoke'`/`'Pinch'`/`'Prowler'`/`'Suitcase'`) — UI «⭐ Топ-коррекция»
  показывал фантомы (тот же класс дефекта, что C3 в инъекции).
- Фикс в движке ранжира: извлечённые id валидируются по канону фаз, чужое —
  заменяется каноном + дедуп; канон `SM_FALLBACK_BY_WP` экспортирован (единый
  источник ранжир + инъекция, локальный дубль из инъекции удалён).
- Проверено: `sm-corrective` 13→**14/14** (lock «все топ-id × 16 фаз — из канона») +
  соседи pro-v3/diagnostics-pro/intake/hub/pro3/corrective — итого **118/118
  (9 файлов)**; `tsc` **0 по своим**; `verify:apk-design` OK.

## §9. Добивка C5: всё по списку (волна + откат + L/R + мелочь)

Честный реаудит остатков дал 4 пункта — все закрыты кодом:

- **(1) Волна в недели.** Инъекция шила только `weeksData[0]` (у TA — `weekIdxs`):
  NEW `opts.weekIdxs` (дефолт `[0]` — поведение 1-в-1; бюджет/дедуп/делод честно
  по каждой неделе, маркер `(нед N)` только при мультивставке) + кнопка
  «🌊 Волна коррекции (N фазы)» (`data-ss="apply-sm-corrective"`, opt-in — сборка
  шьёт нед 1 автоматом) со снапшотом до вставки.
- **(2) Откат.** Хелперы `snapshot/rollbackSMPlanForInject` существовали без
  потребителей: NEW `handleRollbackSMCorrective` (`data-ss="rollback-sm-corrective"`),
  снапшот в `sessionStorage` с id-плана (пересборка инвалидирует — честный отказ);
  новая сборка гасит флаг и чистит ключ.
- **(3) L/R-добивка в сетах.** Хинт обещал «+1 сет», план его не видел: NEW
  `opts.unilateralBoost` (фаза → left/right; +1 сет + пометка «слаб. слева/справа»,
  мусор игнится) end-to-end: хаб `smUnilateral` (только grip-фазы при ≥7%) → мост
  (валидация left/right, кап 4) → wizard → сборка и волна.
- **(4) Мелочь.** Mobility-хинт динамический (фазы с причиной mobility из моста,
  fallback — статичное трио).
- По пути: чуть не снёс `handleRollbackSpec` точечной заменой (граница oldString) —
  восстановлено сразу, проверено grep (3 хендлера на месте); тест писал неверную
  сторону (100 vs 90 — слабее справа, чинил тест); в тесте дважды затирал соседний
  `it(` коротким oldString — восстановлено чтением (урок: якоря с запасом).
- Проверено: injection 6→**10/10** (недели/делод/LR) + NEW `sm-corrective-wave`
  **2/2** (E2E мост→сборка→волна→откат через живой маунт; в логе

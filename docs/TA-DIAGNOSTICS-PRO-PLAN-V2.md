...
## 11. V7-круг «закрытие остатков» (✅ ВЫПОЛНЕН)

Всё из §«что осталось» аудита, что реально закрываемо в своих файлах:
- **V7-A FvR взятия**: тоггл рывковая/толчковая тяга, нормы clean, пометка «оценка (валидировано
  только для рывка, Sandau 2021)»; заявки попыток больше не подхватывают чужой замер
  (FvR-дефолт рывка только в snatch-режиме — исправлена потенциальная мисмаркировка).
- **V7-B aux-аудит**: `TA_AUX_PHASES` + строка покрытия присед/тяги/жим в аудите
  (разбор aux — через OHS/VBT, без нового таба — осознанно).
- **V7-C честность**: пометка цикла Sinclair в хабе + конец года синка («по N», хвост — поддержание).
- Проверено: область **690/690 (48 файлов)**, hub **36/36**, **tsc 0**, коммит pathspec, push.
...
## 14. V10-круг «персист и worst» (✅ ВЫПОЛНЕН)

Два реальных недодела из ревизии:
- **V10-A персист недель синка**: `annualStartWeek`/`annualEndWeek` жили в обычном `useState`
  (тот же баг, что был с нотами) — переехали в `WLState`, переживают remount (тест).
- **V10-B worst при полном CORE**: кнопка «разобрать» указывала на покрытую фазу —
  теперь fallback на минимум AUX (тест: полное покрытие → worst `squat_mid` из AUX).
- Проверено: таргеты **47/47**, **tsc exit 0**, коммит pathspec, push.

## 12. V8-круг «текущий цикл Sinclair» (✅ ВЫПОЛНЕН)

Сверка с интернетом показала: действует цикл **2025–2028** (M A=0.700767819/B=201.159 —
сверено с официальной таблицей PZPC: 81.5кг → 1.28201 ✓; F A=0.674107991/B=163.918),
движок сидел на устаревшем 2021–2024:
- Параметр цикла (`2025-2028` дефолт / `2021-2024` legacy), снимки хранят свой цикл,
  тренд считает каждый снимок своим циклом; чипы цикла в хабе; цикл в экспорте/bridge.
- Найден и задокументирован конфликт источников: часть калькуляторов до сих пор показывает
  2021–2024 как «current»; SM-зона проекта (`calcSinclair` в finalize) тоже на старом цикле —
  файл чужой, не тронут, владельцу на заметку (одному атлету хабы покажут разный Sinclair).
- **Инцидент процесса**: V8-правка случайно снесла `TA_PROGRESS_KEY` — `saveTAProgress` падал
  в ReferenceError, глотаемый try/catch (молча нет записи при зелёных тестах).
  Поймано отладкой через временный лог (файл удалён). Выводы: storage-хелперы обязаны иметь
  прямые save/load-тесты (добавлен регрессионный) + импорт константы в тесте роняет коллекцию
  при повторении.
- Проверено: прогресс **11/11**, экспорт, hub, **tsc 0**, коммит pathspec, push.

## 10. V6-круг «аудит остатков» (✅ ВЫПОЛНЕН)

Честная ревизия своих файлов после V5; закрыты блоки A+B:
- **V6-A гигиена**: удалён dead-export `buildWLPlanHtml` (никто не импортировал) и мёртвый
  `diagnoseBarPathPro` (только определение; живой — `diagnoseBarPath` в хабе); снесены мёртвые импорты
  (`loadLVPProfiles`, `velocityForLVP`, `LIMITER_CATEGORIES`); схлопнута мёртвая ветка
  `candidateTAWeakPointsFromDiary` (if/else делали одно и то же) + убраны висячие импорты
  (`phaseForReps`, `groupOfExerciseName`) с честной правкой шапки.
- **V6-B незамкнутое**: ноты инъекции переехали в `WLState` (переживают remount);
  снапшот `{raw, planId, savedAt}` + legacy-совместимость — откат запрещён при пересборке плана
  (другой id): план не тронут, stale очищен, тост «План пересобран…»; ручной вес IMTP
  (приоритет над профилем); маркер пола ♂/♀ в строке Sinclair.
- Проверено: область **654/654 (45 файлов, +SM-файлы коллег тоже зелёные)**, hub **33/33**,
  **tsc 0 по всему проекту**, коммит pathspec, push.

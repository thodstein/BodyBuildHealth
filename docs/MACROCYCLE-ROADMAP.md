...
- [x] **E1. Самостоятельная модель CardioCycle** — `cardio.engine.ts`: многонедельный цикл с фазами
      (base/build/maintenance/contest_prep/taper/peak/transition), Zone 2/MISS/HIIT/recovery, делодами,
      kcal, соревнованиями, сериализацией/библиотекой, миграцией CardioPlan.
- [x] **E2. Отдельный «Кардио-конструктор»** — 4-й режим зоны «Планировщик» (`CardioConstructor.tsx`,
      `nav.ts`/`shared.ts`/`TrainingScreen.tsx`): параметры, соревнования, предпросмотр недель,
      библиотека (активный/дублировать/удалить), экспорт `.ics` и печать, подключение к ПЛ/ББ/ручному.

...
bb-UI 26 файлов / 213 passed, tsc --noEmit 0 по проекту. Коммиты ce96a4d8, 4569dc1a,
c291a7d6, d1008a8d (+ этот) — без пуша.
...
3) Гигиена/остатки (безопасно): `applyTaperToFinalWeeks(plan, totalWeeks)` — параметр
   не читается (либо использовать, либо переименовать в `_totalWeeks` с комментом);
   `bbProgramPath` (roundtrip-маркер вариантов) и `zoneSpec`/`donorsForZone` (API-запас) —
   решить судьбу; ревизия 157 test-only экспортов движков по политике «API сохранён»
   (удалять только при 0 потребителей и 0 тестов).
4) Тесты: периодически гонять тяжёлую матрицу `$env:BB_CYCLE_AUDIT_FULL='1';
   npx vitest run src/engines/bb/__tests__/bb-cycle-audit-*.test.ts` (20 тестов скрыты env-гейтом;

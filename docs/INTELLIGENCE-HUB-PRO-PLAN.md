...
## §8. Добивка-2 E/G/F (Sep 12 2026, без пуша)
- **E1 мост честно** (`a2ce081`): reaudit показал — `pri`-контракт всей кодобазы только `{volumeMult, rirShift}`,
  deload-флаг и топ-сет/% молча отбрасывались. Сначала слал вторым пейлоадом — своим же тестом поймано, что батчинг
  React съедает первый (приёмник `SRCBBScreen:761-767` обрабатывает только последний setState). Финал: РОВНО ОДИН
  пейлоад — при флаге `kind='deload'` (несёт те же объём/RIR + флаг), иначе `kind='pri'`; тест считает события
  (`dispatched==1` в обоих путях). Топ-сет подписан ориентиром.
- **E2 подпись калибровки**: «учтена в авторегуляции» врало (`autoRegulate` bias не читает) — честное указание на
  живой тумблер RIR-калибрации (дневник/RIR-хаб). Доказано UI-тестом с сидом.
- **F1 паритет отчёта** (`2588b5a4`): `trainingLoadReport` += `opts` (рекомендации тем же методом, что пульт;
  остальные — дефолт байт-в-байт) + тест паритета. **F2 покрытие**: reduced/elevated/need_base, streak w3, custom окна.
- **G-баг E1** (этот раунд): двухпейлоадная версия снесена до однопейлоадной (см. выше) + тач-норма 44px нав-пилюлям/
  журнальным кнопкам (были 30/40px) + §8. Проверено: область 127+/UI 8/8 + `tsc` 0 по своим.

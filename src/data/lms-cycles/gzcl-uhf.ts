...
const weeks: SRDaySpec[][] = Array.from({ length: 9 }, (_, wi) => {
  ...
  return [
    day(ex('Присед T1', 'ПР', 1.2, [s(Math.min(0.92, t1Pct), Math.max(1, 3 - Math.floor(w/3)), 4)]), ex('Жим лежа T2', 'ЖМ', 1.0, [s(t2Pct, 6, 4)]), ex('Тяга в наклоне T3', 'ТГ', 0.5, [s(0.5,12,3)])),
    day(ex('Жим лежа T1', 'ЖМ', 1.0, [s(Math.min(0.92, t1Pct), Math.max(1, 3 - Math.floor(w/3)), 4)]), ex('Присед T2', 'ПР', 1.0, [s(t2Pct, 6, 4)]), ex('Подтягивания T3', 'ТГ', 0.5, [s(0.3,12,3)])),
    day(ex('Становая тяга T1', 'ТГ', 1.4, [s(Math.min(0.92, t1Pct), Math.max(1, 3 - Math.floor(w/3)), 3)]), ex('Жим стоя T2', 'ЖМ', 0.5, [s(t2Pct, 6, 3)]), ex('Гиперэкстензия T3', 'ТГ', 0.3, [s(0.3,12,3)])),

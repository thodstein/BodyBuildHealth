...
function buildWeek(w: number): SSDaySpec[] {
  ...
  return [
    day('squat_day', rot[0] === 'explosive' ? 'памп' : 'тяж',
      ...
      ex('box_jump', 'Футворк: прыжки/шаги (agility)', 'legs', 0.4, [s(0, 5, 3)], { bodyweight: true, role: 'accessory' }),
    ),
    day('overhead_day', rot[1] === 'explosive' ? 'памп' : 'тяж',
      ...
      ex('pin_press', 'Дожим (локаут)', 'shoulders', 0.7, [s(0.62, 8, 3)], { base: 'bench', role: 'accessory' }),
    ),
    day('deadlift_day', rot[2] === 'explosive' ? 'памп' : 'тяж',
      ...
      ex('rdl', 'Румынская (задняя цепь)', 'legs', 0.7, [s(0.60, 8, 3)], { role: 'accessory' }),
    ),
    day('event_day', 'тяж',

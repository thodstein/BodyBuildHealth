...
function buildWeek(w: number): SSDaySpec[] {
  ...
  return [
    day('deadlift_day', char,
      ...
      ex('kettlebell_swing', 'Махи гирей (взрыв)', 'legs', 0.5, [s(0.15, 10, 3)], { base: 'deadlift', baseMult: 1 }),
      ex('leg_press', 'Жим ногами', 'legs', 0.6, [s(1.00, 8, 3)], { base: 'backSquat' }),
    ),
    day('overhead_day', char,
      ex('log_press', 'Лог-лифт', 'strongman', 1.4, [s(load, rep, sets)]),
      ex('db_press', 'Жим гантелей', 'shoulders', 0.9, [s(Math.max(0.55, load - 0.10), 8, 3)], { base: 'bench' }),
      ex('bench_bar', 'Жим лёжа', 'shoulders', 1.0, [s(Math.max(0.60, load - 0.05), rep + 2, 3)]),
      ex('lateral_raise', 'Махи гантелями в стороны', 'shoulders', 0.4, [s(0.15, 10, 4)], { base: 'overheadPress', baseMult: 1 }),
      ex('pin_press', 'Трицепс: узкий дожим', 'shoulders', 0.6, [s(0.55, 8, 3)], { base: 'bench' }),
      ex('row_db', 'Бицепс/спина: тяга гантели', 'back', 0.5, [s(0.45, 8, 3)], { base: 'deadlift', baseMult: 1 }),
    ),
    day('event_day', char,
      ...
      ex('calf_raise', 'Икры стоя', 'legs', 0.3, [s(0.50, 15, 3)], { base: 'backSquat' }),
      ex('box_jump', 'Прыжки на тумбу (взрыв)', 'legs', 0.4, [s(0, 4, 3)], { bodyweight: true }),
    ),
  ];
}

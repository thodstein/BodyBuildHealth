...
function foundationWeek(w: number): SSDaySpec[] {
  ...
  return [
    day('squat_day', 'тяж',
      ...
      ex('bulgarian_split', 'Выпады шагающие', 'legs', 0.6, [s(0.30, 12, 3)], { base: 'backSquat' }),
      ex('leg_press', 'Жим ногами (высокая постановка)', 'legs', 0.7, [s(1.10, 12, 3)], { base: 'backSquat' }),
      ex('plank', 'Планка / колесо', 'accessory', 0.3, [s(0, 30, 3)], { bodyweight: true }),
    ),
    day('overhead_day', 'тяж',
      ...
      ex('row_db', 'Тяга гантели (верх спины)', 'back', 0.7, [s(0.35, 10, 4)], { base: 'deadlift', baseMult: 1 }),
      ex('pin_press', 'Дожим узким хватом (локаут)', 'shoulders', 0.7, [s(0.60, 12, 3)], { base: 'bench' }),
    ),
    day('deadlift_day', 'тяж',
      ex('deadlift', 'Становая тяга (сброс каждую)', 'back', 1.4, [s(dl, 5, 4)]),
      ex('row_bar', 'Тяга штанги в наклоне', 'back', 0.8, [s(0.35, 8, 4)], { base: 'deadlift', baseMult: 1 }),
      ...
      ex('hanging_leg_raise', 'Подносы ног в висе', 'accessory', 0.3, [s(0, 12, 3)], { bodyweight: true }),
    ),
  ];
}
...
function eventIntroWeek(w: number): SSDaySpec[] {
  ...
  return [
    day('squat_day', 'тяж',
      ...
      ex('good_morning', 'Наклоны со штангой (шарнир)', 'back', 0.6, [s(0.40, 8, 3)], { base: 'deadlift', baseMult: 1 }),
      ex('plank', 'Кор-цепь: планка/паллоф/колесо', 'accessory', 0.3, [s(0, 45, 3)], { bodyweight: true }),
    ),
    day('overhead_day', 'тяж',
      ...
      ex('face_pull', 'Протяжка к лицу (здоровье плеч)', 'shoulders', 0.4, [s(0.40, 15, 4)], { base: 'overheadPress', baseMult: 1 }),
    ),
    day('event_day', 'тяж',
...
function compPrepWeek(w: number): SSDaySpec[] {
  if (w === 12) {
    return [
      ...
      day('deadlift_day', 'лёг',
        ex('face_pull', 'Пул-апарты/растяжка', 'shoulders', 0.3, [s(0.30, 25, 2)], { base: 'overheadPress', baseMult: 1 }),
        ex('plank', 'Мобильность: бёдра/грудь/плечи', 'accessory', 0.2, [s(0, 20, 1)], { bodyweight: true }),
      ),
    ];
  }
  const mock = w === 11;
  ...
  return [
    day('squat_day', 'тяж',
      ...
      ex('hip_thrust', 'Ягодичный мост (камни/тяги)', 'legs', 0.6, [s(0.60, 8, 3)], { base: 'deadlift', baseMult: 1 }),
    ),
    day('overhead_day', 'тяж',
      ex('log_press', mock ? 'Лог — тяжёлый сингл (взятие каждое)' : 'Лог 90-95%', 'strongman', 1.4, [s(0.90, 1, 3), s(top, 1, 1)]),
      ex('pin_press', 'Жим узким (локаут)', 'shoulders', 0.8, [s(0.78, 5, 4)], { base: 'bench' }),
    ),
    day('event_day', 'тяж',

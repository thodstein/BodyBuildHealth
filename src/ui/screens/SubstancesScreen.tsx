...
export const SubstancesScreen: React.FC = () => {
  ...
  return (
    <div className="screen substances">
      <h2>РЎРїСЂР°РІРѕС‡РЅРёРє РІРµС‰РµСЃС‚РІ</h2>
      ...
      <div className="card" style={{ marginBottom: 12 }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
          <button className={'btn secondary' + (selectedClass === 'all' ? ' active' : '')} style={{ fontSize: 11, padding: '4px 10px' }} onClick={() => setSelectedClass('all')}>Р’СЃРµ</button>
          {allClasses.map(cls => (
      ...
      <div style={{ fontSize: 12, color: 'var(--text-dim)', marginBottom: 8 }}>
        РќР°Р№РґРµРЅРѕ: {filteredSubstances.length} РІРµС‰РµСЃС‚РІ
      </div>
            ...
            <div key={s.id} className="card" style={{ margin: 0, cursor: 'pointer', borderLeft: `3px solid ${color}` }} onClick={() => toggleExpand(s.id)}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                  ...
                  <span style={{ fontSize: 16, color: 'var(--text-dim)' }}>{isExpanded ? 'в–І' : 'в–ј'}</span>
                </div>
              </div>
                ...
                <div style={{ marginTop: 10 }} onClick={e => e.stopPropagation()}>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, fontSize: 12, marginBottom: 8 }}>
                    <div style={{ background: 'var(--bg-secondary)', borderRadius: 8, padding: 8, textAlign: 'center' }}>
                      <div style={{ color: 'var(--text-dim)', fontSize: 10 }}>TВЅ (С‡Р°СЃРѕРІ)</div>
                      <div style={{ fontWeight: 700, fontSize: 16, color }}>{s.pk.halfLifeHours}</div>
                    </div>
                    <div style={{ background: 'var(--bg-secondary)', borderRadius: 8, padding: 8, textAlign: 'center' }}>
                      <div style={{ color: 'var(--text-dim)', fontSize: 10 }}>Р‘РёРѕРґРѕСЃС‚СѓРїРЅРѕСЃС‚СЊ</div>
                      <div style={{ fontWeight: 700, fontSize: 16 }}>{(s.pk.bioavailability * 100).toFixed(0)}%</div>
                    </div>
                  </div>

                  <div style={{ fontWeight: 600, fontSize: 12, marginBottom: 6, color: 'var(--accent)' }}>Р¤Р°СЂРјР°РєРѕРґРёРЅР°РјРёРєР°</div>
                  <div style={{ display: 'grid', gap: 4 }}>
                    {[
                      { label: 'AR СЃСЂРѕРґСЃС‚РІРѕ', value: s.pd.AR_affinity },
                      { label: '', value: s.pd.aromatization },
                      { label: '5-О± СЂРµРґСѓРєС‚Р°Р·Р°', value: s.pd.five_alpha_reduction },
                  ...
                  <div style={{ marginTop: 8, fontSize: 11, color: 'var(--text-dim)' }}>
                    EC50: {s.ec50} В· РҐРёР»Р»: {s.n_hill} В· РњР°РєСЃ. СЌС„С„РµРєС‚: {(s.maxEffect * 100).toFixed(0)}%
                  </div>
                </div>
              )}
            </div>
      ...
      {filteredSubstances.length === 0 && (
        <div className="card" style={{ textAlign: 'center', padding: 24 }}>
          ...
          <div style={{ color: 'var(--text-dim)' }}>РќРёС‡РµРіРѕ РЅРµ РЅР°Р№РґРµРЅРѕ. РџРѕРїСЂРѕР±СѓР№С‚Рµ РґСЂСѓРіРѕР№ Р·Р°РїСЂРѕСЃ.</div>
        </div>
      )}
    </div>
  );

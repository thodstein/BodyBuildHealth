...
export function calculateSleepDebt(entries: SleepEntry[], targetHours: number) {
  const safeTarget = Number.isFinite(targetHours) ? targetHours : 8;
  const totalDebt = entries.reduce((sum, entry) => sum + Math.max(0, safeTarget - entry.hours), 0);
  const avgDebtPerDay = entries.length ? totalDebt / entries.length : 0;
  const recommendations: string[] = [];
  if (avgDebtPerDay > 0.5) recommendations.push('Увеличьте продолжительность сна и время в постели');
  if (
    entries.some(
      (entry) =>
        (entry as SleepEntry & { latency?: number }).latency !== undefined &&
        (entry as SleepEntry & { latency?: number }).latency! > 30,
    )
  ) {
    recommendations.push('Работайте над улучшением засыпание: стабильный режим и меньше экранов');
  }
  return { totalDebt, avgDebtPerDay, recommendations };
}

export function calculateGoalsProgress(entries: SleepEntry[], goals: SleepGoals, days = 7) {
  if (!entries.length) {
    return {
      hoursProgress: 0,
      qualityProgress: 0,
      latencyProgress: 0,
      awakeningsProgress: 0,
      overallScore: 0,
      metGoals: [],
      missedGoals: [],
    };
  }
  const recent = entries.slice(0, Math.max(1, days));
  const average = (values: number[]) => values.reduce((sum, value) => sum + value, 0) / values.length;
  const hours = average(recent.map((entry) => entry.hours));
  const quality = average(recent.map((entry) => entry.quality));
  const latencyValues = recent
    .map((entry) => (entry as SleepEntry & { latency?: number }).latency)
    .filter((value): value is number => Number.isFinite(value));
  const awakenings = average(recent.map((entry) => entry.awakenings));
  const hoursProgress = Math.min(100, Math.round((hours / goals.targetHours) * 100));
  const qualityProgress = Math.min(100, Math.round((quality / goals.targetQuality) * 100));
  const latencyProgress = latencyValues.length
    ? Math.min(100, Math.round((goals.targetLatency / average(latencyValues)) * 100))
    : 100;
  const awakeningsProgress = Math.min(100, Math.round((goals.targetAwakenings / Math.max(0.1, awakenings)) * 100));
  const checks = [
    ['Продолжительность сна', hoursProgress >= 100],
    ['Качество сна', qualityProgress >= 100],
    ['Засыпание', latencyProgress >= 100],
    ['Пробуждения', awakenings <= goals.targetAwakenings],
  ] as const;
  const metGoals = checks.filter(([, met]) => met).map(([name]) => name);
  const missedGoals = checks.filter(([, met]) => !met).map(([name]) => name);
  return {
    hoursProgress,
    qualityProgress,
    latencyProgress,
    awakeningsProgress,
    overallScore: Math.round((hoursProgress + qualityProgress + latencyProgress + awakeningsProgress) / 4),
    metGoals,
    missedGoals,
  };
}
...
function loadSleepDiary(): SleepEntry[] {
  try {
    return JSON.parse(localStorage.getItem(SLEEP_DIARY_KEY) || '[]');
  } catch {
    return [];
  }
}
...
function calcSleepTrend(diary: SleepEntry[]): {
  ...
  const hCV =
    diary.length >= 7 ? std(diary.slice(0, 7).map((e) => e.hours)) / avg(diary.slice(0, 7).map((e) => e.hours)) : 0;
  const cs =
    diary.length >= 3
      ? Math.max(
          0,
          Math.min(
            ...
            Math.round(
              (1 - hCV) * 50 +
                (avg(diary.slice(0, 7).map((e) => e.quality)) / 10) * 30 +
                (1 - Math.min(avg(diary.slice(0, 7).map((e) => e.awakenings)) / 5, 1)) * 20,
            ),
          ),
        )
      : 0;
...
function weeklyAverages(
  diary: SleepEntry[],
): { weekLabel: string; avgHours: number; avgQuality: number; avgAwakenings: number; days: number }[] {
...
function dayOfWeekAvg(diary: SleepEntry[]): { dayName: string; avgHours: number; avgQuality: number; count: number }[] {
  ...
  for (const e of diary) {
    ...
    if (day) {
      day.hours.push(e.hours);
      day.quality.push(e.quality);
    }
  }
  return days.map((d) => ({
    dayName: d.dayName,
    ...
    avgQuality:
      d.quality.length > 0 ? Math.round((d.quality.reduce((s, v) => s + v, 0) / d.quality.length) * 10) / 10 : 0,
...
export const SleepDiaryTab: React.FC<{ settings: any; save: (data: any) => void }> = ({ settings, save }) => {
  ...
  const hoursLine =
    chartDays.length > 0
      ? chartDays
          .map((e, i) => `${i === 0 ? 'M' : 'L'}${xScale(i).toFixed(1)},${yScaleHours(e.hours).toFixed(1)}`)
          .join(' ')
      : '';
  const qualityLine =
    chartDays.length > 0
      ? chartDays
          .map((e, i) => `${i === 0 ? 'M' : 'L'}${xScale(i).toFixed(1)},${yScaleQuality(e.quality).toFixed(1)}`)
          .join(' ')
      : '';
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      ...
      {showForm && (
        <div style={glassCard}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 8 }}>
            <div>
              ...
              <input
                ...
                style={{ width: '100%' }}
              />
              <div style={{ fontSize: 11, fontWeight: 700, color: qualityColor(quality), textAlign: 'center' }}>
                {quality}/10
              </div>
            </div>
            <div>
              ...
              <input
                ...
                style={{ width: '100%' }}
              />
              <div style={{ fontSize: 11, fontWeight: 700, color: apple.textPrimary, textAlign: 'center' }}>
                {awakenings}
              </div>
            </div>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 8 }}>
            <div>
              <div style={{ fontSize: 9, color: apple.textDim, marginBottom: 3 }}>Засыпание</div>
              <input
                type="time"
                value={bedtime}
            ...
            <div>
              <div style={{ fontSize: 9, color: apple.textDim, marginBottom: 3 }}>Подъём</div>
              <input
                type="time"
                value={wakeTime}
          ...
          <div style={{ marginBottom: 8 }}>
            ...
            <input
              ...
              placeholder="Стресс, кофеин, алкоголь..."
              style={{
                width: '100%',
                padding: '8px 10px',
                borderRadius: 8,
          ...
          <div style={{ display: 'flex', gap: 6 }}>
            ...
            <button
              onClick={() =>
                save({
                  baselineSleepHours: hours,
                  baselineSleepQuality: quality,
      ...
      {viewMode === 'stats' && diary.length > 0 && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          <div style={glassCard}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6 }}>
              <TrendCard
                icon="🕐"
                label="Часы"
                value={`${trend.hoursDiff >= 0 ? '+' : ''}${trend.hoursDiff}ч`}
                trend={trend.hoursTrend}
                goodUp
              />
              <TrendCard
                icon="⭐"
                label="Качество"
                value={`${trend.qualityDiff >= 0 ? '+' : ''}${trend.qualityDiff}`}
                trend={trend.qualityTrend}
                goodUp
              />
            </div>
            <div style={{ marginTop: 6, display: 'flex', gap: 8, fontSize: 9, color: apple.textDim, flexWrap: 'wrap' }}>
              <span>
                🌙 Пробуждений: ср. {trend.avgAwakenings} ({trend.awakeningsDiff >= 0 ? '+' : ''}
                {trend.awakeningsDiff})
              </span>
              <span>📅 Всего записей: {diary.length}</span>
              {trend.consistencyScore > 0 && (
                <span>
                  🎯 Консистентность: {trend.consistencyScore}%{' '}
                  {trend.consistencyScore >= 70 ? '✅' : trend.consistencyScore >= 40 ? '⚠️' : '🔴'}
                </span>
              )}
            </div>
          </div>
          ...
          {weekly.length >= 2 && (
            <div style={glassCard}>
              <div style={{ fontSize: 10, fontWeight: 700, color: apple.textPrimary, marginBottom: 6 }}>
                📅 Средние по неделям
              </div>
                ...
                {dowAvg.map((d, i) => (
                  <div
                    ...
                    style={{
                      ...
                      padding: '4px 2px',
                      borderRadius: 6,
                      background: d.count > 0 ? 'rgba(0,230,138,0.04)' : 'transparent',
                    }}
                  >
                    <div style={{ fontSize: 7, color: apple.textDim }}>{d.dayName}</div>
                    <div style={{ fontSize: 11, fontWeight: 700, color: d.count > 0 ? apple.accent : apple.textDim }}>
                      {d.avgHours > 0 ? `${d.avgHours}ч` : '—'}
                    </div>
                    <div style={{ fontSize: 7, color: d.count > 0 ? qualityColor(d.avgQuality) : apple.textDim }}>
                      {d.avgQuality > 0 ? `⭐${d.avgQuality}` : ''}
                    </div>
                  </div>
                ))}
              </div>
            ...
            <svg viewBox={`0 0 ${chartW} ${chartH}`} style={{ width: '100%', height: chartH }}>
              {hoursYTicks.map((t) => (
                <text
                  ...
                  y={yScaleHours(t) + 3}
                  textAnchor="end"
                  fill="rgba(0,230,138,0.35)"
                  fontSize={6}
                >
                  {t}
                </text>
              ))}
              <text x={padL - 4} y={yScaleHours(9) - 4} textAnchor="end" fill="rgba(0,230,138,0.35)" fontSize={5}>
                часы
              </text>
              {qualityYTicks
                .filter((t) => t !== 0)
              ...
              {chartDays.length >= 2 && (
                <path
                  d={`${hoursLine} L${xScale(chartDays.length - 1)},${yScaleHours(yMinH)} L${xScale(0)},${yScaleHours(yMinH)} Z`}
              ...
              {chartDays.length > 14 && (
                <>
                  <text x={xScale(0)} y={chartH - 2} textAnchor="middle" fill="rgba(255,255,255,0.2)" fontSize={6}>
                    {chartDays[0].date.slice(5)}
                  </text>
                  <text
                    x={xScale(chartDays.length - 1)}
                    y={chartH - 2}
                    textAnchor="middle"
                    fill="rgba(255,255,255,0.2)"
                    fontSize={6}
                  >
                    {chartDays[chartDays.length - 1].date.slice(5)}
                  </text>
                </>
              )}
            </svg>
              ...
              <span>
                <span
                  style={{
                    ...
                    marginRight: 3,
                    verticalAlign: 'middle',
                    borderTop: '1px dashed rgba(0,230,138,0.15)',
                  }}
                />{' '}
                7ч
              </span>

        ...
        <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
          ...
          {plan.weeksData.map(wk => {
            ...
            <div key={wk.week} data-ss="week" style={{ ...CARD, padding:0, overflow:'hidden', borderColor: border, background: isOpen ? 'linear-gradient(180deg, rgba(26,24,38,0.82), rgba(18,16,28,0.66))' : CARD.background }}>
              <button onClick={()=> setExpandedWeek(isOpen? null : wk.week-1)} style={{ width:'100%', display:'flex', alignItems:'center', gap:12, padding:'16px 18px', background: wk.deload? 'rgba(245,158,11,0.07)' : (wk as any).taper? 'rgba(59,130,246,0.07)':'transparent', border:'none', cursor:'pointer', textAlign:'left', minHeight:72 }}>
                      ...
                      {sess.exercises.map(ex => (
                        <div key={ex.id} data-ss="exercise" style={{ background:'linear-gradient(180deg, rgba(255,255,255,0.05), rgba(255,255,255,0.02))', border:'0.5px solid rgba(255,255,255,0.08)', borderLeft:'3px solid rgba(0,230,138,0.35)', borderRadius:16, padding:14, display:'flex', flexDirection:'column', gap:10 }}>
                          ...
                          <div style={{ display:'grid', gridTemplateColumns:'76px 76px 76px auto', gap:8, alignItems:'center' }}>
                            <input type="number" value={ex.weight} onChange={e=> updateEx(wk.week-1, sess.day, ex.id, { weight: Number(e.target.value)||0 })} style={{ ...INPUT, padding:'12px 8px', fontSize:15, fontWeight:700, textAlign:'center', fontVariantNumeric:'tabular-nums', minHeight:52 }} placeholder="кг" />
                            <input type="text" value={ex.reps} onChange={e=> updateEx(wk.week-1, sess.day, ex.id, { reps: e.target.value })} style={{ ...INPUT, padding:'12px 8px', fontSize:15, fontWeight:700, textAlign:'center', minHeight:52 }} placeholder="повт" />
                            <input type="number" value={ex.rir} onChange={e=> updateEx(wk.week-1, sess.day, ex.id, { rir: Number(e.target.value)||0 })} style={{ ...INPUT, padding:'12px 8px', fontSize:15, fontWeight:700, textAlign:'center', fontVariantNumeric:'tabular-nums', minHeight:52 }} placeholder="RIR" />
                            <div style={{ display:'flex', gap:6 }}><button onClick={()=> moveEx(wk.week-1, sess.day, ex.id, -1)} style={{ width:46, height:52, borderRadius:12, background:'rgba(255,255,255,0.06)', border:'0.5px solid rgba(255,255,255,0.09)', color:'#fff', cursor:'pointer', fontSize:15 }}>↑</button><button onClick={()=> moveEx(wk.week-1, sess.day, ex.id, 1)} style={{ width:46, height:52, borderRadius:12, background:'rgba(255,255,255,0.06)', border:'0.5px solid rgba(255,255,255,0.09)', color:'#fff', cursor:'pointer', fontSize:15 }}>↓</button></div>
                          </div>
                          ...
                          {ex.warmupSets && ex.warmupSets.length>0 && <div style={{ fontSize:10.5, color:TEXT_3, fontFamily:'-apple-system, system-ui, sans-serif' }}>Разминка: {ex.warmupSets.map(s=> `${s.reps}×${s.weight}кг`).join(' → ')} → рабочие</div>}
                            <div style={{ display:'flex', gap:6, flexWrap:'wrap' }}>
                              ...
                              <span key={si} data-ss="set-row" style={{ display:'flex', gap:6, alignItems:'center', background:'rgba(255,255,255,0.045)', padding:'8px 10px', borderRadius:12, fontSize:12, fontWeight:600, color:'#fff', border:'0.5px solid rgba(255,255,255,0.07)', fontVariantNumeric:'tabular-nums', minHeight:52 }}>
                                ...
                                <input type="number" value={s.reps} onChange={e=> updateSet(wk.week-1,sess.day,ex.id,si,{reps:Number(e.target.value)||0})} style={{ width:48, minHeight:44, padding:'8px 6px', fontSize:14, fontWeight:700, background:'rgba(255,255,255,0.07)', color:'#fff', border:'0.5px solid rgba(255,255,255,0.12)', borderRadius:10, textAlign:'center', fontVariantNumeric:'tabular-nums' }} />×
                                <input type="number" value={s.rir} onChange={e=> updateSet(wk.week-1,sess.day,ex.id,si,{rir:Number(e.target.value)||0})} style={{ width:46, minHeight:44, padding:'8px 6px', fontSize:14, fontWeight:700, background:'rgba(255,255,255,0.07)', color:'#fff', border:'0.5px solid rgba(255,255,255,0.12)', borderRadius:10, textAlign:'center', fontVariantNumeric:'tabular-nums' }} />RIR
                                <input type="number" step="0.05" placeholder="м/с" value={vbtMap[`${wk.week}-${sess.day}-${ex.id}-${si}`] ?? ''} onChange={e=> { const v=parseFloat(e.target.value); const k=`${wk.week}-${sess.day}-${ex.id}-${si}`; setVbtMap(m=> ({...m, [k]: Number.isFinite(v)?v:0})); }} style={{ width:58, minHeight:44, padding:'8px 6px', fontSize:13, background:'rgba(255,255,255,0.07)', color:'#fff', border:'0.5px solid rgba(255,255,255,0.12)', borderRadius:10, textAlign:'center', fontVariantNumeric:'tabular-nums' }} />
                                {(ex.id.includes('yoke')||ex.id.includes('farmers')||ex.id.includes('carry')||ex.id.includes('husafell')||ex.id.includes('frame')||ex.id.includes('sled')||ex.id.includes('tire')||ex.id.includes('stone')||ex.id.includes('sandbag')) && <>
                                  <input type="number" placeholder="м" title="дистанция м" value={(s as any).distanceM ?? ''} onChange={e=> updateSet(wk.week-1,sess.day,ex.id,si,{distanceM: Number(e.target.value)||0} as any)} style={{ width:54, minHeight:44, padding:'8px 6px', fontSize:13, fontWeight:700, background:'rgba(255,159,10,0.10)', color:'#fff', border:'0.5px solid rgba(255,159,10,0.20)', borderRadius:10, textAlign:'center' }} />
              ...
              <div style={{ display:'flex', gap:4, flexWrap:'wrap' }}>
                {annual.blocks.map((b:any, idx:number)=> (
                  <div key={b.id} style={{ display:'flex', gap:2, alignItems:'center', background:'rgba(255,255,255,0.04)', padding:'4px 6px', borderRadius:8, border:'0.5px solid rgba(255,255,255,0.06)' }}>
                    ...
                    <button disabled={idx===0} onClick={()=> { const n=moveAnnualBlock(annual, idx, idx-1); if(n){ saveAnnualSS(n); setAnnual(n); setMsg(`◀ блок ${idx+1} → ${idx}`); setTimeout(()=>setMsg(''),1500);} }} style={{ ...BTN_SMALL, padding:'8px 12px', fontSize:13, opacity: idx===0?0.4:1 }}>◀</button>
                    <button disabled={idx===annual.blocks.length-1} onClick={()=> { const n=moveAnnualBlock(annual, idx, idx+1); if(n){ saveAnnualSS(n); setAnnual(n); setMsg(`▶ блок ${idx+1} → ${idx+2}`); setTimeout(()=>setMsg(''),1500);} }} style={{ ...BTN_SMALL, padding:'8px 12px', fontSize:13, opacity: idx===annual.blocks.length-1?0.4:1 }}>▶</button>
                  </div>
                ))}
              </div>

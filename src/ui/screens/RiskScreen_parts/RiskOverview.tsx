      ...
      <div className="card">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 6 }}>
          <div style={{ background: 'var(--bg-secondary)', padding: 10, borderRadius: 8, textAlign: 'center' }}>
            <div style={{ fontSize: 10, color: 'var(--text-dim)' }}>Raw</div>
            <div style={{ fontSize: 22, fontWeight: 700, color: getRiskColor(riskResult.overallRaw) }}>{Math.round(riskResult.overallRaw)}%</div>
          </div>
          <div style={{ background: 'var(--bg-secondary)', padding: 10, borderRadius: 8, textAlign: 'center' }}>
            <div style={{ fontSize: 10, color: 'var(--text-dim)' }}>Net</div>
            <div style={{ fontSize: 22, fontWeight: 700, color: overallColor }}>{Math.round(riskResult.overallNet)}%</div>
          </div>
          <div style={{ background: 'var(--bg-secondary)', padding: 10, borderRadius: 8, textAlign: 'center' }}>
            <div style={{ fontSize: 10, color: 'var(--text-dim)' }}>Статус</div>
              ...
              return (
                <div key={risk.id} style={{ background: 'var(--bg-secondary)', padding: '8px 10px', borderRadius: 6, borderLeft: `3px solid ${levelColor}` }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span style={{ fontWeight: 600, fontSize: 12 }}>{risk.name}</span>
                  ...
                  <div style={{ fontSize: 11, color: 'var(--text-dim)', marginTop: 2 }}>{risk.description}</div>
                </div>
              );
            })}
          </div>
            ...
            {relevantRecs.map(rec => {
              ...
              return (
                <div key={rec.recId} style={{ background: 'var(--bg-secondary)', padding: '8px 10px', borderRadius: 6 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span style={{ fontWeight: 600, fontSize: 12 }}>{rec.title}</span>
                  ...
                  <div style={{ fontSize: 11, color: 'var(--text-dim)', marginTop: 2 }}>{rec.text}</div>
                </div>
              );
            })}
          </div>

...
export const RiskScreen: React.FC<{ initialSubTab?: string }> = ({ initialSubTab }) => {
  ...
  return (
    <div className="screen risk" style={{ flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column', overflow: 'auto', padding: 0 }}>
      ...
      {mainTab === 'hero' && (
        <div style={{ position:'fixed', inset:0, width:'100%', height:'100dvh', minHeight:'100dvh', zIndex:100, display:'flex', flexDirection:'column', background:'transparent' }}>
          ...
          <div style={{ position:'absolute', inset:0, background:'linear-gradient(transparent 82%, rgba(0,0,0,0.08) 92%, rgba(0,0,0,0.16) 100%)' }} />
          <div style={{ position:'relative', zIndex:2, flex:1, display:'flex', flexDirection:'column', justifyContent:'flex-end', padding:'12px 12px calc(64px + env(safe-area-inset-bottom,0px))', gap:10, overflowY:'auto' }}>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
              {[
                ...
                <div key={card.id} role="button" tabIndex={0} onClick={() => { setMainTab(card.id as any); setSubTab(card.id === 'info' ? 'info' : card.id === 'tz_spec' ? 'overview' : 'overview'); }} onKeyDown={e=>{ if(e.key==='Enter'||e.key===' '){ e.preventDefault(); (e.currentTarget as HTMLDivElement).click(); }}} onMouseEnter={e=>{ (e.currentTarget as HTMLDivElement).style.transform='translateY(-1px)'; (e.currentTarget as HTMLDivElement).style.borderColor=`${card.color}40`; (e.currentTarget as HTMLDivElement).style.boxShadow=`0 6px 18px rgba(0,0,0,0.32), 0 0 0 1px ${card.color}18 inset`; }} onMouseLeave={e=>{ (e.currentTarget as HTMLDivElement).style.transform='translateY(0)'; (e.currentTarget as HTMLDivElement).style.borderColor='rgba(255,255,255,0.12)'; (e.currentTarget as HTMLDivElement).style.boxShadow='0 3px 12px rgba(0,0,0,0.22)'; }} style={{ display:'flex', alignItems:'center', gap:10, padding:'10px 12px', borderRadius:14, cursor:'pointer', textAlign:'left', width:'100%', border:'1px solid rgba(255,255,255,0.10)', boxShadow:'0 3px 12px rgba(0,0,0,0.22)', background:'rgba(18,18,20,0.28)', backdropFilter:'blur(8px)', WebkitBackdropFilter:'blur(8px)', transition:'transform 0.15s ease, border-color 0.15s ease, box-shadow 0.15s ease, background 0.15s ease' }}>

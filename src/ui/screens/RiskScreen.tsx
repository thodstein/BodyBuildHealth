...
export const getRiskColor = getBarColor;
      ...
      <div className="card" style={{ marginBottom: 16 }}>
        ...
        <div className="risk-overview-grid">
          <div className="risk-overview-item">
            ...
            <div className="risk-overview-value" style={{ color: getBarColor(rawRiskResult.overallRaw) }}>{Math.round(rawRiskResult.overallRaw)}%</div>
          </div>
          <div className="risk-overview-arrow">&rarr;</div>
          <div className="risk-overview-item">
            <div className="risk-overview-label">С поддержкой</div>
            <div className="risk-overview-value" style={{ color: getBarColor(riskResult.overallNet) }}>{Math.round(riskResult.overallNet)}%</div>
          </div>
          <div className="risk-overview-item">
                    ...
                    {RISK_MECHANISMS.map((m) => {
                      ...
                      return (
                        <td key={m.id} className={isSelected ? 'risk-matrix-cell risk-matrix-cell-selected' : 'risk-matrix-cell'} style={{ backgroundColor: colors.bg, cursor: 'pointer' }} onClick={() => { setSelectedCell({ system, mechanism: m.id }); setSelectedSystem(system); setTab('detail'); }}>
                          <div className="risk-matrix-cell-raw" style={{ color: getBarColor(cell.raw), fontSize: '0.65em' }}>{cell.raw.toFixed(1)}%</div>
                          <div className="risk-matrix-cell-net" style={{ color: colors.text, fontWeight: 'bold' }}>{cell.net.toFixed(1)}%</div>
                        </td>
                      );
                    })}
                    <td className="risk-matrix-total" style={{ color: getBarColor(sysRaw) }}>{Math.round(sysRaw)}%</td>
                    <td className="risk-matrix-total" style={{ color: getBarColor(sysNet) }}>{Math.round(sysNet)}%</td>
                  </tr>
                );
              })}
            </tbody>
            ...
            return (
              <div key={system} className="card" style={{ marginBottom: 8, cursor: 'pointer' }} onClick={() => setSelectedSystem(isExpanded ? null : system)}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <span style={{ fontSize: 18, fontWeight: 700, color: getBarColor(sysRaw) }}>{Math.round(sysRaw)}%</span>
                    <span style={{ color: 'var(--text-dim)', fontSize: 12 }}>→</span>
                    <span style={{ fontSize: 18, fontWeight: 700, color: getBarColor(sysNet) }}>{Math.round(sysNet)}%</span>
                      ...
                      return (
                        <div key={m.id} style={{ background: 'var(--bg-primary)', borderRadius: 6, padding: '8px 10px', marginBottom: 4 }}>
                          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', cursor: 'pointer' }} onClick={() => setSelectedCell({ system, mechanism: m.id })}>
                            <div>
                              <span style={{ fontWeight: 600, fontSize: 12, color: getBarColor(cell.net) }}>{m.label}</span>
                            ...
                            <div style={{ marginTop: 6, fontSize: 11, lineHeight: 1.5, color: 'var(--text-light)' }}>
                              ...
                              <div style={{ marginTop: 4 }}>
                                <div style={{ display: 'flex', gap: 8, fontSize: 11 }}>
                                  <span>Сыр. риск: <strong style={{ color: getBarColor(cell.raw) }}>{cell.raw.toFixed(1)}%</strong></span>
                                  <span>Нетто: <strong style={{ color: getBarColor(cell.net) }}>{cell.net.toFixed(1)}%</strong></span>
                ...
                <div className="risk-comparison-bars">
                  <div className="risk-comparison-bar-container">
                    ...
                    <div className="risk-comparison-bar-track">
                      <div className="risk-comparison-bar-fill" style={{ width: Math.min(100, raw) + '%', backgroundColor: getBarColor(raw) }} />
                    </div>
                  ...
                  <div className="risk-comparison-bar-container">
                    ...
                    <div className="risk-comparison-bar-track">
                      <div className="risk-comparison-bar-fill" style={{ width: Math.min(100, net) + '%', backgroundColor: getBarColor(net) }} />
                    </div>

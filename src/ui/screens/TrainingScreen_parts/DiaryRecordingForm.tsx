    ...
    <div className="train-recordform" style={{ padding: 12, borderRadius: 14, background: 'rgba(24,24,27,0.42)', backdropFilter: 'blur(12px)' as any, border: '1px solid rgba(255,255,255,0.04)', marginBottom: 8 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
        ...
        <div style={{ display: 'flex', gap: 4 }}>
          {undoRef.current.length > 0 && (
            <button onClick={undo} style={{ padding: '3px 8px', borderRadius: 5, border: '1px solid rgba(255,255,255,0.1)', background: 'transparent', color: '#fff', cursor: 'pointer', fontSize: 10 }}>↩</button>
          )}
          {totalSets > 0 && <span style={{ fontSize: 10, color: '#fff' }}>{totalSets} сетов · {totalVolume.toLocaleString()} кг</span>}
        </div>
      </div>

      {/* Session fields */}
      <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr 1fr' : '1fr 1fr 1fr', gap: 6, marginBottom: 6 }}>
        <div><label style={{ fontSize: 10, color: '#fff' }}>Дата</label><input type="date" value={logDate} onChange={e => setLogDate(e.target.value)} style={style.input} /></div>
        <div><label style={{ fontSize: 10, color: '#fff' }}>Длительность (мин)</label><input type="number" min={1} value={logDuration} onChange={e => setLogDuration(parseInt(e.target.value) || 0)} style={style.input} /></div>
        {!isMobile && <div><label style={{ fontSize: 10, color: '#fff' }}>RPE (1-10)</label><input type="number" min={1} max={10} value={logRPE} onChange={e => setLogRPE(parseInt(e.target.value) || 5)} style={style.input} /></div>}
      </div>
      {isMobile && (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6, marginBottom: 6 }}>
          <div><label style={{ fontSize: 10, color: '#fff' }}>RPE (1-10)</label><input type="number" min={1} max={10} value={logRPE} onChange={e => setLogRPE(parseInt(e.target.value) || 5)} style={style.input} /></div>
          <div><label style={{ fontSize: 10, color: '#fff' }}>Восстановление</label><input type="number" min={1} max={10} value={logRecovery} onChange={e => setLogRecovery(parseInt(e.target.value) || 5)} style={style.input} /></div>
        </div>
      )}
      <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr 1fr' : '1fr 1fr 1fr', gap: 6, marginBottom: 6 }}>
        {!isMobile && <div><label style={{ fontSize: 10, color: '#fff' }}>Восстановление (1-10)</label><input type="number" min={1} max={10} value={logRecovery} onChange={e => setLogRecovery(parseInt(e.target.value) || 5)} style={style.input} /></div>}
        ...
        <div><label style={{ fontSize: 10, color: '#fff' }}>Самочувствие</label>
        ...
        <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap', marginTop: 4 }}>
          {[
            ...
            <button key={qt.tag} type="button" onClick={() => {
              ...
              color: logNotes.includes(qt.tag) ? '#00e68a' : '#fff',
      ...
      {draftRestored && exercises.length > 0 && (
        <div style={{ padding: '6px 10px', borderRadius: 8, background: 'rgba(96,165,250,0.1)', border: '1px solid rgba(96,165,250,0.2)', marginBottom: 8, fontSize: 10, color: '#60a5fa', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <button onClick={() => { setExercises([]); clearDraft(); }} style={{ background: 'none', border: 'none', color: '#fff', cursor: 'pointer', fontSize: 10 }}>✕ очистить</button>
        </div>
      )}
      ...
      {planApplied && exercises.length > 0 && (
        <div style={{ padding: '6px 10px', borderRadius: 8, background: 'rgba(0,230,138,0.08)', border: '1px solid rgba(0,230,138,0.25)', marginBottom: 8, fontSize: 10, color: '#00e68a', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <button onClick={() => { setExercises([]); setPlanApplied(false); }} style={{ background: 'none', border: 'none', color: '#fff', cursor: 'pointer', fontSize: 10 }}>✕</button>
        </div>
      )}
      ...
      <div style={{ display: 'flex', gap: 3, flexWrap: 'wrap', marginBottom: 6 }}>
        {[
          ...
          <button key={g.id} type="button" onClick={() => {
            ...
            color: searchQuery === g.label ? '#00e68a' : '#fff',
          ...
          <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, background: '#18181b', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 10, zIndex: 100, maxHeight: 200, overflowY: 'auto', marginTop: 4 }}>
            {searchResults.map(ex => (
              <div key={ex.id} onClick={() => addExercise(ex)} style={{ padding: '10px 12px', cursor: 'pointer', borderBottom: '1px solid rgba(255,255,255,0.04)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <span style={{ fontSize: 10, color: '#fff' }}>{ex.group}</span>
              </div>
            ))}
          </div>
        )}
              ...
              {recent.map(r => {
                ...
                return (
                  <button key={r.name} onClick={() => { const ex = EXERCISE_CATALOG.find((c: any) => c.name === r.name); if (ex) addExercise(ex); }}
                    style={{ padding: '4px 8px', borderRadius: 6, border: '1px solid rgba(255,255,255,0.08)', background: 'rgba(255,255,255,0.03)', color:'#fff', cursor: 'pointer', fontSize: 10 }}>
                    {r.name}
                  </button>
                );
              })}
      ...
      {exercises.length === 0 && (
        <div style={{ textAlign: 'center', padding: 24, borderRadius: 12, background: 'rgba(255,255,255,0.02)', border: '1px dashed rgba(255,255,255,0.08)', marginBottom: 8 }}>
          ...
          <div style={{ fontSize: 11, color: '#fff' }}>Начните с поиска упражнения выше</div>
        </div>
      )}
      ...
      {exercises.length >= 2 && (
        <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 4, gap: 6 }}>
          <button onClick={() => setCurrentExIdx(-1)} style={{ fontSize: 9, color: '#fff', background: 'none', border: 'none', cursor: 'pointer', padding: '2px 4px' }}>▾ Все свернуть</button>
          <button onClick={() => setCurrentExIdx(0)} style={{ fontSize: 9, color: '#fff', background: 'none', border: 'none', cursor: 'pointer', padding: '2px 4px' }}>▸ Первое</button>
        </div>
      )}
        ...
        return (
          <div key={exIdx} style={{
              ...
              <div style={{ display: 'flex', gap: 4 }}>
                <button onClick={e => {
                  ...
                  style={{
                    ...
                    color: ex.notes !== undefined ? '#a855f7' : '#fff',
                ...
                {exIdx > 0 && (
                  <button onClick={e => { e.stopPropagation(); toggleSuperset(exIdx); }}
                    style={{
                      ...
                      color: ex.isSuperset ? '#a855f7' : '#fff',
            ...
            {isCurrent && ex.notes !== undefined && (
              <div style={{ padding: '0 10px 4px' }}>
                <input type="text" value={ex.notes || ''}
                  ...
                  style={{ width: '100%', padding: '4px 8px', borderRadius: 5, background: 'transparent', border: '1px solid rgba(168,85,247,0.15)', color: '#fff', fontSize: 10, boxSizing: 'border-box' as any }} />
              </div>
            )}
            ...
            {isCurrent && !prev && ex.sets.length <= 1 && (() => {
              ...
              return (
                <div style={{ padding: '4px 10px', marginBottom: 4, fontSize: 9, color: '#fff', background: 'rgba(255,255,255,0.02)', borderRadius: 6, border: '1px dashed rgba(255,255,255,0.06)' }}>
                  💡 {suggested.note}
                </div>
              );
            })()}
            ...
            {isCurrent && (
              <div style={{ padding: '0 10px 8px' }}>
                ...
                {/* Set header */}
                <div style={{ display: 'grid', gridTemplateColumns: '20px 1fr 1fr 1fr 1fr 96px', gap: 3, marginBottom: 4, fontSize: 10, color: '#fff', textTransform: 'uppercase', paddingLeft: 2 }}>
                  ...
                  <React.Fragment key={setIdx}>
                      ...
                      <div style={{ display: 'flex', gap: 2 }}>
                        <button onClick={() => setMmcOpen(prev => ({ ...prev, [`${exIdx}_${setIdx}`]: !prev[`${exIdx}_${setIdx}`] }))}
                          style={{ width: 22, height: 22, borderRadius: 4, border: 'none', cursor: 'pointer', fontSize: 9,
                            ...
                            color: mmcOpen[`${exIdx}_${setIdx}`] ? '#00e68a' : '#fff' }}
                        ...
                        <button onClick={() => {
                          ...
                          style={{ width: 22, height: 22, borderRadius: 4, border: 'none', cursor: 'pointer', fontSize: 10,
                            ...
                            color: set.technique ? (set.technique === 5 ? '#22c55e' : set.technique === 4 ? '#60a5fa' : '#ef4444') : '#fff' }}
                        ...
                        <button onClick={() => {
                          ...
                          style={{ width: 22, height: 22, borderRadius: 4, border: 'none', background: set.notes ? 'rgba(168,85,247,0.15)' : 'rgba(255,255,255,0.05)', color: set.notes ? '#a855f7' : '#fff', cursor: 'pointer', fontSize: 9 }}>📝</button>
                    ...
                    {set.notes !== undefined && (
                      <input type="text" value={set.notes || ''}
                        ...
                        style={{ width: '100%', padding: '3px 6px', borderRadius: 4, background: 'transparent', border: '1px solid rgba(168,85,247,0.15)', color: '#fff', fontSize: 9, marginBottom: 3, boxSizing: 'border-box' as any }} />
                    )}
                ...
                <div style={{ display: 'flex', gap: 4, marginTop: 4 }}>
                  ...
                  <button onClick={() => setRestTimer(restTarget)}
                    style={{ padding: '6px 10px', borderRadius: 6, border: '1px solid rgba(255,255,255,0.1)', background: 'transparent', color: '#fff', cursor: 'pointer', fontSize: 10 }}>⏱ {restTarget}с</button>
                </div>
              </div>
            )}
          </div>
          ...
          {[60, 90, 120, 180].map(sec => (
            <button key={sec} onClick={() => setRestTarget(sec)}
              style={{
                ...
                color: restTarget === sec ? ACCENT : '#fff',
      ...
      {savedSummary ? (
        <div style={{ padding: 10, borderRadius: 10, background: 'rgba(0,230,138,0.06)', border: '1px solid rgba(0,230,138,0.2)', marginBottom: 8 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 4, fontSize: 10, color:'#fff' }}>
            <div><span style={{ color: '#fff' }}>Упражнения:</span> <strong style={{ color: '#fff' }}>{savedSummary.exercises}</strong></div>
            <div><span style={{ color: '#fff' }}>Подходы:</span> <strong style={{ color: '#fff' }}>{savedSummary.sets}</strong></div>
            <div><span style={{ color: '#fff' }}>Тоннаж:</span> <strong style={{ color: '#fff' }}>{savedSummary.volume.toLocaleString()} кг</strong></div>
          </div>
        </div>
      ) : (
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8, fontSize: 10, color: '#fff' }}>

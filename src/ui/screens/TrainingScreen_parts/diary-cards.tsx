            ...
            <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
              ...
              {confirmDeleteId === workout.id ? (
                <span style={{ display: 'flex', gap: 4 }}>
                  ...
                  <button onClick={e => { e.stopPropagation(); onCancelDelete?.(); }} style={{ padding: '2px 6px', borderRadius: 4, background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.15)', color: '#fff', cursor: 'pointer', fontSize: 10 }}>✕</button>
                </span>
              ) : (
                ...
                <div key={exercise.id} style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 10 }}>
                  ...
                  {deltaPct !== null && (
                    <span style={{
                      ...
                      color: isPR ? '#22c55e' : delta === 0 ? '#fff' : '#ef4444',
                      fontSize: 10,
                    }}>
                      {isPR ? '+' : ''}{deltaPct}%
                    </span>
        ...
        return (
          <div key={item.key} style={{ marginBottom: 6 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 2 }}>
              <span style={{ fontSize: 10, color: '#fff' }}>{item.label}</span>
              <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                <span style={{ fontSize: 10, fontWeight: 700, color: p >= 100 ? '#22c55e' : '#fff' }}>
                  ...
                  <span style={{ color: '#fff', fontWeight: 400 }}> / {item.key === 'tonnage' ? (item.target / 1000).toFixed(1) + 'т' : item.target}</span>
                </span>
                <input type="number" value={item.target} onChange={e => saveTarget({ [item.key]: Math.max(1, +e.target.value || 1) })}
                  style={{ width: 40, padding: '1px 3px', borderRadius: 3, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.06)', color: '#fff', fontSize: 9, textAlign: 'right' }} />
              </div>
            </div>
...
export const ProgressChartsCard: React.FC<{ historyWorkouts: WorkoutLog[] }> = ({ historyWorkouts }) => {
  ...
  return (
    <div className="card" style={{ padding: 10, marginTop: 8 }}>
      ...
      {top.length === 0 ? (
        <div style={{ fontSize: 10, color: '#fff' }}>Недостаточно данных (нужно ≥2 тренировок на упражнение с весами).</div>
      ) : (
        <>
  ...
  return (
    <div style={style.card}>
      ...
      <div style={{ display: 'flex', gap: 6, marginBottom: 6 }}>
        ...
        <span style={{ fontSize: 10, color: '#fff' }}>vs</span>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr auto 1fr', gap: 4, fontSize: 10, marginBottom: 4 }}>
        <div style={{ textAlign: 'center' }}>
          ...
          <div style={{ fontSize: 8, color: '#fff' }}>объём</div>
        </div>
        <div style={{ color: volA > volB ? '#22c55e' : volA < volB ? '#ef4444' : '#fff', fontSize: 10 }}>
          {volA > volB ? '◀' : volA < volB ? '▶' : '='}
        </div>
        <div style={{ textAlign: 'center' }}>
        ...
        <div style={{ textAlign: 'center' }}>
          ...
          <div style={{ fontSize: 8, color: '#fff' }}>сетов</div>
        </div>
        <div style={{ color: setsA > setsB ? '#22c55e' : setsA < setsB ? '#ef4444' : '#fff' }}>
      ...
      {shared.length > 0 && <div style={{ fontSize: 9, color: '#fff' }}>🔄 Совпадение: {shared.join(', ')}</div>}
  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 4 }}>
        {alts.map(ex => (
          <div key={ex} onClick={() => setSelEx(selEx === ex ? '' : ex)} style={{ padding: '5px 8px', borderRadius: 6, background: selEx === ex ? 'rgba(0,230,138,0.1)' : 'rgba(255,255,255,0.03)', border: `1px solid ${selEx === ex ? 'rgba(0,230,138,0.25)' : 'rgba(255,255,255,0.06)'}`, cursor: 'pointer', fontSize: 10, color: selEx === ex ? ACCENT : '#fff' }}>{ex}</div>
        ))}
      </div>
    </div>
  );
...
export const SectionHeader: React.FC<{ icon: string; title: string; hint?: string }> = ({ icon, title, hint }) => (
  <div style={{ display: 'flex', alignItems: 'center', gap: 8, margin: '10px 2px 6px' }}>
    ...
    {hint && <span style={{ fontSize: 9, color: '#fff' }}>{hint}</span>}
  </div>
);
  ...
  return (
    <div style={style.card}>
      ...
      <div style={{ marginBottom: 6 }}>
        <label style={{ fontSize: 9, color: '#fff' }}>Рабочий вес (кг)</label>
      ...
      {ramp.length === 0 ? (
        <div style={{ fontSize: 10, color: '#fff' }}>Введите рабочий вес — появится разминочная рампа.</div>
      ) : (
        <div style={{ display: 'grid', gap: 2 }}>
          {ramp.map((r, i) => (
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 10, padding: '2px 0' }}>
              <span style={{ minWidth: 30, color: i === ramp.length - 1 ? ACCENT : '#fff', fontWeight: i === ramp.length - 1 ? 700 : 400 }}>{r.bar ? '—' : `${Math.round(r.pct * 100)}%`}</span>
              <span style={{ flex: 1, color:'#fff' }}>{r.bar ? 'Пустой гриф (20 кг)' : `${r.load}кг`}</span>
              <span style={{ color: '#fff' }}>{r.reps} повт</span>
            </div>
          ))}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
        {weekTotals.map(row => (
          <div key={row.id} style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 10 }}>
            <span style={{ minWidth: 118, color: '#fff', fontWeight: 600 }}>
              {row.icon} {row.label}
            </span>
            ...
            <span style={{ minWidth: 24, textAlign: 'right', color: row.count >= 3 ? '#22c55e' : row.count > 0 ? '#f59e0b' : '#fff', fontWeight: 700 }}>
              {row.count}/7
            </span>
          </div>
        ))}

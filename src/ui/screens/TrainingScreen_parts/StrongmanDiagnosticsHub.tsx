...
export const StrongmanDiagnosticsHub: React.FC = () => {
  ...
  return (
    <div className="train-strongdiag" style={{ padding: '14px 12px 22px', color: '#fff', maxWidth: 880, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 14 }}>
      <style>{`.train-strongdiag input[type="checkbox"]{ width:22px; height:22px; flex-shrink:0; accent-color:#f59e0b; cursor:pointer; }`}</style>
      <div style={{ ...CARD, padding: '18px 18px 16px', background: 'linear-gradient(135deg,rgba(239,68,68,0.12),rgba(245,158,11,0.12))', border: '1px solid rgba(239,68,68,0.22)', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 8 }}>
          <div style={{ width: 52, height: 52, borderRadius: 15, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(135deg,#ef4444,#f59e0b)', color: '#fff', fontWeight: 900, fontSize: 24, flexShrink: 0, boxShadow: '0 6px 20px rgba(239,68,68,0.35), inset 0 1px 0 rgba(255,255,255,0.25)' }}>🏋️‍♂️</div>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: 19, fontWeight: 800, color: '#fff', lineHeight: 1.1, letterSpacing: -0.4 }}>Стронгмен-диагностика — хаб PRO</div>
            <div style={{ fontSize: 12.5, color: '#fff', lineHeight: 1.45, opacity: 0.85, marginTop: 4 }}>16 фаз (лог 4 + carry 6 + stone 3 + grip/core/cond) × углы + биомеханика + VBT carry 15% + LVP + OHS 6 + hold + grip tri-modal + sway + симулятор + попытки + физика.</div>
          </div>
          <div style={{ textAlign: 'center', flexShrink: 0 }}>
            <div style={{ width: 68, height: 68, borderRadius: 34, background: `conic-gradient(${sColor} ${score}%, rgba(255,255,255,0.06) 0)`, display: 'flex', alignItems: 'center', justifyContent: 'center', border: `3px solid ${sColor}`, fontWeight: 900, color: '#fff', fontSize: 18, boxShadow: `0 0 20px ${sColor}44` }}>{score}</div>
            <div style={{ fontSize: 12, color: sColor, fontWeight: 800, marginTop: 4 }}>{level==='ok'?'ОК':level==='warn'?'WARN':'CRITICAL'} · v{scoring.verification}</div>
          </div>
        </div>
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', fontSize: 12, marginBottom: 8 }}>
          <span style={{ padding: '12px 16px', borderRadius: 20, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.06)', color: DIM }}>ACWR {acwr ? acwr.ratio.toFixed(2) : '—'} {acwr ? (acwr.zone === 'dangerous' ? '🔴' : acwr.zone === 'caution' ? '🟠' : '🟢') : ''}</span>
          <span style={{ padding: '12px 16px', borderRadius: 20, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.06)', color: DIM }}>{weakPoints.length? `${weakPoints.length} слабые` : 'баланс'}</span>
          {swayDiag && <span style={{ padding: '12px 16px', borderRadius: 20, background: swayDiag.severity==='ok'?'rgba(34,197,94,0.12)':'rgba(239,68,68,0.12)', border: '1px solid rgba(255,255,255,0.06)', color: swayDiag.severity==='ok'?'#22c55e':'#ef4444' }}>sway {swayDiag.swayCm}см</span>}
          {vbtLoss && <span style={{ padding: '12px 16px', borderRadius: 20, background: vbtLoss.exceeded?'rgba(239,68,68,0.12)':'rgba(34,197,94,0.12)', border: '1px solid rgba(255,255,255,0.06)', color: vbtLoss.exceeded?'#ef4444':'#22c55e' }}>VBT {vbtLoss.lossPct}%</span>}
          <span style={{ padding: '12px 16px', borderRadius: 20, background: ohs.level==='ok'?'rgba(34,197,94,0.12)':'rgba(239,68,68,0.12)', border: '1px solid rgba(255,255,255,0.06)', color: ohs.level==='ok'?'#22c55e':'#ef4444' }}>OHS {ohs.totalScore}/6</span>
          <span style={{ padding: '12px 16px', borderRadius: 20, background: gripFails>0?'rgba(245,158,11,0.12)':'rgba(34,197,94,0.12)', border: '1px solid rgba(255,255,255,0.06)', color: gripFails>0?'#f59e0b':'#22c55e' }}>grip {gripFails? `${gripFails}/3` : 'OK'}</span>
          {scoring.floors.length>0 && <span style={{ padding: '12px 16px', borderRadius: 20, background: 'rgba(239,68,68,0.12)', border: '1px solid rgba(239,68,68,0.22)', color: '#ef4444' }}>floor: {scoring.floors[0]}</span>}
        </div>
        ...
        {enodeCorrected != null && swayCm != null && <div style={{ marginTop: 8, padding: '10px 14px', borderRadius: 14, background: 'rgba(59,130,246,0.08)', border: '1px solid rgba(59,130,246,0.18)', fontSize: 12, color: '#60a5fa' }}>Enode xLoop {swayCm}см → {enodeCorrected}см (Chavda 2024 bias) · VBT yoke {VBT_SS_THRESHOLDS.yoke_walk.optimalMin}/{VBT_SS_THRESHOLDS.yoke_walk.stopMin} камень {VBT_SS_THRESHOLDS.atlas_stone_load.optimalMin}/{VBT_SS_THRESHOLDS.atlas_stone_load.stopMin}</div>}
        <div style={{ marginTop: 8, padding: '10px 14px', borderRadius: 14, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.06)', fontSize: 12, color: DIM }}>Axial {axialProgress.curM}м / {axialProgress.mrv}м MRV ({axialProgress.pct}%) · {axialQuant.text} · {axialQuant.recipe} · cond {state.conditioningFail? 'FAIL — prowler 10×100ft' : 'ok — alactic 8×10с/50с'} · {swayDiag? `sway ${swayDiag.swayCm}см` : 'sway —'}</div>
        ...
        {contestSim && <div style={{ marginTop: 8, padding: '12px 14px', borderRadius: 14, background: 'rgba(168,85,247,0.08)', border: '1px solid rgba(168,85,247,0.18)', fontSize: 12, color: '#a78bfa' }}>🏆 Симулятор: {contestSim.predictedPlace} место из 10 · total {contestSim.totalPoints}pts · слабые {contestSim.weakEvents.join(', ') || '—'} · порядок {contestSim.recOrder.join(' → ')}</div>}
        {attemptsBridge && attemptsBridge.rationale.length > 1 && <div style={{ marginTop: 8, padding: '10px 14px', borderRadius: 14, background: 'rgba(34,197,94,0.08)', border: '1px solid rgba(34,197,94,0.18)', fontSize: 12, color: '#22c55e' }}>🎯 Попытки: {attemptsBridge.rationale.slice(1, 4).join(' · ')}{attemptsBridge.medley ? ` · Medley ${attemptsBridge.medley.totalTimeS}с/cap ${attemptsBridge.medley.timeCapS}с` : ''}</div>}
        ...
        {smSpec && <div style={{ marginTop: 8, padding: '10px 14px', borderRadius: 14, background: 'rgba(239,68,68,0.08)', border: '1px solid rgba(239,68,68,0.18)', fontSize: 12, color: '#fca5a5' }}>📅 Спец-блок: {smSpec.weakPoints.join(', ')} × {smSpec.totalWeeks}нед · {smSpec.weeks[0]?.note || ''} · dayMap event-день</div>}
        ...
        {holdDiag && <div style={{ marginTop: 8, padding: '10px 14px', borderRadius: 14, background: 'rgba(34,197,94,0.08)', border: '1px solid rgba(34,197,94,0.18)', fontSize: 12, color: '#22c55e' }}>✊ Hold: {holdDiag.verdict}</div>}
        {anthroDiag && <div style={{ marginTop: 8, padding: '10px 14px', borderRadius: 14, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.06)', fontSize: 12, color: DIM }}>📏 Антро: {anthroDiag.loadAdvice} {anthroDiag.pickupAdvice}</div>}
        ...
        {gripAsymDiag && gripAsymDiag.isAsym && <div style={{ marginTop: 8, padding: '10px 14px', borderRadius: 14, background: 'rgba(245,158,11,0.08)', border: '1px solid rgba(245,158,11,0.22)', fontSize: 12, color: '#f59e0b' }}>⚖️ Grip-асимметрия: {gripAsymDiag.text}</div>}
      ...
      <div style={{ ...CARD, padding: 14 }}>
        ...
        {tab==='press' && (
          <div>
            ...
            {state.pressWeak.map(id=>{
              ...
              if (!bio) return null;
              return (
                <div key={id} style={{ padding:'12px 14px', borderRadius:14, background:'rgba(22,30,52,0.88)', border:'1px solid rgba(140,190,255,0.16)', marginBottom:6 }}>
                  <div style={{ fontSize:13, fontWeight:700, color:'#fff' }}>{bio.label} <span style={{ color:DIM }}>· {bio.joint} {bio.angleRangeDeg[0]}-{bio.angleRangeDeg[1]}° · {bio.keyJoint}</span></div>
                  <div style={{ fontSize:12, color:DIM }}>{bio.weakMuscles.join(', ')} · {bio.references.join(', ')}</div>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8, marginTop:8 }}>
              <label style={{ fontSize:13, color:DIM }}>Лог кг<br/><input value={state.logKg} onChange={e=>setState(s=>({...s, logKg:e.target.value}))} placeholder="100" style={{ width:'100%', marginTop:4, background:'rgba(22,30,52,0.88)', color:'#fff', border:'1px solid rgba(140,190,255,0.16)', borderRadius:14, padding:'12px 12px', fontSize:14 }} /></label>
              <label style={{ fontSize:13, color:DIM }}>Аксель кг<br/><input value={state.axleKg} onChange={e=>setState(s=>({...s, axleKg:e.target.value}))} placeholder="120" style={{ width:'100%', marginTop:4, background:'rgba(22,30,52,0.88)', color:'#fff', border:'1px solid rgba(140,190,255,0.16)', borderRadius:14, padding:'12px 12px', fontSize:14 }} /></label>
            </div>
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8, marginTop:8 }}>
              <label style={{ fontSize:13, color:DIM }}>VBT лог best м/с<br/><input value={state.vbtLogBest} onChange={e=>setState(s=>({...s, vbtLogBest:e.target.value}))} placeholder="0.85" style={{ width:'100%', marginTop:4, background:'rgba(22,30,52,0.88)', color:'#fff', border:'1px solid rgba(140,190,255,0.16)', borderRadius:14, padding:'12px 12px', fontSize:14 }} /></label>
              <label style={{ fontSize:13, color:DIM }}>VBT лог last м/с<br/><input value={state.vbtLogLast} onChange={e=>setState(s=>({...s, vbtLogLast:e.target.value}))} placeholder="0.65" style={{ width:'100%', marginTop:4, background:'rgba(22,30,52,0.88)', color:'#fff', border:'1px solid rgba(140,190,255,0.16)', borderRadius:14, padding:'12px 12px', fontSize:14 }} /></label>
            </div>
            <div style={{ fontSize:12, color:DIM, marginTop:6 }}>Ивенты: {Object.keys(EVENT_META).slice(0,4).join(', ')} — {VBT_SS_THRESHOLDS.log_press ? `VBT log ${VBT_SS_THRESHOLDS.log_press.optimalMin}/${VBT_SS_THRESHOLDS.log_press.stopMin} м/с` : ''}</div>
            <div style={{ marginTop:8, padding:'12px 14px', borderRadius:14, background:'rgba(22,30,52,0.88)', border:'1px solid rgba(140,190,255,0.16)' }}>
              <div style={{ fontSize:13, fontWeight:700, color:'#fff' }}>Контест пакет</div>
              ...
              <label style={{ display:'flex', alignItems:'center', gap:6, fontSize:13, color:DIM, marginTop:6 }}><input type="checkbox" checked={state.turnNeeded} onChange={e=>setState(s=>({...s, turnNeeded:e.target.checked}))}/> Разворот 180° (йок/фермер)</label>
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8, marginTop:6 }}>
                <label style={{ fontSize:13, color:DIM }}>Платформа см<br/><input value={state.platformHeightCm} onChange={e=>setState(s=>({...s, platformHeightCm:e.target.value}))} placeholder="140" style={{ width:'100%', marginTop:4, background:'rgba(22,30,52,0.88)', color:'#fff', border:'1px solid rgba(140,190,255,0.16)', borderRadius:14, padding:'12px 12px', fontSize:14 }} /></label>
                <label style={{ fontSize:13, color:DIM, display:'flex', alignItems:'center', gap:6, marginTop:16 }}><input type="checkbox" checked={state.tackyUsed} onChange={e=>setState(s=>({...s, tackyUsed:e.target.checked}))}/> Tacky есть</label>
              </div>
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8, marginTop:6 }}>
                <label style={{ fontSize:13, color:DIM }}>Диаметр лога см<br/><input value={state.diameterCm} onChange={e=>setState(s=>({...s, diameterCm:e.target.value}))} placeholder="30" style={{ width:'100%', marginTop:4, background:'rgba(22,30,52,0.88)', color:'#fff', border:'1px solid rgba(140,190,255,0.16)', borderRadius:14, padding:'12px 12px', fontSize:14 }} /></label>
                <label style={{ fontSize:13, color:DIM }}>Покрытие<br/><select value={state.surface} onChange={e=>setState(s=>({...s, surface:e.target.value}))} style={{ width:'100%', marginTop:4, background:'rgba(22,30,52,0.88)', color:'#fff', border:'1px solid rgba(140,190,255,0.16)', borderRadius:14, padding:'12px 12px', fontSize:13 }}><option value="">—</option><option value="резина">резина</option><option value="трава">трава</option><option value="асфальт">асфальт</option><option value="песок">песок</option></select></label>
              </div>
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr 1fr', gap:8, marginTop:6 }}>
                <label style={{ fontSize:13, color:DIM }}>Дип см (8-12)<br/><input value={state.logDipCm} onChange={e=>setState(s=>({...s, logDipCm:e.target.value}))} placeholder="10" style={{ width:'100%', marginTop:4, background:'rgba(22,30,52,0.88)', color:'#fff', border:'1px solid rgba(140,190,255,0.16)', borderRadius:14, padding:'12px 12px', fontSize:14 }} /></label>
                <label style={{ fontSize:13, color:DIM }}>Дип мс (~200)<br/><input value={state.logDipMs} onChange={e=>setState(s=>({...s, logDipMs:e.target.value}))} placeholder="200" style={{ width:'100%', marginTop:4, background:'rgba(22,30,52,0.88)', color:'#fff', border:'1px solid rgba(140,190,255,0.16)', borderRadius:14, padding:'12px 12px', fontSize:14 }} /></label>
                <label style={{ fontSize:13, color:DIM }}>Вес тела кг<br/><input value={state.bodyweightKg} onChange={e=>setState(s=>({...s, bodyweightKg:e.target.value}))} placeholder="105" style={{ width:'100%', marginTop:4, background:'rgba(22,30,52,0.88)', color:'#fff', border:'1px solid rgba(140,190,255,0.16)', borderRadius:14, padding:'12px 12px', fontSize:14 }} /></label>
                <label style={{ fontSize:13, color:DIM }}>Тяга кг (axle)<br/><input value={state.deadliftKg} onChange={e=>setState(s=>({...s, deadliftKg:e.target.value}))} placeholder="250" style={{ width:'100%', marginTop:4, background:'rgba(22,30,52,0.88)', color:'#fff', border:'1px solid rgba(140,190,255,0.16)', borderRadius:14, padding:'12px 12px', fontSize:14 }} /></label>
              </div>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8, marginTop:6 }}>
                <label style={{ fontSize:13, color:DIM }}>Стратегия попыток<br/><select value={state.strategy} onChange={e=>setState(s=>({...s, strategy:e.target.value}))} style={{ width:'100%', marginTop:4, background:'rgba(22,30,52,0.88)', color:'#fff', border:'1px solid rgba(140,190,255,0.16)', borderRadius:14, padding:'12px 12px', fontSize:13 }}><option value="conservative">conservative 85/92/98</option><option value="balanced">balanced 88/95/100</option><option value="aggressive">aggressive 90/97/102</option></select></label>
                <label style={{ fontSize:13, color:DIM }}>Хват тяги<br/><select value={state.mixGrip} onChange={e=>setState(s=>({...s, mixGrip:e.target.value}))} style={{ width:'100%', marginTop:4, background:'rgba(22,30,52,0.88)', color:'#fff', border:'1px solid rgba(140,190,255,0.16)', borderRadius:14, padding:'12px 12px', fontSize:13 }}><option value="overhand">overhand/hook</option><option value="mixed">mixed (риск)</option><option value="straps">straps</option></select></label>
              </div>
              <label style={{ display:'flex', alignItems:'center', gap:6, fontSize:13, color:DIM, marginTop:6 }}><input type="checkbox" checked={state.armsBent} onChange={e=>setState(s=>({...s, armsBent:e.target.checked}))}/> Руки согнуты на камне/шине (риск бицепса)</label>
        ...
        {tab==='carry' && (
          <div>
            ...
            {state.carryWeak.map(id=>{
              ...
              return (
                <div key={id} style={{ padding:'12px 14px', borderRadius:14, background:'rgba(22,30,52,0.88)', border:'1px solid rgba(140,190,255,0.16)', marginBottom:6 }}>
                  <div style={{ fontSize:13, fontWeight:700, color:'#fff' }}>{bio.label} <span style={{ color:DIM }}>· {bio.angleRangeDeg.join('-')}°</span></div>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8 }}>
              <label style={{ fontSize:13, color:DIM }}>Йок кг<br/><input value={state.yokeKg} onChange={e=>setState(s=>({...s, yokeKg:e.target.value}))} placeholder="300" style={{ width:'100%', marginTop:4, background:'rgba(22,30,52,0.88)', color:'#fff', border:'1px solid rgba(140,190,255,0.16)', borderRadius:14, padding:'12px 12px', fontSize:14 }} /></label>
              <label style={{ fontSize:13, color:DIM }}>Фермер кг (на руку)<br/><input value={state.farmersKg} onChange={e=>setState(s=>({...s, farmersKg:e.target.value}))} placeholder="120" style={{ width:'100%', marginTop:4, background:'rgba(22,30,52,0.88)', color:'#fff', border:'1px solid rgba(140,190,255,0.16)', borderRadius:14, padding:'12px 12px', fontSize:14 }} /></label>
            </div>
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:8, marginTop:8 }}>
              <label style={{ fontSize:13, color:DIM }}>Sway см<br/><input value={state.swayCm} onChange={e=>setState(s=>({...s, swayCm:e.target.value}))} placeholder="2.5" style={{ width:'100%', marginTop:4, background:'rgba(22,30,52,0.88)', color:'#fff', border:'1px solid rgba(140,190,255,0.16)', borderRadius:14, padding:'12px 12px', fontSize:14 }} /></label>
              <label style={{ fontSize:13, color:DIM }}>VBT йок best м/с<br/><input value={state.vbtYokeBest} onChange={e=>setState(s=>({...s, vbtYokeBest:e.target.value}))} placeholder="1.45" style={{ width:'100%', marginTop:4, background:'rgba(22,30,52,0.88)', color:'#fff', border:'1px solid rgba(140,190,255,0.16)', borderRadius:14, padding:'12px 12px', fontSize:14 }} /></label>
              <label style={{ fontSize:13, color:DIM }}>VBT йок last м/с<br/><input value={state.vbtYokeLast} onChange={e=>setState(s=>({...s, vbtYokeLast:e.target.value}))} placeholder="1.20" style={{ width:'100%', marginTop:4, background:'rgba(22,30,52,0.88)', color:'#fff', border:'1px solid rgba(140,190,255,0.16)', borderRadius:14, padding:'12px 12px', fontSize:14 }} /></label>
            </div>
            {swayDiag && <div style={{ fontSize:12, color: swayDiag.severity==='ok'?'#22c55e': swayDiag.severity==='warn'?'#f59e0b':'#ef4444', marginTop:4 }}>{swayDiag.text} · SRD 3/5см · VBT yoke {VBT_SS_THRESHOLDS.yoke_walk.optimalMin}/{VBT_SS_THRESHOLDS.yoke_walk.stopMin} м/с</div>}
            ...
            {carryPhys ? <div style={{ fontSize:12, color:'#60a5fa', marginTop:4 }}>Физика: {carryPhys.note} · Hindle 1.69м/с stride 1.14м rate 1.62Hz</div> : <div style={{ fontSize:12, color:DIM, marginTop:4 }}>Физика йока: введи вес тела (таб Жим) + йок кг → speed/stride/cadence/feasible</div>}
            <div style={{ marginTop:8, padding:'12px 14px', borderRadius:14, background:'rgba(22,30,52,0.88)', border:'1px dashed rgba(140,190,255,0.16)', textAlign:'center' }}>
              <div style={{ fontSize:13, color:DIM }}>📹 Видео переноски — sway из Kinovea (xLoop)</div>
              <div style={{ fontSize:12, color:DIM, marginTop:4 }}>Сними сбоку 30fps → Kinovea CSV → вкладка Видео → sway автоматически</div>
            </div>
          </div>
        )}
        ...
        {tab==='load' && (
          <div>
            ...
            {state.loadWeak.map(id=>{
              ...
              return (
                <div key={id} style={{ padding:'12px 14px', borderRadius:14, background:'rgba(22,30,52,0.88)', border:'1px solid rgba(140,190,255,0.16)', marginBottom:6 }}>
                  <div style={{ fontSize:13, fontWeight:700, color:'#fff' }}>{bio.label}</div>
                  <div style={{ fontSize:12, color:'#fff', marginTop:4 }}>{bio.biomechanicalReason}</div>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8 }}>
              <label style={{ fontSize:13, color:DIM }}>Камень кг<br/><input value={state.stoneKg} onChange={e=>setState(s=>({...s, stoneKg:e.target.value}))} placeholder="140" style={{ width:'100%', marginTop:4, background:'rgba(22,30,52,0.88)', color:'#fff', border:'1px solid rgba(140,190,255,0.16)', borderRadius:14, padding:'12px 12px', fontSize:14 }} /></label>
              <label style={{ fontSize:13, color:DIM }}>Sway камня см<br/><input value={state.swayCm} onChange={e=>setState(s=>({...s, swayCm:e.target.value}))} placeholder="2.0" style={{ width:'100%', marginTop:4, background:'rgba(22,30,52,0.88)', color:'#fff', border:'1px solid rgba(140,190,255,0.16)', borderRadius:14, padding:'12px 12px', fontSize:14 }} /></label>
            </div>
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8, marginTop:8 }}>
              <label style={{ fontSize:13, color:DIM }}>VBT камень best м/с<br/><input value={state.vbtStoneBest} onChange={e=>setState(s=>({...s, vbtStoneBest:e.target.value}))} placeholder="0.75" style={{ width:'100%', marginTop:4, background:'rgba(22,30,52,0.88)', color:'#fff', border:'1px solid rgba(140,190,255,0.16)', borderRadius:14, padding:'12px 12px', fontSize:14 }} /></label>
              <label style={{ fontSize:13, color:DIM }}>VBT камень last м/с<br/><input value={state.vbtStoneLast} onChange={e=>setState(s=>({...s, vbtStoneLast:e.target.value}))} placeholder="0.55" style={{ width:'100%', marginTop:4, background:'rgba(22,30,52,0.88)', color:'#fff', border:'1px solid rgba(140,190,255,0.16)', borderRadius:14, padding:'12px 12px', fontSize:14 }} /></label>
            </div>
            <div style={{ fontSize:12, color:DIM, marginTop:6 }}>Контест пресеты: {Object.values(CONTEST_PRESETS as any).slice(0,3).map((c:any)=>c.name).join(', ')} · stone VBT {VBT_SS_THRESHOLDS.atlas_stone_load.optimalMin}/{VBT_SS_THRESHOLDS.atlas_stone_load.stopMin} м/с · платформа {state.platformHeightCm || '—'}см</div>
            <div style={{ fontSize:12, color: state.tackyUsed ? '#22c55e' : '#f59e0b', marginTop:4 }}>{state.tackyUsed ? '✓ Tacky учтён — руки не сгибать' : '⚠ Без tacky — риск сгибания рук + бицепс tear'}</div>
          </div>
        )}

        {tab==='grip' && (
          <div>
            <div style={{ fontSize:13, fontWeight:700, color:ACCENT, marginBottom:6 }}>Хват / Кор / Кондиция (tri-modal + axial)</div>
            <div style={{ display:'flex', gap:6, flexWrap:'wrap', marginBottom:8 }}>
              {GRIP_OPTS.map(o=>(
                <button key={o.id} onClick={()=>toggle('gripWeak', o.id)} aria-pressed={state.gripWeak.includes(o.id)} style={{ padding:'10px 14px', borderRadius:999, border:'1px solid', borderColor: state.gripWeak.includes(o.id) ? '#a855f7' : 'rgba(140,190,255,0.16)', background: state.gripWeak.includes(o.id) ? 'rgba(168,85,247,0.14)' : 'rgba(22,30,52,0.88)', color: state.gripWeak.includes(o.id) ? '#a855f7' : DIM, fontSize:13 }}>{o.label}</button>
              ))}
            </div>
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:8 }}>
              <label style={{ fontSize:13, color:DIM }}>Support (фермер) сек<br/><input value={state.gripHoldSec} onChange={e=>setState(s=>({...s, gripHoldSec:e.target.value}))} placeholder="60" style={{ width:'100%', marginTop:4, background:'rgba(22,30,52,0.88)', color:'#fff', border:'1px solid rgba(140,190,255,0.16)', borderRadius:14, padding:'12px 12px', fontSize:14 }} /></label>
              <label style={{ fontSize:13, color:DIM }}>Pinch сек<br/><input value={state.pinchHoldSec} onChange={e=>setState(s=>({...s, pinchHoldSec:e.target.value}))} placeholder="20" style={{ width:'100%', marginTop:4, background:'rgba(22,30,52,0.88)', color:'#fff', border:'1px solid rgba(140,190,255,0.16)', borderRadius:14, padding:'12px 12px', fontSize:14 }} /></label>
              <label style={{ fontSize:13, color:DIM }}>Crush (axle) сек<br/><input value={state.axleHoldSec} onChange={e=>setState(s=>({...s, axleHoldSec:e.target.value}))} placeholder="30" style={{ width:'100%', marginTop:4, background:'rgba(22,30,52,0.88)', color:'#fff', border:'1px solid rgba(140,190,255,0.16)', borderRadius:14, padding:'12px 12px', fontSize:14 }} /></label>
            </div>
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8, marginTop:8 }}>
              <label style={{ fontSize:13, color:DIM }}>Планка сек<br/><input value={state.corePlankSec} onChange={e=>setState(s=>({...s, corePlankSec:e.target.value}))} placeholder="120" style={{ width:'100%', marginTop:4, background:'rgba(22,30,52,0.88)', color:'#fff', border:'1px solid rgba(140,190,255,0.16)', borderRadius:14, padding:'12px 12px', fontSize:14 }} /></label>
              <label style={{ fontSize:13, color:DIM, display:'flex', alignItems:'center', gap:6, marginTop:16 }}><input type="checkbox" checked={state.conditioningFail} onChange={e=>setState(s=>({...s, conditioningFail:e.target.checked}))}/> Кондиция провал (medley &gt;60с)</label>
            </div>
            ...
            <div style={{ fontSize:12, color:DIM, marginTop:6 }}>ACWR {acwr? `${acwr.ratio.toFixed(2)} ${acwr.zone}` : '—'} · conditioning как в strength-sport-conditioning (alactic 8×10с/50с)</div>
            {(() => {
              ...
              return (
                <div style={{ marginTop:8, padding:'12px 14px', borderRadius:14, background:'rgba(59,130,246,0.08)', border:'1px solid rgba(59,130,246,0.18)' }}>
                  ...
                  <div style={{ fontSize:12, color:DIM, marginTop:2 }}>{sess.sets}× {sess.work} / отдых {sess.rest} · {sess.hrZone} · {sess.note}</div>
                  <div style={{ fontSize:12, color:DIM, marginTop:4 }}>Все системы: {allSMCondSessions().map((s) => `${s.goal} ${s.sets}×${s.work}`).join(' · ')}</div>
                </div>
              );
            })()}
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:8, marginTop:8 }}>
              <label style={{ fontSize:13, color:DIM }}>Pinch-блок<br/><select value={state.pinchWidth} onChange={e=>setState(s=>({...s, pinchWidth:e.target.value}))} style={{ width:'100%', marginTop:4, background:'rgba(22,30,52,0.88)', color:'#fff', border:'1px solid rgba(140,190,255,0.16)', borderRadius:14, padding:'12px 12px', fontSize:13 }}><option value="2in">2″ (норма 30с)</option><option value="3in">3″ (норма 20с)</option><option value="4in">4″ (норма 15с)</option></select></label>
              <label style={{ fontSize:13, color:DIM }}>CoC<br/><select value={state.cocLevel} onChange={e=>setState(s=>({...s, cocLevel:e.target.value}))} style={{ width:'100%', marginTop:4, background:'rgba(22,30,52,0.88)', color:'#fff', border:'1px solid rgba(140,190,255,0.16)', borderRadius:14, padding:'12px 12px', fontSize:13 }}><option value="coc1">CoC 1 (20с)</option><option value="coc1_5">CoC 1.5 (30с)</option><option value="coc2">CoC 2 (40с)</option></select></label>
              <label style={{ fontSize:13, color:DIM }}>FatGrip мм<br/><select value={state.fatGripMm} onChange={e=>setState(s=>({...s, fatGripMm:e.target.value}))} style={{ width:'100%', marginTop:4, background:'rgba(22,30,52,0.88)', color:'#fff', border:'1px solid rgba(140,190,255,0.16)', borderRadius:14, padding:'12px 12px', fontSize:13 }}><option value="38">38 (стандарт)</option><option value="50">50 (axle)</option><option value="60">60 (толстый)</option></select></label>
            </div>
            <div style={{ display:'flex', gap:6, marginTop:6 }}>
              ...
              <span style={{ fontSize:12, color:DIM, alignSelf:'center' }}>SBS r=0.40 — support/pinch/crush раздельно</span>
            </div>
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:8, marginTop:8 }}>
              <label style={{ fontSize:13, color:DIM }}>Лог-hold сек<br/><input value={state.logHoldSec} onChange={e=>setState(s=>({...s, logHoldSec:e.target.value}))} placeholder="10" style={{ width:'100%', marginTop:4, background:'rgba(22,30,52,0.88)', color:'#fff', border:'1px solid rgba(140,190,255,0.16)', borderRadius:14, padding:'12px 12px', fontSize:14 }} /></label>
              <label style={{ fontSize:13, color:DIM }}>Фермер-hold сек<br/><input value={state.farmersHoldSec} onChange={e=>setState(s=>({...s, farmersHoldSec:e.target.value}))} placeholder="60" style={{ width:'100%', marginTop:4, background:'rgba(22,30,52,0.88)', color:'#fff', border:'1px solid rgba(140,190,255,0.16)', borderRadius:14, padding:'12px 12px', fontSize:14 }} /></label>
              <label style={{ fontSize:13, color:DIM }}>Axle DOH кг<br/><input value={state.axleDohKg} onChange={e=>setState(s=>({...s, axleDohKg:e.target.value}))} placeholder="140" style={{ width:'100%', marginTop:4, background:'rgba(22,30,52,0.88)', color:'#fff', border:'1px solid rgba(140,190,255,0.16)', borderRadius:14, padding:'12px 12px', fontSize:14 }} /></label>
            </div>
            {holdDiag && <div style={{ fontSize:12, color: holdDiag.profile === 'balanced' ? '#22c55e' : '#f59e0b', marginTop:4 }}>{holdDiag.verdict} · {holdDiag.details.join(' · ')}</div>}
            {weakPoints.map(id=>{
              const bio = smBiomechForWeak(id);
              if (!bio || !['grip_support','core_brace','conditioning','farmers_grip'].includes(bio.weakPoint)) return null;
              return <div key={id} style={{ padding:'12px 14px', borderRadius:14, background:'rgba(22,30,52,0.88)', border:'1px solid rgba(140,190,255,0.16)', marginTop:6 }}><div style={{ fontSize:13, fontWeight:700, color:'#fff' }}>{bio.label}</div><div style={{ fontSize:12, color:'#fff' }}>{bio.biomechanicalReason}</div><div style={{ fontSize:13, color:'#5ee' }}>{bio.corrections.join(' · ')}</div></div>;
            })}
          </div>
        )}
        ...
        {tab==='mobility' && (
          <div>
            <div style={{ fontSize:13, fontWeight:700, color:ACCENT, marginBottom:6 }}>Мобильность — OHS 6 + асимметрия + sway</div>
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:6, marginBottom:8 }}>
              <label style={{ fontSize:13, color:DIM, display:'flex', alignItems:'center', gap:6 }}><input type="checkbox" checked={state.ohsHeelsFlat} onChange={e=>setState(s=>({...s, ohsHeelsFlat:e.target.checked}))}/> Пятки плоско</label>
              <label style={{ fontSize:13, color:DIM, display:'flex', alignItems:'center', gap:6 }}><input type="checkbox" checked={!state.ohsKneeValgus} onChange={e=>setState(s=>({...s, ohsKneeValgus:!e.target.checked}))}/> Колени без вальгуса</label>
              <label style={{ fontSize:13, color:DIM, display:'flex', alignItems:'center', gap:6 }}><input type="checkbox" checked={state.ohsHipBelowParallel} onChange={e=>setState(s=>({...s, ohsHipBelowParallel:e.target.checked}))}/> Таз ниже параллели</label>
              <label style={{ fontSize:13, color:DIM, display:'flex', alignItems:'center', gap:6 }}><input type="checkbox" checked={state.ohsTrunkUpright} onChange={e=>setState(s=>({...s, ohsTrunkUpright:e.target.checked}))}/> Корпус upright</label>
              <label style={{ fontSize:13, color:DIM, display:'flex', alignItems:'center', gap:6 }}><input type="checkbox" checked={state.ohsArmsOverMidfoot} onChange={e=>setState(s=>({...s, ohsArmsOverMidfoot:e.target.checked}))}/> Руки над стопой</label>
              <label style={{ fontSize:13, color:DIM, display:'flex', alignItems:'center', gap:6 }}><input type="checkbox" checked={state.ohsLumbarNeutral} onChange={e=>setState(s=>({...s, ohsLumbarNeutral:e.target.checked}))}/> Нейтраль поясницы</label>
            </div>
            <div style={{ padding:'12px 14px', borderRadius:14, background: ohs.level==='ok'?'rgba(34,197,94,0.08)': ohs.level==='warn'?'rgba(245,158,11,0.08)':'rgba(239,68,68,0.08)', border:`1px solid ${ohs.level==='ok'?'rgba(34,197,94,0.18)': ohs.level==='warn'?'rgba(245,158,11,0.18)':'rgba(239,68,68,0.18)'}`, marginBottom:8 }}>
              ...
              <div style={{ fontSize:12, color:DIM, marginTop:4 }}>{ohs.recommendation} {ohs.needsPhysio?'· нужен физио':''}</div>
              <div style={{ fontSize:12, color:DIM, marginTop:4 }}>Нормы knee-to-wall ≥{OHS_NORMS.kneeToWallCm.optimal}см (cutoff {OHS_NORMS.kneeToWallCm.cutoff}), ankle {OHS_NORMS.ankleDeg.range}</div>
              <div style={{ display:'flex', gap:6, marginTop:6, alignItems:'center' }}>
                ...
                <span style={{ fontSize:12, color:DIM }}>{smOhsTrend && smOhsTrend.n >= 2 ? `тренд ${smOhsTrend.delta >= 0 ? '+' : ''}${smOhsTrend.delta} за ${smOhsTrend.n} зам.` : `история ${smOhsHist.length}/10`}</span>
              </div>
            </div>
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8, marginBottom:8 }}>
              <label style={{ fontSize:13, color:DIM }}>Knee-to-wall см<br/><input value={state.kneeToWallCm} onChange={e=>setState(s=>({...s, kneeToWallCm:e.target.value}))} placeholder="12" style={{ width:'100%', marginTop:4, background:'rgba(22,30,52,0.88)', color:'#fff', border:'1px solid rgba(140,190,255,0.16)', borderRadius:14, padding:'12px 12px', fontSize:14 }} /></label>
              <label style={{ fontSize:13, color:DIM }}>Голеностоп °<br/><input value={state.ankleDeg} onChange={e=>setState(s=>({...s, ankleDeg:e.target.value}))} placeholder="35" style={{ width:'100%', marginTop:4, background:'rgba(22,30,52,0.88)', color:'#fff', border:'1px solid rgba(140,190,255,0.16)', borderRadius:14, padding:'12px 12px', fontSize:14 }} /></label>
            </div>
            <div style={{ display:'flex', gap:6, marginBottom:8, alignItems:'center' }}>
              <span style={{ fontSize:13, color:DIM }}>Heel-raise 2.5см</span>
              <button onClick={()=>setState(s=>({...s, heelRetest:'better'}))} style={{ padding:'10px 16px', borderRadius:999, border:'1px solid', borderColor: state.heelRetest==='better'?'#22c55e':'rgba(140,190,255,0.16)', background: state.heelRetest==='better'?'rgba(34,197,94,0.14)':'rgba(22,30,52,0.88)', color: state.heelRetest==='better'?'#22c55e':DIM, fontSize:13 }}>Лучше</button>
              <button onClick={()=>setState(s=>({...s, heelRetest:'same'}))} style={{ padding:'10px 16px', borderRadius:999, border:'1px solid', borderColor: state.heelRetest==='same'?'#f59e0b':'rgba(140,190,255,0.16)', background: state.heelRetest==='same'?'rgba(245,158,11,0.14)':'rgba(22,30,52,0.88)', color: state.heelRetest==='same'?'#f59e0b':DIM, fontSize:13 }}>Без изм</button>
              <button onClick={()=>setState(s=>({...s, heelRetest:''}))} style={{ padding:'10px 16px', borderRadius:999, border:'1px solid rgba(140,190,255,0.16)', background:'rgba(22,30,52,0.88)', color:DIM, fontSize:13 }}>Сброс</button>
            </div>
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8 }}>
              <label style={{ fontSize:13, color:DIM }}>Левая макс кг<br/><input value={state.leftMax} onChange={e=>setState(s=>({...s, leftMax:e.target.value}))} placeholder="100" style={{ width:'100%', marginTop:4, background:'rgba(22,30,52,0.88)', color:'#fff', border:'1px solid rgba(140,190,255,0.16)', borderRadius:14, padding:'12px 12px', fontSize:14 }} /></label>
              <label style={{ fontSize:13, color:DIM }}>Правая макс кг<br/><input value={state.rightMax} onChange={e=>setState(s=>({...s, rightMax:e.target.value}))} placeholder="102" style={{ width:'100%', marginTop:4, background:'rgba(22,30,52,0.88)', color:'#fff', border:'1px solid rgba(140,190,255,0.16)', borderRadius:14, padding:'12px 12px', fontSize:14 }} /></label>
            </div>
            {asymmetry && (
              <div style={{ marginTop:8, padding:'12px 14px', borderRadius:14, background: asymmetry.isCrit?'rgba(239,68,68,0.08)': asymmetry.isAsym?'rgba(245,158,11,0.08)':'rgba(34,197,94,0.08)', border:`1px solid ${asymmetry.isCrit?'rgba(239,68,68,0.2)': asymmetry.isAsym?'rgba(245,158,11,0.2)':'rgba(34,197,94,0.2)'}` }}>
                ...
                <div style={{ fontSize:12, color:DIM }}>Пороги 7/12% — предиктор distal biceps tear (Heazlewood). {gripAsymDiag ? gripAsymDiag.text : ''}</div>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8, marginTop:8 }}>
              <label style={{ fontSize:13, color:DIM }}>Рост см (антро)<br/><input value={state.anthroHeight} onChange={e=>setState(s=>({...s, anthroHeight:e.target.value}))} placeholder="182" style={{ width:'100%', marginTop:4, background:'rgba(22,30,52,0.88)', color:'#fff', border:'1px solid rgba(140,190,255,0.16)', borderRadius:14, padding:'12px 12px', fontSize:14 }} /></label>
              <label style={{ fontSize:13, color:DIM }}>Размах рук см<br/><input value={state.anthroArmSpan} onChange={e=>setState(s=>({...s, anthroArmSpan:e.target.value}))} placeholder="186" style={{ width:'100%', marginTop:4, background:'rgba(22,30,52,0.88)', color:'#fff', border:'1px solid rgba(140,190,255,0.16)', borderRadius:14, padding:'12px 12px', fontSize:14 }} /></label>
            </div>
            {anthroDiag && <div style={{ fontSize:12, color:DIM, marginTop:4 }}>{anthroDiag.loadAdvice} {anthroDiag.pickupAdvice} (tacky ≈{anthroDiag.tackyHeightCm}см)</div>}
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8, marginTop:8 }}>
              <label style={{ fontSize:13, color:DIM }}>Sway см (lateral)<br/><input value={state.swayCm} onChange={e=>setState(s=>({...s, swayCm:e.target.value}))} placeholder="2.5" style={{ width:'100%', marginTop:4, background:'rgba(22,30,52,0.88)', color:'#fff', border:'1px solid rgba(140,190,255,0.16)', borderRadius:14, padding:'12px 12px', fontSize:14 }} /></label>
              <label style={{ fontSize:13, color:DIM }}>Tacky<br/><select value={state.tackyUsed?'yes':'no'} onChange={e=>setState(s=>({...s, tackyUsed:e.target.value==='yes'}))} style={{ width:'100%', marginTop:4, background:'rgba(22,30,52,0.88)', color:'#fff', border:'1px solid rgba(140,190,255,0.16)', borderRadius:14, padding:'12px 12px', fontSize:14 }}><option value="no">нет</option><option value="yes">есть</option></select></label>
            </div>
        ...
        {tab==='video' && (
          <div>
            <div style={{ fontSize:13, fontWeight:700, color:ACCENT, marginBottom:6 }}>Видео — Kinovea sway + carry VBT</div>
            <div style={{ fontSize:12, color:DIM, marginBottom:6 }}>Полевая методика: телефон сбоку 30fps → Kinovea (free) → трек центра масс/йока → xLoop = sway. VBT carry: скорость ходьбы м/с.</div>
            ...
            <div style={{ display:'flex', gap:6, marginTop:6 }}>
              ...
              <span style={{ fontSize:12, color:DIM, alignSelf:'center' }}>Или введи sway/VBT вручную</span>
            </div>
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:6, marginTop:8 }}>
              <label style={{ fontSize:13, color:DIM }}>Sway см<br/><input value={state.swayCm} onChange={e=>setState(s=>({...s, swayCm:e.target.value}))} placeholder="3.2" style={{ width:'100%', marginTop:4, background:'rgba(22,30,52,0.88)', color:'#fff', border:'1px solid rgba(140,190,255,0.16)', borderRadius:14, padding:'12px 12px', fontSize:14 }} /></label>
              <label style={{ fontSize:13, color:DIM }}>yMax см<br/><input value={state.stoneKg} onChange={e=>setState(s=>({...s, stoneKg:e.target.value}))} placeholder="85" style={{ width:'100%', marginTop:4, background:'rgba(22,30,52,0.88)', color:'#fff', border:'1px solid rgba(140,190,255,0.16)', borderRadius:14, padding:'12px 12px', fontSize:14 }} /></label>
              <label style={{ fontSize:13, color:DIM }}>VBT yoke м/с<br/><input value={state.vbtYokeLast} onChange={e=>setState(s=>({...s, vbtYokeLast:e.target.value}))} placeholder="1.25" style={{ width:'100%', marginTop:4, background:'rgba(22,30,52,0.88)', color:'#fff', border:'1px solid rgba(140,190,255,0.16)', borderRadius:14, padding:'12px 12px', fontSize:14 }} /></label>
            </div>
            {swayDiag && <div style={{ marginTop:6, padding:'12px 14px', borderRadius:14, background: swayDiag.severity==='critical'?'rgba(239,68,68,0.08)': swayDiag.severity==='warn'?'rgba(245,158,11,0.08)':'rgba(34,197,94,0.08)', border:`1px solid ${swayDiag.severity==='ok'?'rgba(34,197,94,0.2)': swayDiag.severity==='warn'?'rgba(245,158,11,0.2)':'rgba(239,68,68,0.2)'}` }}><div style={{ fontSize:13, fontWeight:700, color: swayDiag.severity==='ok'?'#22c55e': swayDiag.severity==='warn'?'#f59e0b':'#ef4444' }}>{swayDiag.text}</div><div style={{ fontSize:12, color:DIM }}>SRD sway 3/5см — {swayDiag.isReal?'реально >SRD':'в пределах шума'}</div></div>}
            {vbtLoss && <div style={{ marginTop:6, padding:'12px 14px', borderRadius:14, background: vbtLoss.exceeded?'rgba(239,68,68,0.08)':'rgba(34,197,94,0.08)', border:`1px solid ${vbtLoss.exceeded?'rgba(239,68,68,0.2)':'rgba(34,197,94,0.2)'}` }}><div style={{ fontSize:13, fontWeight:700, color: vbtLoss.exceeded?'#ef4444':'#22c55e' }}>VBT потеря {vbtLoss.lossPct}% · {vbtLoss.zone} · {vbtLoss.recommendation}</div><div style={{ fontSize:12, color:DIM }}>Порог carry 15% (Hindle stride 1.83м) vs TA 10% — VBT yoke {VBT_SS_THRESHOLDS.yoke_walk.optimalMin}/{VBT_SS_THRESHOLDS.yoke_walk.stopMin} м/с</div></div>}
            <div style={{ marginTop:6, padding:'12px 12px', borderRadius:14, background:'rgba(255,255,255,0.04)', border:'1px solid rgba(255,255,255,0.06)', fontSize:12, color:DIM }}>VBT зоны: yoke {VBT_SS_THRESHOLDS.yoke_walk.optimalMin}-{VBT_SS_THRESHOLDS.yoke_walk.stopMin} · farmers {VBT_SS_THRESHOLDS.farmers_walk_heavy.optimalMin}/{VBT_SS_THRESHOLDS.farmers_walk_heavy.stopMin} · stone {VBT_SS_THRESHOLDS.atlas_stone_load.optimalMin}/{VBT_SS_THRESHOLDS.atlas_stone_load.stopMin} · log {VBT_SS_THRESHOLDS.log_press.optimalMin}/{VBT_SS_THRESHOLDS.log_press.stopMin} м/с</div>
            <div style={{ marginTop:6, display:'grid', gridTemplateColumns:'1fr 1fr', gap:6 }}>
              <div style={{ padding:'12px 12px', borderRadius:14, background: swayCm!=null ? 'rgba(59,130,246,0.08)' : 'rgba(255,255,255,0.04)', border:'1px solid rgba(59,130,246,0.18)', fontSize:12, color:DIM }}>Enode: raw {swayCm ?? '—'}см → {enodeCorrected ?? '—'}см (xLoop bias Chavda 2024)<br/><span style={{ fontSize:10, color:'#60a5fa' }}>raw {swayCm ?? 0} ×1.08 −0.45 = {enodeCorrected ?? 0}</span></div>
              <div style={{ padding:'12px 12px', borderRadius:14, background:'rgba(255,255,255,0.04)', border:'1px solid rgba(255,255,255,0.06)', fontSize:12, color:DIM }}>Сетка: yoke 1.30/1.00<br/>farmers 1.40/1.10<br/>stone 0.45/0.30<br/>log 0.32/0.20 м/с — стоп при &lt;stopMin</div>
            </div>
            ...
            {carryPath && <div style={{ marginTop:6, padding:'12px 14px', borderRadius:14, background: carryPath.verdict === 'ok' ? 'rgba(34,197,94,0.08)' : carryPath.verdict === 'warn' ? 'rgba(245,158,11,0.08)' : 'rgba(239,68,68,0.08)', border:'1px solid rgba(255,255,255,0.06)' }}><div style={{ fontSize:13, fontWeight:700, color: carryPath.verdict === 'ok' ? '#22c55e' : carryPath.verdict === 'warn' ? '#f59e0b' : '#ef4444' }}>Траектория переноски: {carryPath.type} · {carryPath.verdict.toUpperCase()}</div><div style={{ fontSize:12, color:DIM, marginTop:2 }}>{carryPath.lines.join(' · ')}</div></div>}
            <div style={{ marginTop:8, padding:'12px 14px', borderRadius:14, background:'rgba(22,30,52,0.88)', border:'1px solid rgba(140,190,255,0.16)' }}>
              <div style={{ fontSize:13, fontWeight:700, color:'#fff' }}>Углы суставов с видео (Hindle-нормы)</div>
              <div style={{ fontSize:12, color:DIM, marginTop:2 }}>Трекер поз → экспорт CSV (t,hip,knee,ankle,shoulder) → вставь ниже. Йок: hip ROM [30,46] / knee [43,65]; лог: shoulder ≥150°.</div>
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 2fr', gap:6, marginTop:6 }}>
                <label style={{ fontSize:13, color:DIM }}>Лифт<br/><select value={state.poseLift} onChange={e=>setState(s=>({...s, poseLift:e.target.value}))} style={{ width:'100%', marginTop:4, background:'rgba(22,30,52,0.88)', color:'#fff', border:'1px solid rgba(140,190,255,0.16)', borderRadius:14, padding:'12px 12px', fontSize:13 }}><option value="yoke_walk">yoke</option><option value="farmers_walk">farmers</option><option value="log_press">log</option></select></label>
                <label style={{ fontSize:13, color:DIM }}>Пол<br/><select value={state.poseSex} onChange={e=>setState(s=>({...s, poseSex:e.target.value as '' | 'male' | 'female'}))} style={{ width:'100%', marginTop:4, background:'rgba(22,30,52,0.88)', color:'#fff', border:'1px solid rgba(140,190,255,0.16)', borderRadius:14, padding:'12px 12px', fontSize:13 }}><option value="">—</option><option value="male">М</option><option value="female">Ж</option></select></label>
              ...
              <div style={{ display:'flex', gap:6, marginTop:6, alignItems:'center' }}>
                <button onClick={checkPoseLive} style={{ padding:'12px 16px', borderRadius:14, background:'rgba(255,255,255,0.04)', border:'1px solid rgba(255,255,255,0.06)', color:DIM, fontSize:13, cursor:'pointer' }}>📷 Live-проверка модели</button>
                {state.poseLive === 'loading' && <span style={{ fontSize: 12, color: DIM }}>проверяем CDN…</span>}
            ...
            <div style={{ marginTop:8, padding:'12px 14px', borderRadius:14, background:'rgba(22,30,52,0.88)', border:'1px solid rgba(140,190,255,0.16)' }}>
              <div style={{ fontSize:13, fontWeight:700, color:'#fff' }}>LVP-калибровка SM (Wood PLOS 2026: population ±0.15 — нужен ramp)</div>
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr 1fr 1fr', gap:6, marginTop:6 }}>
                <label style={{ fontSize:13, color:DIM }}>Лифт<br/><select value={state.lvpLift} onChange={e=>setState(s=>({...s, lvpLift:e.target.value}))} style={{ width:'100%', marginTop:4, background:'rgba(22,30,52,0.88)', color:'#fff', border:'1px solid rgba(140,190,255,0.16)', borderRadius:14, padding:'12px 12px', fontSize:13 }}><option value="yoke_walk">yoke</option><option value="farmers_walk">farmers</option><option value="stone_load">stone</option><option value="log_press">log</option></select></label>
                <label style={{ fontSize:13, color:DIM }}>50% м/с<br/><input value={state.lvp50} onChange={e=>setState(s=>({...s, lvp50:e.target.value}))} placeholder="1.90" style={{ width:'100%', marginTop:4, background:'rgba(22,30,52,0.88)', color:'#fff', border:'1px solid rgba(140,190,255,0.16)', borderRadius:14, padding:'12px 12px', fontSize:14 }} /></label>
                <label style={{ fontSize:13, color:DIM }}>65% м/с<br/><input value={state.lvp65} onChange={e=>setState(s=>({...s, lvp65:e.target.value}))} placeholder="1.60" style={{ width:'100%', marginTop:4, background:'rgba(22,30,52,0.88)', color:'#fff', border:'1px solid rgba(140,190,255,0.16)', borderRadius:14, padding:'12px 12px', fontSize:14 }} /></label>
                <label style={{ fontSize:13, color:DIM }}>75% м/с<br/><input value={state.lvp75} onChange={e=>setState(s=>({...s, lvp75:e.target.value}))} placeholder="1.40" style={{ width:'100%', marginTop:4, background:'rgba(22,30,52,0.88)', color:'#fff', border:'1px solid rgba(140,190,255,0.16)', borderRadius:14, padding:'12px 12px', fontSize:14 }} /></label>
                <label style={{ fontSize:13, color:DIM }}>90% м/с<br/><input value={state.lvp90} onChange={e=>setState(s=>({...s, lvp90:e.target.value}))} placeholder="1.10" style={{ width:'100%', marginTop:4, background:'rgba(22,30,52,0.88)', color:'#fff', border:'1px solid rgba(140,190,255,0.16)', borderRadius:14, padding:'12px 12px', fontSize:14 }} /></label>
              </div>
              <div style={{ display:'flex', gap:6, marginTop:6, alignItems:'center' }}>
                ...
                <span style={{ fontSize:12, color:DIM }}>{state.lvpResult || (smLvpStored ? `сохранён r² ${smLvpStored.r2}${smLvpStored.valid ? ' ✓' : ' ⚠'}` : 'ramp 50/65/75/90 → r²≥0.85')}</span>
              </div>
            </div>
            <div style={{ marginTop:8, padding:'12px 14px', borderRadius:14, background:'rgba(22,30,52,0.88)', border:'1px solid rgba(140,190,255,0.16)' }}>
              <div style={{ fontSize:13, fontWeight:700, color:'#fff' }}>Прогресс стронга (йок 20м / фермер 40м / лог / лестница)</div>
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr 1fr 1fr', gap:6, marginTop:6 }}>
                <label style={{ fontSize:13, color:DIM }}>Йок 20м с<br/><input value={state.progYoke20m} onChange={e=>setState(s=>({...s, progYoke20m:e.target.value}))} placeholder="12.0" style={{ width:'100%', marginTop:4, background:'rgba(22,30,52,0.88)', color:'#fff', border:'1px solid rgba(140,190,255,0.16)', borderRadius:14, padding:'12px 12px', fontSize:14 }} /></label>
                <label style={{ fontSize:13, color:DIM }}>Фермер 40м с<br/><input value={state.progFarmers40m} onChange={e=>setState(s=>({...s, progFarmers40m:e.target.value}))} placeholder="28.0" style={{ width:'100%', marginTop:4, background:'rgba(22,30,52,0.88)', color:'#fff', border:'1px solid rgba(140,190,255,0.16)', borderRadius:14, padding:'12px 12px', fontSize:14 }} /></label>
                <label style={{ fontSize:13, color:DIM }}>Лог кг<br/><input value={state.progLogMax} onChange={e=>setState(s=>({...s, progLogMax:e.target.value}))} placeholder="110" style={{ width:'100%', marginTop:4, background:'rgba(22,30,52,0.88)', color:'#fff', border:'1px solid rgba(140,190,255,0.16)', borderRadius:14, padding:'12px 12px', fontSize:14 }} /></label>
                <label style={{ fontSize:13, color:DIM }}>Лестница кг<br/><input value={state.progStoneLadder} onChange={e=>setState(s=>({...s, progStoneLadder:e.target.value}))} placeholder="140" style={{ width:'100%', marginTop:4, background:'rgba(22,30,52,0.88)', color:'#fff', border:'1px solid rgba(140,190,255,0.16)', borderRadius:14, padding:'12px 12px', fontSize:14 }} /></label>
                <label style={{ fontSize:13, color:DIM }}>Вес кг<br/><input value={state.progBw} onChange={e=>setState(s=>({...s, progBw:e.target.value}))} placeholder="105" style={{ width:'100%', marginTop:4, background:'rgba(22,30,52,0.88)', color:'#fff', border:'1px solid rgba(140,190,255,0.16)', borderRadius:14, padding:'12px 12px', fontSize:14 }} /></label>
              </div>
              <div style={{ display:'flex', gap:6, marginTop:6, alignItems:'center' }}>
                ...
                <span style={{ fontSize:12, color:DIM }}>{smTrend ? `n=${smTrend.n} Δscore ${smTrend.scoreDelta} · best ${smTrend.bestScore} (${smTrend.bestDate})` : `история ${smProgressHist.length}/60`}</span>
              </div>
            </div>
            <div style={{ marginTop:8, padding:'12px 14px', borderRadius:14, background:'rgba(22,30,52,0.88)', border:'1px dashed rgba(140,190,255,0.16)', textAlign:'center' }}>
              <div style={{ fontSize:13, color:DIM }}>📹 Видео sway — измеряй lateral как max(x)-min(x) в Kinovea</div>
              <div style={{ marginTop:6, width:'100%', height:60, background:'rgba(255,255,255,0.03)', borderRadius:14, display:'flex', alignItems:'center', justifyContent:'center', color:DIM, fontSize:13, border:'1px solid rgba(255,255,255,0.04)' }}>preview — sway 3см норма, &gt;5см критично (McGill lateral bend)</div>
            </div>
          </div>
        )}
      </div>
      ...
      <div style={{ ...CARD, padding: 12, background:'rgba(239,68,68,0.08)', border:'1px solid rgba(239,68,68,0.16)' }}>
        <div style={{ fontSize:12, color:DIM, marginBottom:6 }}>Findings: {scoring.findings.map(f=>f.text).join(' · ') || '—'}</div>
        {scoring.floors.length>0 && <div style={{ fontSize:12, color:'#ef4444', marginBottom:6 }}>Floors: {scoring.floors.join(' · ')}</div>}
        <div style={{ fontSize:13, color:DIM, marginBottom:6 }}>Выбрано: {weakPoints.length? weakPoints.join(' · ') : '— баланс'} {smWeakPoints.length? `→ ${smWeakPoints.join(' · ')}` : ''}</div>
        {smRankTop.length > 0 && <div style={{ fontSize:12, color:'#5ee', marginBottom:6 }}>Ранжир: {smRankTop.map((c) => `${c.name} ${c.protocol.sets}×${c.protocol.reps} @${c.protocol.pct}% (score ${c.score})`).join(' · ')}</div>}
        {smSpec && <div style={{ fontSize:12, color:DIM, marginBottom:6 }}>Спец-блок {smSpec.totalWeeks}нед: {smSpec.weeks.slice(0, 3).map((w) => `нед${w.week} ${Object.values(w.targetSets)[0]}×5`).join(' · ')}… · {smSpec.rationale[0]}</div>}
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:6, marginBottom:8 }}>
          <label style={{ fontSize:13, color:DIM }}>Спец-блок нед (4-8)<br/><input value={state.specWeeks} onChange={e=>setState(s=>({...s, specWeeks:e.target.value}))} placeholder="6" style={{ width:'100%', marginTop:4, background:'rgba(22,30,52,0.88)', color:'#fff', border:'1px solid rgba(140,190,255,0.16)', borderRadius:14, padding:'12px 12px', fontSize:14 }} /></label>
          <label style={{ fontSize:13, color:DIM }}>Год: старт-неделя<br/><input value={state.annualStartWeek} onChange={e=>setState(s=>({...s, annualStartWeek:e.target.value}))} placeholder="1" style={{ width:'100%', marginTop:4, background:'rgba(22,30,52,0.88)', color:'#fff', border:'1px solid rgba(140,190,255,0.16)', borderRadius:14, padding:'12px 12px', fontSize:14 }} /></label>
        </div>
        ...
        <div style={{ display:'flex', gap:8, marginTop:10 }}>
          <button onClick={handleExport} style={{ flex:1, padding:'12px 8px', minHeight:52, borderRadius:14, background:'rgba(255,255,255,0.045)', border:'1px solid rgba(255,255,255,0.08)', color:DIM, fontSize:13, fontWeight:700, cursor:'pointer' }}>🖨 HTML</button>
          <button onClick={handleExportCsv} style={{ flex:1, padding:'12px 8px', minHeight:52, borderRadius:14, background:'rgba(255,255,255,0.045)', border:'1px solid rgba(255,255,255,0.08)', color:DIM, fontSize:13, fontWeight:700, cursor:'pointer' }}>📥 CSV</button>
          <button onClick={handleExportIcs} style={{ flex:1, padding:'12px 8px', minHeight:52, borderRadius:14, background:'rgba(255,255,255,0.045)', border:'1px solid rgba(255,255,255,0.08)', color:DIM, fontSize:13, fontWeight:700, cursor:'pointer' }}>📅 ICS</button>
          <button onClick={handleSaveAnnual} style={{ flex:1, padding:'12px 8px', minHeight:52, borderRadius:14, background:'rgba(255,255,255,0.045)', border:'1px solid rgba(255,255,255,0.08)', color:DIM, fontSize:13, fontWeight:700, cursor:'pointer' }}>🗓 Год</button>
          <button onClick={handleSMBackup} style={{ flex:1, padding:'12px 8px', minHeight:52, borderRadius:14, background:'rgba(255,255,255,0.045)', border:'1px solid rgba(255,255,255,0.08)', color:DIM, fontSize:13, fontWeight:700, cursor:'pointer' }}>📦 Бэкап</button>
        </div>
        <div style={{ fontSize:12, color:DIM, marginTop:6 }}>SM-storage: {(smStoreBytes.total / 1024).toFixed(1)}КБ · quota-safe (истории урезаются при переполнении, чужие ключи не трогаем)</div>
      </div>
    </div>
  );
};

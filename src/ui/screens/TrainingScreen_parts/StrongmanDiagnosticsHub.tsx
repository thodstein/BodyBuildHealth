...
export const StrongmanDiagnosticsHub: React.FC = () => {
  ...
  const proSnapExtra = () => ({
    ...
    contestSim: contestSim ? (contestSim.hasEstimate ? `${contestSim.predictedPlace} место/10 · слабые ${contestSim.weakEvents.join(', ') || '—'} · ${contestSim.recOrder.join(' → ')}` : `прогноза нет (${contestSim.noData || 0} ив. без данных)`) : null,

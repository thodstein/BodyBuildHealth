/**
 * bb-step-params.tsx — шаг 1 ББ-авто («📋 Базовые параметры»).
 *
 * Рефактор дедупликации (аудит «дубли шаг 1-2», план
 * `docs/BB-AUTO-STEPS-DEDUP-PLAN.md`):
 *  - РОВНО ОДИН контрол на настройку: раньше общий хвост дублировал 7 настроек
 *    в режиме «Программы + Адаптировать» (интенсив-техника дублировалась ещё и
 *    в генерике двумя разными селектами);
 *  - единый стиль всех карточек — `BbCard`/`BbFoldCard` (иконка-тайл, подпись,
 *    бейдж, акцентная кромка);
 *  - честные гейты режима: «женский цикл»/«целевой % жира» живут в генерик-ветке
 *    (там их применяет движок), packing в режиме источника заблокирован с пояснением;
 *  - «Объёмный тренинг» прямо перечисляет эффекты (было непонятно «что даёт»),
 *    «Цель объёма» честно помечается перебитой, селект схемы показывает реальное
 *    состояние без display-подмены.
 */
...
import { BbRowSwitch, BbCard, BbFoldCard, WEAK_GROUPS, type PlanMode, type Step } from './bb-auto-constructor-shared';
import { BTN, BTN_GHOST, H, IN } from './training-ui';
...
export const BbParamsStep: React.FC<BbParamsStepProps> = ({
  ...
  const isPrograms = planMode === 'programs';
  const isGeneric = planMode === 'generic_split';
  const limitText = (() => {
  ...
  return (
    <div>
      ...
      {/* 1. Источник программы: генерик-сплит или готовый цикл/программа */}
      <BbCard
        icon="📌" accent="#a855f7"
        title="Источник программы"
        desc="Генерик-сплит — полная авто-генерация по вашим параметрам. Программы — готовый цикл/программа из библиотеки: «точно» (дословно) или «адаптировать» под профиль."
      >
        ...
        {isPrograms && (
          <>
            ...
            {bbSource === 'cycle' && (
              <div style={{ marginTop: 8 }}>
                <PopupSelect label="BB-цикл" value={selectedCycleId} onChange={v => { setSelectedCycleId(v); const c = getCycleById(v); if (c) { setBbDays(c.meta.sessionsPerWeek); setBbWeeks(c.meta.weeks); } }} options={
                  bbCyclesList.map(c => ({ id: c.meta.id, label: `${c.meta.title} (${c.meta.weeks} нед, ${c.meta.sessionsPerWeek}×/нед)`, description: c.meta.description?.slice(0, 120) }))
                } />
                {selectedCycleId && (() => {
                  ...
                  return (
                    <div style={{ marginTop: 6, ...NOTE, lineHeight: 1.5 }}>
                      <div><b>Уровень:</b> {c.meta.level}{c.meta.targetFocus ? ` · <b>Фокус:</b> ${c.meta.targetFocus}` : ''}</div>
                      {c.meta.deloadWeeks && c.meta.deloadWeeks.length > 0 && <div><b>Разгрузка:</b> нед {c.meta.deloadWeeks.join(', ')}</div>}
                      {c.meta.rirProgression && <div><b>RIR:</b> {c.meta.rirProgression.start}→{c.meta.rirProgression.end}</div>}
                      {c.meta.phases && c.meta.phases.length > 0 && <div><b>Фазы:</b> {c.meta.phases.map(ph => ph.title || `нед ${ph.weekStart}-${ph.weekEnd}`).join(', ')}</div>}
                      <div style={{ marginTop: 4, padding: '4px 8px', borderRadius: 8, background: 'rgba(0,230,138,0.06)' }}>{c.meta.description?.slice(0, 200)}</div>
                    </div>
                  );
                })()}
              </div>
            ...
            {bbSource === 'program' && (
              <div style={{ marginTop: 8 }}>
                ...
                {selectedProgramId && !customCycle && (() => {
                  ...
                  return (
                    <div style={{ marginTop: 6, ...NOTE, lineHeight: 1.5 }}>
                      <div><b>Программа:</b> {p.name} · {p.author} · {p.level} · {p.daysPerWeek} дн/нед · {p.durationWeeks} нед · цель {p.goal}{p.direction && p.direction !== p.goal ? ` (${p.direction})` : ''}</div>
                      {p.targetAudience && <div style={{ marginTop: 3 }}><b>Кому:</b> {p.targetAudience.slice(0, 160)}{p.targetAudience.length > 160 ? '…' : ''}</div>}
                      {p.warnings && p.warnings.length > 0 && (
                        <div style={{ marginTop: 3, padding: '4px 8px', borderRadius: 8, background: 'rgba(245,158,11,0.08)', color: '#fbbf24' }}>
                          ⚠️ {p.warnings.slice(0, 2).join(' · ')}{p.warnings.length > 2 ? '…' : ''}
                        </div>
                      )}
                      <div style={{ marginTop: 4, padding: '4px 8px', borderRadius: 8, background: 'rgba(96,165,250,0.06)' }}>{p.description?.slice(0, 240)}</div>
                    </div>
                  );
                })()}
              </div>
            ...
            {/* Режим конвертации источника */}
            <div style={{ marginTop: 10, padding: '8px 10px', borderRadius: 10, background: 'rgba(96,165,250,0.06)', border: '1px solid rgba(96,165,250,0.15)' }}>
              ...
              <div style={{ marginTop: 6, ...NOTE, lineHeight: 1.4 }}>
                {bbAdaptMode === 'faithful'
                  ...
                  : 'Структура программы сохраняется, но применяются настройки секций ниже: слабые группы (+добивка), интенсив-техники, авто-делод, стратегия прогрессии, DUP/суперсеты, PED-фаза.'}
              </div>
            </div>
      ...
      {/* 2. Личный MEV — калибровка (необязательная) */}
      <BbFoldCard
        icon="🧪" accent="#60a5fa"
        title="Личный MEV (калибровка)"
        badge={mevCal ? (isMEVCalibrationComplete(mevCal) ? '✅ калиброван' : `📈 нед ${mevCalibrationProgress(mevCal).weeksDone}`) : 'не начата'}
        desc="Протокол RP: старт с MEV−2, +1 сет/нед, пока крепатура ≤2 и производительность не падает; 2 подряд недели деградации → фикс личного MEV. Применяется вместо таблицы (внутри капов)."
      >
        {!mevCal ? (
      ...
      {/* 3. Атлет — уровни/цель/объём (в режиме источника берутся из программы) */}
      {isGeneric && (
        <BbCard icon="🧬" accent="#f59e0b" title="Атлет и горизонт"
          desc="Уровень, стаж и цель задают капы, фазы и фоновый подбор сплита. В режиме источника эти данные берутся из выбранной программы."
        >
          ...
          {trainingVolumeMode === 'high' && (
            <div style={{ marginTop: 6, padding: '5px 8px', borderRadius: 8, background: 'rgba(245,158,11,0.08)', border: '1px solid rgba(245,158,11,0.22)', ...NOTE }}>
              ⤵ Цель объёма перебита режимом «Объёмный» (см. секцию «Объём»): сборка идёт на MRV.
            </div>
          )}
        </BbCard>
      )}

      {/* 4. Объём — один пульт: цель объёма + режим + схема памп-дней */}
      <BbCard icon="📦" accent="#3b82f6" title="Объём тренировки"
        desc="Три рычага: цель объёма (MEV/MAV/MRV), режим (обычный/объёмный) и схема памп-дней. Режим «Объёмный» перебивает цель на MRV и добавляет объём — см. разбор ниже."
      >
        <div style={{ padding: '8px 10px', borderRadius: 10, background: 'rgba(59,130,246,0.06)', border: '1px solid rgba(59,130,246,0.18)', marginBottom: 8 }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: '#3b82f6', marginBottom: 6 }}>📦 Объёмный тренинг <span style={{ fontSize: 9, fontWeight: 400, color: '#fff' }}>капы от уровня — новичок без фармы 60 сетов недоступно</span></div>
          <div style={{ display: 'flex', gap: 6 }}>
            ...
            <button onClick={() => { if (bbLevel === 'beginner' && peds.length === 0) flash('Объёмный для новичка: капы 24/10, объём MRV, GVT/FST-7 доступны.'); setTrainingVolumeMode('high'); }} style={{ flex: 1, padding: '8px 10px', borderRadius: 10, cursor: 'pointer', fontWeight: 700, fontSize: 11, border: trainingVolumeMode === 'high' ? '2px solid #f59e0b' : '1px solid rgba(255,255,255,0.08)', background: trainingVolumeMode === 'high' ? 'rgba(245,158,11,0.15)' : 'rgba(255,255,255,0.02)', color: trainingVolumeMode === 'high' ? '#f59e0b' : '#fff' }}>Объёмный</button>
          </div>
          <div style={{ marginTop: 6, ...NOTE, lineHeight: 1.5 }}>
        ...
        <div style={GRID2}>
          ...
          <PopupSelect
            ...
            hint={'Методики для памп-изоляций (кап 5 сетов/упр; FST-7 финишер — 7 одним движением). В коде: standard / gvt / fst7 / gironda.' + fst7Hint}
            options={[
              { id: 'standard', label: trainingVolumeMode === 'high' ? 'Стандартная (при «Объёмном» исполняется как GVT 10×10)' : 'Стандартная (авто)', desc: 'Авто-распределение: тяж — база, памп — изоляция по необходимости. Баланс сила/объём.' },
              ...
              ...(fst7Allowed ? [{ id: 'fst7', label: 'FST-7 (7×8-12 одним финишером)', desc: 'Rambod: 7 сетов одной изоляцией в конце мышцы, 30-45с. Только enhanced без joint-guard.' } as any] : []),
      ...
      {/* 5. Методики — один контрол на настройку */}
      <BbCard icon="🧩" accent="#a855f7" title="Методики"
        desc="Как строится каждый день: порядок упражнений, интенсив-техники, волновая периодизация, суперсеты, темп, калорийный контекст, вариативность и плотность отдыха."
      >
        <div style={GRID2}>
          <PopupSelect label="🎯 Фокус тренировки" value={bbTrainingFocus} onChange={v => setBbTrainingFocus(v as 'strength' | 'hypertrophy' | 'endurance')} options={[
            { id: 'strength', label: 'Сила: RIR 1-2', desc: 'Тяжёлые веса, низкая скорость, RIR 1-2 — максимальный механический натяг.' },
          ...
          <PopupSelect label="🧩 Методика порядка" value={bbMethodology} onChange={v => setBbMethodology(v as SessionMethodology)} hint="Порядок упражнений в дне — влияет на силу и утомление. В коде: compound_first / pre_exhaust / post_exhaust / mountain_dog / fst7 / hyperemia" options={[
            { id: 'compound_first', label: 'Базовые → изоляция (по умолчанию)', desc: 'Сначала тяжёлые многосуставные на свежие мышцы — максимум веса и безопасная техника, затем изоляция.' },
            { id: 'pre_exhaust', label: 'Pre-exhaust: изоляция первой', desc: 'Изоляция целевой мышцы до базы — утомляет заранее, база добивает. Вес в базе −10-15%.' },
            { id: 'post_exhaust', label: 'Post-exhaust: база → изоляция', desc: 'База в полную силу, сразу изоляция без отдыха — «пробить» мышцу двойным стимулом.' },
            { id: 'mountain_dog', label: 'Mountain Dog: активация → база → памп → стретч', desc: 'Лёгкая активация (RIR≥3), тяжёлая база, памп и loaded stretch 30-60с. Для MGF-фаз и суставов.' },
            { id: 'fst7', label: 'FST-7 порядок: праймер → финишер', desc: 'Памп-праймер первым, тяжёлая работа в середине, 7-сетовый финишер и стретч в конце. Под GH+инсулин окно.' },
          ...
          {isGeneric
            ? <PopupSelectSmart label="🎯 Интенсив-техника" value={intensityTech} onChange={onUserIntensityTech} suggestedIds={bbSuggest.intensityTechnique} suggestionReason="По цели и уровню (цепочка)" options={[
                { id: 'none', label: 'Авто по фазе', desc: 'Accumulation → пауза-репс, intensification/peaking → rest-pause/dropset по профилю мышцы.' },
                { id: 'rest_pause', label: 'Рест-пауза', desc: 'Финал 8 повт + 15с пауза → 3-4 + 15с → 3-4. Продлевает подход без сброса веса.' },
                { id: 'drop_set', label: 'Дроп-сет', desc: 'После отказа −20% веса → 6 повт, ещё −20% → 4 повт.' },
                { id: 'myo_reps', label: 'Myo-reps', desc: 'Активация 12-15 повт, затем 4×4 с 5с паузой.' },
                { id: 'pause_rep', label: 'Пауза-репс', desc: 'Пауза 2-3с внизу каждого повтора — убирает читинг, усиливает растянутую.' },
                { id: 'mechanical_drop', label: 'Мех. дроп-сет', desc: 'Смена угла/хвата без отдыха (жим гантелей → разводка).' },
                { id: 'negative', label: 'Негативы (3-4с)', desc: 'Медленный негатив 3-4с, быстрый подъём — акцент на эксцентрике.' },
              ]} />
            : <PopupSelect label="🎯 Интенсив-техника" value={intensityTech} onChange={v => setIntensityTech(v as IntensityTechnique)} hint='Техника на последнем подходе. В коде: none / rest_pause / drop_set / myo_reps / pause_rep / mechanical_drop / negative' options={[
          ...
          <PopupSelect label='🌊 Волновая периодизация (DUP)' value={dupMode} onChange={v => setDupMode(v as DUPMode)} hint='Чередование стимулов внутри недели — в коде: none / heavy_light / strength_hypertrophy / full_dup (Schoenfeld 2017)' options={[
            { id: 'none', label: 'Выкл (стандартная периодизация)', desc: 'Блочная периодизация: накопление → интенсификация → разгрузка.' },
            { id: 'heavy_light', label: 'Тяж/лёг (2 дня)', desc: 'Чередование тяжёлых и лёгких дней — тяж сила, лёгк объём/техника.' },
            { id: 'strength_hypertrophy', label: 'Сила/гипертрофия (2 дня)', desc: 'День силы 4-6 повт RIR 1-2 + день гипертрофии 10-15 RIR 2-3.' },
            { id: 'full_dup', label: 'Полный DUP (3 дня)', desc: 'Сила / гипертрофия / выносливость. Максимум вариативности, нужен опыт ≥2 года.' },
          ]} />
          <PopupSelect label='🔗 Суперсеты' value={supersetMode} onChange={v => setSupersetMode(v as 'none' | 'antagonist' | 'same_muscle' | 'giant')} hint='В коде: none / antagonist / same_muscle / giant. Выполняются без отдыха между упражнениями пары.' options={[
            { id: 'none', label: 'Выкл', desc: 'По очереди с полным отдыхом. Максимум силы в каждом движении.' },
            { id: 'antagonist', label: 'Антагонисты (пары)', desc: 'Грудь ↔ спина, бицепс ↔ трицепс. Экономия 30% времени.' },
            { id: 'same_muscle', label: 'Одна группа (пробить)', desc: 'Компаунд + изоляция одной мышцы без отдыха — сильный памп.' },
            { id: 'giant', label: 'Гигант-сет (3 упр. одной группы)', desc: 'Три упражнения одной группы подряд без отдыха. Для продвинутых, RIR 3+.' },
          ]} />
          <PopupSelect label="⬇️ Эксцентрик" value={String(eccentricMult)} onChange={v => setEccentricMult(parseFloat(v))} hint="Множитель эксцентрики — в коде eccentricMult: 1.0 / 1.1 / 1.2 (Schoenfeld 2021)." options={[
            ...
            { id: '1.1', label: '1.1 — +10%', desc: 'Медленнее опускание, умеренный рост стимула.' },
            { id: '1.2', label: '1.2 — +20%', desc: 'Выраженный акцент на негативе, требует техники и восстановления.' },
          ]} />
          ...
          <PopupSelect label="🔄 Вариативность упражнений" value={rotationMode} onChange={v => setRotationMode(v as any)} hint="Как часто менять упражнения — в коде rotationMode: forbid / strict / variety." options={[
            { id: 'forbid', label: '🚫 Запрет — одни и те же', desc: 'Стабильная прогрессия по весам весь мезоцикл.' },
            { id: 'strict', label: '📅 Строгий — смена раз в 4 недели', desc: 'Смена на границе фаз — баланс стабильности и разнообразия.' },
            { id: 'variety', label: '🎨 Разнообразие — при 2×/мышцу', desc: 'Чередование углов при 2+ тренировках мышцы — снижает привыкание.' },
          ]} />
          <PopupSelect label="⏱ Плотность (отдых)" value={intensityLevel} onChange={v => setIntensityLevel(v as any)} hint="Управляет паузой между подходами (не интенсивностью-техникой). В коде intensityLevel: light / moderate / high." options={[
            { id: 'light', label: '🌿 Лёгкая — отдых +20%', desc: 'Тяж 3 мин, памп 75с. Подходит при плохом восстановлении.' },
      ...
      {/* 6. Прогрессия и разгрузка — общий блок для обоих режимов */}
      <BbCard icon="📈" accent="#00e68a" title="Прогрессия и разгрузка"
        desc="Как растёт нагрузка по неделям и когда автоматически ставится разгрузка."
      >
        {isGeneric
          ? <PopupSelectSmart label="Стратегия прогрессии" value={loadStrategy} onChange={onUserLoadStrategy} suggestedIds={bbSuggest.loadStrategy} suggestionReason={bbSuggest.goalDesc.split('.')[0]} options={[
          ...
          : <PopupSelect label="📈 Стратегия прогрессии" value={loadStrategy} onChange={v => setLoadStrategy(v as LoadStrategy)} options={[
              { id: 'double_progression', label: 'Двойная прогрессия', desc: 'Сначала добить повторы, затем +5% веса.' },
              { id: 'linear', label: 'Линейная', desc: '+2.5 кг/нед компаунд, +1 кг изоляция.' },
              { id: 'wave', label: 'Волновая', desc: '3-нед волны тяж/сред/лёг.' },
              { id: 'rpe_based', label: 'RPE-based', desc: 'Вес по ощущению RPE (авто-регуляция).' },
            ]} />}
        <div style={{ marginTop: 4, ...NOTE }}>
        ...
        <div style={{ marginTop: 8 }}>
          <BbRowSwitch checked={autoDeload} onChange={setAutoDeload} icon="🛡️" accent="#22c55e"
            title="Авто-разгрузка при ACWR > 1.3" desc="Автоматически вставить разгрузочную неделю при перегрузке (ACWR > 1.3)" ariaLabel="Авто-разгрузка" />
        </div>
        {autoDeload && (
          <div style={{ marginTop: 2 }}>
            {isGeneric
              ...
              : <PopupSelect label="📉 Тип разгрузки" value={deloadType} onChange={v => setDeloadType(v as DeloadType)} hint='В коде: pump / neural / full_rest / mini' options={[
                  { id: 'pump', label: 'Pump-разгрузка', desc: 'Лёгкие веса 15-20 повт.' },
                  { id: 'neural', label: 'Нейр-разгрузка', desc: 'Мало сетов, умеренный вес.' },
                  { id: 'full_rest', label: 'Полный отдых', desc: '20% объёма, 40% веса.' },
                  { id: 'mini', label: 'Мини-делоад', desc: '−1-2 сета, вес почти тот же.' },
                ]} />}
            <div style={{ marginTop: 4, padding: '4px 8px', borderRadius: 8, background: 'rgba(34,197,94,0.06)', ...NOTE }}>
      ...
      <BbCard icon="🏋️" accent="#22d3ee" title="Подбор упражнений"
        desc="Фильтры пула, авто-регуляция и персональные предпочтения. Синхронизируется с профилем (🧬 Профиль тренированности)."
      >
        <BbRowSwitch checked={avoidAxialLoadUi} onChange={setAvoidAxialLoadUi} icon="🚫" accent="#ef4444"
          title="Исключить осевую нагрузку" desc="Убрать упражнения с нагрузкой на позвоночник (приседы, тяги со штангой)" />
        <BbRowSwitch checked={fewerCompound} onChange={setFewerCompound} icon="🏗️" accent="#f59e0b"
          title="Меньше многосуставных" desc="Больше замен на тренажёры и изолирующие упражнения" />
        <BbRowSwitch checked={allowStrengthLifts} onChange={setAllowStrengthLifts} icon="🏋️" accent="#3b82f6" disabled={bbGoal !== 'strength_mass'}
          title="Становая / жим стоя" desc={bbGoal === 'strength_mass' ? 'Включить становую и жим стоя в план' : 'Доступно в цели «Сила + Масса»'} />
        <BbRowSwitch checked={abRotation} onChange={setAbRotation} icon="🔀" accent="#22d3ee"
          title="A/B ротация паттернов" desc="Одинаковые дни недели — разные движения (generic + adapt; faithful дословно)" />
        <BbRowSwitch checked={autoRegOn} onChange={setAutoRegOn} icon="🤖" accent="#22c55e"
        ...
        <div style={{ marginTop: 4, ...NOTE }}>
          Любимые получают приоритет при отборе, не любимые исключаются из генерации. Синхронизируется с профилем.
        </div>
      </BbCard>

      {/* 9. Оборудование и загрузка */}
      <BbCard icon="🏗️" accent="#60a5fa" title="Оборудование и загрузка"
        desc="Ограничивает пул отбора и округляет веса к реально достижимым на вашем наборе пластин."
      >
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
          {([['barbell', 'Штанга'], ['dumbbell', 'Гантели'], ['cable', 'Блок/кроссовер'], ['machine', 'Тренажёр'], ['kettlebell', 'Гири'], ['bodyweight', 'Свой вес'], ['bands', 'Резинки']] as const).map(([id, label]) => {
            ...
            return (
              <button key={id} onClick={() => setBbEquipment(prev => prev.includes(id) ? prev.filter(x => x !== id) : [...prev, id])}
                style={{ padding: '5px 10px', borderRadius: 14, fontSize: 10, fontWeight: 700, cursor: 'pointer', minHeight: 38, border: on ? '1px solid #60a5fa' : '1px solid rgba(255,255,255,0.08)', background: on ? 'rgba(96,165,250,0.15)' : 'rgba(255,255,255,0.02)', color: on ? '#60a5fa' : '#fff' }}>
                {label}{on ? ' ✓' : ''}
              </button>
            );
          })}
        </div>
        <div style={{ marginTop: 4, ...NOTE }}>Если ничего не выбрано — используются все упражнения. Выбор ограничивает пул отбора.</div>
        <div style={{ marginTop: 8 }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: '#93c5fd', marginBottom: 4 }}>🏋️ Набор пластин (микрозагрузка)</div>
          <div style={{ ...NOTE, opacity: 0.9, marginBottom: 6 }}>Веса плана округляются к реально достижимым на вашем наборе (стандарт / микрозагрузка / домашний / машины-шаг 1).</div>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
      ...
      >
        <div style={{ padding: '8px 10px', borderRadius: 10, background: 'rgba(96,165,250,0.04)', border: '1px solid rgba(96,165,250,0.14)', marginBottom: 8 }}>
          ...
          <div style={{ ...NOTE, opacity: 0.9, marginBottom: 6 }}>Утренний HRV и сон перекрывают профиль в recovery (пишется в he_wearable_daily).</div>
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 8 }}>
            {isFemaleProfile && (
              <div style={{ flex: '1 1 220px', padding: 10, borderRadius: 10, background: 'rgba(244,114,182,0.04)', border: '1px solid rgba(244,114,182,0.14)' }}>
                ...
                <div style={{ ...NOTE, opacity: 0.9, marginBottom: 4 }}>День цикла (1-28): в лютеиновую фазу объём −5%.</div>
            ...
            {(bbGoal === 'cut' || bbGoal === 'recomp') && (
              <div style={{ flex: '1 1 220px', padding: 10, borderRadius: 10, background: 'rgba(239,68,68,0.04)', border: '1px solid rgba(239,68,68,0.14)' }}>
                <div style={{ fontSize: 11, fontWeight: 700, color: '#f87171', marginBottom: 4 }}>🎯 Целевой % жира</div>
                <div style={{ ...NOTE, opacity: 0.9, marginBottom: 4 }}>Далеко от цели → объём ближе к MEV (сохранение мышц); близко → полный объём.</div>
        ...
        <div style={{ marginTop: 8, padding: 10, borderRadius: 10, background: 'rgba(16,185,129,0.04)', border: '1px solid rgba(16,185,129,0.15)' }}>
          ...
          <div style={{ ...NOTE, opacity: 0.9, marginBottom: 6 }}>Прогрессивная рампа: нед 1-2 ×50% объёма / ×70% вес, далее ramp к полному (не пересекается со щадящими травмами).</div>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
            {WEAK_GROUPS.map(([id, l]) => {
        ...
        <div style={{ marginTop: 8, padding: 10, borderRadius: 10, background: 'rgba(245,158,11,0.04)', border: '1px solid rgba(245,158,11,0.18)' }}>
          <div style={{ fontSize: 11, fontWeight: 800, color: '#f59e0b', marginBottom: 4 }}>🦴 Ограничения мобильности <span style={{ fontSize: 9, fontWeight: 700, padding: '2px 8px', borderRadius: 999, background: 'rgba(245,158,11,0.12)', border: '1px solid rgba(245,158,11,0.3)' }}>биомеханика</span></div>
          <div style={{ ...NOTE, opacity: 0.9, marginBottom: 6 }}>Такие упражнения будут заменены на биомеханически безопасные альтернативы.</div>
          ...
          <div style={{ marginTop: 8, fontSize: 9, color: '#fff', lineHeight: 1.45 }}>
            💡 <b>Чем отличается от «Травм»:</b> травмы защищают <b>мышцу</b> (исключение или щадящая нагрузка), а мобильность — конкретные <b>движения</b>. Работают вместе, не дублируя друг друга.
          </div>
        </div>
      </BbCard>

      {savedPlans.length > 0 && (
        <BbRowSwitch checked={usePreviousPlan} onChange={setUsePreviousPlan} icon="🔗" accent="#00e68a"
          title="Cross-mesocycle: прогрессия из последнего плана" desc={savedPlans[0]?.name || ''} ariaLabel="Прогрессия из последнего плана" />
      )}

  ...
  return (
    <div className="train-cardiovol" style={CARD}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
        ...
        <button style={metric === 'minutes' ? { ...BTN_SMALL, border: '1px solid rgba(0,230,138,0.50)', color: '#4ade80', boxShadow: '0 0 10px rgba(0,230,138,0.16)' } : BTN_SMALL} onClick={() => setMetric('minutes')} aria-pressed={metric === 'minutes'}>мин</button>
        <button style={metric === 'kcal' ? { ...BTN_SMALL, border: '1px solid rgba(0,230,138,0.50)', color: '#4ade80', boxShadow: '0 0 10px rgba(0,230,138,0.16)' } : BTN_SMALL} onClick={() => setMetric('kcal')} aria-pressed={metric === 'kcal'}>ккал</button>
        <button style={metric === 'trimp' ? { ...BTN_SMALL, border: '1px solid rgba(139,92,246,0.50)', color: '#a78bfa', boxShadow: '0 0 10px rgba(139,92,246,0.16)' } : BTN_SMALL} onClick={() => setMetric('trimp')} title="TRIMP = нагрузка (Banister)" aria-pressed={metric === 'trimp'}>TRIMP</button>
        <button style={metric === 'ctl' ? { ...BTN_SMALL, border: '1px solid rgba(59,130,246,0.50)', color: '#60a5fa', boxShadow: '0 0 10px rgba(59,130,246,0.16)' } : BTN_SMALL} onClick={() => setMetric('ctl')} title="CTL/ATL/TSB (Fitness-Fatigue)" aria-pressed={metric === 'ctl'}>CTL</button>
        {fact && (

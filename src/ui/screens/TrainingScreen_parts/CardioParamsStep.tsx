...
const GOAL_CARD: React.CSSProperties = {
  flex: '1 1 150px', padding: '11px 13px', borderRadius: 13, cursor: 'pointer', textAlign: 'left',
  border: '1px solid rgba(255,255,255,0.10)', background: 'linear-gradient(180deg, rgba(255,255,255,0.05), rgba(255,255,255,0.02))', color: '#fff',
  transition: 'all 0.15s ease', boxShadow: 'inset 0 1px 0 rgba(255,255,255,0.04)',
};
const GOAL_CARD_ACTIVE: React.CSSProperties = {
  ...GOAL_CARD, border: '1px solid rgba(0,230,138,0.55)', background: 'linear-gradient(180deg, rgba(0,230,138,0.16), rgba(0,230,138,0.06))', color: '#fff',
  boxShadow: '0 0 16px rgba(0,230,138,0.20), inset 0 1px 0 rgba(255,255,255,0.07)',
};
const PRESET: React.CSSProperties = {
  flex: '0 0 160px', padding: '11px 13px', borderRadius: 13, cursor: 'pointer', textAlign: 'left',
  border: '1px solid rgba(255,255,255,0.10)', background: 'linear-gradient(180deg, rgba(255,255,255,0.05), rgba(255,255,255,0.02))', color: '#fff',
  transition: 'all 0.15s ease', boxShadow: 'inset 0 1px 0 rgba(255,255,255,0.04)',
};
...
/**
 * Подсекции шага параметров для ББ-оболочки (шаги 1-3: Цель / Атлет / Нагрузка).
 * Только подача: JSX скопирован из CardioParamsStep 1-в-1, движки/строки не менялись.
 * Сам CardioParamsStep ниже не тронут (совместимость существующих вызовов).
 */
export interface CardioParamsModel {
  goal: CardioGoal;
  totalWeeks: number;
...
export function useCardioParamsPreview(m: CardioParamsModel): {
  preview: { cycle: CardioCycle | null; warnings: string[] };
  s: ReturnType<typeof cardioCycleSummary> | null;
  tidPreview: { pi: number | null; model: string; label: string } | null;
} {
  ...
  return { preview, s, tidPreview };
}

/** Шаг 1 ББ-оболочки: цель цикла + горизонт и структура фаз (бывшие sec-goal/sec-horizon). */
export const CardioGoalHorizonSection: React.FC<{
  goal: CardioGoal; setGoal: (g: CardioGoal) => void;
  totalWeeks: number; setTotalWeeks: (n: number) => void;
  daysAvailable: number; setDaysAvailable: (n: number) => void;
  recoveryLow: boolean; setRecoveryLow: (v: boolean) => void;
  phaseSplit: PhaseSplitState; setPhaseSplit: (s: PhaseSplitState) => void;
  comps: CardioCompetitionRef[];
  taperWeeks: number; setTaperWeeks: (n: number) => void;
  taperEnabled: boolean; setTaperEnabled: (v: boolean) => void;
  peakWeek: boolean; setPeakWeek: (v: boolean) => void;
  previewFactors?: CardioPreviewFactors;
  periodizationModel?: CardioPeriodizationModel; setPeriodizationModel?: (m: CardioPeriodizationModel) => void;
  taperModel?: CardioTaperModel; setTaperModel?: (m: CardioTaperModel) => void;
  maxHrFormula?: 'classic' | 'tanaka' | 'gulati'; setMaxHrFormula?: (f: 'classic' | 'tanaka' | 'gulati') => void;
  wizardMode?: 'simple' | 'pro';
}> = ({ goal, setGoal, totalWeeks, setTotalWeeks, daysAvailable, setDaysAvailable, recoveryLow, setRecoveryLow, phaseSplit, setPhaseSplit, comps, taperWeeks, setTaperWeeks, taperEnabled, setTaperEnabled, peakWeek, setPeakWeek, previewFactors, periodizationModel, setPeriodizationModel, taperModel, setTaperModel, maxHrFormula, setMaxHrFormula, wizardMode = 'pro' }) => {
...
/** Шаг 2 ББ-оболочки: атлет — пользователь + PRO полевые тесты (бывшие sec-user/sec-field). */
export const CardioAthleteSection: React.FC<{
  age: string; setAge: (v: string) => void;
  bodyWeight: number; setBodyWeight: (n: number) => void;
  restingHr: string; setRestingHr: (v: string) => void;
  sex: 'male' | 'female'; setSex: (s: 'male' | 'female') => void;
...
/** Шаг 3 ББ-оболочки: нагрузка — оборудование, дни ног, факторы (бывшие sec-equip/sec-factors). */
export const CardioLoadSection: React.FC<{
  equipment: CardioEquipment[]; setEquipment: (e: CardioEquipment[]) => void;
  lowImpact: boolean; setLowImpact: (v: boolean) => void;
  legDays: number[]; setLegDays: (d: number[]) => void;
  ...
  wizardMode?: 'simple' | 'pro';
}> = ({ equipment, setEquipment, lowImpact, setLowImpact, legDays, setLegDays, factorsOn, onToggleFactor, factorsSummary, wizardMode = 'pro' }) => {
...
/** Компактный итог параметров для шагов ББ-оболочки (тот же hero sec-preview). */
export const CardioParamsPreviewHero: React.FC<{
  preview: { cycle: CardioCycle | null; warnings: string[] };
  s: ReturnType<typeof cardioCycleSummary> | null;
  tidPreview: { pi: number | null; model: string; label: string } | null;
  totalWeeks: number;
  ...
  peakWeek: boolean;
  onReset: () => void;
}> = ({ preview, s, tidPreview, totalWeeks, goal, taperEnabled, taperWeeks, peakWeek, onReset }) => (
  <div style={CARD_HERO} id="sec-preview">
    ...
    {preview.cycle && (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
        ...
        <div style={{ display: 'flex', gap: 3, flexWrap: 'nowrap', overflowX: 'auto', scrollbarWidth: 'none', padding: '3px 0' }}>
          {preview.cycle.weeks.map(w => (
            <div key={w.week} style={{ flex: '1 0 12px', minWidth: 10, height: 22, borderRadius: 6, background: PHASE_COLOR[w.phase] ?? '#888', opacity: w.deload ? 0.5 : 1, boxShadow: w.deload ? 'inset 0 0 0 1px rgba(255,255,255,0.20)' : '0 1px 4px rgba(0,0,0,0.30)' }} title={`Нед ${w.week} · ${w.phase}${w.deload ? ' · делод' : ''}${w.taper ? ' · taper' : ''}`} />
          ))}
        </div>
        <div style={{ display: 'flex', gap: 9, flexWrap: 'wrap' }}>
          {PHASE_ORDER.filter(p => (s?.phaseWeeks[p.phase] ?? 0) > 0).map(p => (
            <span key={p.phase} style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 11.5, color: 'rgba(255,255,255,0.78)', fontWeight: 700, fontVariantNumeric: 'tabular-nums' }}>
              <span style={{ width: 9, height: 9, borderRadius: 3, background: PHASE_COLOR[p.phase], boxShadow: `0 0 6px ${PHASE_COLOR[p.phase]}88` }} />

  ...
  return (
    <div className="train-microcyc" style={{ padding: 12, borderRadius: 14, background: 'rgba(24,24,27,0.4)', border: '1px solid rgba(0,230,138,0.15)', marginBottom: 10 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 10 }}>
        <div>
          <label style={{ display: 'block', fontSize: 10, color: '#fff', marginBottom: 2 }}>Недель: <b style={{ color: ACCENT }}>{weeks}</b></label>
          <input type="range" min={4} max={16} step={1} value={weeks} onChange={e => setWeeks(+e.target.value)} style={{ width: '100%' }} />
        </div>
        <div>
          <label style={{ display: 'block', fontSize: 10, color: '#fff', marginBottom: 2 }}>Цель</label>
      ...
      <div style={{ fontSize: 10, color: '#fff', marginBottom: 8 }}>Модель периодизации: <b style={{ color: ACCENT }}>{styleLabel}</b>. Фаз: {plan.phases.length}.</div>
        ...
        {plan.phases.map((ph, i) => (
          <div key={i} style={{ padding: 10, borderRadius: 10, background: 'rgba(255,255,255,0.03)', borderLeft: `4px solid ${phaseColor[ph.phase]}` }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <span style={{ fontSize: 10, color: '#fff' }}>нед {rows.filter(r => r.phase === ph.phase)[0]?.week}–{rows.filter(r => r.phase === ph.phase).slice(-1)[0]?.week} ({ph.weeks} нед)</span>
            </div>
            <div style={{ fontSize: 10, color: '#fff', marginTop: 4, lineHeight: 1.5 }}>{PHASE_HINTS[ph.phase] || ''}</div>
            <div style={{ display: 'flex', gap: 8, fontSize: 10, color:'#fff', marginTop: 6, flexWrap: 'wrap' }}>
      ...
      <div style={{ marginTop: 10 }}>
        <div style={{ fontSize: 10, fontWeight: 700, color: '#fff', textTransform: 'uppercase', letterSpacing: '0.3px', marginBottom: 6 }}>Таймлайн по неделям</div>
        <div style={{ display: 'flex', gap: 3, flexWrap: 'wrap' }}>
          {rows.map(r => (
            <div key={r.week} title={`Нед ${r.week}: ${PHASE_LABELS[r.phase] || r.phase} · объём ${r.params.volumeLevel}`} style={{ flex: '1 0 28px', minWidth: 28, padding: '6px 4px', borderRadius: 6, textAlign: 'center', background: phaseColor[r.phase] + '18', border: `1px solid ${phaseColor[r.phase]}40` }}>
              ...
              <div style={{ fontSize: 10, color: '#fff' }}>{PHASE_LABELS[r.phase] || ''}</div>
            </div>
          ))}
        </div>
      </div>
      <div style={{ marginTop: 8, padding: 12, borderRadius: 12, background: 'rgba(0,230,138,0.06)', border: '1px solid rgba(0,230,138,0.2)' }}>
        <div style={{ fontSize: 10, color: '#fff', marginBottom: 8 }}>🔗 Применить первую фазу «{PHASE_LABELS[plan.phases[0].phase] || plan.phases[0].phase}» (объём {plan.phases[0].params.volumeLevel}) к планировщику. Последовательность: {plan.phases.map(ph => (PHASE_LABELS[ph.phase] || ph.phase) + ' ' + ph.weeks + 'нед').join(' → ')}.</div>

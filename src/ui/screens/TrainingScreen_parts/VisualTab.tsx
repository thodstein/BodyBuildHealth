...
export const VisualTab: React.FC<{ sessions: any[] }> = ({ sessions }) => {
  ...
  if (sessions.length < 2) return (
    <div style={{ padding:24, textAlign:'center', background:'rgba(24,24,27,0.08)', borderRadius:16, border:'1px solid rgba(255,255,255,0.04)' }}>
      ...
      <div style={{ fontSize:12, color:'rgba(255,255,255,0.85)' }}>Нужно минимум 2 тренировки для визуализации</div>
    </div>
  );
  ...
  const gLabel: React.CSSProperties = { fontSize:10, color:'rgba(255,255,255,0.85)', fontWeight:500, letterSpacing:'0.3px', textTransform:'uppercase', marginBottom:8 };
  ...
  return (<div style={{ display:'flex', flexDirection:'column', gap:4 }}>
    ...
    {dashboard && <div style={glassCard}>
      ...
      <div style={{ display:'flex', alignItems:'flex-end', gap:3, height:90, padding:'4px 2px' }}>
        {weekly.map((w,i) => { const maxV = Math.max(...weekly.map(x=>x.volume),1); return <div key={i} style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center' }}>
          ...
          <span style={{ fontSize:10, color:'rgba(255,255,255,0.85)', marginTop:4, fontWeight:500 }}>Н{w.week}</span>
        </div>})}
      </div>
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:4, fontSize:10, marginTop:6, padding:'6px 4px', background:'rgba(255,255,255,0.03)', borderRadius:8 }}>
        <span style={{ color:'rgba(255,255,255,0.85)' }}>Пик <b style={{color:'#fff'}}>{(dashboard.summary as any).peakVolume || dashboard.summary.totalVolume}</b></span>
        <span style={{ color:'rgba(255,255,255,0.85)' }}>Интенс. <b style={{color:'#fff'}}>{dashboard.summary.avgIntensity}%</b></span>
        <span style={{ color:'rgba(255,255,255,0.85)' }}>Тренд <b style={{color:(dashboard.summary as any).trend==='up'?'#34d399':(dashboard.summary as any).trend==='down'?'#f87171':'#9ca3af'}}>{(dashboard.summary as any).trend==='up'?'↑ +':(dashboard.summary as any).trend==='down'?'↓ ':'→'}</b></span>
      </div>
    </div>}
    ...
    {muscleVol.length > 0 && <div style={glassCard}>
      ...
      {muscleVol.map((mv,i) => <div key={i} style={{ display:'flex', alignItems:'center', gap:8, marginBottom:3 }}>
        <span style={{ width:55, fontSize:10, color:'rgba(255,255,255,0.85)', fontWeight:500, textAlign:'right' }}>{mv.muscle}</span>

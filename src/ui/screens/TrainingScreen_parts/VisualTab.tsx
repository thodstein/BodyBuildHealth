  ...
  return (<div style={{ display:'flex', flexDirection:'column', gap:4 }}>
    ...
    {dashboard && <div style={glassCard}>
      ...
      <div style={{ display:'flex', alignItems:'flex-end', gap:3, height:90, padding:'4px 2px' }}>
        {weekly.map((w,i) => { const maxV = Math.max(...weekly.map(x=>x.volume),1); return <div key={i} style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center' }}>
          ...
          <span style={{ fontSize:10, color:'#fff', marginTop:4, fontWeight:500 }}>Н{w.week}</span>
        </div>})}
      </div>
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:4, fontSize:10, marginTop:6, padding:'6px 4px', background:'rgba(255,255,255,0.03)', borderRadius:8 }}>
        <span style={{ color:'#fff' }}>Пик <b style={{color:'#fff'}}>{(dashboard.summary as any).peakVolume || dashboard.summary.totalVolume}</b></span>
        <span style={{ color:'#fff' }}>Интенс. <b style={{color:'#fff'}}>{dashboard.summary.avgIntensity}%</b></span>
        <span style={{ color:'#fff' }}>Тренд <b style={{color:(dashboard.summary as any).trend==='up'?'#34d399':(dashboard.summary as any).trend==='down'?'#f87171':'#9ca3af'}}>{(dashboard.summary as any).trend==='up'?'↑ +':(dashboard.summary as any).trend==='down'?'↓ ':'→'}</b></span>
      </div>
    </div>}
    ...
    {muscleVol.length > 0 && <div style={glassCard}>
      ...
      {muscleVol.map((mv,i) => <div key={i} style={{ display:'flex', alignItems:'center', gap:8, marginBottom:3 }}>
        <span style={{ width:55, fontSize:10, color:'#fff', fontWeight:500, textAlign:'right' }}>{mv.muscle}</span>
        ...
        <span style={{ fontSize:10, fontWeight:700, color:'#fff', minWidth:32, textAlign:'right' }}>{mv.percent}%</span>
      </div>)}
    </div>}
        ...
        {prog.slice(0,5).map((p,i) => {
          ...
          return <div key={i} style={{ background:'rgba(255,255,255,0.03)', borderRadius:10, padding:'6px 8px' }}>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:4 }}>
              <span style={{ fontSize:10, fontWeight:600, color:'#fff' }}>{p.exercise}</span>

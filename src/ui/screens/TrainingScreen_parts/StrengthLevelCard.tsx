...
export const StrengthLevelCard: React.FC = () => {
  ...
  return (<div className="card" style={{ marginTop:8, padding:10 }}>
    ...
    <div style={{ display:'grid',gridTemplateColumns:'1fr 1fr',gap:4 }}>
      ...
      <div><label style={{ fontSize:10 }}>Вес тела (кг)</label><input type="number" value={slWt} onChange={e=>setSlWt(parseFloat(e.target.value) || 0)} style={{ width:'100%',padding:'4px',borderRadius:4,background:'rgba(255,255,255,0.04)',border:'1px solid rgba(255,255,255,0.08)',color:'#fff',fontSize:11,boxSizing:'border-box' }} /></div>
      <div><label style={{ fontSize:10 }}>1RM (кг)</label><input type="number" value={sl1RM} onChange={e=>setSl1RM(parseFloat(e.target.value) || 0)} style={{ width:'100%',padding:'4px',borderRadius:4,background:'rgba(255,255,255,0.04)',border:'1px solid rgba(255,255,255,0.08)',color:'#fff',fontSize:11,boxSizing:'border-box' }} /></div>
    </div>

...
export function targetLabelFor(ex: TargetableExercise): string {
  ...
  // ПЛЕЧИ
  if (m === 'shoulders' || m === 'delt_mid' || m === 'delt_front' || m === 'delt_rear') {
    if (/жим|press|армей/i.test(n)) return 'Плечи (передняя+средняя): хват чуть шире плеч, гриф к ключице, таз напряжён, без прогиба поясницы';
    if (/мах|lateral|отведен/i.test(n)) return 'Средняя дельта: мах до уровня плеч, локти чуть согнуты, пауза 1с вверху, медленно вниз';
    if (/задн|rear|обратн|наклон/i.test(n)) return 'Задняя дельта: наклон корпуса 30-45°, локти ведут вверх, сводите лопатки, без рывков';
    return 'Плечи: контролируйте траекторию, без читинга, полная амплитуда';
  }
  // ТРАПЕЦИИ
  if (m === 'traps') {
    if (/шраг|shrug/i.test(n)) return 'Трапеции: плечи к ушам строго вверх, пауза 1-2с вверху, медленно вниз 2с, без вращения плечами';
    return 'Трапеции: подъём плеч к ушам, задержка вверху, под контролем';
  }
  // ИКРЫ
  if (m === 'calves') {
    if (/сидя|seated/i.test(n) || /сидя/i.test(n)) return 'Икры (камбаловидная): колено 90°, подъём на носки с паузой 1-2с в растяжке внизу и пике вверху';
    if (/стоя|standing|жим.*ног/i.test(n)) return 'Икры (икроножная): колено прямое, максимум растяжения 2с внизу, подъём с паузой 1с вверху';
    return 'Икры: полная амплитуда, пауза в растянутой и сокращённой, без рывков';
  }
  // ПРЕСС
  if (m === 'abs') {
    if (/скручиван|crunch|пресс/i.test(n)) return 'Пресс: подъём лопаток, поясница прижата, подбородок к груди, выдох на усилии, пауза 1с';
    if (/подъём.*ног|ногами|leg.*raise/i.test(n)) return 'Пресс (низ): ноги прямые/согнутые, таз чуть подкрутить к груди, без раскачки';
    return 'Пресс: контролируемое скручивание, без рывков, держите напряжение';
  }
  // ПРЕДПЛЕЧЬЯ
  if (m === 'forearms') {
    if (/запяст|wrist|зоттман/i.test(n)) return 'Предплечья: сгибание/разгибание запястий с паузой 1с, хват крепкий, без читинга корпусом';
    return 'Предплечья/хват: медленный контроль, полная амплитуда';
  }
  // ЯГОДИЦЫ — дополнительно
  if (m === 'glutes' && !/мост|hip/i.test(n)) {
    if (/отведен|abduct|махи.*ног/i.test(n)) return 'Ягодицы (средняя): отведение бедра с паузой 1с, корпус стабилен, без наклона';
  }

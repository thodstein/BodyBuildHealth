      ...
      <g key={col + pts.length}>
        ...
        {dots && pts.map((v, i) => (
          <circle key={i} cx={px(i, pts.length)} cy={py(v)} r={lastBold && i === pts.length - 1 ? 3 : 1.8}
            fill={lastBold && i === pts.length - 1 ? col : '#fff'} stroke={lastBold && i === pts.length - 1 ? '#000' : 'none'} strokeWidth={0.5}>
  ...
  return (
    <div>
      <svg width="100%" viewBox={`0 0 ${width} ${height}`} style={{ display: 'block', maxWidth: width }}>
        ...
        <text x={2} y={height - 4} fontSize={7} fill="#fff">{Math.round(min)}{ySuffix}</text>
        <text x={2} y={10} fontSize={7} fill="#fff">{Math.round(max)}{ySuffix}</text>
        {series
      ...
      {data.map((d, i) => {
        ...
        return (
          <g key={i}>
            ...
            {d.value > 0 && <text x={x + bw / 2} y={y - 3} textAnchor="middle" fill="#fff" fontSize={7}>{Math.round(d.value)}</text>}
            {d.label && <text x={x + bw / 2} y={height - 1} textAnchor="middle" fill="#fff" fontSize={7}>{d.label}</text>}
          </g>
        );
      })}
    </svg>

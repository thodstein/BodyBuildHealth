    ...
    <div style={{ ...CARD, padding: 10, borderLeft: '2px solid rgba(167,139,250,0.3)' }}>
      ...
      <div style={{ padding: '6px 8px', background: 'rgba(167,139,250,0.06)', borderRadius: 6, marginBottom: 8 }}>
        ...
        <div style={{ fontSize: 10, color: DIM, marginTop: 4 }}>
          Процентки/сеты/повторения цикла <b>не редактируются</b> — это профессиональная методика. Ниже ваш оверлей.
        </div>
      </div>

      <div style={{ fontSize: 10, fontWeight: 800, color: ACCENT, marginBottom: 4 }}>🎯 Ваши ПМ (для расчёта весов из %)</div>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 6, marginBottom: 8 }}>
        <WmField label="Присед" value={wm.squat} />
        <WmField label="Жим" value={wm.bench} />
        <WmField label="Тяга" value={wm.dead} />
      </div>

      <div style={{ fontSize: 10, fontWeight: 800, color: ACCENT, marginBottom: 4 }}>⚠ Слабые точки (приоритет акцента)</div>
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4, marginBottom: 8 }}>
        {weakPts.length === 0 ? (
          <div style={{ fontSize: 10, color: DIM }}>Не выбраны. Используйте оверлей ниже, чтобы отметить.</div>
        ) : (
          weakPts.map((w) => (
            <span key={w} style={{ padding: '3px 8px', borderRadius: 6, background: 'rgba(0,230,138,0.18)', border: '1px solid var(--accent)', color: '#fff', fontSize: 10 }}>
              {GROUP_RU[w] ?? w}
            </span>
          ))
        )}
      ...
      <div style={{ fontSize: 10, fontWeight: 800, color: ACCENT, marginBottom: 4 }}>📅 Расписание (сессия → день)</div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 3, marginBottom: 8 }}>
        {(cycle.meta.sessionsPerWeek ?? pl.schedule.length ?? 0) > 0 ? (
          pl.schedule.map((s, i) => (
          ...
          <div style={{ fontSize: 10, color: DIM }}>Расписание пустое.</div>
        )}
      </div>
...
const WmField: React.FC<{ label: string; value: number | undefined }> = ({ label, value }) => (
  <div style={{ padding: '4px 6px', background: 'rgba(255,255,255,0.02)', borderRadius: 6, textAlign: 'center' }}>

 ...
 *
 * P0-аудит: при мед-блоке (красные флаги / 14–15 лет) секция не предлагает
 * добавление и показывает ЧЕСТНУЮ причину (`blockReason` от родителя) —
 * раньше кнопки были активны и ломали план (`medical_block`).
 */
...
export const CardioHiitSection: React.FC<{
  ...
  totalWeeks?: number;
  disabled?: boolean;
  /** Честная причина мед-блока (из cardioHiitInjectGate): показывается вместо кнопок. */
  blockReason?: string | null;
}> = ({ onAdd, totalWeeks, disabled, blockReason }) => {
  ...
  return (
    <div style={CARD} data-cardio="hiit-section" data-blocked={blocked ? 'true' : 'false'}>
      ...
      {blocked ? (
        <div
          role="status"
          data-cardio="hiit-block"
          style={{ padding: '10px 12px', borderRadius: 12, background: 'rgba(239,68,68,0.10)', border: '1px solid rgba(239,68,68,0.32)', borderLeft: '3px solid #f87171', color: '#fca5a5', fontSize: 12.5, fontWeight: 700, lineHeight: 1.5 }}
        >
          ⛔ {blockReason}
          <div style={{ color: '#fff', fontWeight: 400, marginTop: 4 }}>
            Протоколы скрыты: только Zone 2 / recovery до очной консультации врача. Скрининг, не диагноз.
          </div>
        </div>
      ) : (
        <>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(240px, 1fr))', gap: 8 }}>
            {CARDIO_INTERVAL_PRESETS.map(p => (
              <div key={p.id} style={{ padding: 10, borderRadius: 12, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', display: 'flex', flexDirection: 'column', gap: 6 }}>
                ...
                <div style={{ fontSize: 10.5, color: '#fff', lineHeight: 1.5 }}>{p.calibration} · {p.frequency}</div>
                <div style={ROW}>

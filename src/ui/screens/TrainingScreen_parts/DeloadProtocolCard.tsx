        ...
        {Object.entries(DELOAD_PROTOCOLS).map(([key, p]) => (
          <button key={key}
            style={{
              ...
              color: protocol.protocolType === p.protocolType ? '#00e68a' : '#fff',
            }}
        ...
        {protocol.weeklySchedule.map((day) => (
          <div key={day.day}>
            ...
            >
              ...
              <span style={{ color:'#fff', flex: 1 }}>{day.description}</span>
              <span style={{ fontSize: 10, color: '#fff' }}>{expandedDay === day.day ? '▲' : '▼'}</span>
            </button>

            {expandedDay === day.day && (
              <div style={{ padding: '6px 10px 6px 36px', fontSize: 10, color:'#fff' }}>
                {day.notes && <div style={{ marginBottom: 4, fontStyle: 'italic' }}>{day.notes}</div>}
                {day.exercises && day.exercises.length > 0 && (
                  <div style={{ marginTop: 4 }}>
                    {day.exercises.map((ex, i) => (
                      <div key={i} style={{ padding: '2px 0', fontSize: 10, color: '#fff' }}>
      ...
      {protocol.warnings.length > 0 && (
        <div style={{ marginTop: 6, padding: 8, borderRadius: 6, background: 'rgba(239,68,68,0.06)', border: '1px solid rgba(239,68,68,0.15)', marginBottom: 6 }}>
          ...
          {protocol.warnings.map((w, i) => (
            <div key={i} style={{ fontSize: 10, color:'#fff', marginBottom: 2 }}>{w}</div>
          ))}
        </div>
      )}

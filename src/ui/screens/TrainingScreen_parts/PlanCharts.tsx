...
export const VolumeByWeekChart: React.FC<{
  ...
  return (
    <svg viewBox={'0 0 ' + W + ' ' + H} style={{ width: '100%', height: 'auto', fontFamily: 'system-ui, sans-serif' }}>
        ...
        return (
          <g key={d.week}>
            ...
            <text x={x + barW / 2} y={PAD.top + chartH + 14} textAnchor="middle"
              fill="rgba(255,255,255,0.5)" fontSize={9}>н{d.week}</text>
      ...
      {/* Y-axis labels */}
      <text x={6} y={PAD.top + 4} fill="rgba(255,255,255,0.3)" fontSize={8}>{maxSets}</text>
      <text x={6} y={PAD.top + chartH - 1} fill="rgba(255,255,255,0.3)" fontSize={8}>0</text>
      {/* Legend */}
      <g transform={'translate(' + PAD.left + ', ' + (H - 4) + ')'}>
        {top.map((m, i) => (
          <g key={m} transform={'translate(' + (i * 70) + ', 0)'}>
            ...
            <text x={11} y={3} fill="rgba(255,255,255,0.4)" fontSize={7}>{m}</text>
          </g>
        ))}
      </g>
    </svg>
        ...
        return (
          <g key={p.week}>
            ...
            <text x={x} y={PAD.top + chartH + 14} textAnchor="middle"
              fill="rgba(255,255,255,0.5)" fontSize={9}>н{p.week}</text>
            <text x={x - 6} y={y - 6} textAnchor="end" fill="#00e68a" fontSize={8}>{p.avg.toFixed(1)}</text>
          </g>
        );
      })}
      {/* Y-axis */}
      <text x={6} y={PAD.top + 4} fill="rgba(255,255,255,0.3)" fontSize={8}>RIR {maxRir.toFixed(0)}</text>
      <text x={6} y={PAD.top + chartH - 1} fill="rgba(255,255,255,0.3)" fontSize={8}>{minRir.toFixed(0)}</text>
      {/* RIR zone bg */}

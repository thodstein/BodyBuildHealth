    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10, padding: isMobile ? 8 : 12 }}>
      ...
      {showPR && (
        <div style={{
          ...
          <div style={{ textAlign: 'center', padding: 32 }}>
            ...
            <div style={{ fontSize: 12, color:'#fff', marginTop: 4 }}>e1RM: {Math.round(showPR.e1rm)}кг</div>
          </div>
        </div>
      )}
      ...
      {restTimer > 0 && (
        <div style={{
          ...
          <div>
            <div style={{ fontSize: 10, color: '#fff' }}>Отдых</div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
          ...
          {redoRef.current.length > 0 && (
            <button onClick={redo} style={{ padding: '3px 8px', borderRadius: 5, border: '1px solid rgba(255,255,255,0.1)', background: 'transparent', color: '#fff', cursor: 'pointer', fontSize: 10 }}>↷</button>
          )}
      ...
      {/* Rest timer settings */}
      {exercises.length > 0 && (
        <div style={{ display: 'flex', gap: 8, alignItems: 'center', fontSize: 10, color: '#fff' }}>
          <span>Отдых:</span>
      ...
      {savedSummary ? (
        <div style={{ padding: 12, borderRadius: 12, background: 'rgba(0,230,138,0.06)', border: '1px solid rgba(0,230,138,0.2)' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 6, fontSize: 11 }}>
            <div style={{ textAlign: 'center' }}><div style={{ color: '#fff' }}>Упр.</div><div style={{ fontWeight: 700, color: '#fff' }}>{savedSummary.exercises}</div></div>
            <div style={{ textAlign: 'center' }}><div style={{ color: '#fff' }}>Сетов</div><div style={{ fontWeight: 700, color: '#fff' }}>{savedSummary.sets}</div></div>
            <div style={{ textAlign: 'center' }}><div style={{ color: '#fff' }}>Тоннаж</div><div style={{ fontWeight: 700, color: '#fff' }}>{savedSummary.volume.toLocaleString()} кг</div></div>
          </div>
        </div>
      ) : exercises.length > 0 && (
        <>

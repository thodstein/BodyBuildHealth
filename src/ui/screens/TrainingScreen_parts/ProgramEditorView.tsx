...
import { ManualHeader, ManualStepper, Badge, ProgressBar, InfoBanner, MethodHint, CARD_BTN, MANUAL_STYLE_TAG } from './ManualUI';
import { buildProgramIcs, downloadIcs } from './ManualExport';
import { buildProgramCsv, downloadCsv, buildProgramXlsx, downloadXlsx, buildProgramJsonForCoach, downloadJson, buildProgramPrintHtmlFile, downloadHtml, buildProgramQrPayload, buildProgramQrDataUrl } from './ManualExportPro';
...
return (
      <div className="manual-constructor manual-constructor--editor" ref={editorRootRef} style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        ...
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', paddingTop: 8, alignItems: 'center', borderTop: '1px solid rgba(255,255,255,0.06)' }}>
          ...
          <div style={{ display: 'flex', gap: 4, alignItems: 'center', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 10, padding: 3, flexWrap: 'wrap' }}>
            ...
            <button style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 4, padding: '5px 8px', borderRadius: 8, cursor: 'pointer', background: 'transparent', border: '1px solid transparent', color: '#22c55e', fontSize: 11, fontWeight: 700, minHeight: 32 }} onClick={() => { try { const xlsx = buildProgramXlsx(program, program.meta.level); downloadXlsx((program.meta.title || 'program').replace(/[^\wа-яА-ЯёЁ -]/g, '') + '.xlsx', xlsx); showToast('📊 XLSX скачан'); } catch(e){ showToast('⚠ Не удалось собрать XLSX: '+(e as Error).message, 'error'); } }} title="Скачать XLSX (лист Программа + Сводка)">📊 XLSX</button>
            ...
            <button style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 4, padding: '5px 8px', borderRadius: 8, cursor: 'pointer', background: 'transparent', border: '1px solid rgba(255,255,255,0.10)', color: 'rgba(255,255,255,0.60)', fontSize: 11, minHeight: 32 }} onClick={async () => { try { const payload = buildProgramQrPayload(program); await navigator.clipboard.writeText(payload); const dataUrl = await buildProgramQrDataUrl(program); const w = window.open('', '_blank'); if (w) { w.document.write(`<html><head><meta charset="utf-8"><title>QR — ${program.meta.title}</title></head><body style="display:flex;flex-direction:column;align-items:center;gap:12px;padding:20px;font-family:sans-serif"><h3>${program.meta.title}</h3><img src="${dataUrl}" style="width:320px;height:320px;border:1px solid #e5e7eb;border-radius:12px"/><pre style="max-width:600px;word-break:break-all;background:#f9fafb;padding:10px;border-radius:8px;font-size:11px;white-space:pre-wrap">${payload}</pre><p style="font-size:11px;color:#6b7280">Payload скопирован в буфер — отсканируйте QR для импорта</p></body></html>`); w.document.close(); } showToast('🔗 QR payload скопирован + QR открыт'); } catch { showToast('⚠ Не удалось скопировать QR', 'error'); } }} title="QR payload + QR-картинка">🔗 QR</button>
          </div>
          {(dir === 'bb' || dir === 'pl') && (
        ...
        {showMore && (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(132px, 1fr))', gap: 8, paddingTop: 8, borderTop: '1px solid rgba(255,255,255,0.06)' }}>
            ...
            {isPro && <button style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', gap: 2, padding: '10px 11px', borderRadius: 12, cursor: 'pointer', background: 'rgba(245,158,11,0.08)', border: '1px solid rgba(245,158,11,0.22)', color: '#f59e0b', minHeight: 56, textAlign: 'left' }} onClick={() => { setMacroLevel(program.meta.level); setMacroGoal(mapGoalToMacro(program.meta.goal)); setEditorLibOpen('macro'); }}><span style={{ fontSize: 13 }}>🗓</span><span style={{ fontSize: 11, fontWeight: 700 }}>Годовой план</span><span style={{ fontSize: 10, color: 'rgba(245,158,11,0.70)' }}>макроцикл</span></button>}
            {isPro && <button style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', gap: 2, padding: '10px 11px', borderRadius: 12, cursor: 'pointer', background: 'rgba(167,139,250,0.08)', border: '1px solid rgba(167,139,250,0.18)', color: '#a78bfa', minHeight: 56, textAlign: 'left' }} onClick={() => setEditorLibOpen('methods')}><span style={{ fontSize: 13 }}>📚</span><span style={{ fontSize: 11, fontWeight: 700 }}>Методики</span><span style={{ fontSize: 10, color: 'rgba(167,139,250,0.70)' }}>энциклопедия</span></button>}
      ...
      <div className="constructor-surface editor-meta-card" style={{ ...CARD, padding: 10, display: 'flex', flexDirection: 'column', gap: 8 }}>
         ...
         <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0,1fr))', gap: 8 }}>
            <EditorPopupSelect value={program.meta.goal} options={(program.meta.direction === 'pl' ? GOAL_OPTS_PL : program.meta.direction === 'bb' ? GOAL_OPTS_BB : program.meta.direction === 'hybrid' ? GOAL_OPTS_HYBRID : GOAL_OPTS_BB).map(o => ({ id: o.id, label: o.label }))} onChange={v => updateMeta({ goal: v })} ariaLabel="Цель программы" title="Цель программы" buttonStyle={{ width: '100%' }} />

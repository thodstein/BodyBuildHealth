...
export const OneRmCalcTab: React.FC = () => {
  ...
  return (
    <div style={{ maxWidth: 720, margin: '0 auto', padding: 12, color: '#fff' }}>
      ...
      <div style={{ fontSize: 12, color: '#fff', marginBottom: 10, lineHeight: 1.5 }}>Введите вес и повторения в рабочем сете — оценка 1ПМ по 7 формулам + консенсус и таблица %1ПМ.</div>
      ...
      {!results ? (
        ...
        <>
          <div style={{ padding: 14, borderRadius: 12, background: 'rgba(0,230,138,0.06)', border: '1px solid rgba(0,230,138,0.25)', marginBottom: 12, textAlign: 'center' }}>
            <div style={{ fontSize: 10, color: '#fff' }}>Консенсус 1RM ({weight}кг × {reps})</div>
            <div style={{ fontSize: 30, fontWeight: 800, color: ACCENT }}>{results.cons} <span style={{ fontSize: 14 }}>кг</span></div>
            <div style={{ fontSize: 10, color: '#fff' }}>диапазон: {results.min}–{results.max} кг</div>
          </div>
          ...
          <div style={{ fontSize: 11, fontWeight: 700, color: '#fff', textTransform: 'uppercase', letterSpacing: '0.3px', marginBottom: 6 }}>Таблица %1RM (от консенсуса)</div>
          ...
          <div style={{ fontSize: 10, color: '#fff', marginTop: 8, lineHeight: 1.5 }}>💡 Консенсус — среднее по формулам. Epley/Brzycki точнее для 1–10 повт; для &gt;10 повт оценка грубее. Обновите workMax в профиле, если 1RM выше текущего.</div>
        </>
      )}
      {results && (
        <div style={{ marginTop: 10, padding: 12, borderRadius: 12, background: 'rgba(0,230,138,0.06)', border: '1px solid rgba(0,230,138,0.2)' }}>
          <div style={{ fontSize: 10, color: '#fff', marginBottom: 8 }}>🔗 Применить 1RM (<b style={{ color: '#00e68a' }}>{results.cons} кг</b>) как ПМ движения к активному планировщику:</div>

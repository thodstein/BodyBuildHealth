    ...
    <div className="screen labs" style={{ flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column', overflow: 'auto', padding: 0 }}>
      ...
      {mainTab === 'hero' && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 100, display: 'flex', flexDirection: 'column' }}>
          <img src="/lab-hero.png" alt="" onError={e=>{ (e.currentTarget as HTMLImageElement).style.display='none'; }} style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', objectPosition: 'center top' }} />
          ...
          <div style={{ position: 'relative', zIndex: 2, flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', padding: '12px 12px calc(64px + env(safe-area-inset-bottom, 0px))', gap: 10, overflowY: 'auto' }}>
            <div>
              <h1 style={{ fontSize: 22, fontWeight: 900, color: '#fff', margin: '8px 0 4px', textShadow: '0 2px 12px rgba(0,0,0,0.9)', letterSpacing: '-0.6px', lineHeight: 1 }}>Лаборатория</h1>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {[
                { id: 'lab', icon: '🔬', title: 'Анализы', desc: 'Ввод, каталог, динамика, дневник и графики', color: LABS_ACCENT, bg: 'rgba(0,230,138,0.14)' },
                { id: 'risks', icon: '⚠️', title: 'Риски и индексы', desc: 'ASI/HMI/CR, риски по системам, верификация', color: '#f97316', bg: 'rgba(249,115,22,0.14)' },
              ].map(card => (
                <div
                  key={card.id}
                  role="button"
                  tabIndex={0}
                  onClick={() => setMainTab(card.id as MainLabTab)}
              ...
              {trendAlertList.length > 0 && (
                <div
                  role="button"
                  tabIndex={0}
                  onClick={() => { setMainTab('lab'); setSubTab('trends'); }}
                  onKeyDown={e => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); (e.currentTarget as any).click(); } }}
                  style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 12px', borderRadius: 14, cursor: 'pointer', textAlign: 'left', width: '100%', border: '1px solid rgba(239,68,68,0.22)', boxShadow: '0 3px 12px rgba(0,0,0,0.30)', background: 'rgba(18,18,20,0.62)' }}
                >
                  <div style={{ width: 38, height: 38, borderRadius: 11, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, background: 'rgba(239,68,68,0.14)', fontSize: 18, border: '1px solid rgba(239,68,68,0.18)' }}>⚠️</div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 13, fontWeight: 800, marginBottom: 2, color: '#ef4444', letterSpacing: '-0.2px', lineHeight: 1.2 }}>Критические тренды ({trendAlertList.length})</div>
                    <div style={{ fontSize: 10.5, color: '#fff', lineHeight: 1.3 }}>{trendAlertList.slice(0, 2).map(a => `${a.name} ${a.direction === 'up' ? '↑' : '↓'}`).join(', ')}{trendAlertList.length > 2 ? ` +${trendAlertList.length - 2}` : ''}</div>
                  </div>
                  <span style={{ width: 26, height: 26, borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(239,68,68,0.12)', border: '1px solid rgba(239,68,68,0.18)', color: '#ef4444', fontSize: 13, flexShrink: 0, fontWeight: 700 }}>→</span>
                </div>
              )}
            </div>
            <div style={{ fontSize: 9, color: '#fff', textAlign: 'center', lineHeight: 1.3 }}>
              Нажми на раздел — откроются инструменты и данные
            </div>
          </div>
        </div>
      )}
              ...
              <div style={{ marginTop: 14 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 0' }}>
                  ...
                  <button onClick={() => setChartGridOpen(g => !g)} style={{
                    ...
                    borderRadius: 6, padding: '3px 8px', fontSize: 10, color: '#fff', cursor: 'pointer', whiteSpace: 'nowrap',
                ...
                <div style={{ display: 'flex', gap: 4, overflowX: 'auto', marginBottom: 8, scrollbarWidth: 'none', paddingBottom: 4 }}>
                  <button onClick={() => { setChartMarkerSearch(''); setChartFilterSys('all'); }} style={{
                    ...
                    color: chartFilterSys === 'all' ? '#000' : '#fff',
                  ...
                  {['hepatic','renal','endocrine','hematologic','cardio','metabolic','reproductive','neuro','other'].map(sys => {
                    ...
                    return (
                      <button key={sys} onClick={() => setChartFilterSys(prev => prev === sys ? 'all' : sys)} style={{
                        ...
                        color: chartFilterSys === sys ? '#000' : '#fff',
        ...
        <div>
            ...
            <div className="card" style={{ marginBottom: 10, padding: 10, border: '1px solid rgba(0,230,138,0.25)' }}>
                    ...
                    {codes.map(code => {
                      ...
                      return (
                        <div key={code} style={{
                          ...
                          />
                          <span style={{ fontSize: 7, color: '#fff', minWidth: 16 }}>{info?.prefUnit || ''}</span>
                        </div>
                      );
                    })}
                  </div>
              ...
              <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
                <button onClick={() => { setShowNewLabsInline(false); setBatchValues({}); }} style={{
                  ...
                  background: 'var(--bg-secondary)', color: '#fff', fontWeight: 600, fontSize: 11, cursor: 'pointer',
                }}>✕ Отмена</button>
          ...
          <div className="card" style={{ marginBottom: 10, padding: 10, background: anyNoLabs ? 'rgba(239,68,68,0.08)' : 'var(--glass-bg)', borderColor: anyNoLabs ? 'rgba(239,68,68,0.3)' : 'var(--glass-border)' }}>
            ...
            {anyNoLabs ? (
              ...
              <div style={{ fontSize: 10, color: '#fff' }}>Нажмите чтобы применить штраф или введите анализы</div>
            )}
          </div>
          ...
          <div className="card" style={{ marginBottom: 10 }}>
            ...
            {missingLabs.length > 0 && missingLabs.length < requiredLabs.length && (
              <div style={{ marginTop: 4, padding: '4px 8px', background: 'rgba(239,68,68,0.06)', borderRadius: 6, fontSize: 9, color: '#fff' }}>
          ...
          {['baseline','pct','post_pct'].includes(selectedPhase) && (
            <div style={{ marginTop: 10 }}>
              ...
              <div className="card" style={{ marginBottom: 10, padding: 10 }}>
                ...
                <div style={{ display:'grid', gridTemplateColumns:'1fr auto', gap:'3px 12px', fontSize:9, color:'#fff' }}>
            ...
            <div>
              ...
              {labReportGenerated && (
                <div style={{ borderRadius:12, padding:14, marginBottom:10, background:'rgba(24,24,27,0.15)', border:'1px solid rgba(255,255,255,0.04)' }}>
                  ...
                  <div style={{ fontSize:10, color:'#fff' }}><b>Маркеров:</b> {labs.length} ({deviationCount} с откл.)</div>
                  ...
                  <div style={{ fontSize:8, color:'#fff', textAlign:'center', marginTop:6 }}>Сохранено в архив. Доступно в Профиле → Отчёты.</div>
                </div>
              )}
              {labArchive.length > 0 && (
                <div>
                        ...
                        <div style={{ marginTop:6, padding:6, background:'rgba(0,0,0,0.15)', borderRadius:6 }}>
                          ...
                          {r.trends && (
                            <div style={{ marginTop:6, paddingTop:6, borderTop:'1px solid rgba(255,255,255,0.06)' }}>
                              ...
                              <div style={{ fontSize:8, color:'#fff', marginBottom:3 }}>{r.trends.summary}</div>
              ...
              {!labReportGenerated && labArchive.length === 0 && (
                <div style={{ textAlign:'center', padding:30, fontSize:10, color:'#fff' }}>Нажмите «Сгенерировать отчёт»</div>
              )}
            </div>
          )}
                ...
                <div style={{ display: 'grid', gap: 3 }}>
                  {labPharmaAlerts.map((a, i) => (
                    <div key={i} style={{
                      ...
                      <div style={{ color: '#fff', fontSize: 8 }}>{(a.drugCause || []).map((id: string) => { const p = PHARMA_DB[id]; return p?.name || id.replace(/_/g, ' '); }).join(', ')} — {a.recommendation}</div>
                    </div>
                  ))}
                </div>
              ) : (
                  ...
                  <div key={item.label} style={{
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <div>
                        ...
                        <div style={{ fontSize: 8, color: '#fff', marginTop: 2 }}>{item.desc}</div>
                      </div>
          ...
          <div style={{ width: '100%', maxWidth: 480, zIndex: 201, background: 'var(--bg)', borderRadius: 20, maxHeight: '85vh', overflow: 'hidden', display: 'flex', flexDirection: 'column', boxShadow: '0 12px 48px rgba(0,0,0,0.5)' }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '12px 16px', maxHeight: '70vh' }}>
              ...
              {!ocrLoading && !ocrResult && (
                <div>
                  <p style={{ fontSize: 12, color: '#fff', marginBottom: 12 }}>Загрузите PDF, фото или вставьте текст результатов анализов.</p>

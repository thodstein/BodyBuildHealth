              ...
              {labReportGenerated && (
                <div style={{ ...LABS_CARD, marginBottom:10 }}>
                  ...
                  <div style={{ display:'flex', gap:8, marginTop:10, flexWrap:'wrap' }}>
                    <button onClick={() => {
                      const txt = `Анализы ${localIsoDate()} · ${labs.length} маркеров (${deviationCount} с откл.)\n` + labs.map((l: any) => `${l.name || l.code}: ${l.value} ${l.unit || ''} (${l.date || ''})`).join('\n');
                    ...
                    <button onClick={() => {
                      ...
                      void saveCsvApk(`labs-${localIsoDate()}.csv`, csv).then(o => flashShare(shareOutcomeLabel(o)));
            ...
            {report.trends.length > 0 && (
              <div className="labs-filter-row" style={{ display:'flex', gap:8, overflowX:'auto', padding:'2px 2px 8px', scrollbarWidth:'none', marginBottom:4 }}>
                  ...
                  <button onClick={() => {
                    ...
                    void saveCsvApk(`lab-trends-${localIsoDate()}.csv`, csv).then(o => flashShare(shareOutcomeLabel(o)));
                  ...
                  <button onClick={() => {
                    ...
                    `).join('');
                    const html = `<!DOCTYPE html><html><head><title>Lab Trends ${localIsoDate()}</title>
                    ...
                    void printHtmlApk(html, `lab-trends-${localIsoDate()}.html`, `${report.summary}\n${insights.join('\n')}`).then(o => flashShare(shareOutcomeLabel(o)));
                  }} style={{ padding:'10px 14px', borderRadius:999, border:'1px solid rgba(140,190,255,0.14)', background:'rgba(21,38,66,0.60)', color:'#fff', fontWeight:800, fontSize:11, cursor:'pointer', whiteSpace:'nowrap', minHeight:44, flexShrink:0 }}>
                    🖨 Print
                  </button>
                </div>

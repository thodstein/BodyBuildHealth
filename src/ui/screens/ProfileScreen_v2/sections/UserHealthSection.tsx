    ...
    >
      ...
      <div style={{ marginBottom: 16, padding: 12, borderRadius: 12, background: 'rgba(139,92,246,0.05)', border: `1px solid ${colors.purpleDim}` }}>
        ...
        <FieldRow cols={3}>
          ...
          <SliderInput label="Агрессия (1-5)" value={health.aggressionScore} onChange={v => updateHealth({ aggressionScore: v })} min={1} max={5} color={colors.purple} direction="bad" minLabel="спокойный, сдержанный" maxLabel="вспыльчивый, легко завестись" />
        </FieldRow>
        <div style={{ display: 'flex', gap: 6, marginTop: 8, flexWrap: 'wrap' }}>
          {[
      ...
      <div style={{ marginBottom: 16, padding: 12, borderRadius: 12, background: 'rgba(236,72,153,0.05)', border: `1px solid ${colors.pinkDim}` }}>
        ...
        <FieldRow cols={3}>
          ...
          <SliderInput label="Апатия off-cycle (1-5)" value={health.apathyOffCycle} onChange={v => updateHealth({ apathyOffCycle: v })} min={1} max={5} color={colors.pink} direction="bad" minLabel="нормальное состояние без курса" maxLabel="сильная апатия, ничего не хочется" />
        </FieldRow>
      </div>

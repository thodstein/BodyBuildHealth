...
const CalcTab = () => { const [w,sw]=useState(100);const[r,sr]=useState(5);return (<div className="card" style={{padding:10}}><h4 style={{margin:'0 0 6px',fontSize:12}}>🧮 Ultimate Calculators</h4><div style={{display:'flex',gap:4,marginBottom:6}}><input type="number" value={w} onChange={e=>sw(parseFloat(e.target.value) || 0)} style={{width:80,padding:'4px',borderRadius:4,background:'var(--bg-secondary)',border:'1px solid var(--border)',color:'var(--text)',fontSize:12}} placeholder="Weight"/><input type="number" value={r} onChange={e=>sr(parseFloat(e.target.value) || 0)} style={{width:60,padding:'4px',borderRadius:4,background:'var(--bg-secondary)',border:'1px solid var(--border)',color:'var(--text)',fontSize:12}} placeholder="Reps"/></div><div style={{fontSize:10}}>Epley: <b>{epley1RM(w,r).toFixed(0)}</b> | Brzycki: <b>{brzycki1RM(w,r).toFixed(0)}</b></div></div>);};

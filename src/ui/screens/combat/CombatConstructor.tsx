    ...
    <div style={{ padding: 12, display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      {step === 'params' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, background: 'rgba(255,255,255,0.04)', padding: 10, borderRadius: 10 }}>
          ...
          {showExactWM && (()=>{ const WM_GROUPS: Record<string,{label:string,ids:string[]}> = { push:{label:'Жим/плечи',ids:['bench_bar','ohp','push_press','landmine_press']}, pull:{label:'Тяги',ids:['row_bar','pullup','fat_bar_row','single_arm_row','high_pull','towel_pullup','rope_climb']}, legs:{label:'Ноги',ids:['squat','front_squat','rdl','trap_bar_dead','zercher_squat','bulgarian_split_heavy','single_leg_rdl_combat','cossack_squat','step_up','hip_thrust']}, neck:{label:'Шея',ids:['neck_harness_ext','neck_flexion','neck_lateral_flex','neck_rotation']}, grip:{label:'Хват',ids:['plate_pinch','farmer_carry','wrist_roller','wrist_flexion']}, core:{label:'Кор/ротация',ids:['deadbug','ab_wheel','copenhagen_plank','landmine_rotation','pallof_rotation_press','sled_push','sled_pull','band_external_rotation']}}; return (<div style={{ display:'flex', flexDirection:'column', gap:8, background:'rgba(255,255,255,0.03)', padding:6, borderRadius:6, maxHeight:320, overflowY:'auto' }}>{Object.entries(WM_GROUPS).map(([key, grp])=> (<div key={key}><div style={{ fontSize:10, color:'#a855f7', fontWeight:700, marginBottom:4 }}>{grp.label} · {grp.ids.length}</div><div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:4 }}>{grp.ids.map(id=> (<label key={id} style={{ color:'#fff', fontSize:9 }}>{id}: <input type="number" value={workMaxByExercise[id]||0} onChange={e=> { const v=Number(e.target.value)||0; setWorkMaxByExercise(s=> { const n={...s}; if(v>0) n[id]=v; else delete n[id]; return n; }); }} style={{ width:'100%', padding:2, borderRadius:4, fontSize:9 }} /></label>))}</div></div>))}</div>); })()}

...
// Таймауты 30с, а не дефолтные 5с: каждый тест рендерит весь CombatConstructor
// в jsdom и строит полный PRO-план с веткой год/питание/кардио/ICS. Изолированно
// файл занимает ~22с, поэтому под нагрузкой параллельных прогонов (42 файла
// зоны) 20с хватало не всегда. Это честный бюджет тяжёлого теста, а не
// ослабление проверки: таймаут не влияет ни на что, кроме времени ожидания.
describe('Combat hard-block сборки', () => {
  it('concussion×2: план показан красным, но НЕ сохранён и НЕ разослан', async () => {
    ...
    await screen.findAllByText(/Сборка заблокирована/, {}, { timeout: 25000 });

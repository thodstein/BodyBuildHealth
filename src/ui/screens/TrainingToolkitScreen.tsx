...
export const TrainingToolkitScreen: React.FC = () => {
  ...
  const labels: Record<string,string> = {movement:'🏃 Движение',peaking:'🏔 Пикинг',calendar:'📅 Календарь',autoreg:'⚙ Авторег',reps:'🔁 Повторы',gym:'🏋️ Зал',patterns:'📐 Паттерны',design:'🎨 Дизайн',logger:'📝 Логгер',ortho:'🦴 Орто'};
...
const CalendarTab: React.FC = () => {
  ...
  const waterDisplay = typeof water === 'object' ? `💧 Вода: ${(water as any).today || 0}мл / 3000мл` : '💧 Данные недоступны';
  return (<div>
    <div className="card" style={{ marginBottom:8,padding:10 }}>
      ...
      <div style={{ display:'grid',gridTemplateColumns:'repeat(7,1fr)',gap:2,textAlign:'center' }}>
        {['Пн','Вт','Ср','Чт','Пт','Сб','Вс'].map(d=><div key={d} style={{ fontSize:8,color:'var(--text-dim)',fontWeight:600 }}>{d}</div>)}
...
const DesignTab: React.FC = () => {
  ...
  return (<div>
    ...
    <div className="card" style={{ padding:10 }}><h4 style={{ margin:'0 0 4px',fontSize:12 }}>📋 По умолчанию (сила)</h4><div style={{ fontSize:10 }}>{def.name || 'Силовой цикл'}: {def.blocks?.length || 0} блоков</div></div>
  </div>);
};

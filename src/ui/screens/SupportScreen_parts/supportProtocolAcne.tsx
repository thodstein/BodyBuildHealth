...
export const SupportProtocolAcne: React.FC<{ s: Record<string, any> }> = ({ s }) => {
  ...
  return (
          <InfoErrorBoundary label="Акне">
            <div className="sup-proto-acne" style={{ paddingBottom: 30, display:'flex', flexDirection:'column', gap:8 }}>
              <div style={cardBg}>
              ...
              <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
                ...
                {[
                  ...
                  },
                  {
                    ...
                    items:[
                        { name:'Цинк (пиколинат)', dose:'50 мг', timing:'На ночь', note:'Ингибирует 5α-редуктазу → снижение DHT в коже. Антивоспалительное. Антибактериальное (биоплёнки C. acnes) [C]' },
                      ...
                      { name:'Витамин А (ретинол)', dose:'5000-10000 МЕ', timing:'С жирной едой', note:'Регулирует кератинизацию. Снижает гиперкератоз фолликулов. При беременности — НЕЛЬЗЯ. ⚠ Не превышать 5000 МЕ при одновременном приёме изотретиноина (аддитивная токсичность). 10000 МЕ — только коротким курсом при подтверждённом дефиците.' },
                    ]
                  },
                  {
                    ...
                    items:[
                      ...
                      { name:'Клендовит гель 💊', dose:'Тонкий слой', timing:'Утро локально', note:'Клиндамицин 1% + адапален 0.05%. Меньшая концентрация ретиноида. ⚠ То же действующее вещество, что в Клензит-С — не дублировать. По назначению врача' },
                        { name:'Бензоил пероксид 5%', dose:'Тонкий слой', timing:'Утро локально', note:'Окислитель → уничтожает C. acnes. Не вызывает резистентности. Может сушить кожу [A]' },
                        { name:'Салициловая кислота 2%', dose:'Точечно', timing:'Вечер', note:'Кератолитик. Открывает поры. Для жирной кожи. Чередовать: утро БПО, вечер салициловая [B]' },
                    ]
                  },
                  {
                    ...
                    items:[
                      ...
                      { name:'Изотретиноин (Роаккутан) 💊', dose:'0.5-1 мг/кг/день', timing:'С жирной едой', note:'⚠ Категория X (беременность). Строгая контрацепция за 1 мес до, во время и 1 мес после. Риск депрессии/суицида (FDA) — расспрос о настроении на каждом визите. НЕ комбинировать с витамином A (гипервитаминоз) — отмена вит. A за 2 нед до начала. ⚠ ТОЛЬКО по назначению дерматолога. Контроль АЛТ/АСТ + липиды + КФК ЕЖЕМЕСЯЧНО. No донорство крови во время + 1 мес после. No воск/лазер/SPF обязателен. Тератогенность! [A]' },
              ...
              <div style={{ borderRadius:12, padding:12, background:'rgba(245,158,11,0.06)', border:'1px solid rgba(245,158,11,0.2)' }}>
                ...
                <div style={{ fontSize:8, color:'var(--text-dim)', lineHeight:1.5 }}>
                  ...
                  • Системные антибиотики ≤ 3 мес (развитие резистентности C. acnes)<br/>
                  • ⚠ <b>Кросс-модульный лимит:</b> Zn ≤50 мг/сут суммарно (Акне+Волосы+Иммунитет). Избыток Zn → дефицит Cu, ↓ ЛПВП
                </div>
              </div>
            </div>
          </InfoErrorBoundary>

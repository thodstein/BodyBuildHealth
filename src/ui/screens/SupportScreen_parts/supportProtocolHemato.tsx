          ...
          <InfoErrorBoundary label="Гематология">
            <div className="sup-proto-hemato" style={{ paddingBottom: 30, display:'flex', flexDirection:'column', gap:8 }}>
              ...
              <StopBanner title="Критические гематологические пороги" thresholds={[
                'Гематокрит >48% — усиление контроля вязкости (гидратация 3+ л/день; изотоник в жаре); донорство/флеботомия — ТОЛЬКО при симптомах. Перед решением — регидратация + повтор Hct В ПОКОЕ (≥24 ч после HIIE/максимума, без тренировки/сауны накануне; утренняя дегидратация завышает на 2-3%)',
                'Гематокрит >52% — первое: снижение дозы/смена эфира/отмена + сон (СОАС)/курение/ИМТ; донация 300-400 мл 1-2×/нед до Hct <48% — shared decision с врачом + контроль ферритина (повторные кровопускания → Fe-дефицит → реактивный тромбоцитоз)',
                'Гематокрит >54% — стоп/снижение дозы + экстренное кровопускание 400-500 мл по ситуации с врачом (ни аспирин, ни донация не заменяют снижение дозы), риск тромбоза/инфаркта',
              ...
              {hematoTab === 'mechanisms' && (
                <div style={cardBg}>
                  ...
                  {[
                    { m:'Эритроцитоз (Hct ↑)', e:'ААС стимулируют эритропоэз (EPO-пик + ↓гепсидин + новый EPO/Hb set-point). Hct растёт на 5-10% в первые 4-6 нед. Первое при росте — снижение дозы/смена эфира/отмена (Bond 2024), донация — shared decision при очень высоком Hct.', a:'Снижение дозы первым; флеботомия при Hct {'>'}54% по ситуации с врачом. Аспирин — только при ≥2 факторах тромбориска (Hct >50%, курение, возраст >40, ожирение, тромбоанамнез), Hct не снижает' },
                ...
                <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
                  {[
                    { phase:'ФАЗА 1 · ПРОФИЛАКТИКА', label:'Базовый курс (Hct {'<'}48%)', color:'#22c55e', condition:'Hct {'<'}48%, PLT в норме', desc:'Профилактика гиперкоагуляции на любом курсе ААС',
                      items:[
                        { name:'Аспирин кардио 100 мг (опционально)', dose:'100 мг', timing:'Утро после еды', note:'НЕ обязателен в фазе 1 (профилактика). Назначать ТОЛЬКО при ≥2 факторах тромбориска (Hct >50%, курение, возраст >40, ожирение, тромбоанамнез) — иначе риск ЖКТ-кровотечения > пользы (USPSTF 2022: первичка — малое ↓MACE ценой ↑кровотечений, без ↓смертности). Hct и массу эритроцитов НЕ снижает. Обязательно + ИПП' },
                        { name:'Гидратация', dose:'3+ л/день', timing:'Равномерно', note:'Снижение относительной вязкости. В жаре — изотоник (держит PV лучше воды). Перед контрольным Hct — обычная гидратация + покой ≥24 ч после HIIE/максимума. Особенно важно при высоких дозах ААС и летом' },
                        { name:'Омега-3 (EPA+DHA)', dose:'2-4 г/день', timing:'С едой', note:'Реологический эффект слабый; MACE не снижает (STRENGTH 2020) — не замена снижению дозы/донации. Безрецептурный рыбий жир ≠ икозапент 4 г (REDUCE-IT — только hyperTG на статине). При >4 г — ↑кровоточивости + риск ФП [B — только липиды/реология]' },
                        { name:'Шаги + Zone 2 (базис)', dose:'Шаги ежедневно + Zone 2 150+ мин/нед', timing:'Ежедневно', note:'Хроническая гиперволемия плазмы (+6-16%): Hct покоя ниже, ↓АД, лучше липиды/эндотелий, контроль ИМТ/сна. Не лечит эритроцитоз и не заменяет снижение дозы. После интенсивного — заминка обязательна (критическая зона вязкости 7-10 мин). Замер Hct — только в покое' },
                    ...
                    { phase:'ФАЗА 2 · РАННЯЯ КОРРЕКЦИЯ', label:'Hct 48-52%', color:'#f59e0b', condition:'Hct 48-52%, без симптомов', desc:'Усиление антикоагуляции',
                      items:[
                        ...
                        { name:'Пентоксифиллин 400 мг', dose:'400 мг', timing:'2×/день с едой', note:'Улучшает деформируемость эритроцитов, ↓ вязкость. Курс 4-6 нед [B]' },

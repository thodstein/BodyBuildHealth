      ...
      <div style={{ position: 'fixed', inset: 0, zIndex: 100, display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ position: 'relative', zIndex: 2, flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', padding: '12px 12px calc(64px + env(safe-area-inset-bottom, 0px))', gap: 10, overflowY: 'auto' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {renderCard(() => { setSection('generator'); setTab('calculator'); setSupportView('calc'); setCalcView('main'); }, '🧮', 'rgba(0,230,138,0.14)', 'Калькулятор поддержки', '#00e68a', 'Расчёт рисков · стеки · нейропротекция', '#00e68a')}
            {renderCard(() => { setSection('home'); setTab('main'); setSupportView('calc'); setCalcView('info'); setInfoView('catalog'); }, '📚', 'rgba(96,165,250,0.14)', 'Общая информация', '#60a5fa', 'Каталог · синергии · исследования', '#60a5fa')}
            {renderCard(() => { setSection('protocols'); setProtocolTab('pct'); }, '📋', 'rgba(139,92,246,0.14)', 'Протоколы поддержки', '#a78bfa', 'ПКТ · Фертильность · ГЗТ · Нейро · Суставы', '#8b5cf6')}
          </div>

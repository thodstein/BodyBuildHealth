...
export const SupportHomeView: React.FC<{ s: Record<string, any> }> = ({ s }) => {
  ...
  const renderCard = (key: string, onClick: () => void, icon: NativeIconName, bg: string, title: string, titleColor: string, desc: string, accent: string) => (
    <div
      role="button"
      tabIndex={0}
      onClick={onClick}
    ...
    >
      <div style={{ width: 38, height: 38, borderRadius: 11, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, background: bg, color: accent, border: `1px solid ${accent}18` }}><NativeIcon name={icon} size={19} /></div>
        ...
        <div style={{ position: 'relative', zIndex: 2, flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', padding: '12px 12px calc(64px + env(safe-area-inset-bottom, 0px))', gap: 10, overflowY: 'auto' }}>
          <div>
            ...
            <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap', marginTop: 8 }}>
              ...
              <span style={{ fontSize: 9, fontWeight: 700, padding: '3px 7px', borderRadius: 20, background: 'rgba(18,18,20,0.55)', border: '1px solid rgba(var(--sup-accent-rgb, 0,230,138),0.16)', color: 'var(--sup-accent, #00e68a)' }}>ТЗ-модель</span>
            </div>
          </div>
          <div className="support-hero-cards" style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {renderCard('calc', () => { setSection('generator'); setTab('calculator'); setSupportView('calc'); setCalcView('main'); }, 'cpu', 'rgba(0,230,138,0.14)', 'Калькулятор поддержки', '#00e68a', 'Расчёт рисков · стеки · нейропротекция', '#00e68a')}
            {renderCard('info', () => { setSection('home'); setTab('main'); setSupportView('calc'); setCalcView('info'); setInfoView('catalog'); }, 'bookOpen', 'rgba(96,165,250,0.14)', 'Общая информация', '#60a5fa', 'Каталог · синергии · исследования', '#60a5fa')}
            {renderCard('protocols', () => { setSection('protocols'); setProtocolTab('pct'); }, 'shield', 'rgba(139,92,246,0.14)', 'Протоколы поддержки', '#a78bfa', 'ПКТ · Фертильность · ГЗТ · Нейро · Суставы', '#8b5cf6')}
          </div>

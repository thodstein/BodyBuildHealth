...
export const SupportResearch: React.FC<{ s: Record<string, any> }> = ({ s }) => {
  ...
  return (
                <div className="sup-research">
                  ...
                  <div style={{display:'flex',gap:4,marginBottom:10,overflowX:'auto',scrollbarWidth:'none',flexShrink:0}}>
                    {([
                      ...
                      <button key={s.key} className="support-pill" data-active={researchSource===s.key} aria-pressed={researchSource===s.key} onClick={() => {setResearchSource(s.key);if(s.key==='pubchem')handlePubchemSearch();if(s.key==='fda')handleFDASearch();}} style={{
                        padding:'7px 14px',borderRadius:20,fontSize:10,fontWeight:700,whiteSpace:'nowrap',cursor:'pointer',flexShrink:0,minHeight:44,
                  ...
                  {/* Shared search input (у Каталога свой живой поиск — общий тут мёртв и только путает) */}
                  ...
                  <div style={{display:'flex',gap:6,marginBottom:10}}>
                    ...
                    <button data-sup-io="research-search" onClick={()=>{
                      ...
                      style={{padding:'8px 14px',borderRadius:12,border:'none',cursor:'pointer',minHeight:48,minWidth:52,background:`linear-gradient(135deg,${researchSource==='pubmed'?'#3b82f6,#2563eb':researchSource==='pubchem'?'#8b5cf6,#7c3aed':researchSource==='fda'?'#ef4444,#dc2626':'#f59e0b,#d97706'})`,color:'#fff',fontWeight:700,fontSize:13,opacity:(researchSource==='pubmed'&&pubMedLoading)||(researchSource==='pubchem'&&pubchemLoading)||(researchSource==='fda'&&fdaLoading)?0.6:1}}>
                    ...
                    <div className="card" style={{marginBottom:12}}>
                      ...
                      <div style={{display:'flex',gap:4,marginBottom:6,flexWrap:'wrap'}}>
                        {[{label:'Кофеин',q:'caffeine'},{label:'Креатин',q:'creatine'},{label:'L-цитруллин',q:'L-citrulline'},{label:'Таурин',q:'taurine'},{label:'L-тирозин',q:'L-tyrosine'},{label:'Бета-аланин',q:'beta-alanine'}].map((p: any) =>(
                          <button key={p.q} onClick={()=>{setPubMedQuery(p.q);handlePubchemSearch(p.q);}} style={{padding:'8px 10px',borderRadius:10,fontSize:10,minHeight:40,cursor:'pointer',border:'1px solid var(--border)',background:'var(--bg-secondary)',color:'var(--text-light)'}}>{p.label}</button>
                        ))}
                      </div>
                    ...
                    <div className="card" style={{marginBottom:12}}>
                      ...
                      <div style={{display:'flex',gap:4,marginBottom:6,flexWrap:'wrap'}}>
                        {[
                          ...
                          <button key={p.q} onClick={()=>{setPubMedQuery(p.q);}} style={{padding:'8px 10px',borderRadius:10,fontSize:10,minHeight:40,cursor:'pointer',border:'1px solid var(--border)',background:'var(--bg-secondary)',color:'var(--text-light)'}}>{p.label}</button>
                        ))}
                      </div>
                    ...
                    <div className="card" style={{marginBottom:12}}>
                      ...
                      <div style={{display:'flex',gap:4,marginBottom:6,flexWrap:'wrap'}}>
                        {[{label:'Аспирин',q:'aspirin'},{label:'Метформин',q:'metformin'},{label:'Тестостерон',q:'testosterone'},{label:'Тамоксифен',q:'tamoxifen'},{label:'Кломифен',q:'clomiphene'}].map((p: any) =>(
                          <button key={p.q} onClick={()=>{setPubMedQuery(p.q);handleFDASearch(p.q);}} style={{padding:'8px 10px',borderRadius:10,fontSize:10,minHeight:40,cursor:'pointer',border:'1px solid var(--border)',background:'var(--bg-secondary)',color:'var(--text-light)'}}>{p.label}</button>
                        ))}
                      </div>
                    ...
                    <div className="card" style={{marginBottom:12}}>
                      ...
                      <div style={{display:'flex',flexDirection:'column',gap:4,maxHeight:300,overflowY:'auto'}}>
                        {pharmaSearchResults.map((r: any) =>(
                          <div key={r.id} role="button" tabIndex={0} onKeyDown={e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();setSearchQuery(r.name);setTab('catalog' as any);}}} style={{padding:'6px 10px',borderRadius:6,background:r.cls==='supplement'?'rgba(0,230,138,0.04)':'rgba(139,92,246,0.04)',border:`1px solid ${r.cls==='supplement'?'rgba(0,230,138,0.15)':'rgba(139,92,246,0.15)'}`,cursor:'pointer',fontSize:10}} onClick={()=>{
                            // Ведём в каталог с подставленным поиском — раньше клик
                            // работал только для PHARMA_DB, остальное молча игнорилось.
                            setSearchQuery(r.name); setTab('catalog' as any);
                          }}>
                      ...
                      {Object.entries(SUPPORT_RESEARCH)
                        ...
                        .map(([id, entries]) => (
                          <div key={id} style={{padding:'10px 12px',borderRadius:10,background:'var(--bg-secondary)',border:'1px solid var(--border)',marginBottom:8}}>
                            ...
                            {entries.map((e, i) => (
                              <a key={i} href={`https://pubmed.ncbi.nlm.nih.gov/?term=${encodeURIComponent(e.study)}`} target="_blank" rel="noopener noreferrer" style={{display:'block',padding:'6px 8px',borderRadius:6,background:'rgba(0,0,0,0.08)',marginBottom:i < entries.length - 1 ? 4 : 0,textDecoration:'none',color:'inherit'}}>
                                <div style={{fontSize:9,color:'#60a5fa',fontWeight:600,marginBottom:2,fontStyle:'italic'}}>📄 {e.study} <span style={{fontStyle:'normal',fontSize:8}}>↗</span></div>
                                <div style={{fontSize:9,color:'var(--text-dim)',lineHeight:1.4,marginBottom:2}}>{e.conclusion}</div>
                                <div style={{fontSize:8,color:'rgba(255,255,255,0.3)'}}>Год: {e.year} · открыть в PubMed ↗</div>
                              </a>
                            ))}
                          </div>
                        ))}

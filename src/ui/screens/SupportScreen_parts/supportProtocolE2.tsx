          ...
          <InfoErrorBoundary label="Эстрадиол">
            <div className="sup-proto-e2" style={{ paddingBottom: 30, display:'flex', flexDirection:'column', gap:8 }}>
              ...
              <StopBanner title="Критические пороги по эстрадиолу" thresholds={[
                ...
                'AI титруется ступенчато: старт 0.25-0.5 мг 1×/нед, контроль E2 через 14 дней, цель 20-40 пг/мл + T/E2 10-30 (не ниже 20!). Профилактически без E2 >60 и симптомов — не стартовать',
              ]} />
                ...
                <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
                  ...
                  {[
                     { phase:'ФАЗА 1 · ПРОФИЛАКТИКА', label:'E2 в норме', color:'#22c55e', condition:'E2 20-60 пг/мл', desc:'Мониторинг + превентивные меры. ЦЕЛЬ E2: 20-40 пг/мл (не ниже, не выше)',
                      items:[
                        { name:'DIM (дииндолилметан)', dose:'200-400 мг', timing:'2×/день', note:'Модулятор метаболизма E2: ↑ 2-OH (антипролиферативный), ↓ 16α-OH (пролиферативный). Уровень C (нет РКИ у мужчин на ААС) — профилактика, не замена AI при E2 >60 [C]' },
                        { name:'Цинк (пиколинат)', dose:'30-50 мг', timing:'Вечер', note:'Слабая ингибиция ароматазы. Здоровый уровень E2. Уровень C (не замена AI). Сумма Zn со всеми модулями ≤50 мг/сут (избыток → дефицит Cu, ↓ЛПВП)' },
                        { name:'Кальций-D-глюкарат', dose:'1-2 г', timing:'2×/день', note:'Связывает конъюгированные эстрогены в кишечнике → ↓ рециркуляции E2. Уровень C — adjunct, не замена AI [C]' },
                    ...
                    { phase:'ФАЗА 2 · ПОВЫШЕННЫЙ E2', label:'E2 60-120 пг/мл', color:'#f59e0b', condition:'E2 {'>'}60, лёгкие симптомы', desc:'Коррекция ароматизации',
                      items:[
                        { name:'Анастрозол 0.25-0.5 мг 💊', dose:'0.25-0.5 мг', timing:'1×/нед (макс. 2×/нед)', note:'Ингибитор ароматазы (half-life 40-50 ч — чаще 1×/нед достаточно). Старт 0.25-0.5 мг × 1/нед, контроль E2 через 14 дней, шаг +0.25 мг только при E2 >60 + симптомы. Цель 20-40 пг/мл + T/E2 10-30. ⚠ ↓ ЛПВП 15-30%, суставы/кость/либидо при E2 <20 — не перелечить [B — off-label у мужчин]' },
                        { name:'Тамоксифен 10-20 мг 💊', dose:'10-20 мг', timing:'Ежедневно', note:'Селективный модулятор ER. Блокирует ERα в груди → профилактика гинекомастии. Не снижает E2 — только блокирует рецепторы. ⚠ AI+SERM одновременно — только коротко (2-4 нед) при ОСТРОЙ болезненной гинекомастии по врачу (AI ↓ мишень SERM); базисно — ОДИН механизм. Редко (<1%) тромбоз — D-димер [B]' },
                    ...
                    { phase:'ФАЗА 3 · ВЫСОКИЙ E2', label:'E2 {'>'}120 пг/мл', color:'#f97316', condition:'E2 {'>'}120, симптомы (отёки, чувствительность груди)', desc:'Активная терапия',
                      items:[
                         { name:'Анастрозол — титрация вверх (макс. 0.5 мг 2×/нед)', dose:'0.25-0.5 мг 2×/нед', timing:'Фикс. дни (ср/сб)', note:'Только если E2 >120 держится на 0.5 мг/нед + симптомы. Шаг +0.25 мг, контроль E2 через 2 нед. Цель 20-40 пг/мл (не снижать <20!). 1 мг/день (онкодоза) мужчинам НЕ применяется — crash E2. Параллельно — снизить дозу T' },
                        { name:'Тамоксифен 20 мг', dose:'20 мг', timing:'Ежедневно', note:'Продолжить. При уже имеющейся sensitive груди' },
                        { name:'Кальций-D-глюкарат 2 г', dose:'2 г', timing:'2×/день', note:'Усиление экскреции эстрогенов через ЖКТ [C]' },
              ...
              {e2Tab === 'timing' && (
                <div style={cardBg}>
                  ...
                  <p style={{ fontSize:8, color:'var(--text-dim)', margin:'0 0 8px', lineHeight:1.3 }}>Анастрозол 1×/нед (макс. 2×/нед) в одно и то же время. Тамоксифен ежедневно. DIM 2×/день. DIM/глюкарат — уровень C, не замена AI при E2 {'>'}60.</p>
                  {[
                    { time:'🌅 Утро (06:00–09:00)', color:'#f59e0b', items:[
                      { n:'Анастрозол 0.25-0.5 мг (1×/нед, макс. 2×/нед)', why:'Фиксированный день (напр. ср; второй — сб только при титрации). После еды. Half-life 40-50 ч' },

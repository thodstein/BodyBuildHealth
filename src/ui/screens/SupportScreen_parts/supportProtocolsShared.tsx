...
export const PROTOCOL_CARDS = [
  ...
  { id:'women', icon:'♀️', label:'Женщины и ААС', desc:'Вирилизация и Virilization Score, дозы ААС/пептидов/SARMs, примерные протоколы поддержки, женские лабы, таймлайн, либидо, контрацепция', color:'#f472b6', system:'Репродуктивная', tags:['Вирилизация','Дозы','Поддержка','Лабы','Либидо','Контрацепция'], kind:'reference' },
...
export const EVIDENCE_LEGEND = 'Уровни доказательности в протоколах: A — РКИ/гайды/лейблы (телмисартан, УДХК 13-15 мг/кг, эзетимиб, каберголин, вирилизация по лейблам ААС); B — когорты/мета-анализы и суррогатные исходы (силимарин, берберин-глюкоза, омега-3 — только ↓ТГ, MACE не снижает по STRENGTH); C — механизм/консенсус без РКИ у мужчин на ААС (DIM, кальций-D-глюкарат, NMN/NR, PQQ, D-рибоза, ГАМК, красный рис — нет CV-пользы, ниацин по исходам — не замена препаратам). Теги [A]/[B]/[C] в карточках — уровень именно этого утверждения.';

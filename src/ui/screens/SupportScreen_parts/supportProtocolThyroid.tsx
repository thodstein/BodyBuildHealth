...
export const SupportProtocolThyroid: React.FC<{ s: Record<string, any> }> = ({ s }) => {
  ...
  return (
          <InfoErrorBoundary label="Тиреоидный">
            <div className="sup-proto-thyroid" style={{ paddingBottom: 30, display:'flex', flexDirection:'column', gap:8 }}>
              <div style={cardBg}>
                ...
                <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
                  ...
                  {[
                    ...
                    { phase:'ФАЗА 2 · СУБКЛИНИЧЕСКИЙ ГИПОТИРЕОЗ', label:'ТТГ 2.5-4.5 / T4 низкий', color:'#f59e0b', condition:'ТТГ {'>'}2.5, T4 св {'<'}10', desc:'Мягкая коррекция',
                      items:[
                        { name:'L-T4 (левотироксин) 25-50 мкг 💊', dose:'25-50 мкг', timing:'Утро натощак за 30-60 мин до еды', note:'ТОЛЬКО при ТТГ >4.5 + низкий св.T4 + симптомы (не при ТТГ 0.5-2.5 — супрессия собственной оси + кардиориск ФП/ГЛЖ, особенно с кленом/GH). Старт с 25 мкг. Титрация каждые 4-6 нед по ТТГ. Цель ТТГ 0.5-2.0 [A]' },
                    ...
                    { phase:'ФАЗА 4 · ТИРЕОТОКСИКОЗ / ГИПЕРТИРЕОЗ', label:'ТТГ {'<'}0.1 / T3/T4 высокий', color:'#ef4444', condition:'ТТГ {'<'}0.1, T3 св {'>'}6.5, пульс {'>'}100', desc:'Неотложно. Исключить тиреоидит/тиреотоксикоз от ААС',
                      items:[
                        ...
                        { name:'Тионамиды (тиамазол 5-20 мг) 💊', dose:'5-20 мг', timing:'1-2×/день', note:'Ингибитор синтеза T3/T4. Только по назначению эндокринолога. ⚠ Агранулоцитоз (боль в горле/лихорадка — срочный ОАК) + гепатотоксичность: ОАК + АЛТ/АСТ до старта и каждые 2-4 нед первые месяцы [A]' },

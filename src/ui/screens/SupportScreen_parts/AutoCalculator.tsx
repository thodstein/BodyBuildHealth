  ...
  return (
    <div style={embedded ? {} : { padding: '0 12px 80px', maxWidth: 600, margin: '0 auto' }}>
      ...
      {tab === 'cards' && <div>
        ...
        {(() => {
          const allRecs = evaluateRecommendations(state, result, effectiveWeek);
          const effectiveBudget = boostOverride ? 'boost' : budget;
          const sevOrder: Record<string,number> = { critical:0, high:1, medium:2, low:3, info:4 };
          allRecs.sort((a,b) => (sevOrder[a.severity]||9) - (sevOrder[b.severity]||9));
          const { recs: filtered, synergyScore, allIds } = applyBudget(allRecs, effectiveBudget);
          const budgetResult = computeBudgetRisk(allRecs, effectiveBudget, state, result);
          const totalSubs = allIds.length;
          return <Card icon="🧠" title={`Расчёт поддержки (${totalSubs} шт · synergy ${synergyScore}%)`}>
            {/* Warning if risk still high */}
            {budgetResult.warning && (
              <div style={{ marginBottom:6, padding:'8px 10px', borderRadius:8, background:'rgba(239,68,68,0.08)', border:'1px solid rgba(239,68,68,0.15)' }}>
                <div style={{ display:'flex', alignItems:'center', gap:4, marginBottom:2 }}>
                  <span style={{ fontSize:12 }}>⚠️</span>
                  <span style={{ fontSize:9, fontWeight:700, color:'#ef4444' }}>Недостаточный уровень поддержки</span>
                </div>
                <div style={{ fontSize:8, color:'#ef4444', marginBottom:4 }}>{budgetResult.warning}</div>
                {effectiveBudget !== 'boost' && (
                  <button onClick={() => { setBoostOverride(true); }} style={{
                    padding:'6px 14px', borderRadius:6, fontSize:9, fontWeight:700, cursor:'pointer',
                    background:'rgba(239,68,68,0.12)', border:'1px solid #ef4444', color:'#ef4444',
                  }}>
                    💎 Усилить до максимума
                  </button>
                )}
              </div>
            )}
            {/* Budget stats */}
            <div style={{ display:'flex', justifyContent:'space-between', marginBottom:4, padding:'4px 6px', borderRadius:4, background:'rgba(0,0,0,0.08)' }}>
              <span style={{ fontSize:8, color:'var(--text-dim)' }}>Риск: <b style={{color:'#ef4444'}}>{budgetResult.riskBefore}%</b> → <b style={{color:'#22c55e'}}>{budgetResult.riskAfter}%</b></span>
              <span style={{ fontSize:8, color:'var(--text-dim)' }}>Синергия: <b style={{color:'#818cf8'}}>{budgetResult.synergyScore}%</b></span>
              <span style={{ fontSize:8, color:'var(--text-dim)' }}>Позиций: <b>{budgetResult.substanceCount}</b></span>
            </div>
            {/* Week change notification */}
            {(() => {
              const weekChange = allRecs.find(r => r.id === '__week_change');
              if (!weekChange) return null;
              return <div style={{ marginBottom:6, padding:'6px 10px', borderRadius:6, background:'rgba(96,165,250,0.08)', border:'1px solid rgba(96,165,250,0.12)' }}>
                <div style={{ fontSize:8, color:'#60a5fa' }}>{weekChange.title}</div>
              </div>;
            })()}
            {filtered.length === 0 && <div style={{ fontSize:9, color:'var(--text-dim)', padding:8, textAlign:'center' }}>Нет активных рекомендаций для уровня «{effectiveBudget}». Заполните данные или повысьте уровень.</div>}
            {filtered.map(rec => {
              const severityColors: Record<string,string> = { critical:'#ef4444', high:'#f59e0b', medium:'#fbbf24', low:'#22c55e', info:'#60a5fa' };
              const color = severityColors[rec.severity] || '#60a5fa';
              const iconMap: Record<string,string> = { critical:'🔴', high:'🟠', medium:'🟡', low:'🟢', info:'🔵' };
              return <div key={rec.id} style={{ marginBottom:6, padding:'8px 10px', borderRadius:8, background:'rgba(0,0,0,0.08)', border:'1px solid rgba(255,255,255,0.04)' }}>
                <div style={{ display:'flex', alignItems:'center', gap:4, marginBottom:4 }}>
                  <span style={{ fontSize:10 }}>{iconMap[rec.severity] || 'ℹ️'}</span>
                  <span style={{ fontSize:9, fontWeight:700, color:'var(--text-light)', flex:1 }}>{rec.systemLabel}</span>
                  <span style={{ fontSize:8, padding:'1px 6px', borderRadius:4, background:color + '22', color }}>{rec.severity === 'critical' ? 'Критично' : rec.severity === 'high' ? 'Высокий' : rec.severity === 'medium' ? 'Средний' : rec.severity === 'low' ? 'Низкий' : 'Инфо'}</span>
                </div>
                <div style={{ fontSize:8, color:'var(--text-dim)', marginBottom:3 }}>{rec.title}</div>
                {rec.substances.map(sub => (
                  <div key={sub.id} style={{ display:'flex', justifyContent:'space-between', padding:'2px 0', fontSize:8, borderBottom:'1px solid rgba(255,255,255,0.03)' }}>
                    <span style={{ color:'var(--accent)', fontWeight:700 }}>{sub.name}</span>
                    <span style={{ color:'var(--text)', fontWeight:600 }}>{sub.dose}</span>
                  </div>
                ))}
                {rec.substances.some(s => s.tier === 'second' || s.tier === 'third') && (
                  <div style={{ marginTop:3, padding:'3px 6px', borderRadius:4, background:'rgba(245,158,11,0.06)', border:'1px solid rgba(245,158,11,0.1)' }}>
                    <span style={{ fontSize:7, color:'#f59e0b' }}>⚠ {rec.escalation}</span>
                  </div>
                )}
                {rec.monitoring && (
                  <div style={{ marginTop:2, fontSize:7, color:'#60a5fa' }}>📊 {rec.monitoring}</div>
                )}
              </div>;
            })}
          </Card>;
        ...
        {/* Недельная динамика риска */}
        {(() => {
          const dw = weeklyDynamics;
          if (!dw || dw.weeks.length === 0) return null;
          const maxR = Math.max(100, ...dw.weeks.map(w => w.overallRaw));
          const curWeek = Math.min(dw.weeks.length, Math.max(1, Math.round(state.goals.cycleWeeks / 2)));
          const cur = dw.weeks.find(w => w.week === curWeek) || dw.weeks[dw.weeks.length - 1];
          return <Card icon="📈" title={`Динамика риска (${dw.weeks.length} нед)`}>
            <div style={{ fontSize:8, color:'var(--text-dim)', marginBottom:4 }}>
              Пик риска: <b style={{color:'#ef4444'}}>{dw.peakRiskWeek} нед</b> ({Math.round(dw.peakRiskValue)}%) · 
              Мин: <b style={{color:'#22c55e'}}>{dw.minRiskWeek} нед</b> ({Math.round(dw.minRiskValue)}%)
            </div>
            <div style={{ display:'flex', alignItems:'flex-end', gap:1, height:40, marginBottom:4, padding:'4px 0', borderBottom:'1px solid rgba(255,255,255,0.05)' }}>
              {dw.weeks.filter((_, i) => i % Math.max(1, Math.floor(dw.weeks.length / 24)) === 0 || i === dw.weeks.length - 1 || i === dw.peakRiskWeek).map(w => {
                const h = Math.max(3, (w.overallRaw / maxR) * 36);
                const isPeak = w.week === dw.peakRiskWeek;
                const isCur = w.week === curWeek;
                return <div key={w.week} style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center', gap:1 }}>
                  <div style={{ width:'100%', height:h, borderRadius:2, background: isPeak ? 'linear-gradient(180deg,#ef4444,#dc2626)' : isCur ? 'linear-gradient(180deg,#00e68a,#00c853)' : 'rgba(255,255,255,0.15)', position:'relative' }}>
                    {isPeak && <span style={{ position:'absolute', top:-10, left:'50%', transform:'translateX(-50%)', fontSize:7, color:'#ef4444', fontWeight:700 }}>⚠</span>}
                  </div>
                  <span style={{ fontSize:6, color:'var(--text-dim)', marginTop:1 }}>{w.week}</span>
                </div>;
              })}
            </div>
            <div style={{ display:'flex', justifyContent:'space-between', fontSize:7, color:'var(--text-dim)' }}>
              <span>1 нед</span>
              <span>📅 Тек. нед: {curWeek} — риск {Math.round(cur.overallRaw)}% → {Math.round(cur.overallNet)}%</span>
              <span>{dw.weeks.length} нед</span>
            </div>
          </Card>;
        })()}
        ...
        {/* Прогноз анализов */}
        {(() => {
          const lf = labForecast;
          if (!lf) return null;
          const markers = Object.entries(lf).filter(([_, v]) => v !== null) as [string, { w4: number; w8: number; w12: number }][];
          if (markers.length === 0) return null;
          return <Card icon="🔬" title={`Прогноз анализов (${markers.length} маркеров)`}>
            {markers.map(([name, f]) => {
              const baseVal = f.w4 - (f.w4 - (f.w8 - f.w4)); // approximate current
              const maxAbs = Math.max(Math.abs(f.w12), Math.abs(f.w8), Math.abs(f.w4), 1);
              const color = f.w12 > f.w4 && f.w12 > baseVal * 1.1 ? '#ef4444' : f.w12 < f.w4 && f.w12 < baseVal * 0.9 ? '#22c55e' : '#fbbf24';
              return <div key={name} style={{ marginBottom:4, padding:'4px 6px', borderRadius:4, background:'rgba(0,0,0,0.06)' }}>
                <div style={{ display:'flex', justifyContent:'space-between', fontSize:8, marginBottom:2 }}>
                  <span style={{ fontWeight:700, color:'var(--text-light)' }}>{name}</span>
                  <span style={{ color }}>{Math.round(f.w4)} → {Math.round(f.w8)} → {Math.round(f.w12)}</span>
                </div>
                <div style={{ display:'flex', gap:2, height:8, alignItems:'flex-end' }}>
                  {[f.w4, f.w8, f.w12].map((v, i) => {
                    const h = Math.max(2, (v / maxAbs) * 8);
                    return <div key={i} style={{ flex:1, height:h, borderRadius:1, background: i === 2 ? color : 'rgba(255,255,255,0.15)' }} />;
                  })}
                </div>
                <div style={{ display:'flex', justifyContent:'space-between', fontSize:6, color:'var(--text-dim)', marginTop:1 }}>
                  <span>4 нед</span><span>8 нед</span><span>12 нед</span>
                </div>
              </div>;
            })}
          </Card>;
        ...
        {/* Сохранение / Загрузка полного расчёта */}
        <button onClick={() => onApply({ level: state.powerLevel, subs: result.selectedSubstances || [], result })} style={{
          width:'100%', padding:'14px', borderRadius:12, border:'2px solid var(--accent)', cursor:'pointer',
          background:'linear-gradient(135deg, rgba(0,230,138,0.12), rgba(0,198,83,0.05))', color:'#00e68a', fontWeight:800, fontSize:13, letterSpacing:0.5,
          marginTop:8,
        }}>
          📋 Применить расчёт → результаты внизу
        </button>

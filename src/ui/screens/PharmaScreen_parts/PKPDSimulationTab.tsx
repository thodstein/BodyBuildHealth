...
export const PKPDSimulationTab: React.FC = () => {
  ...
  const chart = useMemo(() => {
    ...
    // Bateman-оверлей первого препарата (форма; нормировано к Cmax суперпозиции — единицы разные, честно)
    let batemanPathD: string | null = null;
    if (showBateman && drugDoses.length > 0) {
      const dd = drugDoses[0];
      const sub0 = PHARMA_DB[dd.substanceId] as any;
      const canon0 = resolveEsterCanon(String(sub0?.esters?.[0] || dd.substanceId));
      const daily = batemanCourse(dd.doseMg || 0, canon0.tHalfDays, (canon0 as any).tmaxDays, dd.frequencyDays || [1, 4], Math.max(7, Math.round(maxWeek * 7)));
      const bPeak = Math.max(...daily, 0.001);
      const scale = (Math.max(...pts.map((p) => p.cp), 1) / bPeak) || 1;
      batemanPathD = daily.map((c, i) => `${i === 0 ? 'M' : 'L'}${toX((i + 1) / 7).toFixed(1)},${toY(c * scale).toFixed(1)}`).join(' ');
    }
    ...
    return (
      <svg viewBox={`0 0 ${W} ${H}`} style={{ width: '100%', height: 'auto', background: 'rgba(0,0,0,0.22)', borderRadius: 12, border:'1px solid rgba(255,255,255,0.06)' }}>
        ...
        {batemanPathD && <path d={batemanPathD} fill="none" stroke="#f472b6" strokeWidth="1.8" strokeDasharray="2 3" opacity={0.9} />}
            ...
            <div key={idx} style={{
              ...
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:9 }}>
                <div style={{ flex:1, display:'flex', alignItems:'center', gap:8, minWidth:0 }}>
                  ...
                  {(() => {
                    const canon = resolveEsterCanon(String((sub as any)?.esters?.[0] || dd.substanceId));
                    return (
                      <span title={canon.source} style={{ fontSize:10, padding:'3px 8px', borderRadius:20, fontWeight:700, whiteSpace:'nowrap',
                        background: canon.estimated ? 'rgba(245,158,11,0.10)' : 'rgba(59,130,246,0.10)',
                        color: canon.estimated ? '#fbbf24' : '#60a5fa',
                        border: `1px solid ${canon.estimated ? 'rgba(245,158,11,0.18)' : 'rgba(59,130,246,0.18)'}` }}>
                        t½ {canon.tHalfDays}д · conf {canon.confidence}/5{canon.estimated ? ' · оценка' : ''}
                      </span>
                    );
                  })()}
                </div>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
          ...
          <div style={{
            ...
            <div style={{ display:'flex', flexWrap:'wrap', justifyContent:'center', gap:10, marginTop:8, fontSize:11, color:'#fff' }}>
              ...
              {showBateman && <span style={{ display:'inline-flex', alignItems:'center', gap:4 }}><span style={{ width:14, height:3, borderRadius:2, background:'#f472b6', display:'inline-block' }} /> Bateman (форма, нормировано)</span>}
            </div>
            ...
            {(() => {
              ...
              const bat = batemanCourse(drugDoses[0]?.doseMg || 0, canon.tHalfDays, (canon as any).tmaxDays, drugDoses[0]?.frequencyDays || [1, 4], 30);
              const batPeak = bat.length ? Math.max(...bat) : 0;
              const batPeakDay = bat.length ? bat.indexOf(batPeak) + 1 : 0;
              return (
                <div style={{ marginTop:8, padding:'10px 11px', borderRadius:12, background:'rgba(59,130,246,0.06)', border:'1px solid rgba(59,130,246,0.14)' }}>
                  ...
                  <div style={{ display:'flex', flexWrap:'wrap', gap:6, fontSize:11, color:'#fff' }}>
                    ...
                    <span style={{ background:'rgba(0,0,0,0.18)', padding:'4px 8px', borderRadius:20, border:'1px solid rgba(255,255,255,0.06)' }}>Bateman-пик: день {batPeakDay} · {batPeak.toFixed(1)} отн. ед (первый препарат)</span>
                  </div>

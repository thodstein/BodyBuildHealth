...
export const CalcMapperCard: React.FC<CalcMapperProps> = ({ state, onStateChange, onApply, onOpenManualPicker, onOpenLabs, planResult }) => {
  ...
  const pairSynergies = useMemo(() => {
    ...
    const synergies: { group: string[]; effect: string; mechanism: string; severity: string; score: number }[] = [];
...
{/* ===== ВЫБОР РЕЖИМА (2 кнопки рядом) ===== */}
      ...
      {showIntellPopup && ReactDOM.createPortal(
        <div style={{ position:'fixed', inset:0, zIndex:300, display:'flex', alignItems:'center', justifyContent:'center', background:'rgba(0,0,0,0.8)' }} onClick={() => setShowIntellPopup(false)}>
          <div onClick={e => e.stopPropagation()} style={{ width:'88%', maxWidth:320, borderRadius:1, overflowWrap:'anywhere', wordBreak:'break-word', background:'#16161a', border:'1px solid rgba(255,255,255,0.1)', overflow:'hidden' }}>
      ...
      {showMegaPopup && ReactDOM.createPortal(
         <div style={{ position:'fixed', inset:0, zIndex:300, display:'flex', alignItems:'center', justifyContent:'center', background:'rgba(0,0,0,0.8)' }} onClick={() => setShowMegaPopup(false)}>
            <div onClick={e => e.stopPropagation()} style={{ width:'92%', maxWidth:400, borderRadius:1, overflowWrap:'anywhere', wordBreak:'break-word', background:'#16161a', border:'1px solid rgba(255,255,255,0.1)', overflow:'hidden', maxHeight:'88vh', display:'flex', flexDirection:'column', color:'#fff' }}>
             <div style={{ height:3, background:'linear-gradient(90deg,#f87171,#ef4444)' }} />
          ...
          {showSynergy && (
            <div style={{ padding:'6px 10px', borderRadius:'0 0 8px 8px', background:'rgba(168,85,247,0.05)', border:'1px solid rgba(168,85,247,0.1)', borderTop:'none' }}>
              ...
              {pairSynergies.length > 0 && (
                <>
                  <div style={{ fontSize:7, fontWeight:700, color:'#93c5fd', margin:'6px 0 4px', textTransform:'uppercase', letterSpacing:'0.3px' }}>Синергии ({pairSynergies.length})</div>
                  {pairSynergies.map((syn, i) => (
                    <div key={`pair-${i}`} style={{ padding:'4px 7px', borderRadius:5, marginBottom:3, background:'rgba(96,165,250,0.05)', border:'1px solid rgba(96,165,250,0.14)', width:'100%', boxSizing:'border-box' }}>
                      <div style={{ fontSize:8, color:'#bfdbfe', lineHeight:1.5 }}>
                        <b>{syn.group.map(id => subNameRu(id)).join(' + ')}</b> — {syn.effect}{syn.score ? ` (+${syn.score})` : ''} <span style={{ fontSize:6, fontWeight:700, color: syn.severity === 'HIGH' ? '#4ade80' : '#fbbf24', padding: '1px 4px', borderRadius: 3, background: syn.severity === 'HIGH' ? 'rgba(34,197,94,0.15)' : 'rgba(245,158,11,0.12)' }}>{syn.severity}</span>
                      </div>

...
export const HealthAnalytics: React.FC = () => {
  ...
  const [activeIssues, setActiveIssues] = useState<string[]>(() => {
    try { return JSON.parse(localStorage.getItem('he_health_issues') || '[]'); } catch { return []; }
  });
  const [activeAllergens, setActiveAllergens] = useState<string[]>(() => {
    try { return JSON.parse(localStorage.getItem('he_health_allergens') || '[]'); } catch { return []; }
  });
    ...
    <div style={{ paddingBottom: 80 }}>
      ...
      {tab === 'labs' && <>
        <div style={{ ...cardBg, padding: '10px 12px', marginBottom: 8 }}>
            ...
            {LABS.map(l => (
              <div key={l.id} style={{ padding: '3px 6px', borderRadius: 6, background: 'rgba(255,255,255,0.02)' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 7, color: 'rgba(255,255,255,0.5)', marginBottom: 1 }}>
                  <span>{l.label}</span><span style={{ color: 'rgba(255,255,255,0.3)' }}>{l.refLow}-{l.refHigh} {l.unit}</span>
                </div>
          ...
          {!hasAnyLabs && <div style={{ marginTop: 6, fontSize: 7, color: 'rgba(255,255,255,0.3)', textAlign: 'center' }}>Введите значения анализов</div>}
        </div>
            ...
            {HEALTH_ISSUES.map(h => {
              ...
              return (
                <div key={h.id} style={{
                  ...
                  <div style={{ fontSize: 8, color: 'rgba(255,255,255,0.5)', marginTop: 2 }}>{h.desc}</div>
                  {isActive && (

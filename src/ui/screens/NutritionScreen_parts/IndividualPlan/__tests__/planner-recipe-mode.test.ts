...
describe('extremeCarbTopUp (§7.2-Р-финал: добор углей на экстрим-полосе)', () => {
  ...
  it('тарелка: приём с 900 г твёрдого (потолок экстрима §3A) не получает добор (комната < 40 г)', () => {
    // §3A EXTREME-SCALE: тарелка рецептурного экстрима расширена 730 → 900
    // (было: 700 г = «полная», стало: полная = 900 г). Тест проверяет тот же инвариант
    // «нет комнаты — нет добора» на новом потолке.
    const meals = recount([
      meal('Обед', 400, [{ id: 'bulk_meal', name: 'блюдо', amount: 900, kcal: 1200, p: 60, f: 20, c: 120 }]),
    ]);
    const notes = extremeCarbTopUp(meals, { kcal: 4800, p: 260, f: 110, c: 950 }, { weightKg: 110 });
    expect(notes.every(n => !n.startsWith('🍚 Экстрим-добор'))).toBe(true);

...
describe('R-1500: assembleRecipeDay на 1500У (реальный поток: products-цели → рецепты)', () => {
  ...
  it('§7.2-Р-финал: экстрим-добор углей доводит день ближе к цели (было 76% — стало ≥85%)', () => {
    // До фикса: рецептурный день финишировал на −24% по углям (1136/1500), потому что
    // перебор белка ядер запирал ккал-гейты всех промежуточных проходов. Финальный добор

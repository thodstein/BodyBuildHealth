...
describe('D4/D5: диетологический потолок углеводов (computeDieteticCarbTarget)', () => {
  ...
  it('с большой дозой инсулина 100 ЕД потолок уступает гликемической безопасности (1000 г)', () => {
    const r = computeDieteticCarbTarget({ weightKg: 120, rawCarbsG: 817, insulinTotalUnits: 100 });
    // Итерация C: окно требует floor=1000 (≈10 г/ЕД), потолок max(960, 1000*1.2)=1200 → 1000.
    // Старый пин 960 («абсолютный потолок») отменён: гипогликемия опаснее перебора углей.
    expect(r).toBe(1000);
  });

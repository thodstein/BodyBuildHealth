...
describe('Кнопки настроек: клик → состояние (батч 2: цели и привязка)', () => {
  ...
  // Решение пользователя (Sep 26 2026): авто-база белка 1.6–2.2 г/кг, верх диапазона —
  // пресет «Актив». Пресета «Макс белка» (2.6) больше нет: выше 2.2 — ручной режим КБЖУ.
  it('пресет белка: верх базового диапазона «Актив» подсвечивается', () => {
    boot();
    clickBtn(/Актив/);
    expect(btns(/Актив/)[0].style.border).toMatch(/2px/);
  });

  it('пресет белка: в UI нет значений выше 2.2 г/кг (ручной режим вместо них)', () => {
    boot();
    expect(btns(/Макс белка/)).toHaveLength(0);
    expect(document.body.textContent || '').not.toMatch(/2\.6 г\/кг/);
  });

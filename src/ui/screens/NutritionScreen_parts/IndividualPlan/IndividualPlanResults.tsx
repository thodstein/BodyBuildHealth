        ...
        <div style={{ position:'fixed', inset:0, zIndex:100, display:'flex', alignItems:'center', justifyContent:'center', background:'rgba(0,0,0,0.6)' }}
          ...
          <div onClick={e => e.stopPropagation()} style={{ width:'88%', maxWidth:380, maxHeight:'85vh', overflowY:'auto', padding:0, borderRadius:20, background:'#1c1c1e', boxShadow:'0 20px 60px rgba(0,0,0,0.5), 0 0 0 1px rgba(255,255,255,0.05)' }}>
            ...
            <div style={{ padding:'12px 20px 20px', display:'flex', flexDirection:'column', gap:8 }}>
              <input value={newRecipe.name} onChange={e => setNewRecipe({...newRecipe, name: e.target.value})} placeholder="Название рецепта" style={{ width:'100%', padding:'12px 14px', borderRadius:12, background:'rgba(255,255,255,0.06)', border:'none', color:'#fff', fontSize:15, boxSizing:'border-box', outline:'none', fontWeight:500 }} />
              <div style={{ display:'flex', gap:6 }}>
                <select value={newRecipe.meal} onChange={e => setNewRecipe({...newRecipe, meal: e.target.value})} style={{ flex:1, padding:'12px 14px', borderRadius:12, background:'rgba(255,255,255,0.06)', border:'none', color:'#fff', fontSize:15, boxSizing:'border-box', outline:'none', appearance:'none', fontWeight:500 }}>
                ...
                <input type="number" value={newRecipe.prepTime} onChange={e => setNewRecipe({...newRecipe, prepTime: +e.target.value || 10})} placeholder="Мин" style={{ width:80, padding:'12px 10px', borderRadius:12, background:'rgba(255,255,255,0.06)', border:'none', color:'#fff', fontSize:15, boxSizing:'border-box', outline:'none', textAlign:'center', fontWeight:500 }} />
              </div>
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr 1fr', gap:5 }}>
                {[{k:'kcal',l:'Ккал',c:'#22c55e'},{k:'protein',l:'Белки',c:'#3b82f6'},{k:'fat',l:'Жиры',c:'#f59e0b'},{k:'carbs',l:'Угл',c:'#f97316'}].map(f => <div key={f.k}><input type="number" value={(newRecipe as any)[f.k]} onChange={e => setNewRecipe({...newRecipe, [f.k]: +e.target.value || 0})} placeholder={f.l} style={{ width:'100%', padding:'14px 6px', borderRadius:12, background:'rgba(255,255,255,0.06)', border:'none', color:f.c, fontSize:15, boxSizing:'border-box', outline:'none', textAlign:'center', fontWeight:700 }} /></div>)}
              </div>
              <textarea value={newRecipe.ingredients} onChange={e => setNewRecipe({...newRecipe, ingredients: e.target.value})} placeholder="Ингредиенты (каждый с новой строки)" style={{ width:'100%', padding:'12px 14px', borderRadius:12, background:'rgba(255,255,255,0.06)', border:'none', color:'#fff', boxSizing:'border-box', outline:'none', minHeight:64, resize:'vertical', fontSize:13, lineHeight:1.4 }} rows={3} />
              <textarea value={newRecipe.instructions} onChange={e => setNewRecipe({...newRecipe, instructions: e.target.value})} placeholder="Инструкция (каждый шаг с новой строки)" style={{ width:'100%', padding:'12px 14px', borderRadius:12, background:'rgba(255,255,255,0.06)', border:'none', color:'#fff', boxSizing:'border-box', outline:'none', minHeight:64, resize:'vertical', fontSize:13, lineHeight:1.4 }} rows={3} />
              <input value={newRecipe.tags} onChange={e => setNewRecipe({...newRecipe, tags: e.target.value})} placeholder="Теги (через запятую)" style={{ width:'100%', padding:'12px 14px', borderRadius:12, background:'rgba(255,255,255,0.06)', border:'none', color:'#fff', boxSizing:'border-box', outline:'none', fontSize:13 }} />
              <button onClick={() => {

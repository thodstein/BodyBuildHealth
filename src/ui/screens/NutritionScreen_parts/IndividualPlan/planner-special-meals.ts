...
export function generateCarbload(deps: SpecialMealDeps): any {
  ...
  return {
    ...
    bju: { c: totalCarbs, p: Math.round(deps.effectiveP), f: Math.round(deps.weight * 0.5), kcal: totalCarbs * 4 + Math.round(deps.effectiveP) * 4 + Math.round(deps.weight * 0.5) * 9 },
  };
}
...
export function generateBUTCH(deps: SpecialMealDeps): any {
  ...
  return {
    ...
    highCarb, lowCarb,
    protein: deps.effectiveP,
    fatHigh: Math.round(deps.effectiveF * 0.8),
    fatLow: Math.round(deps.effectiveF * 1.2),

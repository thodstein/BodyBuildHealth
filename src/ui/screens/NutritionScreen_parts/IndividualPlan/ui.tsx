...
export const GlassCard: React.FC<{ title?: string; icon?: string; color?: string; style?: React.CSSProperties; children: React.ReactNode }> = ({ title, icon, color, style, children }) => (
  <div className="plan-glass" data-color={color || 'none'} style={{
    ...
    {title && <div style={{ position: 'relative', zIndex: 1, padding: '12px 14px 0', fontSize: 13, color: color || '#fff', fontWeight: 800, display: 'flex', alignItems: 'center', gap: 8, letterSpacing: '-0.35px', lineHeight: 1.1 }}>
      {icon && <span style={{
    ...
    <div style={{ position: 'relative', zIndex: 1, padding: title ? '10px 14px 14px' : 14 }}>
      {children}
    </div>
  </div>
);

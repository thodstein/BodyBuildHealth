...
export const FOOD_DB: FoodItem[] = [
  ...
  { id: 'oats_dry', name: 'Овсяные хлопья (сухие)', category: 'grain', kcal: 367, protein: 13, fat: 6, carbs: 60, fiber: 10, gi: 40, servingSize: '40 г (сух)', foodState: 'dry',
    description: 'Сухие овсяные хлопья — основа для хлопьев с протеином. Бета-глюкан, медленные углеводы, удобно брать на работу (не требует варки, достаточно залить).',
    bestFor: ['bulk', 'maintenance', 'recomp'], timing: 'morning', pharmaNote: 'β-глюкан снижает холестерин; GI 40 — стабильная глюкоза', tier: 'basic',
    micros: { Ca: 54, Fe: 4.7, Mg: 177, P: 410, K: 400, Na: 2, Zn: 3.9, Se: 34, VitB1: 0.76, VitB3: 0.9, VitB6: 0.12 } },
  ...
  { id: 'sugar_cookies', name: 'Печенье сахарное', category: 'carb', kcal: 416, protein: 6, fat: 12, carbs: 71, fiber: 1.5, gi: 65, servingSize: '40 г', description: 'Гибкая диета (20% рациона): 2-3 шт, не база', tier: 'mid', allergens: ['gluten'], isVegetarian: true, isVegan: false, isGlutenFree: false, isDairyFree: true, dietTags: ['limited', 'treat'], micros: { Ca: 20, Fe: 1.2, Mg: 12, P: 55, K: 90, Na: 140 } },
...
(function fillFoodState() {
  for (const f of FOOD_DB as FoodItem[]) {
    ...
    if ((id.includes('drink') || id.includes('isotonic') || id.includes('isoton') || id.includes('water') || id.includes('juice') || id.includes('kvas') || id.includes('kombucha')) && !id.includes('amylopectin') && !id.includes('maltodextrin') && !id.includes('vitargo') && !id.includes('dextrose')) { f.foodState = 'liquid'; continue; }
    // carb powder / isoton powder — 30г скуп, не жидкость
    if (id.includes('amylopectin') || id.includes('amylop') || id.includes('maltodextrin') || id.includes('vitargo') || id.includes('dextrose') || id.includes('isoton')) { f.foodState = 'powder'; continue; }
...
export const FOOD_ALLERGEN_DIET: Record<string, { allergens: string[]; isVegetarian: boolean; isVegan: boolean; isGlutenFree: boolean; isDairyFree: boolean; dietTags: string[] }> = {
  ...
  oats_dry: { allergens: [], isVegetarian: true, isVegan: true, isGlutenFree: true, isDairyFree: true, dietTags: ['mediterranean'] },

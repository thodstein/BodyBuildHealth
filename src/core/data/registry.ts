...
class DataRegistry {
  ...
  getInteraction(a: string, b: string): InteractionEntry | undefined {
    return MASTER_DB.interactions.find(i => 
      (i.substanceA === a && i.substanceB === b) || (i.substanceA === b && i.substanceB === a)
    );
  }

  getGoal(id: string): GoalProfile | undefined { return MASTER_DB.goals.find(g => g.id === id); }
  getAnalysis(id: string): AnalysisEntry | undefined { return MASTER_DB.analyses.find(a => a.id.toUpperCase() === id.toUpperCase()); }
}

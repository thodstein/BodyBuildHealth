...
export function exportToCSV(points: LabPoint[]): string {
  ...
  return 'date,code,value,unit,phase,source\n' + points.map(p=>`${p.date},${p.code},${p.value},${p.unit},${p.phase},${p.source||''}`).join('\n');
}

export function generateReportText(data: ExportData): string {
  let txt = `📊 HEALTH ENGINE REPORT | Дата: ${new Date().toISOString().slice(0,10)}\n\n`;
...
export function triggerPrintReport() {
  window.print(); // Браузер сам предложит "Сохранить как PDF"
}
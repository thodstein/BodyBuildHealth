...
export const STRICT_EXERCISE_GROUPS: Record<string, StrictExerciseGroup[]> = {
  chest: [
    {
      key: 'chest_fly', label: 'Разводка гантелей / пек-дек (бабочка)',
      ids: ['fly_db', 'pec_deck'],
      re: /разводк.*гантел|пек.?дек|бабоч|butterfly|pec.?deck/i,
    },
    {
      key: 'chest_incline', label: 'Жим под углом 30° (гантели/Смит/штанга)',
      ids: ['incline_bar', 'incline_db', 'smith_incline'],
      re: /жим.*(наклон|incline)|наклон.*жим/i,
      not: /тренаж|machine/i,
    },
  ],
  back: [
    {
      key: 'back_pulldown', label: 'Тяга верхнего блока широким хватом (прямой/параллельный)',
      ids: ['pulldown_wide', 'pulldown_vbar'],
      re: /тяга верхнего блока.*(широк|v-|параллел|нейтр)/i,
    },
    {
      key: 'back_seal', label: 'Тяга двух гантелей к поясу лёжа на скамье',
      ids: ['row_seal'],
      re: /тяга.*(л[её]жа на скамь|двух гантел)|seal.?row/i,
    },
    {
      key: 'back_tbar', label: 'Т-тяга',
      ids: ['row_tbar', 'tbar_row_v2'],
      re: /т-?гриф|т-тяга|t.?bar/i,
    },
  ],
  hamstrings: [
    {
      key: 'ham_curl', label: 'Сгибание ног лёжа/сидя',
      ids: ['leg_curl', 'leg_curl_lying', 'lying_leg_curl', 'leg_curl_seated', 'leg_curl_seated_v2', 'seated_leg_curl'],
      re: /сгибан.*ног|leg.?curl/i,
      not: /стоя|одной ног|single|nordic|норд/i,
    },
    {
    ...
    },
    {
      ...
      ids: ['rdl', 'rdl_db', 'rdl_v2', 'deadlift_romanian', 'b_stance_rdl'],
      re: /румын|rdl/i,
    },
  ],
  quads: [
    {
      key: 'quad_squat', label: 'Приседание со штангой/гакк',
      ids: ['squat', 'squat_bar', 'front_squat', 'front_squat_v2', 'squat_ssb', 'squat_smith', 'squat_lowbar',
        'squat_pause', 'squat_tempo', 'squat_anderson', 'squat_box', 'squat_zercher',
        'hack_squat', 'hack_squat_v2', 'hack_squat_reverse'],
      re: /присед|squat|хак|hack|гакк/i,
      not: /жим.*ног|leg.?press|выпад|lunge|болгар|bulgarian|пистол|pistol|split|над голов|overhead|сисси|sissy|поясн|belt|пендул|pendulum|сумо|sumo|гоблет|goblet|рывков|clean|snatch/i,
    },
    {

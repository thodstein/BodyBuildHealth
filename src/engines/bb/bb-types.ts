...
export interface BBExercise {
  ...
  supersetWith?: string;
  /** Фаза 2.8: номер суперсет-группы (общий у партнёров пары). Позволяет плееру
   *  честно чередовать A1/B1/A2/B2: первое упражнение группы имеет rest=0
   *  (сразу на партнёра), отдых — после второго. Детерминированный (per-week). */

...
export interface ProfExecutionProfile {
  ...
  setupChecklist?: string[]; // сетап-чеклист «стимул в цель» (пилот руки+дельты)
  leakTo?: string; // куда уходит нагрузка при нарушении сетапа
}
...
const PROF_DB: Record<string, ProfExecutionProfile> = {
  chest_upper: {
    ...
    setupChecklist: ['Скамья ровно 30°, лопатки сведены вниз', 'Локти 75°, гриф к подбородку', 'Пауза 1с внизу в растянутой'],
    leakTo: 'Угол 45° → забирает передняя дельта; локти 90° → стресс плеча вместо верха груди',
  },
  chest_mid: {
    ...
    setupChecklist: ['Лопатки сведены вниз, грудь колесом', 'Локти 75°, гриф к соскам', 'Без отбива, пауза 1с внизу'],
    leakTo: 'Локти 90° → забирает плечо; отбив → моментум вместо середины груди',
  },
  chest_lower: {
    ...
    setupChecklist: ['Брусья: наклон 30° вперёд', 'Глубокое опускание с растяжением', 'Пауза 1с внизу'],
    leakTo: 'Вертикальный корпус на брусьях → забирает трицепс вместо низа груди',
  },
  back_width: {
    ...
    setupChecklist: ['Тяга локтями к карманам, не за голову', 'Лопатки вниз-назад, грудь вперёд', 'Пауза в пике + медленный возврат 2-3с'],
    leakTo: 'Тяга за голову → шея/плечи; раскачка → моментум; тяга кистями → бицепс вместо ширины',
  },
  back_thickness: {
    ...
    setupChecklist: ['Локти в стороны 60-90°', 'Тяга к низу живота, корпус фиксирован', 'Сведение лопаток на 1с'],
    leakTo: 'Локти узко вдоль тела → забирают широчайшие; раскачка → поясница вместо центра спины',
  },
  delt_mid: {
    ...
    setupChecklist: ['Наклон 15°, лопатки опущены', 'Локти ведут движение', 'Стоп на уровне плеч + пауза 1с'],
    leakTo: 'Выше плеч/раскачка → забирает трапеция, дельта недогружена',
  },
  delt_rear: {
    ...
    setupChecklist: ['Наклон 60-70°, спина прямая', 'Локти вверх, мизинец ведёт', 'Пауза 1с в сведении лопаток'],
    leakTo: 'Локти назад вдоль тела → забирают широчайшие',
  },
  delt_front: {
    ...
    setupChecklist: ['Локти под грифом', 'Таз напряжён, поясница нейтраль', 'Полная амплитуда вверх'],
    leakTo: 'Прогиб поясницы → дожимает верх груди вместо дельты',
  },
  quads: {
    ...
    setupChecklist: ['Колени по носкам, пятки прижаты', 'Глубина ниже параллели', 'Таз не клюёт, грудь вверх'],
    leakTo: 'Полуприсед → теряется растянутая (главный драйвер); таз вверх первым → забирают поясница и ягодицы',
  },
  hamstrings: {
    ...
    setupChecklist: ['Таз назад, штанга скользит по ногам', 'Колени мягкие 15-20°, спина нейтраль', 'Растяжение сзади 1с внизу'],
    leakTo: 'Кругление → диски поясницы вместо бицепса бедра; присед вместо шарнира → забирает квадрицепс',
  },
  glutes: {
    ...
    setupChecklist: ['Подбородок прижат, таз ровно до прямой', 'Пауза 2с вверху', 'Толчок пятками'],
    leakTo: 'Переразгибание → поясница вместо ягодиц; толчок носками → квадрицепс',
  },
  biceps: {
    ...
    setupChecklist: ['Локти прижаты (длинная) / впереди на скамье (короткая)', 'Плечо неподвижно', 'Супинация вверху для длинной головки'],
    leakTo: 'Локти вперёд → забирает передняя дельта; раскачка → поясница вместо бицепса',
  },
  triceps: {
    ...
    setupChecklist: ['Блок: локти прижаты; overhead: локти у головы', 'Плечо неподвижно весь подход', 'Пауза 1с в пике (блок) / внизу в растянутой (overhead)'],
    leakTo: 'Локти вперёд/в стороны → забирает передняя дельта; без паузы теряется пик',
  },
  calves: {
  ...
  traps: {
    ...
    setupChecklist: ['Руки прямые, движение строго вверх-вниз', 'Пауза 1-2с вверху', 'Не вращать плечами'],
    leakTo: 'Вращение плечами → суставной стресс; подсед/рывок → моментум вместо верха трапеций',
  },
  forearms: {
    ...
    setupChecklist: ['Предплечья лежат и неподвижны', 'Только кисти, полная амплитуда', '15-20 повторов, вес вторичен'],
    leakTo: 'Помогают локти/плечи → забирает бицепс вместо предплечий',
  },
  abs: {
    ...
    setupChecklist: ['Поясница прижата', 'Скручивание (вис: подкручивание таза)', 'Пауза в пике, медленный возврат'],
    leakTo: 'Подъём корпуса без скручивания → сгибатели бедра; руки за головой → шея; раскачка → моментум',
  },
};

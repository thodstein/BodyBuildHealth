...
// ВАЖНО: добавлены curl_*/сгибани*/планк*/dead_bug/bird_dog/pallof/tricep_*/pushdown (раньше ошибочно в tier 3+).
const CANONICAL_PATTERNS: RegExp = /жим.*штанг.*лёж|жим.*гантел.*лёж|жим.*наклонн|жим.*отрицат|жим.*смите.*лёж|отжиман.*брусь|отжиман.*кольц|алмазн.*отжиман|разводк.*гантел|сведен.*кроссовер|сведен.*тренаж|butterfly|пуловер|жим.*грудн.*тренаж|подтяг|pull-?up|тяга.*верхн.*блок|тяга.*наклон|тяга.*тренаж|тяга.*т-гриф|тяга.*горизонтальн|шраги.*штанг|шраги.*гантел|армейск.*жим|жим.*гантел.*сидя|жим.*смите.*сидя|махи.*гантел.*стор|махи.*кроссовер.*стор|махи.*наклон.*задн|тяга.*лиц|обратн.*сведен|присед.*штанг|фронт.*присед|жим.*ногам|выпад|rdl|румынск|сгибан.*ног|разгибан.*ног|ягодичн.*мост|ягодичн.*мостик|hip.?thrust|подъём.*носки|подъём.*носк.*стоя|подъём.*носк.*сидя|подъём.*штанг.*бицепс|подъём.*гантел.*бицепс|подъём.*гир|подъём.*на бицепс|сгибани.*бицепс|сгибани.*блок|молотк|скамье.*скотт|разгибан.*блок|разгибани.*трицепс|разгибани.*канат|французск.*жим|жим.*узким хват|жим.*узк|pushdown|face.?pull|lateral.?raise|lat.?pulldown|barbell.?row|dumbbell.?row|bench.?press|incline.?press|leg.?press|leg.?curl|leg.?ext|calf.?raise|shrug|планк|мёртв.*жук|dead.?bug|птица.*собак|bird.?dog|паллоф|pallof/i;

/* ───────────────────────── Экзотика / специфика ───────────────────────── */
const EXOTIC_PATTERNS: RegExp = /гир[яеюи]|kettlebell|донышк|кубан|cuban|пугало|scarecrow|lu.?raise|махи.*lu|svend|свенд|hex.?press|железн.*крест|iron.?cross|landmine|лэндмайн|guillotine|гильотин|швунг|jerk|из-за.*голов|behind.?neck|рывок.*гир|взят.*гир|олимп|olymp|прогулк.*фермер|farmer.*walk|турецк.*подъём|tgu|halo|ореол|мельниц.*гир|windmill|waiter.?s?.?walk|прогулк.*гир|spiderman|человек.?паук|плиометр|plyo|дефицит|deficit|trx|кольц|suspension|svend|свенд|жим.*резин|band.*press|цеп|chain|доск|board.?press|pin.?press|жим.*спот|spoto|жим.*пола|floor.?press|гильотин|guillotine|жимовой.*швунг|жим.*широк|wide.?grip.?press|железн|iron/i;
...
const INAPPROPRIATE_PATTERNS: RegExp = /донышк|bottom.?up|гильотин|guillotine|швунг|jerk|турецк.*подъём|tgu|прогулк.*фермер|farmer.?walk|прогулк.*гир|waiter.?s?.?walk|рывок.*гир|snatch|spiderman|человек.?паук|плиометр|plyo|мельниц.*гир|windmill|halo|ореол|жим.*цеп|chain|доск|board|pin.?press|спот|spoto|горизонт.*групп|front.?lever|kb_snatch|kb_dead_snatch|kb_tgu|kb_halo|kb_windmill|bottom_up/i;
...
export function bbExerciseTier(ex: Exercise | any): BBExerciseTier {
  ...
  if (joint === 'high' && !/присед.*штанг|станов|жим.*лёж|squat|deadlift|bench|жим.*ног|overhead/i.test(n)) return 3;

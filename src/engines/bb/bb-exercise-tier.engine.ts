...
export function bbExerciseTier(ex: Exercise | any): BBExerciseTier {
  ...
  // Hex/Svend/Iron cross — не для Pro primary (экзотика, не тяжёлый compound)
  if (/hex.?press|свенд|svend|железн.*крест|iron.?cross/i.test(n) || /hex.?press|свенд|svend/i.test(id)) return 4;
  ...
  // 1) Канонический инструментарий — проверяем ДО jointStress, чтобы тяга в наклоне (high) не ушла в exotic
  ...
  if (joint === 'high' && !/присед.*штанг|станов|жим.*лёж|squat|deadlift|bench|жим.*ног|overhead|жим.*стоя|ohp|тяга.*наклон|row/i.test(n)) return 3;

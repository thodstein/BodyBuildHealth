...
/* ── П1-В: одно число перекоса прячет фазовую асимметрию (26.09.2026) ──
 *
 * Sayers M.G.L. et al. Discrete Symmetry Metrics Mask Frequent Phase-Specific Lower Limb
 * Asymmetry During Bilateral Countermovement Jumping in Youth Football Players.
 * Scand J Med Sci Sports 2026;36(7):e70350 · PMID 42466527.
 * Ключевой вывод: СУММАРНАЯ (дискретная) симметрия может быть почти идеальной, пока
 * асимметрия по ОТДЕЛЬНЫМ ФАЗАМ прыжка (контрдвижение / амортизация / толчок / приземление)
 * остаётся частой и выраженной. То есть «перекос 0 %» у нас — это отсутствие данных,
 * а не доказательство симметрии.
 *
 * Что НЕ сделано (и почему честно): фазовой разбивки L/R у нас нет физически. Дневник хранит
 * СЕТЫ по сторонам (гантели/одна сторона), а фаза определяется внутри повторения — таких данных
 * не пишет ни журнал, ни тест. Поэтому вместо выдуманной разбивки показываем честную плашку.
 * Появятся данные (force-plate / видео-разбор прыжка) — тогда добавим разбивку.
 */
export const LR_PHASE_ASYM_NOTE =
  'Разбивки по фазам движения нет: перекос посчитан по СЕТАМ, а фаза живёт внутри повторения. '
  + 'Поэтому «перекос 0 %» — это отсутствие данных, а не симметрия: в исследовании Sayers 2026 '
  + '(PMID 42466527) суммарная симметрия часто маскировала выраженную асимметрию по отдельным фазам '
  + 'прыжка. Для фазовой оценки нужны force-plate или видео-разбор.';

...
/**
 * Резолв упражнения в каталог для equipment-проверок (единый источник:
 * валидатор + pplPushAllowed финализатора обязаны резолвить одинаково,
 * иначе гард пропустит то, что зафлагает валидатор, и наоборот).
 * Возвращает запись каталога или undefined (неизвестно).
 */
export function resolveExerciseCatalogEntry(name: string, exerciseName?: string): any {
  let catalog: any = EXERCISE_CATALOG.find(item => item.name === name || item.id === exerciseName);
  ...
  if (!catalog) {
    const lower = String(name || '').toLowerCase().replace(/ё/g, 'е').trim();
    ...
    if (!catalog) {
      ...
      if (/разгибан/i.test(name) && /блок/i.test(name)) {
        catalog = EXERCISE_CATALOG.find(item => item.id === 'tricep_cable');
      } else if (/сгибан/i.test(name) && /блок/i.test(name)) {
...
function validateSession(session: BBSession, week: number, sessionIndex: number, options: BBPlanValidationOptions = {}): BBPlanValidationIssue[] {
  ...
  for (const exercise of session.exercises) {
    ...
    if (options.equipment?.length) {
        // Резолв через общий хелпер (паритет с pplPushAllowed в финализаторе).
        const catalog: any = resolveExerciseCatalogEntry(exercise.name, exercise.exerciseName);
    ...
    switch (issue.code) {
      ...
      case 'effective_mrv_overflow': {
        ...
        if (match) {
          ...
          recs.push({
            priority: 'high',
            // 26.09.2026 было→стало: раньше здесь стояло «выше верхней границы MRV» + «риск
            // невосстановления», то есть MRV подавался как физический потолок. Это не так: РКИ
            // Camargo 2026 (PMID 42461790, +120 % объёма за 8 недель) не показало ухудшения
            // адаптации, а делод по Pancar 2026 (PMID 41730991) тоже не мешал. MRV — верх
            // РАБОЧЕГО ДИАПАЗОНА ОРИЕНТИРОВОЧНОЙ модели, выход за него = выход за модель,
            // а не доказанный вред.
            action: `Вне рабочего диапазона модели на «${ru}»: эффективные ${current} против верхней границы MRV ${mrv} (+${excess}). `
              + `Это НЕ «потолок организма»: по РКИ (Camargo 2026, PMID 42461790) даже +120 % объёма не ухудшили адаптацию, `
              + `поэтому решение ваше — снизить объём, если вы не довольны восстановлением, либо оставить. `
              + `Если снижаете: уберите одно изолирующее или уменьшите сеты базы в неделе ${issue.week || ''}.`,
            code: issue.code,
          });
        }
        break;

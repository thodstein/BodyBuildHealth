...
describe('D: DC-лайт', () => {
  ...
  it('ротация-3: топ-лифт мышцы меняется по неделям, объём тот же', () => {
    ...
    // Sep 2026 (keep-first): в ротационный пул вернулись канонические варианты
    // («Сведение в кроссовере средний» и др.) с прописанными 4-5 сетами —
    // недельная дисперсия ±3 при cap 5, направление объёма сохранено.
    expect(Math.abs(sum(plan.weeks[1]) - sum(plan.weeks[0]))).toBeLessThanOrEqual(3);

...
export interface BridgeSession { sessionId: string; date: string; focus: string; exercises: BridgeExercise[]; totalVolume: number; totalSets: number; totalReps: number; weekNumber: number; planned: boolean; source: 'SRC' | 'BB'; macroPhase?: string; taperWeek?: boolean; mockMeet?: boolean; }
...
export function lmsPlanToSessions(plan: LMSBuildOutput): BridgeSession[] {
  ...
  for (const wk of plan.weeks) {
    for (let dayIdx = 0; dayIdx < wk.days.length; dayIdx++) {
      ...
      out.push({ sessionId: uid('src', i), date: '', focus: `Нед${wk.week} День${dayIdx + 1}`, exercises, totalVolume, totalSets, totalReps, weekNumber: wk.week, planned: true, source: 'SRC', macroPhase: wk.macroPhase, taperWeek: wk.taperWeek, mockMeet: wk.mockMeet });
      i++;
    }
  }
  return out;

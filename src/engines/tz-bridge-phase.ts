...
export interface PhaseProtocol {
  ...
  mandatoryNutrition?: string[]; // substanceId нутрицевтиков, добавляемых ВСЕГДА (независимо от покрытия механизмов)
}
...
export const PHASE_PROTOCOL: Record<PhaseKey, PhaseProtocol> = {
  ...
  pct: {
    ...
    mandatoryNutrition: ['dim', 'calcium_d_glucarate', 'fiber', 'sulforaphane'],
    algorithm: 'ПКТ: 1) SERM (тамоксифен 20 мг/день или кломифен 25-50 мг/день) — rep4/rep5. 2) hCG 500-1000 МЕ 2р/нед (если не было на курсе) — rep1/rep2/rep3. 3) T-бустеры (ашваганда, цинк, форсколин) после SERM. 4) адаптогены (аобработы) — cns1. 5) нутрициональный клиренс эстрогенов (DIM/Ca-D-глюкарат/клетчатка/сульфорафан) — rep4. 6) сохраняем гепато/кардиопротекцию. Длительность 4-6 нед.',
  },
  // ─── ФЕРТИЛЬНОСТЬ ───
  fertility: {
    key: 'fertility',

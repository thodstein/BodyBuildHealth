...
describe('витрина долга: date-only вызовы', () => {
  ...
  it('долг не растёт: новых мест быть не должно', () => {
    ...
    // Затем «прочие» движки (periodization-designer/coaching-psychology/cooldown/
    // mindset-protocol/body-composition/diary-insights/mobility-protocol/
    // pct-planner/warmup) сняли 24 -> 129. Из них 7 мест — СЕРИИ (серия прогресса
    // привычки, серия PR, серии разминки/заминки) и окна приверженности; там UTC
    // не «сдвигал подпись», а ломал счётчик: cursor шагает setDate() от момента,
    // и в UTC+10 до полуночи он уезжал на вчерашний ключ, поэтому отметка
    // «сегодня» не попадала в серию. Отдельно НЕ тронуты 3 места в
    // labs-scheduler.engine.ts: там startDate — строка 'YYYY-MM-DD',
    // new Date('YYYY-MM-DD') = полуночь UTC, и прибавление целых недель в мс с
    // извлечением дня в UTC даёт ровно «startDate + N недель» независимо от
    // смещения. На машине с отрицательным смещением localIsoDate сдвинул бы
    // dueDate на сутки назад — это календарная арифметика, а не долг канона.

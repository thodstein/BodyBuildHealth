...
describe('restoreDiaryExtras', () => {
  ...
  it('пустой бэкап → 0 везде', () => {
    expect(restoreDiaryExtras({})).toEqual({ warmup: 0, mind: 0, mob: 0 });
  });

  it('восстанавливает разминку/психо/мобильность из бэкапа', () => {
    ...
    expect(counts).toEqual({ warmup: 2, mind: 1, mob: 1 });
  ...
  it('существующие записи не дублируются (мерж по ключу)', () => {
    ...
    // добавлены: разминка 07-02 (1), психо с sessionId w1 (1), мобильность — нет новых
    expect(counts).toEqual({ warmup: 1, mind: 1, mob: 0 });
    expect(loadWarmupLog().length).toBe(2);

...
describe('restoreDiaryExtras', () => {
  ...
  it('пустой бэкап → 0 везде', () => {
    expect(restoreDiaryExtras({})).toEqual({ warmup: 0, cooldown: 0, stretch: 0, mind: 0, mob: 0 });
  });

  it('восстанавливает разминку/заминку/растяжку/психо/мобильность из бэкапа', () => {
    const counts = restoreDiaryExtras({
      ...
      stretchSessions: [{ date: '2026-07-01', focus: 'legs', durationMin: 15, done: true, quality: 4 }],
    ...
    expect(counts).toEqual({ warmup: 2, cooldown: 1, stretch: 1, mind: 1, mob: 1 });
    ...
    expect(stretchLogForDate('2026-07-01')?.durationMin).toBe(15);
  ...
  it('существующие записи не дублируются (мерж по ключу)', () => {
    ...
    upsertStretchLog({ date: '2026-07-01', focus: 'fullbody', durationMin: 10, done: true, quality: 3 });
    ...
    const counts = restoreDiaryExtras({
      ...
      stretchSessions: [{ date: '2026-07-01', focus: 'legs', durationMin: 20, done: true, quality: 5 }, { date: '2026-07-02', focus: 'upper', durationMin: 10, done: true, quality: 4 }],
    ...
    // добавлены: разминка 07-02 (1), заминка 07-02 (1), растяжка 07-02 (1), психо с sessionId w1 (1), мобильность — нет новых
    expect(counts).toEqual({ warmup: 1, cooldown: 1, stretch: 1, mind: 1, mob: 0 });
    expect(loadWarmupLog().length).toBe(2);
    ...
    expect(loadStretchLog().length).toBe(2);
    expect(stretchLogForDate('2026-07-01')?.durationMin).toBe(10); // оригинал не перезаписан
  ...
  it('мусорные/невалидные записи пропускаются', () => {
    const counts = restoreDiaryExtras({
      ...
      stretchSessions: [null, { date: 'не дата', done: true }, { date: '2026-07-01', focus: 'legs', durationMin: 15, done: true }],

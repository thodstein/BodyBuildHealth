...
describe('E2E: полный цикл распознавания еды', () => {
  ...
  it('фото проходит через серверный OCR endpoint и затем пищевой парсер', async () => {
    ...
    const fetchMock = vi.fn().mockImplementation(async (url: string) => ({
      ok: /\/api\/ocr-scanned-pdf$/.test(url),
      json: async () => /\/api\/ocr-scanned-pdf$/.test(url) ? { ok: true, text: serverText } : {},
    }));
    vi.stubGlobal('fetch', fetchMock);
    try {
      const result = await processUploadedFile(new File(['image'], 'fatsecret.png', { type: 'image/png' }));
      expect(fetchMock).toHaveBeenCalledWith(expect.stringMatching(/\/api\/ocr-scanned-pdf$/), expect.objectContaining({ method: 'POST' }));

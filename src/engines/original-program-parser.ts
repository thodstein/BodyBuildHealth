...
const EXERCISE_ROW_RE = /^\|?\s*([^|]+?)\s*\|\s*(\d+(?:-\d+)?)\s*\|\s*([^|]+?)\s*\|\s*([\d.]+(?:\s*[-–]\s*[\d.]+)?)\s*\|(?:\s*([^|]+?)\s*\|)?/;
const SETS_RE = /^(\d+)\s*[×x*]\s*([^,]+?)(?:,\s*RIR\s*([\d.]+(?:\s*[-–]\s*[\d.]+)?))?(?:,\s*(.*))?$/i;
...
function parseExercise(name: string, sets: string, reps: string, rir: string, note: string): ProgramDay['exercises'][number] {
  ...
  const rirValue = parsed?.[3] || rir || parsedRirFallback(parsedReps);
  const parsedRir = Number(String(rirValue).match(/[\d.]+/)?.[0] || 2);
...
function parseDay(lines: string[], index: number, header: string, source: OriginalSource): ProgramDay {
  ...
  for (let i = index + 1; i < lines.length && !DAY_RE.test(lines[i]); i++) {
    ...
    const plain = /^(?:\d+\s+)?(.+?)\s+(\d+)\s*[×x*]\s*([^,]+?)(?:,\s*RIR\s*([\d.]+(?:\s*[-–]\s*[\d.]+)?))?(?:\s+(.+))?$/i.exec(line);
    ...
    const inlineSets = /^(\d+)\s*[×x*]\s*([^,]+?)(?:,\s*RIR\s*([\d.]+(?:\s*[-–]\s*[\d.]+)?))?$/i.exec(next[0] || '');
    if (inlineSets) {
...
export function parseOriginalProgram(source: OriginalSource, text: string): FullProgram {
  ...
  // Keep recovery/rest days too. Dropping empty days changes the original cycle layout.
  const days = headings.map(heading => parseDay(lines, heading.index, heading.line, source));

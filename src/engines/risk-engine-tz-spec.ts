...
export function calculateTzSpecRisk(input: TzSpecInput): TzSpecResult {
  ...
  const explanation=[
    ...
    'Формула механизма: R = w × m × E × U × Π(1−k*) — вклад каждого ПРЕПАРАТА суммируется (Σ).',
    'Агрегация: RSS (евклидова норма) — механизмы комбинируются НЕ суммой (√ΣR²), системы усредняются арифметически.',
    ...
    // 26 сен 2026 (E0.7): надбавка за неопределённость РАНЬШЕ применялась к числу
    // (U_i = 1 + 0.25·(1−d_cov)) и была видна только в отладке — пользователь получал
    // завышенный процент без единого слова почему. Теперь она названа своим именем.
    // Это НЕ «наказание за отсутствие анализов»: анализы, наоборот, СНИЖАЮт надбавку,
    // потому что верифицируют механизм. Формулировка именно поэтому говорит про
    // неопределённость оценки, а не про риск.
    labCoverage<1
      ?`Лабораторное покрытие ${Math.round(labCoverage*100)}% → надбавка неопределённости оценки +${Math.round((U_i-1)*100)}% (U=${U_i.toFixed(2)}). Чем меньше подтверждено анализами, тем выше оценка; сдайте анализы — надбавка снизится.`
      :'Лабораторное покрытие 100% — надбавка неопределённости не применяется.',
  ].filter(Boolean).join('\n');

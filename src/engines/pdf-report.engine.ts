  ...
  return `
    <div style="font-family:system-ui,-apple-system,sans-serif;max-width:800px;margin:0 auto;padding:24px;color:#111;">
      ...
      <table style="width:100%;border-collapse:collapse;margin:8px 0 16px;font-size:13px;">
        ...
        <tbody>
          ${Object.entries(risks.systemBreakdown).map(([k,v])=>`
          ...
          `).join('')}
          <tr style="background:#f5f5f7;font-weight:600;"><td style="padding:6px;border:1px solid #ddd;">OVERALL</td><td style="padding:6px;text-align:center;border:1px solid #ddd;">${risks.overallRaw.toFixed(1)}%</td><td style="padding:6px;text-align:center;border:1px solid #ddd;">${risks.overallNet.toFixed(1)}%</td></tr>
        </tbody>
      </table>
      ...
      ${pctPlan ? `
        ...
        <ul style="margin:0 0 16px 16px;font-size:13px;">
          ${pctPlan.pctProtocol.map(p=>`<li>${p.drug}: ${p.dose} (${p.durationWeeks} нед)</li>`).join('')}
        </ul>
      ` : ''}

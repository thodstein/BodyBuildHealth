...
/**
 * Переместить блок макроцикла (C10): сдвиг блока с индекса from на индекс to,
 * пересчёт weekOffset всех блоков по порядку. totalWeeks не меняется,
 * связи с соревнованиями (competitionId) сохраняются.
 * Возвращает новый объект макроцикла (PL или BB) или null при невалидных индексах.
 */
export function moveMacroBlock(
  src: Macrocycle | BBMacrocycle,
  fromIdx: number,
  toIdx: number,
): Macrocycle | BBMacrocycle | null {
  if (!src || !Array.isArray(src.blocks) || src.blocks.length < 2) return null;
  if (!Number.isInteger(fromIdx) || !Number.isInteger(toIdx)) return null;
  if (fromIdx < 0 || fromIdx >= src.blocks.length || toIdx < 0 || toIdx >= src.blocks.length) return null;
  if (fromIdx === toIdx) return src;
  const blocks = [...src.blocks] as MacroBlock[];

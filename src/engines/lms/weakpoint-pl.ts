...
const DIAGNOSIS: Record<Lift, Partial<Record<WeakPoint, Omit<WeakPointDiagnosis, 'lift' | 'assistance'> & { assistanceFromCatalog: string[] }>>> = {
  ...
  row: {
    ...
    row_mid: { weakPoint: 'row_mid', label: 'Середина (на пояс)', description: 'Зависание на уровне пояса — слабая концентрика.', assistanceFromCatalog: ['Тяга горизонтального блока', 'Тяга гантели в наклоне', 'Тяга штанги в наклоне'], intensityPct: 0.7, rationale: 'Тяга горизонтального блока — контролируемая концентрика к поясу.' },
  ...
  pulldown: {
    pd_top: { weakPoint: 'pd_top', label: 'Верх (старт сверху)', description: 'Лопатки не опущены при старте — слабые широчайшие в верхней точке.', assistanceFromCatalog: ['Подтягивания (прямой хват)', 'Тяга верхнего блока (прямой)', 'Тяга верхнего блока широким хватом'], intensityPct: 0.65, rationale: 'Подтягивания + тяга блока — сила старта сверху.' },

...
export function calcNutrition(i: NutritionInput): NutritionTargets {
  ...
  const proRange = { bulk:[1.8,2.2], cut:[2.2,2.6], maintenance:[1.6,2.0], recomp:[1.8,2.2], rehab:[2.0,2.4] };
  const [pMin,pMax] = proRange[i.goal]||[1.6,2.0];
  const protein = Math.round(((pMin+pMax)/2)*i.weightKg);
  
  const fatRange = { bulk:[0.9,1.1], cut:[0.7,0.9], maintenance:[0.8,1.0], recomp:[0.8,1.0], rehab:[0.9,1.1] };

...
describe('unified-lift-diagnosis', () => {
  it('bench master содержит 8 геометрий + слабые фазы', () => {
    const d = unifiedLiftDiagnosis({ lift: 'bench', sessions: [] });
    expect(d.lift).toBe('bench');
    expect(d.limiter.techniqueGeometry.length).toBe(8);
  ...
  it('non-bench без геометрии', () => {
    const d = unifiedLiftDiagnosis({ lift: 'squat', sessions: [] });
    expect(d.limiter.techniqueGeometry.length).toBe(0);
  });

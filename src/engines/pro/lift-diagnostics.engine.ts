...
// triceps (локоть 0=согнут → 180=разогнут), calf (голеностоп: 0=растяжение → пик),
// shrug (плечо: 0=внизу → пик удержания).
// Угловая диагностика есть для ВСЕХ 12 движений (ohp/row/pulldown/incline — раунд A3;
// triceps/calf/shrug — PRO P1). Для row/pulldown ключевые суставы — локоть и лопатка.
// Общие сумо-позиции: классическая тяга тестирует их же (WEAK_POINTS_BY_LIFT.deadlift
// включает sumo_start/sumo_lockout осознанно) — один объект, две ссылки, без дубля текста.
const SUMO_START_INFO: StickingPointInfo = { phase: "sumo_start", angleRangeDeg: [0, 20], keyJoint: "таз+колено (срыв сумо)", weakMuscles: ["Ягодицы", "Приводящие бедра"], biomechanicalReason: "В сумо срыв идёт от ягодиц/приводящих при вертикальном корпусе; слабый срыв = недостаток силы бёдер.", corrections: ["Тяга из ямы (широкая постановка)", "Присед в широкой постановке", "Тяга с плинтов"], loadCues: "Дефицит в широкой постановке — перегрузка срыва сумо." };
const SUMO_LOCKOUT_INFO: StickingPointInfo = { phase: "sumo_lockout", angleRangeDeg: [70, 180], keyJoint: "таз (замыкание бёдер)", weakMuscles: ["Ягодицы", "Разгибатели спины"], biomechanicalReason: "Финальная фаза сумо — разгибание таза и замыкание бёдер; слабый дожим = ягодицы/спина.", corrections: ["Тяга с плинтов (выше колен)", "Присед в широкой постановке", "Румынская тяга"], loadCues: "Тяга с плинтов выше колен изолирует замыкание сумо." };
const STICKING_POINTS: Partial<Record<Lift, Partial<Record<WeakPoint, StickingPointInfo>>>> = {
  ...
  deadlift: {
    ...
    sumo_start: SUMO_START_INFO,
    sumo_lockout: SUMO_LOCKOUT_INFO,
  },
  ohp: {
  ...
  triceps: {
    triceps_start: { phase: "triceps_start", angleRangeDeg: [0, 30], keyJoint: "локоть (сгибание, старт)", weakMuscles: ["Трицепс (длинная головка)", "Трицепс (медиальная)"], biomechanicalReason: "Старт из полного сгибания — максимальная нагрузка на растянутый трицепс; слабый старт = сила длинной головки.", corrections: ["Французский жим", "Разгибание на трицепс в верхнем блоке", "Жим узким хватом"], loadCues: "Растянутая позиция (французский/overhead) — стартовая сила длинной головки." },
    triceps_mid: { phase: "triceps_mid", angleRangeDeg: [30, 100], keyJoint: "локоть (разгибание, переход)", weakMuscles: ["Трицепс (латеральная)", "Трицепс (медиальная)"], biomechanicalReason: "Середина — слабый переход/недостаток объёма разгибателей.", corrections: ["Жим узким хватом", "Разгибание на трицепс в верхнем блоке", "Отжимания на брусьях"], loadCues: "Базовые жимы узким хватом + блок — объём середины амплитуды." },
    triceps_lockout: { phase: "triceps_lockout", angleRangeDeg: [100, 160], keyJoint: "локоть (верхнее разгибание)", weakMuscles: ["Трицепс (пиковое сокращение)"], biomechanicalReason: "Слабый дожим — трицепс не замыкает локоть.", corrections: ["Разгибание на трицепс в верхнем блоке", "Французский жим", "Жим узким хватом"], loadCues: "Блок и дожимы — изоляция пикового сокращения." },
  },
  calf: {
    calf_bottom: { phase: "calf_bottom", angleRangeDeg: [0, 20], keyJoint: "голеностоп (растяжение)", weakMuscles: ["Икроножная", "Камбаловидная"], biomechanicalReason: "Старт из полного растяжения — слабый старт = сила в растянутой позиции.", corrections: ["Подъём на носки стоя", "Подъём на носки сидя", "Жим носками в тренажёре"], loadCues: "Полная амплитуда с паузой внизу." },
    calf_mid: { phase: "calf_mid", angleRangeDeg: [20, 45], keyJoint: "голеностоп (подъём)", weakMuscles: ["Икроножная", "Камбаловидная"], biomechanicalReason: "Середина — общий объём икр.", corrections: ["Подъём на носки стоя", "Подъём на носки в Смите", "Подъём на носки сидя"], loadCues: "Базовый объём + разные углы стопы." },
    calf_top: { phase: "calf_top", angleRangeDeg: [45, 70], keyJoint: "голеностоп (пик)", weakMuscles: ["Икроножная (пик)"], biomechanicalReason: "Слабое пиковое удержание вверху.", corrections: ["Подъём на носки стоя с паузой вверху", "Подъём на носки сидя", "Подъём на носки на одной ноге"], loadCues: "Пауза 2с в пике — удержание сокращения." },
  },
  shrug: {
    shrug_start: { phase: "shrug_start", angleRangeDeg: [0, 15], keyJoint: "плечо (съём)", weakMuscles: ["Верх трапеций", "Левая/правая трапеция"], biomechanicalReason: "Слабый съём — трапеции не стартуют движение.", corrections: ["Шраги со штангой", "Шраги с гантелями", "Тяга штанги к подбородку"], loadCues: "Тяжёлые шраги — стартовая сила трапеций." },
    shrug_mid: { phase: "shrug_mid", angleRangeDeg: [15, 40], keyJoint: "плечо (подъём)", weakMuscles: ["Верх трапеций", "Средняя трапеция"], biomechanicalReason: "Середина — недостаточный объём трапеций.", corrections: ["Шраги со штангой", "Шраги в Смите", "Тяга штанги к подбородку"], loadCues: "Объёмные шраги — середина траектории." },
    shrug_top: { phase: "shrug_top", angleRangeDeg: [40, 60], keyJoint: "плечо (пик удержания)", weakMuscles: ["Верх трапеций (пик)"], biomechanicalReason: "Слабая фиксация пика.", corrections: ["Шраги со штангой с паузой вверху", "Шраги с гантелями", "Шраги в тренажёре"], loadCues: "Пауза 2с в пике + изометрия." },
  },
};

...
function kAnatomy(input: JointJsiInput, joint: JointId): number {
  ...
  if ((joint==='knee' || joint==='spine') && (input.lift==='squat' || input.lift==='deadlift')) {
...
function nutraceuticalFor(maxLevel: JsiLevel, maxPain: number): NutraceuticalTier {
  ...
  if (maxLevel==='critical' || maxLevel==='red' || pain>=7) {
    return {
      ...
      basket:['Глюкозамин сульфат 1500мг','Хондроитин сульфат 1200мг','Мукосат / Алфлутоп (инъекц. хондропротектор)','UC-II 40мг','МСМ 2-3г','Босвеллия 300мг'],
      labs:['hs-CRP (ультрачувствительный СРБ)','СОЭ','Креатинкиназа (если отёк)'],
      note:'Доставка субстрата в синовию в обход ЖКТ (инъекц.). Контроль синовита по hs-CRP/СОЭ. При острой боли — снизить объём до делода.',
    };
  }
  if (maxLevel==='yellow' || pain>=4) {
    return {
      tier:'yellow', title:'Жёлтая зона — терапия БАД',
      basket:['UC-II (неденатурированный коллаген II) 40мг','МСМ 1.5-2г','Экстракт Босвеллии 300мг','Куркумин 500мг','Омега-3 2г'],
      note:'Остановка аутоиммунной деградации хряща (UC-II) + снятие отёка (МСМ/Босвеллия).',
    };
  }
  return {
    tier:'green', title:'Зелёная — профилактика',
    basket:['Гидролизат коллагена 10г + Витамин С 500мг (кофакторы синтеза)','Желатин 15г','Витамин D3','Омега-3 1г'],

...
// ТЗ §5.1: Матрица MV-MRV по уровням (унифицировано с LEVEL_CONFIGS)
...
export function calcTraining(i: TrainingInput): TrainingOutput {
  ...
  const weekPlan = isDeload
    ? `НЕДЕЛЯ ${weekNum} (ДЕЛОД): 50% объёма, RIR 4, без отказов, акцент на технику и мобильность`
    : `НЕДЕЛЯ ${weekNum} (${phase.toUpperCase()}): ${Math.round(weeklyProgression[0].volumeTotal)} общих подходов, RIR ${rir}, прогрессия ${weeklyProgression[0].progressionType}`;

...
const SPLITS: Record<string, { name: string; days: number; desc: string; condition: (i: TrainingInput) => boolean; volumeModifier?: Record<string, number> }> = {
  ...
  torso_limbs_4:{ name: 'Торс/Конечности 4x', days: 4, desc: 'Торс отдельно, конечности отдельно', condition: i => i.daysPerWeek === 4 && !!(i.injuries?.some(inv => inv.location === 'lower' || inv.location === 'back')) },

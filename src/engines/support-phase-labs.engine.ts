...
export interface PhaseLabCard {
  ...
  preanalytics?: string[];
  rxNote?: string;
  /** ♀ Женская пометка (текст-ориентир; числа — в FEMALE_LAB_GROUPS таба «Лабы», без дублирования таблиц). */
  femaleNote?: string;
}
...
const K0_BASELINE: PhaseLabCard = {
  ...
  femaleNote: '♀ Женщины: HCT/HGB/RBC, АЛТ/АСТ/ГГТ (женская ULN), ТТ/FT/E2 и креатинин — по женским порогам, ТТ выше женского порога = вирилизация. Полный перечень — таб «Лабы» (♀ группы).',
};
...
const K2_WEEK4: PhaseLabCard = {
  ...
  femaleNote: '♀ Женщины: контроль оралов — по женской ULN АЛТ/АСТ/ГГТ; E2 — по фазе цикла; HCT — женские зоны. См. женский таб «Лабы».',
};
...
const K5_TRT: PhaseLabCard = {
  ...
  femaleNote: '♀ Женщины: PSA не применяется; TRT/ЗГТ — только врач по женским показаниям и женским порогам TT/E2 (см. таб «Лабы»).',
};
...
const K7_FERTILITY: PhaseLabCard = {
  ...
  femaleNote: '♀ Женщины: спермограмма не применяется — женские гормоны/УЗИ по фазе цикла (см. женский таб «Контрацепция/Фертильность»).',
};

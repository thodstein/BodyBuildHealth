...
export function buildSmartStack(
  ...
  return {
    ...
    coverageSummary: `Покрыто систем: ${allSystems.size} (${[...allSystems].join(', ')})`,
    synergySummary: allSynergies.length > 0 ? `${allSynergies.length} синергий в стеке` : 'Синергии не выявлены',
    timingSummary: timing.join('; ') || 'Принимать согласно инструкции',
    monitoring: selected.length > 0 ? 'Контролируйте самочувствие и лабораторные показатели' : 'Нет данных',
  };
}

// ─── Build 3 variants: budget / balanced / max ───

export type BuildVariant = 'economy' | 'balanced' | 'maximum';

export function buildSmartStackMulti(
  goals: RecGoal[],
  profile?: BioStackProfile,
): { variant: BuildVariant; stack: RecStack; estCost: number }[] {
  const variants: { variant: BuildVariant; maxSize: number; label: string }[] = [
    { variant: 'economy', maxSize: 5, label: 'economy' },
    { variant: 'balanced', maxSize: 10, label: 'medium' },
    { variant: 'maximum', maxSize: 18, label: 'premium' },
  ];

  return variants.map(({ variant, maxSize }) => {
    // score threshold: economy accepts lower scores, maximum needs high
    const minScore = variant === 'economy' ? 0 : variant === 'balanced' ? 20 : 40;
    // tier filter: economy → core only, balanced → core+standard, maximum → all
    const allowedTiers = variant === 'economy' ? new Set(['core'])
      : variant === 'balanced' ? new Set(['core', 'standard'])
      : new Set(['core', 'standard', 'advanced', 'specialty']);
    const allMatches = searchBioStack({
      goals,
      constraints: { maxSubstances: maxSize * 2 },
    }, profile);
    
    const filtered = allMatches.filter(m => m.score >= minScore && allowedTiers.has(m.tier));
    const selected: RecMatch[] = [];
    const coveredMechs = new Set<string>();
    
    for (const match of filtered) {
      if (selected.length >= maxSize) break;
      // priority: higher score gets priority, new mechanisms preferred
      const newMechs = (match.mechanisms || []).filter(m => !coveredMechs.has(m));
      if (newMechs.length > 0 || selected.length < 3) {
        selected.push(match);
        newMechs.forEach(m => coveredMechs.add(m));
      } else if (variant === 'maximum' && selected.length < maxSize) {
        selected.push(match);
      }
    }
    ...
    const estCost = selected.reduce((sum, s) => {
      const entry = SUPPORT_CATALOG_DATA[s.id];
      if (!entry) return sum + 500;
      const tm: Record<string, number> = { core: 800, standard: 500, advanced: 300, specialty: 1200 };
      return sum + (tm[entry.tier as string] || 500);
    }, 0);

    return {
      variant,
      stack: {
        id: 'stack_multi_' + variant + '_' + Date.now(),
        name: goals.map(g => g.charAt(0).toUpperCase() + g.slice(1)).join(' + ') + ` (${variant})`,
        substances: selected,
        totalScore: selected.reduce((s, m) => s + m.score, 0),
        coverageSummary: `Покрыто систем: ${allSystems.size} (${[...allSystems].join(', ')})`,
        synergySummary: allSynergies.length > 0 ? `${allSynergies.length} синергий` : 'Синергии не выявлены',
        timingSummary: 'См. карточки веществ',
        monitoring: 'Контролируйте лабораторные показатели',
      },
      estCost,
    };
  });

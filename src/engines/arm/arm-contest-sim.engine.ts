...
/** PRO-5 P1: чек-лист фолов WAF-2025 (канон правил, дословно по смыслу). */
export function waf2025FoulChecklist(): string[] {
  return [
    'Фол: плечо за центрлайн между ручками',
    'Фол: касание тела (подбородок/плечо/голова)',
    'Dangerous: плечо внутрь/вперёд руки или ниже локтя в neutral (до 2/3) — выпрямиться по крику рефери',
    'Фол: локоть оторван от подушки (просвет любой); езда на трицепсе/предплечье за край — фол',
    'Slip-out: пальцы поднял/сжал в кулак/рваная кисть — фол виновнику; обоюдно/неясно — ремни без фола',
    'Ремни: локти в центр, ладони, большие вверх; пряжка ≥2.5см выше линии запястья; умышленный срыв в проигрыше (>2/3) — поражение',
    'Фол в >2/3 пути к подушке — поражение; 2 warnings = 1 foul, 2 fouls = поражение',
  ];
}
...
export function buildContestSimWeek(input: ContestSimInput = {}): ContestSimWeek {
  ...
  const checklist = [
    ...
    ...(isLift ? ['3+ мин между попытками', 'Заявки 90/96/102 записаны'] : ['Готовность к strap при срыве', 'Drain-план на поздние раунды', ...waf2025FoulChecklist()]),

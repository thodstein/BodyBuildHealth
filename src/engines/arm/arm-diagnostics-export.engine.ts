...
export interface ArmExportData {
  ...
  /** Движения P1–P6 (опционально): строки движения схватки в отчёт. */
  movement?: { matchPhase?: string | null; start?: string | null; vector?: string | null; foul?: string | null; tableStrength?: string | null; danger?: string | null } | null;
}
...
export function buildArmDiagnosticsHtml(data: ArmExportData): string {
  ...
  if (data.movement) {
    ...
    const lines: string[] = [];
    if (m.matchPhase) lines.push('Фаза: ' + m.matchPhase);
    if (m.start) lines.push('Старт: ' + m.start);
    if (m.vector) lines.push('Векторы: ' + m.vector);
    if (m.tableStrength) lines.push('Сила стола: ' + m.tableStrength);
    if (m.foul) lines.push('Фолы: ' + m.foul);
    if (m.danger) lines.push(m.danger);
    if (lines.length) topBlock += '<h2>Движение схватки</h2><ul>' + lines.map(li).join('') + '</ul>';
  }
...
export function buildArmDiagnosticsCsv(data: ArmExportData): string {
  ...
  if (data.movement) {
    const m = data.movement;
    const parts = [m.matchPhase, m.start, m.vector, m.tableStrength, m.foul, m.danger].filter(Boolean).join(' | ');
    if (parts) extra += '\nmovement;' + csvCell(parts);
  }

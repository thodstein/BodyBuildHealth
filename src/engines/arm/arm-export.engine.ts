...
export function buildArmPrintHtml(plan: ArmPlan, diagnostics?: { findings?: Array<{ level: string; text: string }>; humerusWarnings?: string[]; balanceWarnings?: string[]; asymmetryPct?: number; benchLevel?: string; fatigue?: string; trend?: string; info?: string[]; movement?: { matchPhase?: string | null; startNote?: string | null; vectorNote?: string | null; foulNote?: string | null; tableStrengthNote?: string | null; humerusDangerNote?: string | null } | null; armliftMovement?: string[] | null }, proSummary?: ArmProSummary | null): string {
  ...
  const mv = diagnostics?.movement;
  const mvLines: string[] = [];
  if (mv) {
    if (mv.matchPhase) mvLines.push(`Фаза: ${esc(mv.matchPhase)}`);
    if (mv.startNote) mvLines.push(`Старт: ${esc(mv.startNote)}`);
    if (mv.vectorNote) mvLines.push(`Векторы: ${esc(mv.vectorNote)}`);
    if (mv.tableStrengthNote) mvLines.push(`Сила стола: ${esc(mv.tableStrengthNote)}`);
    if (mv.foulNote) mvLines.push(`Фолы: ${esc(mv.foulNote)}`);
    if (mv.humerusDangerNote) mvLines.push(`⛔ ${esc(mv.humerusDangerNote)}`);
  }
  const mvBlock = mvLines.length ? `<div class="rows"><b>🥋 Движение схватки:</b> ${mvLines.join(' · ')}</div>` : '';
  /** PRO-6 M11: движение армлифтинга из пака (строки уже готовые, только esc). */
  const alm = Array.isArray(diagnostics?.armliftMovement) ? diagnostics.armliftMovement.filter((x) => typeof x === 'string' && x.trim() !== '').slice(0, 8) : [];
  const almBlock = alm.length ? `<div class="rows"><b>🏋️ Движение армлифтинга:</b> ${alm.map(esc).join(' · ')}</div>` : '';
  const diagBlock = diagnostics ? `<div class="card"><h3>🔬 Диагностика — сустав/сухожилие (механизм)</h3><div class="rows">${jointFindings || '<div>Нет данных</div>'}${humerusBlock}${balanceBlock}${mvBlock}${almBlock}<div>Асимметрия ${diagnostics.asymmetryPct ?? '—'}% · Bench ${esc(diagnostics.benchLevel||'—')}</div><div>${esc(diagnostics.fatigue||'')} ${esc(diagnostics.trend||'')}</div></div><div class="meta">WR RT M 130.5 / F 77.2 · WAF весовые · Каталог 72 · tendonCap 1.2× vs Muscle 1.7×</div>${infoBlock}</div>` : '';

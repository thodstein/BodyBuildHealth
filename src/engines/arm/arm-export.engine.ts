...
export function buildArmPrintHtml(plan: ArmPlan, diagnostics?: { score?: number; level?: string; verification?: number; asymmetryPct?: number; benchLevel?: string; fatigue?: string; trend?: string }): string {
  ...
  const diagBlock = diagnostics ? `<div style="margin:10px 0;padding:10px 12px;border:1px solid #e2e8f0;border-radius:8px;background:#f8fafc"><h3 style="margin:0 0 6px;color:#b45309">🔬 Диагностика — ${esc(diagnostics.level||'')}</h3><div style="font-size:11px;color:#334155">Score ${diagnostics.score ?? '—'}/100 · Верификация ${diagnostics.verification!=null?Math.round((diagnostics.verification as number)*100)+'%':'—'} · Асимметрия ${diagnostics.asymmetryPct ?? '—'}% · Bench ${esc(diagnostics.benchLevel||'—')}<br/>${esc(diagnostics.fatigue||'')} ${esc(diagnostics.trend||'')}</div><div style="font-size:10px;color:#64748b;margin-top:4px">WR RT M 130.5 / F 77.2 · WAF весовые · Каталог 72 · tendonCap 1.2×</div></div>` : '';

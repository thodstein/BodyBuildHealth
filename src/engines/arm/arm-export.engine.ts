...
export function buildArmPrintHtml(plan: ArmPlan): string {
  ...
  const header = `<div style="display:flex;align-items:center;gap:16px;margin-bottom:12px"><div style="flex:1"><h1 style="margin:0">🤝 Арм-план — ${esc(plan.pattern.name)}</h1><p style="margin:4px 0;font-size:11px;color:#64748b">${plan.weeks.length} нед · ${esc(plan.level||'')} · ${esc(plan.discipline||'')} · ${new Date().toLocaleDateString('ru-RU')}</p><p style="margin:6px 0;font-size:11px">${plan.rationale.map(r=>esc(r)).join('<br/>')}</p></div><div style="text-align:center"><img src="${qrUrl}" width="110" height="110" style="border:1px solid #e2e8f0;border-radius:8px" alt="QR"/><div style="font-size:9px;color:#94a3b8;margin-top:4px">QR: ${esc(plan.pattern.id)}</div></div></div>`;
  return `<!doctype html><html><head><meta charset="utf-8"><title>Арм-план ${esc(plan.pattern.name)}</title><style>body{font-family:system-ui;padding:20px} table{border-collapse:collapse;width:100%} h3{color:#0a6} h4{color:#333} @media print{body{padding:10px} img{max-width:110px}}</style></head><body>${header}${gantt}${rows}<p style="margin-top:16px;font-size:10px;color:#94a3b8">PRO: РУ/РА (рабочий угол/амплитуда), tendonCap, humerus guard, table ≥50% — Кузнецов VIII. Печать: Ctrl+P → Save as PDF.</p><script>window.onload=()=>window.print()</script></body></html>`;
}

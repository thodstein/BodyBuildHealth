...
export function buildArmPrintHtml(plan: ArmPlan, diagnostics?: { findings?: Array<{ level: string; text: string }>; humerusWarnings?: string[]; balanceWarnings?: string[]; asymmetryPct?: number; benchLevel?: string; fatigue?: string; trend?: string; info?: string[]; movement?: { matchPhase?: string | null; startNote?: string | null; vectorNote?: string | null; foulNote?: string | null; tableStrengthNote?: string | null; humerusDangerNote?: string | null } | null }, proSummary?: ArmProSummary | null): string {
  ...
  const diagBlock = diagnostics ? `<div class="card"><h3>🔬 Диагностика — сустав/сухожилие (механизм)</h3><div class="rows">${jointFindings || '<div>Нет данных</div>'}${humerusBlock}${balanceBlock}${mvBlock}<div>Асимметрия ${diagnostics.asymmetryPct ?? '—'}% · Bench ${esc(diagnostics.benchLevel||'—')}</div><div>${esc(diagnostics.fatigue||'')} ${esc(diagnostics.trend||'')}</div></div><div class="meta">WR RT M 130.5 / F 77.2 · WAF весовые · Каталог 72 · tendonCap 1.2× vs Muscle 1.7×</div>${infoBlock}</div>` : '';

  ...
  const rows = plan.weeks.map(wk => {
    const sessRows = wk.sessions.map(sess => {
      const exRows = sess.exercises.map(ex => {
        ...
        return `<tr><td><b>${esc(ex.name)}</b>${table}${comment}</td><td>${esc(ex.muscle)}</td><td class="num">${ex.sets}×${ex.repsRange[0]}-${ex.repsRange[1]} RIR${ex.rir}${hold}</td><td>${esc(angle)}</td><td class="num">${esc(ex.tempoSpec || '')}</td></tr>`;
      }).join('');
      const sessNote = (sess as any).note ? `<div class="note">📝 ${esc((sess as any).note)}</div>` : '';
      return `<section class="sess"><h4>День ${sess.day} — ${esc(sess.sessionTag)} <span class="chip">${esc(sess.character)}</span> ${sess.tableTime ? '<span class="chip table">🖐️ стол</span>' : ''}</h4>${sessNote}<table class="t"><thead><tr><th>Упражнение</th><th>Мышца</th><th>Сеты×Повт</th><th>РУ</th><th>Темп</th></tr></thead><tbody>${exRows}</tbody></table></section>`;
    }).join('');
    return `<section class="week"><h3><span class="ph" style="background:${phaseColor[wk.phase]||'#94a3b8'}">${esc(phaseRu[wk.phase]||wk.phase)}</span> Неделя ${wk.week} — ${esc(wk.phase)} ${wk.deload ? '(deload)' : wk.taper ? '(taper)' : ''}</h3>${(wk as any).note ? `<div class="note">📝 ${esc((wk as any).note)}</div>` : ''}${sessRows}</section>`;
  }).join('');

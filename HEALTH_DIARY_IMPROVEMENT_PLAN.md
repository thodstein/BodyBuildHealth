...
## Раунд 3 — сквозная интеграция (11 августа 2026) ✅

| # | Что сделано | Файлы |
|---|-------------|-------|
| 1 | **Связь с протоколами поддержки**: `DiaryWindowProps.onNavigate?` (необязательный), проброс через ProfileDiariesTab; у рекомендаций плана (боль/нейро/гемато/акне) — кнопка «🛡 Протокол поддержки» → `onNavigate('support')` | `DiaryWindow.tsx`, `ProfileDiariesTab.tsx`, `HealthDiary.tsx` |
| 2 | **Прогресс выполнения плана**: «✅ Выполнено N из M» + прогресс-бар в шапке секции плана | `HealthDiary.tsx` |
| 3 | **Прогноз боли на 7 дней**: `fitLinearTrend` + `projectToDate` по точкам боли → «🔮 Прогноз боли через 7 дней: N/70 (↑/↓, r²)» под графиком | `HealthDiary.tsx` |
| 4 | **Сводный отчёт TXT**: `exportHealthReportText(analysis, plan)` — боль/зоны/триггеры, симптомы, нейро/акне/гемато, контекст (сон/АД/курс/вес), + полный план; кнопка «📄 Отчёт TXT» в меню экспорта | `health-improvement-plan.engine.ts`, `HealthDiary.tsx` |

**Тесты: +5 (2 UI: навигация + прогресс; 1 движок: отчёт). Итого по фиче 89.**
**Верификация: `tsc --noEmit` 0 ошибок; полный vitest 3449/3449 (232 файла; единичный прогон с 2 флейк-таймаутами profile-e2e/course-sync — известная особенность полного параллельного прогона, повторные прогоны зелёные); `vite build` OK.**

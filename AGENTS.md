...
- **Добивка-2 «9 остатков» (в worktree, БЕЗ коммита)**: закрыты все 9 честных гэпов — (1) `contestPhase`/`peakWeek` → `phaseTag` в `bbPlanToQualityInput` (taper>peak>deload; глубина по неделям тега); (2) хаб-деривация `deriveV2InputFromProgram` — номера делод-недель + глубина + RIR-сдвиг; (3) монотония Foster через готовый `sRPEAdjustment` + бейдж «сегодня» (`data-bb="quality-v2-today"`); (4) manual RIR-passthrough; (5) `resolveWorkMax` + `LEGACY_WM_DEFAULTS` в проде (графики байт-в-байт); (6) P7-распил: NEW `quality-hub-parts` (`useQualityProgram`/`useQualityCharts`/`QualityScoreCard`/`PerMuscleBars`) + `programForDivision`; (7) NEW паритет-матрица S1/S3/V2 (честный допуск ±12); (8) связка S4↔V2 (`combinedQualitySummary`, `data-q="combined"`); (9) split-фикс clipboard (`buildSplitFixText`) + АПК §117 (без hex). Проверено: **163/163 (12 файлов)**; `tsc --noEmit` **0 по всему проекту** (поймано своим прогоном: 4 типовые ошибки распила `TplTemplate`/implicit-any — починены; чужой синтаксис в том же `BbAutoConstructor` починен владельцем — `dad9c9f08`). НЕ КОММИТИЛ/НЕ ПУШИЛ.

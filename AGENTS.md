...
- **P0-4 (B2): hemato-разбиение (аудит выполнен).** Система `hematologic` = «Гематолого-МЕТАБОЛИЧЕСКИЙ блок» — СЛИТЫ 3 домена:
  ...
  План: (а) сплит на отдельные системы/под-риски — hematologic (hem1 + NEW hem6 тромбоз + NEW hem7 кровотечение), metabolic (hem2/hem3), electrolytes (hem4/hem5); (б) пересчёт maxRaw/шкал; (в) маппинг SUPPLEMENTS_DB organId; (г) UI-панель под-рисков «эритроцитоз 34% · тромбоз 12%»; (д) тесты. Остальные системы проверены — разделены хорошо: hepatic (liv1-3), renal (ren1-4), cns (cns1-6), reproductive (rep1-5), cardio (cv1-5, кроме cv4).

...
## Механизм-модель рисков: union-агрегация + якорные floors + верификация + эритроцитоз (Aug 17 2026, uncommitted)
...
- **Union вместо суммы**: `unionPct` (S = 1 − Π(1−pᵢ)) — каноническая формула competing-risks
  ...
  Применяется: система = union механизмов. **Общий индекс = худшая система (max, триаж)** —
  union по органам переоценивает (6 систем ×25% дали бы 82% «очень высокий» для обычного
  курса; органы коррелированы одним препаратом). Timeline — тоже max. `maxRaw`/формулы
  механизмов/поддержка Π(1−k) не тронуты.
...
- **Эритроцитоз (hem1) — «не так страшно» честными методами**:
  ...
  - **процедуры как k-записи**: NEW `PROCEDURE_DB` — эритроцитаферез (hem1 k=0.45) и
    ...
    Calc.mapper tzFinalRisk + doctor report); **процедура пробивает якорь на afterPercent**
    (меняет состояние: «до процедуры high, после — ниже»), raw остаётся на якоре;
  ...
  - числа сценария (тест 500, без анализов): гемато 34.5% → база 28.8% → +фибринолитики 26.1%
    → полный стек 23.2% (hem1 31→20%); TRT 200 → гемато 11.7%; общий = max: 34.5%, не 85%
    (был union);
...
- Тесты: NEW `tz-spec-union-floors.test.ts` (30: union-свойства, синергии, floors, сценарные
  ...
  доза/полный стек/процедуры не трогают другие системы); обновлён `tz-spec-risk-invariants.test.ts`
  (система ≤ суммы механизмов, ≥ максимума — вместо «сумма сходится»).
- Проверено: tsc 0 по моим файлам (1 ошибка в чужом новом untracked CardioDiary.tsx);
  затронутые области 361/361 (tz-spec-union-floors 30, invariants 6,

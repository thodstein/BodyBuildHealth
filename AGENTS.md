...
- **Аудит-топ-10 зафиксирован** (коды не менялись, где риск): packing-6 съедается perExerciseCap-5 без onCourse; prep early-return пропускает levels/safety; один prep-маркер отменяет taper всему плану; tradeoff не обновляет чужой indirect + хардкод 5 на курсе; level-swap ранжирует штангу выше станков; валидатор шумит low_freq на руках + порог 0.7 прячет дефициты; `course = onCourse || enhanced` даёт курсовые капы натуралу; female-бонусы только в adapt.

...
- **Раунд 12 (структура)** — по требованию «полная структурная перестройка», рамки согласованы (5 шагов, 3 группы, табы замер-first): конструктор 6→5 (новый 🎯 Атлет; TOP-остаток в Стол/хват; цикл целиком в Сплит; качество+веса влиты в План; визард «Далее/Назад»; контент перенесён побайтово, вторая сборка убрана); табы хаба grip/strength/wrist/pressure/recovery (дефолт grip — иначе ложились 10 чужих тестов). NEW `arm-wizard-nav` 5/5. Тесты шагов обновлены (моя зона); мост/корни чужих целы. Замечание: `cycle-catalog-bridge` временно не собирается — чужой `StrengthSportConstructor.tsx` в сломанном WIP (моих файлов в ошибках ноль).

...
### Goal
Устранить исчезновение стеков в попапах калькулятора поддержки: стеки «кликаются в пустой экран, но не отображаются» из-за крошечных шрифтов (`fontSize:7/8/9` vs рабочие 14 в intell-попапе). Довести правку шрифтов до конца в попапе ручного режима И попапе «Усиление» (enhancement), плюс сопутствующие карточки выбранных стеков.

### ✅ Сделано и проверено (esbuild transform OK, EXIT 0, 222.5kb)
- **Попап ручного режима** (`showManualPopup`, ~744-870): все шрифты подняты 7/8/9 → 11-13px (заголовок, ✕, кнопка каталога, chips manualSubs, блок «Выбранные стеки поддержки», строки стеков, раскрываемые детали). Подтверждено чтением 773-807: chips стеков `fontSize:11/13`, ✕ `fontSize:13`.
- **Попап «Усиление»** (`showEnhancementPopup`, ~933-1068): все шрифты подняты 7/8/9 → 10-13px (заголовок/✕, поле поиска, строки стеков, раскрываемые детали: organMechanisms/synergyPrinciple/finalEffect → 11, бейджи механизмов → 10, заголовок веществ → 12, строки веществ → 11, противопоказания/предупреждения → 11, кнопка «закрыть» → 12).
- **Карточка «Усиление» (вне попапа, `level !== 'manual'`, ~884-927)**: подняты шрифты — лейбл 7→9, кнопка «Все стеки» 8→11, табы режима 8→11, chips выбранных стеков 7→10, ✕ 9→12 (и цвет ✕ 0.3→0.6 для видимости/тап-зоны).
- **Кнопка gapFill «Все рекомендованные»** (~953): шрифт 9→12 (повторная правка — первый раз не применилась из-за несовпадения контекста, переапplied с полной строкой).
- **Карточка быстрых протоколов/пресетов вне попапов**: мелкие шрифты оставлены как есть (предсуществующий плотный UI, НЕ в зоне бага).

### ❌ Остаётся (вне зоны бага)
- `fontSize:7/8/9` в предсуществующих компактных попапах: intell-попап (строки 691/703/733 — мелкие лейблы рядом с рабочими строками 14px, пользователь НЕ жаловался), и в карточке результата/модулях суставов/нейро/симптомов (1094+). Эти НЕ reported как сломанные — оставлены.
- `tsc --noEmit` полный не запускался (таймаут проекта); правки — только смена строковых литералов `fontSize`, типобезопасны.

### Key Decisions
- Корень бага: не stale dev-сервер и не `styles.css` (переменные `:root` корректны), а именно `fontSize:7/8/9` с бледными цветами в попапах ручного режима и усиления → строки «невидимы».
- Исправлять ВСЕ шрифты в обоих проблемных попапах + карточке «Усиление», а не только одну (пользователь явно указал, что enhancement-попап тоже пострадал).
- esbuild transform подтверждает отсутствие синтаксических ошибок после правок.

### Relevant Files
- `src/ui/screens/Calculator/Calc.mapper.tsx` — попап ручного режима (744-870), попап «Усиление» (933-1068), карточка «Усиление» (884-927), gapFill (953) — шрифты исправлены; esbuild OK.

---

## Session Summary (Jul 16 — Part 11) — Планировщик: BB-аудит, отчёт, и выполнение логических правок

### Goal
Действовать как главный тренер ББ: аудит BB-авто + ручного планировщика, критический отчёт, затем ВЫПОЛНЕНИЕ логических правок. Визуал/дизайн ДЕЛЕГИРОВАН отдельному агенту (создаст общий файл вывода программы для всех конструкторов).

### Constraints & Preferences
- Отвечать на русском; тех. идентификаторы — английские
- Не отвечать на беседу, только суммировать
- Сохранять точные пути файлов/идентификаторов
- Дизайн/визуал — отдельный агент (общий UI-файл вывода); этот ассистент правит только ЛОГИКУ
- Использовать прямое чтение исходников (отчёты сабагентов обрезаны/недостоверны)

### Progress
#### Done
- Доставлен критический отчёт (что работает; A–D десинхрон; таблица багов 1–4; визуал; план A–H)
- Прочитан bb-builder.engine.ts (140–807) + manual-plan-builder.ts (238–520) + injury-хелперы
- Подтверждено: A+B+C УЖЕ реализованы в bb-builder:
  - distributePhases как ЕДИНЫЙ источник RIR/фаз/deload (коммент строка ~28)
  - bbRir(resolved, phase, phaseWeek) + phaseBaseRir + drift из PHASE_CONFIGS (fix A/B)
  - ACCESSORY_2X_GROUPS + exerciseCount (fix C)
  - normalizeWeekMrv + per-exercise кап 8 + mrvByMuscle×PED×weak/focus (fix D)
...
### Key Decisions
- Дизайн делегирован отдельному агенту → правим только логику
- A-D оказались уже реализованы предыдущей сессией (Part 8) — НЕ изобретать заново
- Доверять прямым чтениям исходников, а не отчётам сабагентов (несколько багов не подтвердились)
...
- SRCBBScreen.tsx — закоммичен в 3 коммитах (47f2172ac PL-авто weakpoint/phase-RIR + d6fd5ffa6 V7/CSS-var/шрифты + 9e35513e0 V7-расширение 1ПМ). Изначально содержал смешанные правки двух агентов, затем обе части были закоммичены.

...
- **Волна 3 (P2, гигиена)**: удалены проверенно мёртвые `computeOverloadTargets`(+тип), `ensurePPLMidDeltFinisher`/`ensurePPLRearDeltFinisher` (заменены семейством `ensurePPL*` в `975772463`), `cnsFor`, `explainBBSelection`/`explainBBMetrics`, `isRepSchemeId`, `BB_RED_FLAGS_EMPTY`, `isAcceptableBB`, `hasBothPhases`, `idealReevesMap`, `SUBGROUP_LABEL_RU`, `BB_PREP_CONFIG_KEY`/`BB_PREP_PLAN_KEY`, `BB_CAUSE_RU`, `indirectArmVolume`, `TECHNIQUE_ALIAS_TO_ENGINE`/`ENGINE_ALIAS_TO_UI`, `splitForDays`, `jointGuardTempoOverride`/`jointGuardRationale`, `SPECIALIZATION_MAX_BLOCK_WEEKS`; в UI снесены 14 мёртвых «открытостей»/`expandedMuscles`/`strategySafe`/`catEx`/`distText`/`past`/`disabled=false`; своп-модалка ББ теперь строго внутри жёсткой группы (`strictGroupMembersOf` + подпись, фолбэк по group вне группы); тавтология `e.sets<=e.sets` заменена сравнением с исходным планом; тест идемпотентности `finalizeBBPlan` усилен сравнением sets/RIR/workSets.length; убран дубль-контрол «Цель объёма», «Дней/нед» 2–7 (паритет с клампом). Осознанно: допуск ±16 blast/cruise оставлен (root-cause в сессионных капах — фикс = отдельная кампания §7), `bbProgramPath` (roundtrip-маркер вариантов) и реестр `zoneSpec/donorsForZone` (API-запас) — не тронуты.

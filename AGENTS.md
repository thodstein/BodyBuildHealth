...
## Даты, раунд 7: движки «прочие» — 24 из 27 мест; 3 оказались НЕ дефектом (Sep 26 2026, без пуша)

Продолжение «контур целиком». Взяты 10 чистых движков вне хабов. Только Edit/Write + vitest/tsc; чужие WIP (WL/BB-хабы, метаболика, кардио, support, Та-движок) не тронуты; **НЕ пушил**.
- **Главная находка — не «подписи», а счётчики.** 7 из 24 мест — это СЕРИИ (`recalcStreaks` привычек, серия ПР, серии разминки/заминки) и окна приверженности. Там `cursor` шагает `setDate()` от момента, и UTC-извлечение давало **вчерашний ключ**: в UTC+10 до полудня отметка «сегодня» не попадала в серию (разминка/заминка = 0 при выполненной разминке). Это не «сдвиг подписи», а сломанный счётчик.
- **3 места НЕ тронуты — и это осознанное решение, а не недосмотр.** `labs-scheduler.engine.ts`: `startDate` — строка `'YYYY-MM-DD'`, `new Date('YYYY-MM-DD')` = полночь UTC, прибавление целых недель в мс + извлечение дня в UTC даёт ровно «startDate + N недель» независимо от смещения. На машине с ОТРИЦАТЕЛЬНЫм смещением `localIsoDate` сдвинул бы `dueDate` на сутки назад. Причина зафиксирована комментарием в движке + тестом (`dueDate === shiftIsoDate(start, week*7)`) и гардом «в контуре это единственный файл с date-only, ровно 3».
- **Побочно найден и исправлен баг, который ценз НЕ видит** (в `coaching-psychology`): `daysSinceLastPR` считал `Math.round((Date.now() - new Date(iso))/86400000)`, где `new Date('YYYY-MM-DD')` — полночь UTC, а `Date.now()` — момент. ПР сегодняшним утром давал **−0 дней** (отрицательный ноль) — то, что пользователю показывается как «0», но со знаком минус; вечерний ПР вчера давал −1. Исправлено на канон: обе стороны приводятся к местной полуночи (`parseLocalIsoDate` + целые дни). Тот же приём применён к `daysDiff` прогноза ПР. **Урок:** ценз ловит только `toISOString().slice(0,10)`; «разобрать строку-дату в UTC-полночь и сравнить с моментом» — тот же класс, но невидимый.
- **Тесты**: NEW `free-engines-date-canon` **24/24** — момент 08:00 местного 27-го (именно тот час, где UTC отстаёт на сутки; на UTC-минус локи-различители вырождены — оговорка печатается, держит source-guard). Покрыты writer/reader-пары ключей (привычка/ПР/сессия дневника/прогресс дня мышления и мобильности), 4 серии, 5 окон приверженности на граничной паре (−N внутри / −N−1 снаружи), 4 прогноза/проекции, labs-инвариант и source-guard (0 date-only + импорт канона во всех 9). **Мутация (возврат UTC в `cooldown.isoOf`, `warmup` floor и `daysSinceIso`) роняет 6 тестов** — 2 поведенческих на `−0`, серия, окно + 2 гарда.
- **Мои ошибки (3, пойманы своими же прогонами).** (1) `createSet` читает `actualReps/actualWeight`, а не `weight/reps` — мой тест считал недельный объём по несуществующим полям. (2) `recordPR` всегда штампует СЕГОДНЯ, поэтому «восходящий тренд для прогноза» из двух записей не строился (daysDiff=0) — старый ПР сею в журнал напрямую. (3) **Знак в новом хелпере**: `last − first` = `daysSinceIso(first) − daysSinceIso(last)`, я написал наоборот и сломал прогноз ПР; поймано тестом. Плюс `daysSinceLastPR` при РАЗРЫВЕ в серии возвращает 1, а не 0 — это предсуществующая семантика другой ветви цикла, её не трогал и явно оговорил в тесте.
- **Проверено**: круг потребителей 14 файлов **276/276** + кросс-зона (periodization/strength-sport×2/mindset-tab/diary-date-canon/pct-UI) **171/171**; NEW 24/24; `tsc --noEmit` **0 по всему проекту**; `verify:apk-design` OK; `git diff --check` чист; кодировка без BOM/U+FFFD.
- **Потолок ценза 153 → 129** (фактический замер, совпал с прогнозом 153−24). Остаток 129: `src/ui/screens` 79, `engines/arm` 11, `engines/bb` 7, `engines/pro` 5, `nutrition-ocr-parser` 4, `engines/combat` 3, labs-scheduler 3 (осознанно), `engines/lms` 2 + 9 одиночных движков по 1.
- **Осознанные границы**: смещение TZ через `process.env` в vitest НЕ применяется — «календарь ≠ UTC» проверяется условно по реальному смещению машины; предсуществующая off-by-one ветвь `daysSince` не менялась (это не канон дат).

## Ручной конструктор: ARM-направление принято + 2 реальных дефекта (Sep 26 2026, коммит pathspec `7584c5cba`, без пуша)

По команде «бери ручной конструктор» принят чужой WIP (14 файлов) и проведён аудит. Только Edit/Write + vitest/tsc; остальные 31 файл worktree (nutrition/cardio/strength-sport/bb) не тронуты; коммит строго pathspec; **НЕ пушил**.
- **Дефект 1 (writer/reader, потеря данных).** `generateCalendarMonth`: выполненная сессия без плановой длительности получала `compliance=0` → статус **`missed`**. Причина: WIP сделал длительность опциональной (убирал фиктивные 60 мин), а процент считался только по ней. Исправлено: при неизвестной длительности день закрывается по факту выполнения (100/done) — так же, как в живом `generateTrainingCalendar`; при известной длительности partial/missed не тронуты. **Важно:** сперва я ошибся, решив что `actual.completed` не существует — в `generateCalendarMonth` вход МАССИВ с полем `completed`, а в `generateTrainingCalendar` вход `Map` без него; два билдера выглядят одинаково, но формы разные.
- **Дефект 2 (тихая пустота).** `computePlanQualityFor` и `analyzeManualVolume` читали только `program.bb` и `hybrid.bbWeeks` — у ARM-программы качество и недельный бюджет считались по **пустым** данным, т.е. бейдж в списке программ врал. Исправлено симметричным `program.arm?.weeks` (порядок `??` сохранён, числа ББ байт-в-байт) + `isPL` явно исключает arm.
- **Принятый WIP**: направление ARM в ручном конструкторе (создание/скелет/визард, `ArmEditor` через общий `BBEditor`, `RirWaveChart`, экспорт HTML/CSV/печать/QR/JSON/ICS, фильтр/иконка) + provenance planned vs actual (`planned*` в `BridgeSet`/`BridgeSession`, `provenanceSource`/`planSnapshotId`/`plannedSessionId`, `duration?` вместо 60 мин, `weekNumber`/`source` стали опциональными).
- **Тесты**: NEW `manual-arm-quality` 4 — ARM по своим неделям, паритет с ББ, ББ/PL не сломаны, недельный бюджет. **Проверено мутацией:** снос `program.arm` из обоих движков роняет 2 теста. `training-calendar-duration` +3 (регрессия missed, partial при известной длительности, паритет живого пути). Круг: manual-constructor + user-program + quality-parity + calendar + UI = **90/90**; `tsc` — по своим 0 (на тот момент 6 ошибок были в чужом `CardioWeekEditor`); `verify:apk-design` OK; `diff --check` чисто.
- **Сознанные границы**: `BBEditor` для ARM собирает quality-memo через tmpProg с `direction:'bb'` (математика идентична — движок читает только недели); в ICS длительность сессии — пользовательский ввод (плоский, по умолчанию 75 мин), `estimatedMin` заполняется только для ARM/кардио.
- **Урок**: `git commit -m` с кириллицей и **backtick** ломает PowerShell-парсер (backtick = escape) — длинные сообщения писать файлом и коммитить через `-F`.

## Даты, раунд 6: хаб стронгмена — 7 мест, из них 3 writer-ключа (Sep 26 2026, коммит pathspec `29a33cc1`, без пуша)

Продолжение «контур целиком». `WLDiagnosticsHub`/`BBDiagnosticsHub` оказались заняты чужим WIP — взят только `StrongmanDiagnosticsHub` (7 мест, ровно как в долге). Только Edit/Write + vitest/tsc; **НЕ пушил**.
- **Главная находка раунда — writer-класс, а не подписи.** Три из семи мест — **ключи записи**: `handleSaveProgress` (`appendSMProgress`), `handleSaveOHSSnap` (`appendOHSSnapshot`), `handleGripAsymSnap` (`appendSMGripSnapshot`). Все три `append*` **ЗАМЕНЯЮТ запись по дате** (`filter(s => s.date !== entry.date)`) → UTC-дата на границе суток даёт **реальную потерю данных**: вечер 26-го (UTC 13:30) + утро 27-го (UTC 23:10) в UTC+10 = один UTC-день → вторая запись затирает первую. Это не «сдвиг на сутки в подписи», а стирание. Остальные 4 места — имена выгрузки (бэкап/ICS/HTML/CSV).
- **Лок построен на последствии, а не только на source.** NEW `strongman-date-canon` 6: source-guard (0 date-only + импорт канона + ровно 3 writer-даты и 4 имени файла) **+ поведенческий** лок на настоящих движках: на UTC-датах `appendSMProgress` оставляет 1 запись, на каноне — 2. Обработчики самого хаба **не вызываются** (тяжёлый god-компонент + выгрузка через нативный мост) — это зафиксировано в шапке теста как граница, но здесь она не ослабляет лок, потому что доказывается ровно то следствие, ради которого правка.
- **Смещение машины +600 мин (UTC+10)** → коллизионная ветка реально выполняется, а не вакуумна (при offset ≤ 0 тест это оговаривает и не делает вид, что проверил).
- **Мутация**: возврат UTC в `handleSaveProgress` роняет тест двумя сигналами (date-only + счётчик writer-дат).
- **Проверено**: `strongman-date-canon` 6/6, census + labs-risk + report-date-canon 18/18, хаб и соседи (`strongman-diagnostics-corrective`, `ss-annual-overlay-consumers`, `rest-hooks-native`) 92/92, `tsc --noEmit` **по всему проекту 0 ошибок** (чужий `CardioWeekEditor` к этому моменту починен владельцем), `verify:apk-design` OK, `diff --check` чисто.
- **Потолок ценза 160 → 153** (фактический замер). Правка файла ценза уехала в коммит `5934ea6fe` параллельного агента (он взял worktree-состояние, в котором я уже понизил потолок) — guard в коде уже 153, в этом коммите файл ценза не менялся.

## Даты, раунд 5: арм-контур — 13 мест, потолок ценза 173 → 160 (Sep 26 2026, 1 коммит pathspec `5934ea6fe`, без пуша)

Продолжение «контур целиком»: взяты оба хаба арма сразу (они зеркальные). Только Edit/Write + скрипт-патч (UTF-8 без BOM) + vitest/tsc; чужие WIP не тронуты; коммит строго pathspec; **НЕ пушил**.
- **Что переведено (12 календарных мест)**: `ArmDiagnosticsHub` 6 (защитный «сегодня» в try/catch — источник дня заменён, сам try/catch сохранён; дата снимка; три имени выгрузки `arm-diagnostics-*.html` / `arm-diagnostics-*.csv` / `arm-plan-*.ics`; `dateIso` новой силовой попытки), `ArmliftingDiagnosticsHub` 6 (недели с прошлой попытки `weeksBetween`; два снимка; три имени выгрузки `armlift-spec-block-*.ics` / `armlifting-*.html` / `armlifting-*.csv`).
- **Главная находка раунда — 13-е место переведено «наоборот».** `he_arm_humerus_checks.touchedAt` в `arm-hub-tabs2` матчился цензом как date-only, но по смыслу это **момент во времени, а не календарный день** (событие «чек-лист тронут»). Переводить его на канон дня было бы враньём с другой стороны, поэтому сделан честный ход: среза до дня там больше нет, поле стало полным UTC-таймстемпом. Ключ **write-only** (проверено: `touchedAt`/`he_arm_humerus_checks` не читаются нигде в проекте), поэтому смена формы безопасна. Это и есть различение контракта в действии: календарная дата — только из `core/local-date`, событие (`updatedAt`/`touchedAt`) — UTC корректен.
- **Мутация доказана тремя независимыми сигналами** (NEW `arm-date-canon` 4): возврат UTC в имени выгрузки `ArmDiagnosticsHub`, возврат UTC в `dateIso` силовой попытки и возврат среза к `touchedAt` — **каждая роняет свой тест**; контрольный прогон зелёный, файлы восстановлены побайтово (откат через Buffer-копию, не через перезапись).
- **Про силу лока честно:** поведенческого лока на рендере нет — экспорт идёт через нативный мост (`downloadArmFile`/`saveCsvApk`/`printHtmlApk`), оба хаба — тяжёлые god-компоненты с провайдером профиля. Поэтому source-guard на 3 файла + проверка, что 3+3 имени выгрузки и `dateIso` идут через канон. Лок на саму запись попытки (где расходились дневные ключи) остаётся в `date-canon-census`/`diary-date-canon`.
- **Проверено**: круг арма — **110 файлов / 1371 тест зелёный** (движки `src/engines/arm` + 5 UI-тестов хабов + `rest-hooks-native`; chart.js-предупреждения — предсуществующий шум, не мой); дата-канон круг **20/20**; `tsc --noEmit` — 1 ошибка и она **чужая** (`strength-sport/sm-bridge-intake.ts:325 isSM`, файл в статусе `M` чужого WIP, моих строк там ноль); кодировка без BOM/U+FFFD, `git diff --check` чист.
- **Остаток долга: 160 мест.** Топ: `WLDiagnosticsHub` 9 **[грязный — чужой WIP]**, `StrongmanDiagnosticsHub` 7, `ArmAutoConstructor` 2, `ExerciseLabPrescription` 2, `FatigueIndexTab` 2, `MobilityTab` 2, `MuscleProgressCard` 2, `StrengthAnalyticsCard` 2, `VolumeRecoveryCorrelationCard` 2, `arm-hub-tabs2` 0, `periodization-designer.engine` 5, `nutrition-ocr-parser` 4, `BbAutoConstructor` 4. Следующий естественный контур — **движки «прочие»** (`periodization-designer` 5 + `coaching-psychology`/`cooldown`/`labs-scheduler`/`mindset-protocol` по 3 + `body-composition`/`diary-insights`/`mobility-protocol`/`pct-planner`/`warmup` по 2 = 20 мест): риск минимальный, локи дешёвые, чистые движки без чужих ханков.

## Даты, раунд 4: лаборатория + риски — 13 мест, потолок ценза 186 → 173 (Sep 26 2026, 1 коммит pathspec `7ee3d7aac`, без пуша)

Продолжение «контур целиком». Тот же приём (связная пара экранов вместо одиночных файлов), но без поведенческого лока на рендере — и это осознанное решение, зафиксированное ниже. Только Edit/Write + скрипт-патч (UTF-8 без BOM) + vitest/tsc; чужие WIP не тронуты; коммит строго pathspec; **НЕ пушил**.
- **Что переведено (13 мест)**: `LabsScreen` 6 (дата снимка трендов; подпись «Анализы <дата>»; три имени выгрузки `labs-*.csv` / `lab-trends-*.csv` / `lab-trends-*.html`; `<title>` печатной версии), `RiskScreen` 7 (дата записи анализа; подпись выгружаемого отчёта; `risk-report-*.json`; **три стартовые даты окон комплаенса** — «3 месяца назад», «N недель назад», «28 дней назад»).
- **Суть дефекта (тот же writer/reader раскол)**: даты анализов пишутся по локальному канону (`lab-diary` → `core/local-date`), а экраны читали «сегодня» и границы окон по UTC. Вечером снимок и текст отчёта получали вчерашнюю дату, в имя выгрузки попадал вчерашний день, стартовые даты окон сдвигались на сутки. **Важно:** сдвиг самой календарной арифметики (`setMonth`/`setDate`) НЕ тронут — менялось только извлечение дня, иначе «3 месяца назад» поехало бы по-разному в месяцах разной длины.
- **Осознанно обошлись без поведенческого лока на рендере — и это не слабость, а расчёт:** экспорт идёт через нативный мост (`saveCsvApk`/`saveTextFileApk`/`printHtmlApk`), а `LabsScreen`/`RiskScreen` — тяжёлые god-компоненты, требующие полного провайдера профиля. Цена обвязки выше ценности мест: сдвиг окна на сутки и вчерашняя дата в имени файла **не меняют решение пользователя**, в отличие от контура дневников (там расходились ключи записей). Поэтому NEW `labs-risk-date-canon` (3) — source-guard на оба файла + проверка, что все три стартовые даты идут через `localIsoDate(d)` и что календарная арифметика осталась на месте. Граница записана в шапке теста.
- **Мутация доказана**: возврат UTC в имени выгрузки `LabsScreen` + в одной из трёх окон `RiskScreen` роняет **2 теста** — гард (0 date-only) и счётчик окон (3 → 2), то есть два независимых сигнала.
- **Проверено**: круг labs + risk — **112/112 (30 файлов)**: 14 UI-тестов labs/risk + весь `RiskScreen_parts` + `LabDiaryTab` + экспорт протоколов; дата-канон круг **22/22**; `tsc --noEmit` **0 ошибок по всему проекту**; кодировка без BOM/U+FFFD; `git diff --check` чист.
- **Остаток долга: 173 места.** Топ: `WLDiagnosticsHub` 9, `StrongmanDiagnosticsHub` 7, `ArmDiagnosticsHub` 6, `ArmliftingDiagnosticsHub` 6, `periodization-designer.engine` 5, `nutrition-ocr-parser` 4, `BbAutoConstructor` 4, `bb/cycle-to-plan` 3. Следующий естественный контур — **хабы диагностики** (`WLDiagnosticsHub` + `StrongmanDiagnosticsHub` = 16 мест): те же снимки по датам и выгрузки, тот же тип правки, что и здесь.

## Калькуляторные хабы: волна 1 «научная честность» — E1.6 закрыт ОДНОЙ находкой (E1.3/E1.5 в прошлом блоке) (Sep 26 2026, без коммита)

Продолжение волны 1. Только Edit/Write + vitest/tsc; чужие WIP не тронуты; **НЕ коммитил**.
- **E1.6 (A4/A7/A8) — план ждал три правки текста, а оказался ОДИН write-only дефект.** Витамин D, омега-3/DHA и EGCG **уже имели честно написанные `note` в реестре** («EFSA прямо называет гиперкальциурию более ранним признаком избытка» + риск на AAS; «UL для добавкового DHA НЕ установлен… ТОЛЬКО для DHA-доминантных… НЕ применять к смесям EPA+DHA»; «гепатотоксичность EGCG идиосинкратическая, COMT/UGT1A1 и HLA-B*35:01, т.е. НЕ предсказуема дозой»). Но `NUTRIENT_LIMITS_V2[id].note` **не читался НИГДЕ**: все 4 ветки `doseWindowFor` собирали `note` только из юрисдикции/года/значения/`criticalEffect`, а UI рендерил лишь `altNote`. Класс тот же, что E0.12 и E1.5: «честно написано — пользователь не видит». Фикс: `DoseWindow.limitNote` (**обязательное** поле — компилятор заставит заполнить во всех ветках, иначе оговорка снова станет write-only) + рендер в карточке дозы (белый текст, 9px). **Один фикс закрыл три эпика.**
- **Честная граница:** из строки A8 не перенесены «smokers should avoid» и «α-омега-3 ухудшают статус витамина A» — в реестр **не внесены осознанно**: проверенного первоисточника нет (`dha` помечена `verified: true`, `websearch` заблокирован 403), а дописывать клиническое утверждение без ссылки в проверенную запись — та же ложь, что E1.5 с железом. Кандидат в E1.12 или `unverifiedLimits`.
- **Тесты:** `support-hub-pro` 69→**77** (E1.6 ×8), включая инварианты «оговорка не теряется ни в одной ветке резолва» и «вещество вне реестра (`vitex`/`serrapeptase`/`bcaa`) НЕ получает выдуманную оговорку». Мутация (обнуление `limitNote` в ветке therapeutic-окна) роняет **4** теста. Круг Support: **337/337 (24 файла)**, `tsc --noEmit` **0 по всему проекту**.
- **Мои ошибки раунда (4, все пойманы своими же прогонами).** (1) Сравнивал оговорку с `NUTRIENT_LIMIT_INFO` из `support-plan/types` — у него **нет поля `note`** (урезанное представление для UI) → `undefined`; канон — `support-limits.NUTRIENT_LIMITS_V2`. (2) `THERAPEUTIC_WINDOWS`/`DOSE_RANGES` импортировал статически не из того модуля → `Object.keys(undefined)`; канон — **top-level await** `await import(...)` из двух РАЗНЫХ модулей (второй — внутри TSX компонента). (3) `new URL(..., import.meta.url)` в vitest даёт `ERR_INVALID_URL_SCHEME` — source-guard на чтение файла заменён на **поведенческий** (UI-рендер) и на компиляторную защиту обязательного поля. (4) **Придумал цитаты реестра, которых в нём нет**: ждал «длительное превышение… ранний признак» и `не "потолок"`, а там «более ранним признаком» и «ТОЛЬКО для DHA-доминантных». Это тот же класс, что в E1.1 — «моя прикидка» вместо чтения; ассерты переписаны по факту, реестр не трогал.

## Калькуляторные хабы: волна 1 «научная честность» — E1.3/E1.5 закрыты (E1.1/E1.2 в прошлом блоке) (Sep 26 2026, без коммита)

Продолжение волны 1. Только Edit/Write + vitest/tsc; чужие WIP (ТА-движок, combat, arm) не тронуты; **НЕ коммитил**.
- **E1.3 — TyG и HOMA-IR.** Они НЕ формулы-дубли, но и **не два независимых сигнала**: `TyG = ln(TG×глюкоза/2)` и `HOMA-IR = (глюкоза/18.018×инсулин)/22.5` **делят вход «глюкоза натощак»**. Одна высокая глюкоза поднимает оба, поэтому два «⚠» рядом читались как двойное подтверждение ИР — на деле это один сигнал, посчитанный дважды. Добавлена `IR_MARKERS_SHARED_INPUT_NOTE` в сводку (где печатаются оба) + факт «HOMA-IR без анализа на инсулин = показателя нет». **Пороги не трогал** (8.8/1.4/2.5 — не мои выдумки). Тест **доказывает утверждение из кода** (глюкоза поднимает оба), а не наличие строки.
- **E1.5 — железо: потребность ≠ доза добавки. С ОТКЛОНЕНИЕМ от плана, и это главное.** План предлагал «доза по весу 12 мг/кг, 16–100 кг → 6–8 мг/кг». Буквальное исполнение — **опасная подмена**: 6–8 мг/кг для 100 кг = 600–800 мг/сут, в **15–20 раз** выше безопасного уровня из реестра (`support-limits`: iron EFSA 2024 SafeLevel 40 мг, альт. IOM 45 мг). Сделано честно: потребность названа по весу, **явно помечена «ПИЩА, а не доза добавки»**, потолок добавки взят из реестра, + честность источника (Smid 2024, PMID 38407751: короткие курсы неэффективны; Hb/ферритин/Tsat/VO2max = **NS**, т.е. цифра ориентировочная, а не Пруф-эффект).
- **Побочно (тот же класс E1.2 — одно значение, две подписи):** в строке железа «40 мг» писалось вручную кириллицей, а «45 mg» подставлялось из реестра латиницей. Канон `limitUnitRu` + убраны оба дубля маппинга (`support-hub-evidence`, `SupportEffectiveDose`); неизвестная единица **не подменяется молча**.
- **Три мои ошибки в этом блоке (чтобы не повторять).** (1) `export function` вставлен **внутрь тела функции** → esbuild `Unexpected "export"`; вынесен на уровень модуля. (2) guard искал оговорку по **первому** вхождению константы, а первое — это импорт: проверка «рядом ли» была бессмысленной. Теперь берётся второе вхождение, порог **из замера** (401 символ), а не из прикидки. (3) Тест искал «не превышает», а в коде акцент «**НЕ** превышает» — виноват тест, не код. **Общий урок:** regex по кириллице с `«»` заменены на `toContain`, потому что `vitest` в сообщении всегда показывает обрезанный префикс строки и непонятно, КАКОЕ утверждение упало — это стоило нескольких итераций.
- **Тесты:** `metabolic-hub` 136→**140** (E1.3 ×4), `metabolic-hub-honesty` 7→**9**, `support-hub-pro` 61→**69** (E1.5 ×7). Мутационно: убираю оговорку «ПИЩА vs добавка» — падает тест E1.5; убираю оговорку IR из UI — падает guard. Круги: Shared+Support UI **332/332** (24 файла), metabolic 140 + honesty 9 + support-hub 69 + лимиты 45 + аудит 42, `tsc --noEmit` **0 по всему проекту**, `verify:apk-design` OK.

## Калькуляторные хабы: волна 1 «научная честность» — E1.1/E1.2 закрыты (Sep 26 2026, без коммита)

Начало Wave 1 (E0.10/E0.11 остаются заблокированы чужим WIP в `TrainingScreen.tsx`). Только Edit/Write + vitest/tsc; чужие WIP (ТА-движок, combat, arm) не тронуты; **НЕ коммитил**.
- **E1.1 — Pontzer ~30%. Ловушка была в формулировке, а не в коде:** «реальный расход может быть на ~30% выше формулы» (Pontzer & Trexler 2026) tempt'ит превратить в «формула +30%» — а это враньё в обе стороны, цель завысится на треть у тех, кому повезло с адаптацией. Сделано честно: `PONTZER_FORMULA_NOTE` («формула BEE×PAL — оценка») в обоих TDEE-выводах + one-answer, и `TDEE_ALREADY_ADAPTED_NOTE` на adaptive-v3 («уже адаптирован по тренду веса — надбавку не применяем, иначе двойной счёт»). Тест фиксирует **отсутствие** надбавки (`tdee === formulaTDEE`, regex на `+30%`), а не наличие подписи.
- **E1.2 — единая метка BMR. Найдено 2 разных подписи для одного значения, одна из них врала:** inline-цепочка в UI покрывала 5 из 8 методов → `livingston`/`harris_revised`/`henry` показывались как «Mifflin», а вторая строка отдавала сырой id `cunningham`. Канон `bmrMethodLabel`/`bmrMethodSpread` → `core/metabolic-constants`, UI переведён на него.
- **Главная находка раунда (обошла дважды):** разброс между BMR-формулами — **46–75%**, а не «единицы процентов», как стояло в моём же первом комментарии. И он НЕ является «погрешностью расчёта»: часть формул к человеку **неприменима** (Livingston валиден для BMI≥35, а при BMI 18 у 55кг даёт 2128 ккал — абсурд, посчитанный «для сравнения»). Поэтому в UI разброс показан **с оговоркой** («часть формул к вам неприменима, это не точность расчёта»), а пороги в тесте — **из замера** (74.7% / 46%), а не из прикидки. **Урок:** моё допущение «разброс <60%» упало тестом — и было бы враньём, если бы я его «починил», ослабив проверку; правильная реакция — пойти и измерить.
- **Тесты:** `metabolic-hub` 128→**136** (E1.1 ×4, E1.2 ×4), NEW `metabolic-hub-honesty` **7** (source-guard: нет inline-фолбэка в UI, нет сырого id, канон покрывает все 8 методов, нет `*1.3` в движке, оговорка видна в обоих TDEE-выводах). **Мутационно проверено 3 раза:** возврат подписи Pontzer роняет 1 тест, возврат `minHeight:40` — 2, возврат inline-цепочки BMR — 2. Круги: Shared 373/373 + metabolic-hub 136 + honesty 7, `tsc` чист по своим (3 ошибки — чужой WIP `combat-measurements`), `verify:apk-design` OK.
- **Побочно:** `course-sync` (5/11) и `profile-e2e` красные **не мои** — `metabolic` в них 0 совпадений, `profile-e2e` изолированно 30/30 (флейк параллельной нагрузки), `course-sync` красный и изолированно (предсуществующий, документирован в истории).

## Калькуляторные хабы: волна 0 «перестать врать» — E0.9/E0.13 закрыты, волна 0 почти вся (Sep 26 2026, без коммита)

Продолжение раунда E0.2/E0.7/E0.8/E0.12. Только Edit/Write + vitest/tsc; чужие WIP (ТА-движок, кардио/combat) не тронуты; **НЕ коммитил**. **Найдено 8 нарушений сенсорной нормы вместо 6, и главный дефект был в САМОМ ТЕСТЕ.**
- **E0.13 — «тест держит норму» был фикцией.** `metabolic-hub-toggles` проверял `.mh-mode` только для `modes.slice(0, 4)` из 16 и с порогом **40**, т.е. настоящие `minHeight: 40` у всех 16 режимов проходили как «норма». Теперь 16/16 и порог 44. Прочие (замерено сканером, а не взято из плана): `.mh-preset` ~26px (только padding), `.mh-compare` 30×30, `.mh-mini` 40 — **три** кнопки (план назвал две; третью нашёл сканер), пот-тумблеры 32. Всё поднято до 44 **инлайн** в TSX, потому что `styles-native.css` — чужой append-only WIP. `.mh-compare` получил область нажатия 44×44 при видимом кружке 30 (`boxSizing:'content-box'`), чтобы ряд сценариев не распух визуально.
- **E0.9 — счётчики, которые врали о собственном коде.** «17 калькуляторов» при фактических 16 → литерал убран, подпись выводит `MODE_DEFS.length` (расхождение теперь невозможно); «5 в 1» → «16 режимов» в **обоих** списках `PlannerToolsPanel` (дефект был продублирован); «Hall adaptive v2» → v3. **Плюс ложное срабатывание плана:** «Biggest Loser» — это прозвище ИССЛЕДОВАНИЯ (Fothergill, Metabolic Adaptation to Weight Loss, 2016), а не название настройки или ключа хранилища; план решил, что такого ключа не существует, и предложил вычистить. Правильно — оставить науку и заменить прозвище на «Fothergill, 2016», потому что пользователю ник прозвища ничего не сообщает.
- **Тесты:** `metabolic-hub-toggles` 5→**10** (новый блок E0.13: 16 пилюль, 4 пресета, все `role=switch`, `.mh-mini` по источнику, `.mh-compare`), `risk-info-honesty-e07-e08` 16→**21** (E0.9: счётчики, MODE_DEFS=16, нет «5 в 1»/v2/прозвища). **Мутационно проверено:** возврат `minHeight:40` роняет 2 теста. Круги: Shared/Support/Calculator UI **373/373** (37 файлов), risk 100/100, engines 447/447, `tsc` чист по своим, `verify:apk-design` OK.
- **Урок сессии (повтор, дорогой):** правка через PowerShell-команду с кириллицей **портит файл** — подпись «5 в 1» превратилась в «16 <нечитаемых символов U+FFFD>» (2 штуки в `PlannerToolsPanel`), и починилось только ASCII-командой с заменой по `\uFFFD`. Контент — ТОЛЬКО Edit/Write; сканеры/замеры — скриптом в temp, и **после любой PowerShell-правки обязательно** проверять `\uFFFD` по своим файлам (в этот раз так найдено и вылечено 2 FFFD, оставшихся от прошлого раунда в `metabolic-hub.engine.ts`). **Следствие:** и в AGENTS нельзя цитировать «испорченный» текст буквально — литерал U+FFFD в доке ломает source-guard'ы и закрепляет мусор в истории.
- **Остаток волны 0:** E0.10/E0.11 — **только** заблокированы чужим WIP в `TrainingScreen.tsx` (`load_safety`, `rir_forecast_hub`, `CALC_TABS`). Всё остальное закрыто; дальше — Wave 1 «научная честность» (E1.x).

## Калькуляторные хабы: волна 0 «перестать врать» — E0.2/E0.7/E0.8/E0.12 закрыты (Sep 26 2026, без коммита)

Продолжение `docs/CALCULATOR-HUBS-PROFESSIONAL-AUDIT-PLAN.md`. Только Edit/Write + vitest/tsc; чужие WIP (кардио/combat) не тронуты; **НЕ коммитил**.
- **E0.2 — `NUTRIENT_UL` выведен из реестра, и тут ДВЕ ловушки, обе пойманы своими же тестами.** (1) **Ключевое пространство:** потребитель `checkUpperLimits` (`engine-helpers.ts:79`) ищет предел по **id вещества** (`vitamin_d3`), а реестр оперирует **нутриентами** (`vitamin_d`) — наивное `Object.fromEntries(Object.values(NUTRIENT_LIMITS_V2)…)` отдало `undefined` и **молча отключило UL-кап** (упало 2 теста «Vitamin D3 dose within UL»). Поэтому ключевой список объявлен явно (это контракт поиска), значения берутся через `resolveNutrient`. (2) **Мост единиц:** реестр хранит витамин D как 4000 **МЕ**, а движок работает в **мкг** — копирование числа давало предел 4000 вместо 100 мкг, т.е. UL-кап ослабался в **40 раз** (тест «200 кг ограничена UL»: 110 мкг вместо ≤100). Лечится `altValue` реестра (100 мкг = тот же предел). Обе ошибки были бы тихими — числовой «защитный» код просто перестал бы защищать.
- **E0.7 — справка «Как считается риск» врала в 6 местах, фантом был продублирован.** Убраны: Σ по механизмам (реально **RSS** `√Σp²`), «среднее геометрическое по системам» (реально **арифметическое**, `reduce/n`), выдуманный `WeightedRisk` (нет в коде), выдуманные веса источников «Фарма 40/Анализы 25/Тренировки 20/Питание 15» (в коде НОЛЬ таких весов), фантомные «Fuzzy Logic» синергии с порогами систем («+10/+8/+12/+15%») — реально `SYNERGY_PAIRS` это пары **механизмов** (cv2+cv4, cv3+cv1, hem1+cv4, liv1+liv2, ren1+ren3, s=0.2–0.25), «штраф ×1.5 при >12 нед» — реально `1 + 0.15×(н−4)` с потолком **×3** (одинаково в `mdss-engine.ts:254` и `compliance-engine.ts:100`), и V7 «используется МАКСИМУМ (не среднее!)» — реально `geomMean(allRaw)` (`risk-engine-v7-matrix.ts:1143`). **Почему это важно:** пользователю показывали несуществующие формулы как авторитетные — это хуже отсутствия формулы.
- **E0.8 — надбавка неопределённости была невидимым числом.** `U_i = 1 + 0.25·(1−d_cov)` реально применялась к индексу, но показывалась только как «×1.22». Теперь рядом с «не верифицировано» — `индекс +X%`, а чип стал `U: ×1.22 (+22% неопределённости)`. **Важно:** `verification` (доля систем с релевантными маркерами) и `U_i` (насколько раздут индекс) — РАЗНЫЕ величины; смешивать = новая ложь, поэтому в коде это зафиксировано комментарием и ассертом.
- **E0.12 — 3 write-only ключа, и у них был реальный побочный эффект.** В `SupportFavoritesView` удалены два дубля срезов одного массива отчётов, причём с **разными кэшами (20 против 10 записей)** — три источника тихо разъезжались; в `DosageCalculatorTab` удалён `he_pharma_course_pending` (реальные каналы — IndexedDB `db.put('course_log')` + буфер обмена, остались). **Почему это не косметика:** `cloud-kv` перехватывает `setItem`, то есть каждая мёртвая запись **улетала в облако целиком**, съедая квоту и трафик ради недостижимых данных.
- **ДВА ЛОЖНЫХ СРАБАТЫВАНИЯ ПЛАНА (проверено чтением, кода не менял).** (1) E0.6: `same` реально существует в `supplements.ts:50`, `SUB_ALIAS` корректен. (2) E0.12: `checkGuardrail` **не мёртвый** — вызывается через `screenGuardrails` из `tz-mapper-engine.ts:1901` и РЕАЛЬНО вырезает вещества с `level='block'`. Оба раза я сначала поверил плану; вывод: **«write-only/мёртвый» проверяется сканером по потребителям, а не чтением плана** (сканер лежит в temp, 69 write-only ключей всего, из них в зоне поддержки/риска/фармы/лаб — 0).
- **Тесты:** NEW `risk-info-honesty-e07-e08` 16 (source-guard: запрещённые строки + обязательные реальные факты; **мутационно проверен** — возврат фантомного `overallRaw = max(...)` роняет тест), `support-limits-registry` 45 (+6 на E0.2), `support-calc-audit` 42 (2 регрессии пойманы и закрыты). Круги: Support/Calculator/Pharma UI **252/252**, risk **100/100** (12 файлов), engines (support-calc/tz-spec/compliance/metabolic/ped/residual/hemato/female) **447/447**, `tsc --noEmit` чист по своим (остальное — чужой WIP кардио/combat), `verify:apk-design` OK.
- **Уроки (сессия):** (1) source-guard по «запрещённой строке» ловит **сам себя**, если честный комментарий цитирует удалённое — имена ключей в комментариях пришлось убрать; (2) шаблонная строка в TSX легко закрывается «на автомате» правкой — поймано трансформ-ошибкой 3 тестовых файлов; (3) мутационная проверка обязательна для guard-тестов, иначе они зелёные на своей же лжи.
- **Остаток волны 0:** E0.9 (тексты «17→16», v3/v2, «5 в 1»→«16», Biggest Loser) и E0.13 (6 контролов <44px, inline — `styles-native.css` чужой append-only WIP). **E0.10/E0.11 заблокированы** чужим WIP в `TrainingScreen.tsx` (`load_safety`, `rir_forecast_hub`, `CALC_TABS`).

## Даты, раунд 3: контур дневников целиком — 22 места, потолок ценза 208 → 186 (Sep 26 2026, 1 коммит pathspec `65fed720a`, без пуша)

Продолжение раунда «два батча по канону». Теперь взят **целый UI-контур** (3 файла дневников тренировок), а не отдельные движки. Только Edit/Write + скрипт-патч (UTF-8 без BOM) + vitest/tsc; чужие WIP не тронуты; коммит строго pathspec; **НЕ пушил**.
- **Что переведено (22 места)**: `TrainingDiaryHub` 10 (ключ напоминания `he_diary_reminder` — чтение и запись; «сегодня» в шести карточках; гейт `trainedToday` дня отдыха; `hasTrainingToday` для миксов), `DiaryToolsView` 8 (7 имён экспортируемых CSV/JSON + окно ПР за 30 дней), `DiaryHistoryView` 4 (неделя назад, границы недельных сумм, «сегодня» для последней тренировки группы).
- **Суть дефекта (writer/reader раскол)**: записи тренировок пишутся по локальному канону (`workout-logger` → `core/local-date`), а эти экраны читали «сегодня» по **UTC**. В вечерние часы при смещении UTC+3…+12 ключи расходились на сутки: подсказка «сегодня нет тренировки — день отдыха» появлялась в день, когда тренировка уже записана; ключ напоминания писался на вчерашний день; в имя выгружаемого файла попадала вчерашняя дата.
- **Третья по счёту пойманная вакуумная версия лока — и это главная находка раунда.** Первая идея (замок на подсказку «тренировка была — добавь запись») оказалась **недостижимой в принципе**: `MixDiarySection` грузит записи в `useEffect` и делает `if (records.length === 0) return null`, поэтому в статическом рендере секция всегда пуста. Вторая версия (закреплённые часы + `vi.useFakeTimers()`) проходила **вхолостую**: полный фейк таймеров обрезает дерево `renderToStaticMarkup` (23 200 против 25 200 символов), и любые проверки «этого текста нет» зеленеют при сломанной логике. Обе границы зафиксированы комментарием в тесте.
- **Рабочий лок (проверен мутацией)**: имя выгружаемого CSV при часах `2026-09-26T20:30:00Z` (UTC = 26-е, календарь = 27-е) — время закрепляется **точечным фейком только `Date`**, планировщик React остаётся настоящим. Возврат UTC в строке имени роняет 2 теста: `diary_export_2026-09-26.csv` вместо `..._2026-09-27.csv`.
- **Честно про силу остальных локов**: два теста на гейт «тренировка сегодня» (подсказка скрыта/видна) **не ловят возврат UTC в дневное время** — локальная и UTC-даты совпадают, это ровно тот же класс ловушки, что и в батче календаря. Их ценность в другом: держат сам гейт; граница записана в тесте. Source-guard (0 date-only + импорт канона в трёх файлах) ловит регресс всегда.
- **Проверено**: UI-круг дневников **225/225 (17 файлов)**, включая живой монтаж хаба в `rest-hooks-native` (предсуществующий шум chart.js и unhandled «DB not init» — не мой); `date-canon-census` + `report-date-canon` + `training-calendar-local-dates` + новый лок — **19/19**; `tsc --noEmit` **0 ошибок по всему проекту**; кодировка без BOM/U+FFFD; `git diff --check` чист.
- **Остаток долга: 186 мест** (было 202 в чистых + 6 в грязных). Топ: `WLDiagnosticsHub` 9, `RiskScreen` 7, `StrongmanDiagnosticsHub` 7, `LabsScreen` 6, `ArmDiagnosticsHub` 6, `ArmliftingDiagnosticsHub` 6, `periodization-designer.engine` 5, `nutrition-ocr-parser` 4. Топ-6 — снова связные контуры (хабы диагностики, labs, risk), т.е. следующий шаг — снова «контур целиком», а не одиночные файлы.

## Даты: два батча по канону — календарь+трекер (writer/reader) и 5 отчётных движков; долг 250 → 208 (Sep 26 2026, 2 коммита pathspec, без пуша)

Продолжение раунда «хвостов» по списку зон, который печатает `date-canon-census`. Батчи брались не по числу мест, а по риску: где **писатель и читатель держат одни и те же дневные ключи** — там неверная дата = потерянная запись. Только Edit/Write + vitest/tsc; чужие WIP не тронуты; **НЕ пушил**.
- **Батч 1 (календарь + трекер питания, 14 мест).** `training-calendar.engine` (9: «сегодня» в трёх календарях, ключи дней диапазона, окна недели/месяца воды) и `nutrition-tracker.engine` (5: ключ записи еды/воды, «сегодня/вчера», серия дней — там стояло `setDate()` локально, а читалось `toISOString()` по UTC, двойная ошибка в одной строке). Окна переведены на календарный `localIsoDateOffset(-7/-14/-30)` вместо `Date.now() − 7×24ч` (последнее едет ещё и по DST). **Главная находка:** чинить ТОЛЬКО читателя было бы НОВЫМ багом — тест на воду показал, что писатель остался на UTC, и запись в 23:30 ушла бы под «вчерашний» ключ, а «сегодня» показывало бы 0 мл. Writer и reader переведены в одной правке, контракт зафиксирован в шапке трекера.
- **Батч 2 (5 отчётных движков, 25 мест).** `log-analytics-progression` (ключи недель, диапазон недели, дата цели прогноза), `comprehensive-report` (`todayIso`, `isoDate`, окна 7/30 дней), `lab-diary` (дата анализа + 3 окна «за N дней»), `symptom-adherence` (`dateStarted`/`dateEnded`, «сегодня» ×3, окно 7 дней), `training-recommendations` (диапазоны недель). В последнем был **смешанный канон**: неделя последней тренировки считалась в UTC (`T00:00:00Z` + `getUTCDay/setUTCDate`), а ключи `w.date` писались по локальному — ровно тот же раскол writer/reader (в UTC−8 «понедельник недели» уезжал на воскресенье); переведено целиком на канон.
- **Честно про локи — обе версии сначала оказались ВАКУУМНЫМИ и пойманы мутацией:** (1) раскол writer/reader не ловился, пока момент времени не закреплён `setSystemTime` на 20:30Z — по UTC+10 днём локальная и UTC-даты совпадают, и первый вариант лока был зелёным при мутации; (2) лок на окно анализов был зелёным при возврате UTC, пока записи стояли заведомо вне окна — усилен до граничной пары (−30 внутри, −31 снаружи). Проверено: без мутации зелёное, с мутацией падает.
- **Потолок ценза понижен дважды: 250 → 233 → 208** (долг может только уменьшаться; зона стронгмена/ТА держится на нуле). Оставшиеся 208 — это в основном `src/ui/screens` (дневники, лабы, риск) и зоны под активным чужим WIP: править чужой незакоммиченный файл нельзя, список зон печатает тест.
- **Проверено**: `date-canon-census` + `report-date-canon` 9, `training-calendar-local-dates` 6, потребители движков **60/60** (8 файлов: comprehensive-report, lab-diary ×2, training-recommendations, measurement-adapter, integration, labs-diary-delete), `tsc` по всему проекту **0 ошибок**, кодировка без BOM, `diff --check` чист.
- **Границы**: `localIsoDateOffset` и `parseLocalIsoDate` покрыты юнитами в цензе; UI-экраны (дневники/лабы/риск) в этом раунде НЕ трогались — они в зонах чужих агентов; проверка «календарь ≠ UTC» остаётся условной по реальному смещению машины (смена TZ через `process.env` в vitest не применяется).

...
- Тесты: обновлён mutation-лок `arm-corrective-pro2` (было `cup_start.exercises не содержит wrist_curl_db` → теперь в базе; проверка «нет дублей в пуле») + NEW lock «пул каждой точки ≥6, все id из каталога». Прогон `src/engines/arm` **92 файла / 1121** + арм-UI **90/90** + `tsc` 0. НЕ ПУШИЛ. Следующие хабы: армлифтинг → ТА → стронг.

...
- **Тесты**: NEW `hub-diary` **6/6** (v2-форма, «пустой `[]` не блокирует v2» — корень бага, легаси-обёртка, плоские записи, битый JSON/мусор, пусто) + `bb-diagnostics-hub` 44/44 + `bb-corrective-*` 10 файлов 89/89 + пачка хаб-UI **74 файла / 597** + `src/engines/strength-sport`+`arm` **161 файл / 2154** + `tsc --noEmit` **0 по всему проекту**. Пробы удалены. НЕ ПУШИЛ.

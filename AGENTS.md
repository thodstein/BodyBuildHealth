...
## ББ-авто: Packing-v2 расширение на грудь/ягодицы — PPL-мандаты exempt, ноги исключены доказанно (Sep 08 2026, НЕ запушено — очередь чужих)

Расширение пилота (грудь/ягодицы в гейт; квадры/хамсы ИСКЛЮЧЕНЫ с доказательством): `mandated`-guard в `planPackingDrops` (PPL flat/incline/fly + колодец — финализатор докинул бы их с 4 сетами), `isPPL` через `BuildSessionParams`, minKeep 3 для квадр в PPL, `PACKING_MUSCLES` = back/chest/glutes. Доказательства дампами: quads angle-coverage докидывает жим ногами (+3 дрейф), hams-фидер докидывает RDL (+1 слот) — им нужен отдельный мэппинг гарантий. Грудь держится на всей матрице без единого падения. Тесты `bb-packing-v2` 20/20 (mandated + матрица 4 сплита×2 профиля×5 мышц с on≤off по ошибкам). Широкий круг **bb 2078/2078 (181 файл)**, tsc по своим 0. Коммит pathspec своих. НЕ ПУШИТЬ.

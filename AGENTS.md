...
- **Волна 4.1 (тот же раунд)**: единый источник ступеней `QUALITY_GRADE_THRESHOLDS` (85/65/45) — `gradeQualityScore` (V2/report) и `gradeFor` (недельная) читают канон (ярлыки поверхностей сохранены — обратная совместимость); NEW lock-тест `bb-quality-grade-scale` 4/4 + потребители (quality-score-v2/parity/weekly/v2/report/quality-actions) **84/84**; `tsc` 0. **4.3** (разрез `BbAutoConstructor.tsx` 8418 строк) и **4.5** (19 нативных select/checkbox того же файла → АПК-кит) отложены осознанно: файл правят параллельные агенты весь день, широкий UI-свип в шторме рискованнее пользы (`verify:apk-design` зелёный).

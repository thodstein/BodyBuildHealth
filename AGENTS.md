...
## Стронг/ТА: интернет-циклы R2 — остатки закрыты: adapt-паритет, weak/contest-селектор, гибрид, год из циклов, +2 шаблона (Sep 06 2026, pathspec свои, НЕ запушено — очередь чужих)

Закрыты все 8 пунктов честного аудита R1 (f1adc98df): adapt знал только ACWR/outside/VBT-скаляр, селектор не видел weak/контест, гибрида не было (план молча менял mode), годового моста из циклов не было, печать молчала про цикл, библиотеки не хватало (Hatch — только постранично + уже есть в lms, добавлен дословный 12-нед стронг по полным недельным раскладкам + гибрид). Strength-sport **671/671 (48 файлов)** + UI циклов 3/3, tsc 0, соседи TrainingScreen_parts 659/660 (1 падение — чужой предсуществующий MesocycleProgressionCard, замыкание только lms, моих файлов ноль).

- **Adapt-паритет**: per-lift VBT-history (пороги 10/15/20 как в билдере), diaryTrend (−5% → ×0.85, плато → +1 сет), HRV-вес (×0.85/×0.94), весогонка-протокол (валидация всегда, срезка объёма в adapt). Поймано тестом: срезка ×0.9 на 4-5 сетах математически ничего не режет (3.6→4) — тест переведён на верхний порог ×0.8, движок не подгонялся (паритет с билдером).
- **Селектор**: weakPoints-токены (+8/покрытие, кап 16) + contestEvents (+5/ивент, кап 15, +5 за тейпер-якорь); UI прокидывает weakPoints и ивенты контеста в ранжирование.
- **B**: NEW `ss-hb-mix-8` (гибрид 4×/нед, mode сохраняется) + честное предупреждение при гибриде с профильным циклом; NEW `strength-sport-ss-annual.engine` (`buildAnnualFromSSCycles` → `buildAnnualWithTaper`, дата старта на последний блок) + кнопка «📚 Год из топ-3 циклов» в annual-карточке (ядро annual не тронуто).
- **C**: NEW `ss-sm-base-12` (12-нед стронг 4×/нед волнами 3+1: нед.1-5 дословно по недельным раскладкам источника, нед.6-12 — по правилу самого источника +шаг/волну, помечено inferred; пойманы 2 мои опечатки `}),`/`});` — esbuild до тестов); Hatch осознанно НЕ дублирован (постраничных % нет + уже есть в lms-cycles/hatch). Печать: строка «Интернет-цикл» (id + дословно/adapt) + print/CSV-тесты (cycle-id, дистанции, AMRAP, шапка Дист).

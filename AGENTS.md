...
## ПЛАН — Калькулятор поддержки (актуальный, Aug 12 2026, pushed 786f7633)

Всё под риск-ориентированную (механизм) модель. Проверено скриптом: **все 396 веществ каталога имеют мех-записи** (SUPPLEMENTS_DB/PHARMACY_DB/TZ_MECH_TO_SUBS, 0 пропущено после алиасов). Механизмы теперь в шкале 0-100% (rawPercent/afterPercent, сумма = системе) — «148 баллов» устранено.

### P0 — риск-модель (выполняется)
- **P0-1: единая риск-модель между экранами.** Вкладка «Риски» — z-score (v7/aggregated/TZ), калькулятор — ТЗ-механизм-модель. Либо единый движок, либо явная маркировка «модель: механизм-индекс 0-100%» на обоих экранах. Внутри калькулятора источник уже единый: `tzFinalRisk` (calculateTzSpecRisk по финальному составу) → попапы/сейфти/мониторинг.
- **P0-2 (C3): убрать локальный hematoScore** в попапе «Кровь» (CalcMapper L~2121: счёт HCT/HGB/PLT/фибрин/D-димер) — перевести цвета/пресеты/статус на системный риск из механизм-модели (уже показан «⚖️ Риск системы»). НЕ убирать механизмы — убрать дублирующую формулу.
- **P0-4 (B2): hemato-разбиение (аудит выполнен).** Система `hematologic` = «Гематолого-МЕТАБОЛИЧЕСКИЙ блок» — СЛИТЫ 3 домена:
  - гематология: hem1 (эритроцитоз) — тромбоз/кровотечение ОТСУТСТВУЮТ как механизмы;
  - метаболизм: hem2 (ИР/глюкоза), hem3 (гипогликемия);
  - электролиты: hem4 (K⁺), hem5 (вода/отёки).
  Также в cardio: cv4 = тромбоз + гипервязкость/реология (слито).
  **ВАЖНО: НОВЫЕ МЕХАНИЗМЫ И НОВЫЕ ОРГАНЫ НЕ ДОБАВЛЯЕМ.** Работаем ТОЛЬКО с существующими 6 органами и 28 мех-кодами. Под-риски — UI-only: из механизмов существующего `hematologic` (hem1 → «эритроцитоз», hem2+hem3 → «метаболизм», hem4+hem5 → «электролиты») — уже показано в попапе «Кровь». Дальше: панель под-рисков в мониторинге/отчётах. Тромбоз остаётся в cardio cv4; кровотечение — вне SYSTEMS (supportRisks, уже есть). Остальные системы проверены — разделены хорошо: hepatic (liv1-3), renal (ren1-4), cns (cns1-6), reproductive (rep1-5), cardio (cv1-5, кроме cv4).
- **P0-5: нижние карточки после плана — детальная проработка.** Инвентаризация (13 блоков): план-чипы → UL-контроль → Образ жизни → Синергии+Риски поддержки → Нутри-корректировки → Ребаунд → Предупреждения курса → Мониторинг (витальные/график/маркеры/системные панели/инструментальный/дневник/экстренные) → CalcSafetyLayer (alerts/guardrails/contra/gaps/protocolWarnings/conflicts/procedures/assayWarnings/labFindings/pillBurden) → Бустеры → Применить → CalcActions → Дисклеймер. Задачи: свести дубли («Текущие показатели» в мониторинге vs labFindings в safety), единый порядок/стиль, пустые состояния, связность.

### P1
- **P1-1 (B4): паритет попапов** — system-contra, system-monitoring, preview «было→станет».
- **P1-2 (B5): LV4/«под контролем врача» визуально отдельно** (врачебная эскалация ≠ БАД).
- **P1-3 (B7): нормализация severity/дедуп safety-источников** (drug-interactions vs biostack-safety vs support-interactions-db).
- **P1-4 (B9): фарм-матрица 10 КЛАССОВ PED** (не 10 веществ): тестостерон, тренболон, нандролон, болденон/DHB, оралы 17α, GH, IGF, инсулин, кленбутерол, T3 → для каждого: фармкласс → механизмы → анализы → фаза → обязательная/условная поддержка → doctor-only → взаимодействия → assay-warnings.
- **P1-5 (C4): ASSAY_INTERFERENCE_DB / PREANALYTIC_EFFECTS_DB** (реально изменяет / искажает assay / гидратация / после нагрузки / сообщить врачу).

### P2
- Экспорт/отчёты с новыми блоками (риски поддержки, мониторинг, под-риски).
- Тексты/переносы во всех попапах; пустые состояния.

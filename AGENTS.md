...
- **Фаза 2 — дубли, безопасная часть (выполнена)**:
  ...
  - Отложено (по решению пользователя, отдельные шаги): один набор % прикидов (пересмотр цифр), слияние двух реестров соревнований (миграция данных), единый экспорт/печать и переключатель «Оригинальный/С тапером».

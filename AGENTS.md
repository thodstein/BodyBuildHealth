...
- **Проверено**: NEW `bb-params-single-source` 8/8 (SSR обоих режимов: каждая настройка ровно 1 раз; честные гейты) + NEW `bb-ped-methodology-program-overlay` 3/3 + `bb-plans-migration` 7/7 + весь `TrainingScreen_parts/__tests__` **145 файлов / 1330 тестов, 0 падений** (1 unhandled `revokeObjectURL` — чужой предсуществующий) + bb-ped-семья + `rest-hooks-native` **231/231** (unhandled `DB not init` — предсуществующий) + `bb-cycle*`/миграция **47/47** + selection/audit/strict/spec **113/113** + `tsc --noEmit` **0 по всему проекту** + `verify:apk-design` OK. **Шторм-инцидент**: параллельный агент откатил (`checkout`) часть worktree — `cycle-to-plan.ts` и хвост `bb-plans-migration.test.ts` восстановлены заново (перечитано перед edit, сверено `git diff`, повторный tsc 0). Коммит `cafb9384` (12 файлов): чужие ханки в моих файлах (`PRO-CORR correctiveDetail`, запись «ББ-коррективка») не swept — остались в worktree; мой AGENTS-слой застейджен через HEAD→tmp→hash-object.
- **Границы (не делали)**: `packingV2`/`cycleDay`/`targetBodyFat`/`recoveryMultOverride` в режиме источника остаются гейтами (движок program-пути их не считает — UI скрыт/disabled, без вранья); `pedPhaseOverride` в faithful не применяется (дословность).

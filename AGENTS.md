...
## Стронг/ТА: «хвосты» закрыты — год из пустоты, единый канон дат, чистый updater (Sep 26 2026, 3 коммита pathspec, без пуша)

По команде «выполняй полностью» закрыты все 4 оставшихся пункта (пустой год, централизация дат, browser E2E, молчащие правки). Только Edit/Write + vitest/tsc; чужие WIP (combat/bb-hybrid/интеллект-хаб/питание) не тронуты; **НЕ пушил**.
- **Год из пустоты.** Проверка «битая история → пустой список» показала: список планов пользователю не показывается (он только внутри annual), поэтому «пустой вариантов» — не дефект. Настоящий дефект на шаг глубже: `buildAnnualFromSS([])` даёт валидный AnnualSS с 0 блоков → он сохранялся, рассылал `he-strength-annual-updated`, а UI рисовал «Год: 0нед · 0 блоков», т.е. пустота выдавалась за собранный год (срабатывало на первом запуске и при битой истории). Фикс: пустая история → год не строится/не пишется/событие не уходит, а `annualNote` добавляется в итоговый тост (иначе провал потерялся бы под «✦ План собран» — ровно тот баг, что чинил кадром ранее). Честная граница: пустая история при УСПЕШНОЙ записи — не ошибка (план попадает в историю, год строится законно).
- **Единый канон дат.** Проверка вскрыла, что канона **не было**: `localIsoDate` жил в ТРЁХ независимых копиях (workout-logger / diary-shared / planner-date-utils) с разной сигнатурой. NEW `src/core/local-date.ts` (`localIsoDate`, `localIsoDateOffset`, `parseLocalIsoDate`, `shiftIsoDate`); три копии делегируют ему, сигнатуры сохранены. NEW `date-canon-census.test.ts` — витрина долга: печатает инвентаризацию по зонам, ставит **потолок 249** (долг только уменьшается) и держит зону стронгмена/ТА на нуле. Смысловая граница в шапке канона: календарная дата — только отсюда, СОБЫТИЕ во времени (`updatedAt/createdAt/builtAt`) — UTC корректен.
- **Молчащие правки → оказались не про сообщение, а про архитектуру.** `saveStrengthSportPlan(copy)` вызывался **внутри updater-функции** `setPlan(prev => …)` (3 места) — side-effect в чистой функции: StrictMode зовёт updater дважды → двойная запись и дублевое событие для хабов; там же внутри стояли два `setMsg`. Фикс: персист вынесен в отдельный эффект по флагу `planEditDirty` (состояние осталось функциональным — быстрые правки подряд не теряются), отказ хранилища даёт «⚠ Правка не сохранена», валидация «Вес 0–500 / RIR 0–5» вынесена ДО `setPlan`.
- **Честно про покрытие:** поведенческие локи (правка пишется / отказ честен) мутацию «вернуть запись в updater» **не поймали** (эффект всё равно пишет). Добавлен source-guard — проверено мутацией, что он роняется на `#4: saveStrengthSportPlan(`.
- **Тесты/проверки:** `ss-constructor-save-honesty` 5→**9** (год 3 + правки 3 + гард 1 + правка проверки видимости→факта записи), `date-canon-census` **7** (по ходу поймал ТРИ мои ошибки: проверка видимости вместо факта, выдуманный потолок 100 вместо измеренных 249, опечатка `now`/`сейчас`; плюс выяснилось, что смена TZ через `process.env` в vitest НЕ применяется — проверка «календарь ≠ UTC» сделана условной по реальному смещению машины), потребители канона **55/55**, зона UI **91/91** (14 файлов), `tsc` 0 по своим (единственная ошибка проекта — чужой `combat-builder.engine.ts`), `verify:apk-design` OK, `diff --check` чист.
- **Осознанные границы этого раунда:** (1) **249 мест долга по датам НЕ тронуты** — 134 в `src/ui/screens` и остальные лежат в зонах с активным чужим WIP (интеллект-хаб, combat, bb-hybrid, питание); правка чужого незакоммиченного файла — запрещённый шторм. Список зон печатает тест, следующий раунд чинит по нему. (2) **browser E2E физически нечем гонять** — в проекте нет ни Playwright, ни Cypress, ни Puppeteer (devDeps пусты, в scripts только `verify:apk-design`/`verify:apk-ci`); покрытие реального поведения даёт jsdom-монтаж компонента с кликом по «Собрать» (это и есть constructor-путь), плюс маркерный чекер APK. (3) Инъекции/откаты (спец-блок, волна СМ) уже были честными — их не трогал.

## Стронг/ТА: аудит-раунд — 4 реальных дефекта закрыты (Sep 26 2026, 3 коммита pathspec, без пуша)

По команде «выполняй полностью» закрыт аудит планировщика стронгмена/ТА (Э1–Э5). Статусы «хвостов 0» в PRO-документах были неполными — это и нашлось. Только Edit/Write + vitest/tsc; чужие WIP (cardio/Articles/planner-bridge) не тронуты; коммиты строго pathspec; **НЕ пушил**.
- **Э1 P0 — гейт болгарского daily-max обходился.** `bulgarianGate` жил только в раннере, `buildSSCyclePlan` его НЕ звал: новичку / 50+ / без `cycleConsent` / при ACWR caution-dangerous 6-дневный протокол максимумов собирался молча, а rationale утверждал «согласие получено» (прямая ложь). Обход #2 — годовой план (`buildAnnualFromSSCycles`), обход #3 — сохранённый `cycleId` (вариант/мост), где раннер не участвует. Фикс: гейт **fail-closed в билдере** (единая точка для всех трёх путей; раннер остался UX-фильтром), `cycleConsent` проброшен в annual base, ошибки build/annual показываются тостом. Локи: `ss-bulgarian-gate` 11 (уровень/возраст/согласие/ACWR/annual) + **паритет «раннер ранкерит ⇔ билдер блочит»** по 11 комбинациям.
- **Э2 P1 — DUP `event_day` для стронгмена был мёртв:** `applyDUP` определяет ветку по `plan.mode`, но `mode` не передавался ни в билдер, ни в cycle-путь. Тесты были **вакуумными** (без `dupMode` во входе + `if`-обёртка + нестрогие ассерты) — зелёные при нерабочей фиче. Прокинул в оба пути, тесты переписаны; **мутационно**: снос `mode` роняет 2 лока.
- **Э3 P1 — симулятор контеста считал по чужим показателям:** кросс-ивентные фолбэки (йок←становя, камень←гантели, царапина←йок) + выдуманные дефолты 180/140/100, хотя `StrengthSportWorkMax` прямо объявляет «отдельный ввод, не фолбэк через deadlift». Фикс: event-specific карта ПМ; тяговые (вес снаряда ≠ нагрузка атлета) не оцениваются; medley по слабейшему элементу; `Math.random()` из чистой функции удалён; нет данных → `noData` + честная строка, а не число из воздуха. `contest-sim-honesty` 9.
- **Э4 P1/P2 — честность хранения/экспорта/дат:** storage больше не молчит при потере записи (warn на всех путях + cloud); `saveStrengthSportPlan` возвращает **boolean**, событие `he-strength-sport-plan-saved` уходит **только при реальной записи** (было: безусловно → хабы перечитывали старое значение), а 4 «✓ встроено/откачено» в конструкторе заменены на честную ветку «⚠ НЕ сохранён»; CSV-экспорты ТА/стронг защищены от формул (`= + - @` → префикс `'`, вектор — пользовательский текст в findings/causes); **даты**: 3 движка (проверка, отчёты ТА/стронг) + 4 date-only места в `StrengthSportConstructor` переведены на канон `localIsoDate` (UTC → в UTC+ «вчера»). Инвентаризация: в движках остальные `toISOString()` — полные таймстемпы (`updatedAt/createdAt/builtAt/savedAt`), где UTC корректен. `ss-export-honesty` 8 + 2 storage-лока.
- **Э5 — тесты/доки:** 3 новых файла (28 тестов) + расширены 3 существующих. Мутационные проверки: UTC-дата (`expected '2026-09-26' to be '2026-09-27'`, TZ=UTC+10 + фикс времени 23:30 UTC — не зависит от времени суток прогона) и безусловное событие (`expected 1 to be +0`). Два документа помечены **историческими** — их «хвостов 0» этих дефектов не покрывали.
- **Проверено**: движки стронгмена/ТА **1044/1044** (68 файлов), UI зоны **80/80** (12), общая матрица `arm-ss-cycle-matrix` 2/2; `tsc` по своим файлам чист (единственные ошибки проекта — в `CardioWeekEditor.tsx`, чужая зона); кодировка без BOM; `git diff --check` чист. **Круги шли по частям** (`--minWorkers=1 --maxWorkers=2`): машина под нагрузкой параллельных агентов, свободно 3.1 ГБ из 7.9 — общий прогон падал по OOM в воркерах, не по коду.
- **Осознанные границы**: `toISOString().slice(0,10)` вне зоны стронгмена (другие блоки, десятки мест) — общая централизация дат, отдельный раунд; browser E2E конструктора не гонял (только jsdom-тесты); правки сетов внутри `setPlan`-updater не трогал (сообщения нет, событие уже не врёт).

## Питание: волны 0–5 закрыты + база белка 1.6–2.2 г/кг с ручным режимом (Sep 26 2026, pathspec, без пуша)

По команде «доделай волны 0–5, потом белок» закрыты все технические волны планировщика рациона и применено решение пользователя по белку. Только Edit/Write + vitest/tsc; чужие WIP (arm/LMS/cardio/SSC/manual/PL-конкуренция/ББ) не тронуты; коммит строго pathspec; НЕ пушил.
- **Волна 0 (UnifiedSettings)**: состояние/поля настроек переведены на `he_profile_v2` (nutrition/training/goals/personal), legacy-ключи только на чтение как фолбэк; `switchKbjuMode('manual')` ставит `manualTargetsLocked` и НЕ обнуляет прочие поля (регрессия сброса режима закрыта).
- **Волна 1 (ограничения)**: канон `resolveAllExcludedFoodIds` + `matchesSelectedAllergen` (теги + текстовый фолбэк) вместо сравнения RU-id с латинскими тегами; лактоза/молочные белки отделены от «не переносимости» и от аллергии; аллергены/исключения участвуют в отчётах, а не только в подборе.
- **Волна 2 (week-edit)**: `DerivedSyncPlans.weekEditDay?` (optional) + `plansSignature` с provenance рецептов и `portionScale` → правка недели корректно пересобирает месяц/закупки/готовку; `usePlannerDerivedSync` реагирует на `mealPrepDays`.
- **Волна 3 (импорт/план/спецконфиг)**: импорт плана валидирует форму и цели; keto и carb-cap получили честные границы и breakdown; отчёты/рекомендации переживают битые/пустые входы; TDEE и микроцели считаются по профилю с фолбэком.
- **Волна 4 (квота/рецепты/стабильность)**: `matchesCategoryPref` + `RecipeMatchOptions.categoryPref` + `recipeViolatesHardRestrictions` — исключённые категории учитываются в `assembleRecipeDay`, `refreshRecipeSuggestions`, `MealComposer` и spec-меals; `isTrainingDayFor` и downstream-гейты учитывают `linkToTraining` и выключенное расписание; очистка provenance второго рецепта + безопасные guards отчётов.
- **Волна 5 (пикер рецептов — «показано = вставляется»)**: NEW `filterRecipesByHardRestrictions` в `recipe-engine.ts` как единый канон (excluded IDs, allergen tags, имена рецептов, категории, `isVegetarian`); `MealComposer` больше не откатывается к нефильтрованному пулу в catch; `IndividualPlanResults` фильтрует picker тем же гейтом; регресс в `planner-restriction-gates.test.ts`.
- **Белок 1.6–2.2 г/кг (решение пользователя)**: `PROTEIN_PRESETS` → 3 уровня (1.6/2.0/2.2), NEW единый источник `PROTEIN_G_PER_KG_RANGE`; авто-ветка `buildDayTargets` жёстко клампит пресет в 1.6–2.2 (legacy 2.6 из старых сохранений пробить потолок не может), `manual`/`profile` возвращаются раньше клампа → ручной режим не ограничен; legacy `proteinPreset/nutrLevel: 'max'` нормализуется в `'enhanced'`; быстрый пресет «🚀 Максимум» ставит `'enhanced'`; тексты «2.5 г/кг» в быстрых пресетах убраны; UI честно пишет про ручной режим как способ выйти за 2.2.
- **Проверено**: область IndividualPlan **111/111 файлов, 1096/1096 тестов passed** (`--pool=forks`); `npx tsc --noEmit` (12GB) **EXIT=0**; `git diff --check` чисто. Полный vitest всего проекта после белковой правки не гонялся (tsc по всему проекту чистый).
- **Осознанные границы**: ссылки на литературу (Jäger 2017 / Morton 2018 / Mountjoy-IOC 2023) НЕ перепроверены — `websearch` отдавал 403, диапазон взят по этим источникам из памяти; готовые/сохранённые планы не пересчитываются задним числом (кламп действует на этапе сборки целей); в ББ-авто и `OrganLoadCalculator` остались старые ориентиры 2.4–2.6 г/кг — чужая зона, не трогалась.
...
- **A-2 `2dd64d27`**: rest 1500У/500Б БЕЗ инсулина 90.7% → **101.6%** (train 99.1%). Корни: добор §7.2-7c заливал «пряники 275 г», а финальный §3D срезал их до 50 (−112 г У); низкоплотные носители не брали цель мейна. Фикс: профиль ёмкости (900/800) и СОБСТВЕННЫЙ кап носителя (comfort/концентрат/EDIBILITY/сухая крупа 240) в доборе; при отсутствии растущих носителей — ВТОРОЙ плотный стейпл в мейн с комнатой; единый кап сухой крупы у билдера/§3D/P5b. Инсулин-дни байт-в-байт (1494.9/1511.4). Тест-матрица train/rest × инсулин/без + контроль 2692/173.5/74.4/332.1.
- **E-carb-room `b905c0a6`**: `carbRoomScore` (65% макро-дистанция + 25% carb-fill + 10% штраф перебора Б ядра; только экстрим-полоса) + честная строка «📐 эффективная цель Б = N г (ядра рецептов)» + `effectiveProteinTargetG`/`deviationToEffectivePct`. R-HV: dev 18.4% к цели пользователя при **8.8% к эффективной цели** (332 г). Обычный рецептурный день строки не пишет.
- **G `38bd7a22`**: ротация носителей окон ДЕНЬ-СКОПОМ (8 плотных: крем/хлопья/рис-хлопья/рис-крем/манка/рис-мука/рисовые хлебцы/кукурузная мука — вместо кольца из 3 на каждый болюс) + чек-инвариант `planner-extreme-diversity` 4/4: ≥6 У-носителей в регулярных приёмах (7–9), цепочка окна без повторов, ≥3 носителя окон, белок ≤3 приёма на id, ≥4 источника белка.
- **I `63550d54`**: `closeExtremeMicroGaps` — ПОЗДНИЙ проход на 8000+ (ранняя вставка съедала 36 г У: train 99%→96.4%): морковь 80 г (VitA +668 мкг) + семечки ≤25 г (VitE), бюджет ≤2% ккал цели, рост существующего источника вместо дубля, граммы по комнате тарелки (≤900); ноты покрытия VitA/VitE пересчитаны по факту; при блокировке источников — честный хинт. **VitA 26–32% → 80–100%, VitE 49–61% → 102–109%** (4 клетки + рецепты), рост +1.8%; рецептурный путь: снятие микро-добавки со входа + срез гибких жировых сайдов (Ж 145/120=+21% → 130.5/120, dev-ось перестала быть жиром). VitD ÷40 не тронут (lock-тест).
- **Проверено**: IndividualPlan **109 файлов / 1038 тестов — 0 падений**; `tsc --noEmit` **0 по всему проекту** (12GB); полный прогон **15048 passed / 6 failed / 20 skipped (1182 файла)** — все 6 чужие/предсуществующие: `course-sync` ×4, `bb-macrocycle` v7, `annual-audit-fixes-2026-08` (2 МБ). **НЕ ПУШИЛ.**
- **Осознанные границы** (в §7 плана): VitD (0%)/VitB9 (1%) тарелки на 8000+ (VitD — модуль добавок; single-slot активно-дефицит тратится на VitD-источник, который уже в дне); рецептурный dev-пол ядер 18.4%; ккал-перебор no-insulin rest-дня +3.7% ради У 101.6% (подписан «Точностью»). Промт следующего раунда — §8 плана.

## Питание: экстремальные КБЖУ (1500У/500Б) — волны A–J (Sep 24 2026, 5 коммитов pathspec, без пуша)

По команде «выполняй полностью» закрыт `docs/NUTRITION-EXTREME-SCALE-PRO-PLAN.md` (эпики A–D, E-anti-thrash, F, H, J; остатки A-2/E-carb-room/G-полный/I — готовый промт в §8 дока). Только Edit/Write + vitest/tsc; чужие WIP не тронуты (в `bb-finalize` были чужие tsc-ошибки — владелец починил, финал 0).
- **A `86b3a03e`**: `extremeCapacityProfile` — тарелка 700→805/910 (потолок 900), сухая крупа 170→200/240, варёный крахмал 250→×1.3, EDIBILITY ×1.15/×1.3 (только У-носители, мясо/яйца не растут), recipePlate 730→850/900; гейт инсулин/≥8 г/кг/≥1200 г/≥7000 ккал; обычные дни и HV 600–1000 г — прежние капы (контроль 3000 ккал байт-в-байт 2692/174/74/332).
- **B `bb4574d6`**: таймлайн приёмов — плотные разрывы ≥90 мин + вынужденная точка «максимум дистанции» + сдвинутый fallback; timeDup 0 (было 3 приёма в 15:45).
- **C `543b6282`**: цепочка инсулин-окон (до 3×120 г на болюс; 40 ЕД → 358/400 ≈90%, было 30%), ротация носителей (крем/хлопья), под-кормления чисто-У, advisory split/пролонг ≥25 ЕД.
- **D `01935015`**: `enforceExtremeCaps` после всех поздних проходов (перенос → срез только при переборе дня; гигиена без дублей id/вторых гарниров/моно-носителя).
- **E/F/H/J `743194ba`**: anti-thrash ребаланса (recipe-dev R-HV 26.7→17.4%); честность — «Точность рациона» перенесена в финал и к цели пользователя (+строка клинической цели), VitD IU→мкг (дрейф 87/500), Mg-UL добавок не флагует еду, MPS-снек по типу дня, «сумма болюсов N ЕД (M инжект.)»; вода/Na/K на ≥4200 ккал; «Сводка MPS» по фактическим приёмам (≥25 г).
- **Проверено**: IndividualPlan **104 файла / 1010 тестов — 0 падений**; `tsc --noEmit` **0 по всему проекту**; финальные дампы: products 1500/500/3×40 ЕД — kcal 8876, Б 485, У 1511, maxPlate 616, timeDup 0, покрытие болюсов 358/400. НЕ ПУШИЛ.
- **Остатки (промт §8)**: no-insulin экстрим rest У 90.7% (нужен A-2), recipe-dev ≤12% (E carb-room), G-полный чек, I микро-плотные на 8000+.

## ББ-авто: единый мастер-план (все прежние BB-auto планы удалены) + волны P0/P1/P2 выполнены (Sep 23 2026, без коммита)

По команде «сделай полный план, сохрани, удали все остальные и начинай полное выполнение»: NEW **`docs/BB-AUTO-MASTER-PLAN.md`** — единственный источник правды. Удалены 19 прежних BB-auto доков (BB-AUTO-* / BB-TAPER-PRO* / BB-FEMALE-POSTERIOR / BB-SPECIALIZATION-MULTIBLOCK / BB-CONTEST-PREP-PLAN / BB-PREP-PRO / FEMALE-CONTEXT-BB-PL-AUTO); открытые пункты сведены в §5–§7 мастер-плана; доки других подсистем (BB-DIAGNOSTICS-*, BB-MOVEMENT-*, BB-CORRECTIVE-*) не тронуты. Только Edit/Write + vitest/tsc; чужие WIP не тронуты; **математика капов/MRV/фаз не менялась** (кроме явно согласованных правок ниже).

- **Волна 1 (P0, мосты/«показано = применяется»)**: `BbAutoConstructor` читает pending `he_planner_apply` при монтировании (≤5 мин, kinds program/weakpoints/pm, не `pl-auto`) и чистит после; program-ветка конвертирует `SRCycleTemplate → FullProgram` (`cycleTemplateToFullProgram` → `applyProgramToBb`) — «→ ББ-авто» из библиотеки реально собирается, форма `{program}` (CardioManageStep) больше не роняет обработчик, `setCustomProgram(null)`-разрыв убран; редактор интенсив-техник пишет канон в `workSets[last].technique` (`canonTechniqueId`: myo_rep→myo_reps, 21s→twenty_ones) — выбор впервые доезжает до плана/SessionPlayer/PDF, мёртвый `intensityTechUsed` удалён; `onGoalChange` годового планировщика — честный флеш вместо no-op; hardcap «10 упражнений» заменён `sessionLimitsFor(...).maxExercises` (L/R-добивка и лаб-предпочтения больше не глохнут у advanced/enhanced); `freqOptResult` впервые рендерится карточкой `data-bb="freq-opt"` (справочно); 6 тихих catch ключевых путей → `console.warn` (PED-адаптация/overlay, автоправка баланса, ACWR/per-muscle/validator дневника, prep-sync).
- **Волна 2 (P1, движок и честность UI)**: `bb-insulin-window` подключён в builder после финализации (GH≥2+инсулин≥5: пометки памп/лёгких дней + rationale, объём не меняется — движок перестал быть мёртвым); `suggestFeeders` уважает оборудование (варианты dumbbell/barbell/cable/band/machine/bodyweight; `[]` — прежнее поведение); cross-meso `peakWeights` берутся ТОЛЬКО из peak-недели (закрыт P0-C2: прогрессия не растёт от делода/пика); `lengthened_partials`/`rest_pause_cluster` реализованы в `applyIntensityTechniqueToExercise`; UI-правда: лимиты сессии в «Проверке плана» с полными входами (стаж/курс/объём), текст «60/18»→«40/15 · 44/16», VBT-ввод едет в generic-сборку (`input.vbt`; в источнике — честная подпись «справочно»), мост в питание считает ккал/белок от веса/пола/цели (+`weightKg/sex/goal` в payload/ноте), `readiness` — реальный recovery (не константа 65), packing в источнике — плашка «включено, но не применяется», FST-7 гейт выровнен с движком (`jointGuardActive`, включая GH≥2+AAS≥500 и lab<0.65) с хинтом «движок вернёт standard»; `legs:140`/`core:80` в `DEFAULT_WORKMAX` (легаси-композиты WEAK_GROUPS без warning/фолбэка 80).
- **Волна 3 (P2, гигиена)**: удалены проверенно мёртвые `computeOverloadTargets`(+тип), `ensurePPLMidDeltFinisher`/`ensurePPLRearDeltFinisher` (заменены семейством `ensurePPL*` в `975772463`), `cnsFor`, `explainBBSelection`/`explainBBMetrics`, `isRepSchemeId`, `BB_RED_FLAGS_EMPTY`, `isAcceptableBB`, `hasBothPhases`, `idealReevesMap`, `SUBGROUP_LABEL_RU`, `BB_PREP_CONFIG_KEY`/`BB_PREP_PLAN_KEY`, `BB_CAUSE_RU`, `indirectArmVolume`, `TECHNIQUE_ALIAS_TO_ENGINE`/`ENGINE_ALIAS_TO_UI`, `splitForDays`, `jointGuardTempoOverride`/`jointGuardRationale`, `SPECIALIZATION_MAX_BLOCK_WEEKS`; в UI снесены 14 мёртвых «открытостей»/`expandedMuscles`/`strategySafe`/`catEx`/`distText`/`past`/`disabled=false`; своп-модалка ББ теперь строго внутри жёсткой группы (`strictGroupMembersOf` + подпись, фолбэк по group вне группы); тавтология `e.sets<=e.sets` заменена сравнением с исходным планом; тест идемпотентности `finalizeBBPlan` усилен сравнением sets/RIR/workSets.length; убран дубль-контрол «Цель объёма», «Дней/нед» 2–7 (паритет с клампом). Осознанно: допуск ±16 blast/cruise оставлен (root-cause в сессионных капах — фикс = отдельная кампания §7), `bbProgramPath` (roundtrip-маркер вариантов) и реестр `zoneSpec/donorsForZone` (API-запас) — не тронуты в волне-3 (решение принято волной-4, см. ниже).
- **Согласование (не делалось)**: §7 мастер-плана — подключение `bb-demographics` (masters/female), «эталонных слоёв» `computeLoading`/`selectDiverseExercises`, `jointGuardTempoOverride`; §5 — target-volume↔feeder allocation-pass, PDF/CSV-колонки «Подмышка/Паттерн/Пояснение».
- **Проверено**: `src/engines/bb` — **253 файла / 2826 passed / 0 failed** (20 skipped; после всех удалений), bb-UI **26 файлов / 213 passed** (было 24/200: +`bb-wave-p0-fixes` 10, +`bb-ex-swap-strict` 3), bridge-handlers/manual/editor/bridge-cycle **51/51**, `tsc --noEmit` **0 по всему проекту**. **НЕ КОММИТИЛ** (коммит — только по отдельной команде, pathspec своих файлов).
- **Коммит `ce96a4d8`** (59 файлов, без пуша) — мастер-план + все волны. **Продолжение после коммита**: §5.2 мастер-плана закрыт — NEW `bbExerciseExplanation` в `bb-summary.engine.ts` (display-only: подгруппа/головка + паттерн по-русски с display-уточнением «наклонный жим» для наклонных жимов + why/how) и колонки «Подмышка | Паттерн | Пояснение» в печати плана и CSV-экспорте (`BbAutoConstructor`); `derivePattern`/валидатор/атрибуция не тронуты; `bb-summary.test.ts` +4 (наклонный жим → «наклонный жим», спина-подгруппа, неизвестная мышца → пустая подгруппа/«прочее», все упражнения плана имеют паттерн). Проверено: bb-summary 6/6, bb-auto-smoke 8/8, tsc 0, bb 253ф/2826, bb-UI 26ф/213.
- **Добивка §4.3 (после §5.2)**: 11 тихих `return`-пропусков в тестах (`bb-pro-quality-phase-b` ×4, `bb-audit-2026-08-extended` ×8, `bb-back-quality.integration` ×2) заменены на precondition-throw — тесты больше не могут «зеленеть» без проверок. Харднинг сразу нашёл **вакуумную проверку**: `programToBBPlan × eccentricMult=1.2` искал primary только в первой сессии недели (там его нет) и молча проходил — теперь primary ищется по всем сессиям и то же упражнение сверяется между планами (фича впервые реально покрыта, проходит). Soft-pass `pre_exhaust` получил не-вакуумный минимум. Тесты 3 файлов **76/76**.
- **Дедуп импортов (та же добивка)**: read-only анализ (`.tmp/find-dead-imports.mjs`) + фиксер (`.tmp/remove-dead-imports.mjs`, удаление имени только при 0 вхождений в теле, BOM-состояние сохраняется) — снято **146 мёртвых именованных импортов**: UI 95 (`BbAutoConstructor` 37, `bb-contest-prep-sections` 57, `bb-step-adjust` 1 — `EXERCISE_CATALOG` осиротел после сноса `catEx`) + движки 51 (25 файлов: `bb-builder` 12, `bb-prep-cycle` 5, `bb-dup` 4, `bb-loading-layer`/`cycle-to-plan` 3 …); повторная проверка — 0 мёртвых; `tsc --noEmit` 0, bb-UI **26ф/213**, bb **253ф/2830**. **Инцидент и фикс**: первая версия фиксера считала использования «после последнего import» — в `bb-finalize.engine.ts` (функция между импортами) ложно удалила `normalizeBBMuscle`; поймано `tsc`, импорт восстановлен, скрипт исправлен (счёт по всему файлу без import-строк). **Промт продолжения другим агентом — `docs/BB-AUTO-MASTER-PLAN.md` §10.**
- **Волна-4 «гигиена/ревизия» (Sep 24 2026, без коммита)**: закрыты безопасные остатки промта §10. `applyTaperToFinalWeeks(plan, _totalWeeks)` — нечитаемый параметр переименован + честный комментарий (длина из `plan.weeks.length`, позиция сохранена для API). **`bbProgramPath` удалён** (состояние всегда было `'library'`: cycle-путь резолвится из `selectedCycleId` через `getCycleById`), вместе с ним — мёртвый `customCycle` (после §2.1 state всегда null) и legacy-поле `programPath` в `SavedBBPlan` (старые варианты читаются: поле опционально). Реестр специализации: `zoneSpec`/`donorsForZone` удалены (0 потребителей/0 тестов), `patternsForZones` снят с экспорта (внутренний хелпер), `donorRecommendations` — данные-резерв. Удалены мёртвые типы `ExerciseCategory`/`BBDay`/`BBVolumeKind`/`CycleSourceCycle` и `insulinWindowRationale`; §7-резервы `mastersAdjust`/`adjustVolumeForDemographic` НЕ удалены — помечены «зарезервировано под согласование» (при ревизии экспортов не трогать). **Ревизия 1147 экспортов движков** (read-only `.tmp/find-dead-exports.mjs`: использования в других файлах + отдельно тесты): dead=322 (pure 9 → 6 удалено + 2 §7-резерва + 1 rationale), exportOnly=313 и testOnly=171 — оставлены по политике «API сохранён» (массовое снятие `export` = churn без функциональной ценности). Проверено: bb **253ф/2830/0** (20 skipped), bb-UI **26ф/213/0** (1 чужой unhandled `revokeObjectURL`), тяжёлая матрица `BB_CYCLE_AUDIT_FULL=1` **5ф/24/0**, `tsc` 0 по своим файлам (6 ошибок — чужой незакоммиченный WIP `nutrition-ocr-parser.ts`, не тронут). Открыто (согласие/отдельная кампания): §7.1 demographics, §7.2 эталонные слои, §7.3 `jointGuardTempoOverride` (удалён в волне-3 — при согласии реализуется заново), §5.1 allocation-pass, ±16 blast/cruise, jsdom-виснет `BbAutoConstructor`. **Согласие запрошено Sep 24 2026 — пользователь выбрал «ничего не подключать»: §7/§5.1 остаются зафиксированными границами.**

## Питание UX-волна 2: центральный тач-гард мелких контролов + свитч/чекбоксы (Sep 23 2026, коммит pathspec, без пуша)

Продолжение волны «микрокнопки» (по команде «продолжай»). Только Edit/Write + vitest/tsc; чужие WIP не тронуты; **математика не менялась**.
- **Центральный тач-гард `.plan-root`** (`styles.css`): `button { min-height:44px }` + интерактивные `span[style*="cursor: pointer"]` → `inline-flex` ≥40px. Скан нашёл **88** мелких интерактивных элементов в выдаче/настройках/композере; гард закрывает их разом, не правя десятки файлов (крупные контролы 44–58px не меняются; инлайновые размеры не переписываются).
- **Нативные контролы дочищены**: «Адаптация веса» (`Settings`) — чекбокс 40×22 → свитч `role=switch` 52×32 (тач 44px); чек-лист шоу (`PeakWeekTab`) — нативные checkbox → строки-тапы 44px `role=checkbox` с ✓-тайлом; карточки быстрых действий компоновщика (`MealQuickControls.btnCard`) — `minHeight:44`.
- **Тесты**: NEW source-guard в `planner-portion-sheet` — «в исходниках планировщика нет `<select>` и `input[type=checkbox]`» (держит волну); `planner-settings-clicks4` переведён на `role=switch`.
- **Проверено**: область IndividualPlan **99 файлов / 991 тест — 0 падений**; `tsc --noEmit` **0**; `verify:apk-design` OK. Не пушил.

## Питание UX: микрокнопки дна плана → нижний лист порции + попапы вместо нативных select (Sep 23 2026, коммит pathspec, без пуша)

По жалобе «микрокнопки очень неудобно — например попап с выбором значений». Только Edit/Write + vitest/tsc; чужие WIP не тронуты; **математика/движки не менялись** — только UI и те же колбэки.
- **`MealListRender.tsx` (день плана)**: микро-редактор в чипе (`fontSize:6` «+25/×2/÷2») и нативный `<select>` замены удалены. Чип продукта — тач-таргет ≥44px (`role=button`, Enter/Space), открывает **нижний лист «🍽 {продукт}»**: степпер −/+ (умный шаг 5/10/25), быстрые `−25/+25/×2/÷2`, порции 50–250 г, инпут 48px/16px, замена — список похожих продуктов (строки 48px), действия 48px (`🔀/🛒/🔒/🚫/✕`), «Отмена/✓ Сохранить» 52px; все колбэки прежние (`updateItemAmount/replaceFoodItem/removeFoodItem/toggleLockFood/addToCart`). Кнопки-действия приёма 32→40px; чипы пресетов рецептов/«Выбрать»/⭐/«Порошок+хлопья» — 36px+ и крупнее шрифт.
- **Нативные select → попапы/чипы**: единицы инъекции — чипы 44px (`Settings`), «Заменить приём» календаря спецприёмов — `PopupSelect` (shared кита `components/PopupXxx`), «Приём» своего рецепта — `PopupSelect` (`Results`); в плане `<select>` больше нет (guard-тест).
- **NEW `planner-portion-sheet` 3/3**: 0 нативных select в плане; клик по чипу → лист `ккал/100 г` → `+25` → «✓ Сохранить» закрывает; «🔀 Заменить продукт» → «Заменить на похожий продукт».
- **Проверено**: область IndividualPlan **99 файлов / 990 тестов — 0 падений**; `tsc --noEmit` **0 по всему проекту**; `verify:apk-design` OK. Не пушил. Правка идёт поверх коммита `20ebe516` (P1/P2-раунд).

## Планировщик питания: полный анализ + исполнение P1/P2 (Sep 23 2026, без коммита)

По команде «полный анализ, что осталось — выполняй полностью». Сначала аудит (2 ридер-агента + чтение/прогоны), затем фиксы. **Только Edit/Write + vitest/tsc; чужие WIP не тронуты** (ПЛ/strongman/arm-файлы в worktree — чужие, не тронуты; другие агенты в это время коммитили свои). Коммитов не делал (не просили). Область IndividualPlan: **98 файлов / 987 тестов — 0 падений** (база 93/966 + 5 новых файлов), `tsc --noEmit` **0 по всему проекту**, `verify:apk-design` OK.

- **P1 аллерген-отчёт (сafety-баг)**: `planner-reports.generateAllergenReportPure` сравнивал РУССКИЕ id (`ALLERGEN_LIST`: 'рыба', 'глютен') с ЛАТИНСКИМИ тегами FOOD_DB ('fish', 'gluten') substring'ом → всегда «✅ аллергенов нет», противореча «🚫 Исключено N» рядом. Локальный матчер удалён → канонический `matchesSelectedAllergen` (`planner-restrictions`, теги+текстовый фолбэк). NEW `planner-allergen-report` 6/6 (позитив/паритет с `resolveAllergenFoodIds`/текст-фолбэк/чистота/все id списка/null-гард).
- **P1 merge-персист `he_planner_prefs`**: массовый эффект 27 настроек писал объект БЕЗ merge и на КАЖДОМ маунте стирал `workSchedule*` (у них нет своего mount-восстановителя) — график работы сбрасывался после перезагрузок. NEW `planner-prefs.ts` (`readPlannerPrefs`/`writePlannerPrefsPatch`), переведены 3 писателя Context (27 полей / variety+carb / `persistWorkPrefs`) + ref-инициализация. NEW `planner-prefs` 4/4 + `planner-prefs-persist` 1/1 (монтирование не теряет work-ключи).
- **P1 UTC-даты (7 мест)**: `setDate()` (локально) + `toISOString()` отдавал вчерашний день в UTC+ до ~03:00. NEW `planner-date-utils.localIsoDate`; `Context` — раскладка недели/3-дней в дневник (×2), `setPeakWeekShowDay`, демо-вес-лог, `SavedPlan.date`; `Results` — факт брифинга, «сегодня» факт-блока, дата файла/имени тренеру. NEW `planner-date-utils` 4/4.
- **P1 «Время приёма» спецприёма — «показано ≠ применяется»**: выбор времени жил только подписью. NEW `SPECIAL_MEAL_TIMING_TARGETS`/`effectiveSpecialMealTarget` (явная цель «Заменить приём» приоритетнее), Context-оверлей использует fallback по времени; движок получил ветку `«Перед сном» → preSleep` (раньше падало в snack-fallback). Toggle «Хочу сладкое»/«Ленивый день»: кнопки выдачи авто-включают режим + честные подписи «Отдельный план… (основной рацион не меняется)» в Settings и Results. NEW `planner-special-meal-timing` 6/6.
- **P2 гигиена**: `scaleItem` рецептурного пути чтит `_fixedGrams` (паритет с corrector); тихие catch → `console.warn` (квоты приёма/рецепта, E4-подбор порций, derived-sync, EA), ручной отчёт даёт тост при сбое; удалены реально мёртвые экспорты (`creamFishClash`, `EXCLUDED_FAMILIES_FROM_PLATE`, `foodPlannable`, `eaZoneColor`, `isPortableWorkDay`, `defaultCookProfile`, `NUTRIENT_SOURCES`, `clearPlanTargets` + тест переписан на «невалидная публикация не затирает валидную»); `readWeightMode/writeWeightMode` ожили (Context без прямого localStorage); мёртвые `mealPrep`/`useProEngine` удалены; эффект синка monthPlan получил полные deps; `dayDeviationPct` — isFinite-гарды; CSS-гард микрошрифтов `.plan-root [style*="font-size: 6/7/8px"]` → 9/10px (246 вхождений, центрально) + степперы сладкое/ленивый 36→44px.
- **Тест-гэпы закрыты**: NEW `product-usefulness-v1` 12/12 (V1-движок живой, тестов не имел) + `nutrition-quality` 8/8 (чистота/масштаб/пороги).
- **Осознанные границы (не делал)**: унификация TOPUP-списков corrector↔recipe-mode и `recalcMealTotals↔mealTotalsOf` — расхождения залочены калиброванными тестами, сведение меняет числа путей (правило «математику не менять»); 6 нативных `<select/checkbox>` в мобильном UI (замена на попапы — отдельный UX-раунд); адаптер `prepToMealPlanInput`/`MealPlanInput` в `bb-contest-prep.engine.ts` — чужая зона BB (тест-онли, оставлен).
- **Не коммитил**: правила сессии — коммиты только по явному запросу; при желании — строго pathspec своих файлов (nutrition-область + styles.css + AGENTS.md).

## ПЛ-авто §10.4: закрытие остатка Фаз 2/4 и §3/§4 — alert→тосты, macro-персист, e1RM/LVP канон, RPE-граница, taper-канон, kinds моста, weakpoint-pl 0 потерь (Sep 23 2026, коммиты pathspec, без пуша)

По решению пользователя («всё включая taper-дубли») закрыт максимальный остаток ПЛ-авто после §10.3. Только Edit/Write + vitest/tsc; чужие WIP не тронуты (чужой untracked `zz-meta-alt-probe.test.ts` цел); коммиты строго pathspec; без пуша. **Математика планов не менялась** — единственное «сведение» (taper-кривая) числа 1-в-1 (паритет-тест).
- **alert/prompt → тосты (§4)**: `AutoregPanel`/`RecoveryPanel`/`PeakingPanel` — канонический `useEditorToast` (`window.showToast` живёт только в контуре питания; в тренинге `alert` был реальным путём); `PlDeadpointsBarPathCard` — 6× `prompt()` → `PopupText compact` (шит; новый опциональный проп, старые потребители целы). NEW `pl-alert-toasts` 4/4 (source-guard без вызовов + поведение тостов/шита).
- **macro-настройки тапера (§3/§4)**: `plMacroTaperMode/plMacroWeightGoal/plMacroMockMeet/plMacroPostMeet` персистятся в `he_pl_session` и восстанавливаются валидаторами `validateSavedMacroTaperMode/WeightGoal/Bool` (`taper-state`); ярлыки макро-контекста отделены («Раскладка тапера макроцикла») — контекст отдельный от циклового (дефолт 'classic' vs 'pl'), слияние меняло бы числа по умолчанию. `taper-state` 6→**8/8**.
- **e1RM/LVP (§3)**: приватная LVP-таблица `estimate1rm.engine` выведена из канона `vbt.engine.LOAD_VELOCITY_PROFILE` (подмножество тех же 6 опорных % — интерполяция 1-в-1; `ohp`/`row` — фолбэк squat, как было; тест поймал и это). `estimate1rm` **18/18** (паритет-якоря + «0.95 = 0.385, не 0.40»).
- **RPE→% (§3)**: сверка показала две ОСОЗНАННЫЕ линейки — Epley (авторегуляция/дневник) и Brzycki (сетка StrengthAnalysisHub/OneRmCalcTab); сведение меняло бы числа одной стороны. Граница заморожена NEW `rpe-formulas-boundary` **4/4** (якоря, монотонность, намеренное расхождение ≤5 п.п.).
- **Taper-дубли (остаток 2.2)**: `pro/taper.taperPlan` строит витринную кривую через канон `buildPLTaperCurve({taperWeeks, mode:'pro', peakIntensityPct:0.90})` + адаптер к прежнему `TaperWeek`-шейпу — прямого дубля кривой нет; числа 1-в-1 (`taper.test` 16/16, `taper-planner-tab` 6/6). `mesocycle-progression.taperCurve` — единственная pro-реализация ВНУТРИ канона (не дубль).
- **Мост (P0-4)**: kinds чужих конструкторов (`program/design/macrocycle/annual_block/methodology/cardio/bb_nutrition/arm_cycle/ss_cycle/combat_cycle/tempo_rollback`) больше не съедаются молча — честная заметка `setMethodNote`; source-guard в `pl-p2-data-hygiene` **18/18**.
- **weakpoint-pl (P0-8)**: core-fallback резолвера (ядра без скобочных уточнений: «Паучий подъём (…)» → каталог) + 8 имён приведены к каталогу (блок-бицепс, трицепс-блок ×3, жим носками, икры ×2, шраги ×2; Смит/тренажёр-вариантов в каталоге нет — взяты ближайшие каталог-записи). Lock: **0 нерезолвленных на всех 36 фазах** (`weakpoint-pl` 14/14).
- **Проверено**: обязательный круг `src/engines/lms` + `SRCBBScreen_parts` + `TrainingScreen_parts` — **230 файлов / 2684 теста: 2683 passed, 1 падение = load-флейк `taper-hub-pro`** (в круге таймаут 5с под 230 параллельными файлами; изолированно **21/21**, файл тяжёлый ~4.8с — предсуществующая тяжесть, мой код его не замедляет) + 1 чужой unhandled `revokeObjectURL`; доп. круг `src/engines/pro` + `weakpoint-pl` + `rest-hooks-native` — **43 файла / 496 passed** (+чужой `DB not init` ReportsScreen, предсуществующий); `tsc --noEmit` **0 по всему проекту**; `verify:apk-design` OK; `npm run build` **OK** (33.84с, PWA 71 precache). Статусы Фаз 2/4 и §3/§4 обновлены в доке (§6 + §10.4).
- **§4-хвост мёртвых экспортов (финальная перепроверка, закрыт)**: удалены реально мёртвые (без потребителей и тестов) — `planAllFrequencies`, `analysesForUnified` + `UNIFIED_LIFT_RU` (+ осиротевшие импорты), `DOTS_CLASS_TABLE` (алиас приватного `DOTS_THRESHOLDS`), `findPlCorrection`. Остальные из §4 — contract-locked своими тестами (policy «API сохранён»: rpeAttempts, liftKeyOf, findBlockByPhase, bbTrainingFocusForWeek, applicableFormulasForLift, adjustedLoad, rpeWeightFor/e1RMFromRpeSet, VL_THRESHOLDS, vblLoad, rpeVbtDiscrepancy, limiter×4, getScheme, lms-metrics×6). Целевые тесты **124/124**; `tsc` — по моим файлам 0 (1 чужая ошибка WIP arm-хаба `buildArmIcs`, не тронут). Границы зафиксированы в доке §10.4 п.10 (сценарии macro/annual — разные слои; ПМ-поля PLPlanView display-only; `activePopup` сеттеры есть).
- Коммиты строго pathspec своих файлов (движки/UI/тесты + доки), НЕ ПУШИЛ.
...
- **Остатки §10.3 — закрыты решением пользователя (Sep 23 2026, код не менялся)**: п.1 BB-ветка `SRCBBScreen` — **оставить как неактивный резерв** (аудит подтвердил: монтируется только `track="pl"`, ни один тест не рендерит `track='bb'`; удаление задело бы `bbLevel`→`baseMrv`→`pedAdapt` PED-карточки ПЛ, `he_bb_session`, `saveTrainingProfile` без ценности); п.3 потребитель `catalogId` в `LMS_EXERCISES` — **оставить на именах** (parity-тест зелёный; перевод менял бы протоколы/re-baseline и дозировку диагностических инъекций без явной ценности; при будущем переводе — только синхронно в обоих контурах). Статусы в доке §10.3 обновлены. Математика планов не менялась.
...
## Коррекция-контент round-10 · Третья ступень порогов: ББ ≥5, ТА/стронг/армлифтинг ≥4, арм ≥7 (Sep 22 2026, коммиты pathspec, без пуша)

Углубление контента ступенью выше (цель «топ-инструмент»): пороги подняты там, где это честно закрывается реальными упражнениями, а не «набивкой».
- **ББ зона/сигнал/драйвер ≥5** (+9 записей, `a56a060c`): `cl-decline-pause` (низ груди + bench-watch/fix), `bt-cs-row-tspine` (толщина + driver:thoracic), `df-front-raise-er` (передняя дельта + rot-gap/erir-low), `tr-suitcase-brace` (трапы + driver:core), `bi-incline-curl-len`, `hm-nhe-ecc` (nhe-weak), `ad-copenhagen-lg` (add-weak), `ank-calf-knw` (ktw-asym + driver:ankle), `pm-throughput-tempo` (pm-yellow) — все `spare` (не вытесняют калиброванный топ) + лок ≥5 (зоны хаба/ББ-авто, 12 сигналов, 5 драйверов).
- **Стронг причина × вид ≥4** (`a56a060c`): +4 записи (`sm_stone_load_vol_stab`, `sm_stone_floor_tech_str`, `sm_farmers_pickup_mob_str2`, `sm_log_drive_str_stab`) с реальными exId в `SM_CORR_EXID_BY_ID` + лок ≥4; поймано своим tsc: `stability` — **не** `SMWeakCause` (это вид) → причины приведены к канону.
- **Армлифтинг причина × фаза ≥4**: +11 записей (raptor-техника под нагрузкой, farmer/fat-bar объём и выносливость, anvil-hub техника максимума, blockbuster-стабильность, plate-pinch холд, country-crush сила/стабильность и усталость-техника, coc-№2 мобильность-техника, coc-№1 стабильность хруста, napalm силовая база) + лок ≥4; поймано лок-ом: `strength`/`stability` — не канонические причины (`max_strength`/`technique`/`fatigue`…), приведено к `ArmliftCause`.
- **Арм пул точки ≥7**: контент уже давал 7–10 (base+`POOL_TOPUP`), лок поднят ≥6→**≥7**.
- **ТА причина-лимитер × фаза ≥4**: +3 записи (`snatch_pause_knee`, `clean_volume_pull`, `catch_series_ta`) — все фазы подняты, ноль тонких ячеек (было 7 по 3); **плюс 3 полные записи каталога** в `exercise-catalog-ta-supplement.ts` (id больше не синтетика) и `CORRECTIVE_HOW_NOT` на каждую (лок требует непустой howNot); re-baseline без потерь: `clean_volume_pull` target `pull_start`→`squat_mid/clean_mid` (индексный C11-лок `deficit_pull=4` цел).
- **Проверено**: `src/engines/strength-sport`+`arm` **161 файл / 2157** + `bb-corrective-*` + хаб-UI **72 файла / 596** + `tsc --noEmit` **0 по всему проекту**. НЕ ПУШИЛ.
...
- **NEW `src/engines/hub-diary.engine.ts`** — единый адаптер: `loadHubDiarySessions()` (v2-форма: `{date, exercises:[{exerciseName, muscleGroup, side, sets:[{weight, weightKg, reps, rpe, velocity}]}]}`) + `loadHubDiaryFlatEntries()` (плоская форма для `buildDiaryTrendSS`/`smWeeklySetsByLift`/`buildLastE1RMIndexSS`). Правила: первый **непустой** ключ побеждает (пустой `'[]'` не блокирует v2); терпимость к реальным легаси-формам (сессия с `exercises[]` ИЛИ плоская запись; упражнение без имени, но с `muscleGroup`/`muscle`; числовой `sets: N`); нормализация **аддитивная** (`...spread` исходных полей + `weight`/`weightKg`, `muscle`/`muscleGroup`, `side`), чтобы ни один потребитель не потерял данные; мусор/битые даты — тихо.
- **Проводка** (5 точек): арм-хаб → `loadHubDiarySessions()` (движок `detectArmWeakByE1rm` ждёт v2), ТА-хаб → sessions (`detectTAWeakFromDiary`, `candidateTAWeakPointsFromDiary`), стронг-хаб → flat (`detectSMWeakFromDiary`, `candidateSMWeakPointsFromDiary`, `smWeeklySetsByLift`), ББ-хаб → sessions (`factVolume`), SS-движки `loadDiaryLogsSS`/`loadDiaryLogsSM` → flat (сохранён фолбэк `he_srpe_sessions`).
- **Поймано своим прогоном** (важное): добавление 7 домашних записей в ББ **вытеснило откалиброванный топ** (tie-break по id поднял новые записи выше) — вернул приоритет честно: NEW флаг `spare` у запасных домашних записей + `score -= 3` в ранжире (не вытесняют основную запись при равном скоре, но выбираются, когда основную отсекли фильтры оборудования/уровня/противопоказаний; пул под домашним залом по-прежнему ≥2).
- **Тесты**: NEW `hub-diary` **6/6** (v2-форма, «пустой `[]` не блокирует v2» — корень бага, легаси-обёртка, плоские записи, битый JSON/мусор, пусто) + **4 сквозных лока на ЖИВОЙ ключ** (`he_workout_log_v2`): ББ «Дневник e1RM (28д)», арм «pronators», ТА «Дневник фазы» (рывок 2×RPE9), стронг «📓 Дневник:» (Yoke Walk) — все падали бы до фикса + `bb-diagnostics-hub` 45/45 + `bb-corrective-*` 10 файлов 89/89 + пачка хаб-UI **74 файла / 601** + `src/engines/strength-sport`+`arm` **161 файл / 2154** + `tsc --noEmit` **0 по всему проекту**. Пробы удалены. НЕ ПУШИЛ.

## Коррекция-контент round-10 · Разрывы «писатель ↔ читатель» плана: арм (не доходил) + ТА/стронг (событие) (Sep 22 2026, коммит pathspec, без пуша)

Реаудит по существу (после паритета поверхностей): проверка «пишет ли конструктор тот ключ, который читает хаб-аудит».
- **Арм — реальный разрыв**: хабы (арм-аудит покрытия 12 точек, аудит плана армлифтинга) читают `he_arm_plan_saved` (фолбэк `he_arm_last_plan`) и слушают `he-arm-plan-saved`, а **конструктор писал только `he_arm_last_plan`** и события не слал → план, собранный в конструкторе, в хабе не виделся (аудит/покрытие пустые). NEW `persistArmPlan(plan)` (единая точка: оба ключа + событие), переведены оба места записи (сборка плана + загрузка варианта). NEW `arm-plan-persist` 3/3 (оба ключа + событие + source-guard «ровно одна прямая запись `he_arm_last_plan` — внутри helper»).
- **ТА/стронг — почти-разрыв**: ключ `he_strength_sport_plan_v1` конструктор пишет через `saveStrengthSportPlan` ✓ (mount-чтение хабов работало), но **событие не слал** → смонтированный живой аудит не обновлялся. Добавлен dispatch `he-strength-sport-plan-saved` в `saveStrengthSportPlan` (guard `typeof window`) + 2 lock-теста (ключ/событие).
- **Проверено**: `arm-plan-persist` 3/3 + арм/армлифтинг-UI 31/31 + SS-UI **77/77 (12 файлов)** + `strength-sport-storage` 6/6 + `tsc --noEmit` **0 по всему проекту**. НЕ ПУШИЛ.

## Коррекция-контент round-10 · Проверка «углублённый контент работает для домашнего зала»: +7 записей ББ, честные пустые стейты ТА/стронг (Sep 22 2026, коммит pathspec, без пуша)

Реаудит по существу: углубление пулов должно помогать ОГРАНИЧЕННОМУ залу (ради чего делалось). Проба всех 5 хабов под профиль `dumbbell+bodyweight` показала: арм 0 тонких точек, армлифтинг 0 (спасибо `equipmentAlt` 66/66), **ББ 7 зон по 1 варианту**, ТА 13/16 фаз 0–1, стронг 9 фаз 0–1.
- **ББ — реальный пробел (бодибилдинг достижим дома), закрыт 7 записями** на свободные реальные id каталога: `lateral_raise_single` (delt_mid), `rear_delt_fly_v2` (delt_rear), `pushup_incline` (chest_upper), `pullup` (back_width), `overhead_tricep_db_both` (triceps), `standing_calf` (calves, только bodyweight-опция), `curl_wrist_db` (forearms) — все с protocol/cues/progression/regression/retest/source/equipmentAlt. Итог: под домашним залом каждая зона хаба **≥2 варианта** (был 1 у 7 зон).
- **NEW lock** (`bb-corrective-coverage`): «под `dumbbell+bodyweight` каждая зона хаба даёт ≥2 варианта» — ловит регресс «контент добавили, но не для ограниченного зала».
- **ТА/стронг — осознанная граница (штангa/снаряды по природе спорта), но честность поверхностей доделана**:
  - ТА: превью моста молча исчезало для фазы с 0 кандидатов → NEW честная плашка `data-wl="corrections-empty"` («⚠ Нет вариантов под фильтр зала/уровня для фаз: … — ослабь фильтры или добавь снаряд (штангу)») + UI-лок (под `bodyweight` плашка есть; со штангой — нет).
  - Стронг: превью моста не показывало давало «фаза → что», и фаза без вариантов не читалась → NEW строка «Фаза → упражнение: …» с честным `⊘ нет вариантов под фильтр` (паритет с ТА) + UI-лок («Свой вес» → ⊘ в превью).
  - Арм/армлифтинг: тонких точек нет, правок не требовалось (проверено пробой).
- **Проверено**: `bb-corrective-*` 5 файлов 39/39 + вся пачка corrective 82/82, ТА-UI **20/20**, стронг-UI **16/16**, пачка ТА/стронг/см-UI **136/136 (10 файлов)**, `tsc --noEmit` **0 по всему проекту**. Пробы (`zz-homegym-probe`, `zz-homeids-probe`) удалены до коммита. НЕ ПУШИЛ.
...
- +7 записей на реальных id main-каталога: `clean_pull_v2` (volume/technique), `overhead_squat` (volume/stability), `smith_split_squat` (fatigue/strength, machine), `squat_overhead` (fatigue/stability, bodyweight-по-сути), `snatch` (volume/strength), `kb_snatch` (mobility/technique, kettlebell), `deadlift_snatch_grip` (fatigue/technique) — с `CORRECTIVE_META` и `CORRECTIVE_HOW_NOT`.
- NEW-порог лока: «каждая причина-лимитер ≥3 в каждой фазе» (было ≥2).
- Re-baseline (осознанно, «было→стало»): `ta-corrective` C11 — под ограничением плеча `tall_snatch` теперь вытесняется из окна топ-5 (плотнее список) → `protocolForPreferred(...)` возвращает `null` честно, а паритет дозы проверяется на `snatch_high_pull` (не спросовый по оверхeду); индексный ассерт заменён на «-1 ИЛИ больше».
- **Проверено**: `src/engines/strength-sport` **65 файлов / 1007** + ТА-UI **5 файлов / 87** + `tsc --noEmit` **0 по всему проекту**. НЕ ПУШИЛ.

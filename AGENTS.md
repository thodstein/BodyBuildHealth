...
- **Раунд 10 (панели анализа + §108, НЕ коммичен)**: кнопки анализа 34–38px (`+упражнение`, табы недель сводки, «Применить» периодизации/сплита/замены), степперы объёма 26px, ✕ шита пикера без тача. NEW хуки `panel-add-ex/week-tab/apply/vol-step` + `lib-sheet-close`. АПК §108: кнопки 44px + press/focus, табы tabular, степперы точечно 36px, ✕ 44px, reduced-motion. Проверено: manual 36/36 (ui 17/17 с рендером PlanSummaryTable) + каталог 34/34 → **70/70**, `tsc` 0, `verify:apk-design` OK. Файлы: `ProgramEditorPanels.tsx` (6 хуков), `BbProgramLibraryPicker.tsx` (1 хук), `styles-native.css` (§108), тест. НЕ КОММИТИЛ/НЕ ПУШИЛ.

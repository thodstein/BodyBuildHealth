...
- **Дедуп импортов (та же добивка)**: read-only анализ (`.tmp/find-dead-imports.mjs`) + фиксер (`.tmp/remove-dead-imports.mjs`, удаление имени только при 0 вхождений в теле, BOM-состояние сохраняется) — снято **95 мёртвых именованных импортов**: `BbAutoConstructor` 37, `bb-contest-prep-sections` 57, `bb-step-adjust` 1 (`EXERCISE_CATALOG` осиротел после сноса `catEx`); повторная проверка — 0 мёртвых; `tsc --noEmit` 0, bb-UI **26ф/213**.

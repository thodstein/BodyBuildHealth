...
1. Box drawing characters (U+2568, U+2591, etc.) inserted BEFORE Cyrillic letters
2. Box drawing characters (U+2550 horizontal line) used as decorative separators

The fix:
- Remove box drawing characters that appear before Cyrillic letters (pattern 1)
- Remove all U+2550 (horizontal line) box characters (pattern 2 - decorative only)
...
def fix_box_cyrillic(bytes_data):
    ...
    while i < len(bytes_data):
        ...
        if bytes_data[i:i+2] == b'\xe2\x95':
            ...
            # Check if next byte is a Cyrillic UTF-8 start byte (d0 or d1)
            if i + 3 < len(bytes_data) and bytes_data[i+2] in (
